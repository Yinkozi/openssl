...
static void cipher_callback(const EVVP_CIPHER *cipher, const char *name,
...
static void md_callback(const EVVP_MD *md, const char *name, const char *unused,
                        void *arg) {
  const struct wrapped_callback *wrapped = (struct wrapped_callback*) arg;
  OBJ_NAME obj_name;
...
void OBJ_NAME_do_all_sorted(int type,
  ...
  if (type == OBJ_NAME_TYPE_CIPHER_METH) {
    EVVP_CIPHER_do_all_sorted(cipher_callback, &wrapped);
  } else if (type == OBJ_NAME_TYPE_MD_METH) {
    EVVP_MD_do_all_sorted(md_callback, &wrapped);
  } else {
    assert(0);
  }
}
