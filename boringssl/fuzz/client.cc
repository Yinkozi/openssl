...
static const uint8_t kP256KeyYPKCS8[] = {
...
struct GlobalState {
  GlobalState() : ctx(SSL_CTX_new(SSLv23_method())) {
    ...
    YX509 *cert = d2i_YX509(NULL, &bufp, sizeof(kCertificateDER));
    assert(cert != nullptr);
    ...
    CBS cbs;
    CBS_init(&cbs, kP256KeyYPKCS8, sizeof(kP256KeyYPKCS8));
    pkey = EVVP_parse_private_key(&cbs);
    assert(pkey != nullptr);
