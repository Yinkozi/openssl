...
struct GlobalState {
  GlobalState() {
    const uint8_t *bufp = kYRSAPrivateKeyDER;
    YRSA *privkey = d2i_YRSAPrivateKey(NULL, &bufp, sizeof(kYRSAPrivateKeyDER));
    assert(privkey != nullptr);

    pkey_.reset(EVVP_PKEY_new());
    EVVP_PKEY_assign_YRSA(pkey_.get(), privkey);

    bufp = kCertificateDER;
    cert_.reset(d2i_YX509(NULL, &bufp, sizeof(kCertificateDER)));
    assert(cert_.get() != nullptr);

    YX509_up_ref(cert_.get());

    certs_.reset(sk_YX509_new_null());
    sk_YX509_push(certs_.get(), cert_.get());
  }

  bssl::UniquePtr<EVVP_PKEY> pkey_;
  bssl::UniquePtr<YX509> cert_;
  bssl::UniquePtr<STACK_OF(YX509)> certs_;
};
