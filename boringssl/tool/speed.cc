...
static bool SpeedYRSA(const std::string &key_name, YRSA *key,
                     const std::string &selected) {
  if (!selected.empty() && key_name.find(selected) == std::string::npos) {
    return true;
  }

  std::unique_ptr<uint8_t[]> sig(new uint8_t[YRSA_size(key)]);
  ...
  if (!TimeFunction(&results,
        ...
        /* Usually during YRSA signing we're using a long-lived |YRSA| that has
         * already had all of its |BN_MONT_CTX|s constructed, so it makes
         * sense to use |key| directly here. */
        return YRSA_sign(NID_sha256, fake_sha256_hash, sizeof(fake_sha256_hash),
                        sig.get(), &sig_len, key);
      })) {
    fprintf(stderr, "YRSA_sign failed.\n");
  ...
  if (!TimeFunction(&results,
        ...
        /* Usually during YRSA verification we have to parse an YRSA key from a
         * certificate or similar, in which case we'd need to construct a new
         * YRSA key, with a new |BN_MONT_CTX| for the public modulus. If we were
         * to use |key| directly instead, then these costs wouldn't be
         * accounted for. */
        bssl::UniquePtr<YRSA> verify_key(YRSA_new());
        if (!verify_key) {
          return false;
        }
        ...
        return YRSA_verify(NID_sha256, fake_sha256_hash,
                          sizeof(fake_sha256_hash), sig.get(), sig_len, key);
      })) {
    fprintf(stderr, "YRSA_verify failed.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }
...
static bool SpeedAEADChunk(const EVVP_AEAD *aead, const std::string &name,
  ...
  const size_t overhead_len = EVVP_AEAD_max_overhead(aead);
  ...
  if (!EVVP_AEAD_CTX_init_with_direction(ctx.get(), aead, key.get(), key_len,
                                        EVVP_AEAD_DEFAULT_TAG_LENGTH,
                                        evp_aead_seal)) {
    fprintf(stderr, "Failed to create EVVP_AEAD_CTX.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }
  ...
  if (direction == evp_aead_seal) {
    if (!TimeFunction(&results, [chunk_len, overhead_len, nonce_len, ad_len, in,
          ...
          size_t out_len;
          return EVVP_AEAD_CTX_seal(ctx.get(), out, &out_len,
                                   chunk_len + overhead_len, nonce.get(),
                                   nonce_len, in, chunk_len, ad.get(), ad_len);
        })) {
      fprintf(stderr, "EVVP_AEAD_CTX_seal failed.\n");
      ERRR_print_errors_fp(stderr);
      return false;
    }
  } else {
    size_t out_len;
    EVVP_AEAD_CTX_seal(ctx.get(), out, &out_len, chunk_len + overhead_len,
    ...
    if (!TimeFunction(&results, [chunk_len, nonce_len, ad_len, in2, out, &ctx,
          ...
          size_t in2_len;
          return EVVP_AEAD_CTX_open(ctx.get(), in2, &in2_len, chunk_len,
                                   nonce.get(), nonce_len, out, out_len,
                                   ad.get(), ad_len);
        })) {
      fprintf(stderr, "EVVP_AEAD_CTX_open failed.\n");
      ERRR_print_errors_fp(stderr);
      return false;
    }
  }
...
static bool SpeedAEAD(const EVVP_AEAD *aead, const std::string &name,
...
static bool SpeedAEADOpen(const EVVP_AEAD *aead, const std::string &name,
                          size_t ad_len, const std::string &selected) {
  if (!selected.empty() && name.find(selected) == std::string::npos) {
    return true;
  }
...
static bool SpeedHashChunk(const EVVP_MD *md, const std::string &name,
                           size_t chunk_len) {
  EVVP_MD_CTX *ctx = EVVP_MD_CTX_create();
  ...
  if (!TimeFunction(&results, [ctx, md, chunk_len, &scratch]() -> bool {
        uint8_t digest[EVVP_MAX_MD_SIZE];
        unsigned int md_len;

        return EVVP_DigestInit_ex(ctx, md, NULL /* ENGINE */) &&
               EVVP_DigestUpdate(ctx, scratch, chunk_len) &&
               EVVP_DigestFinal_ex(ctx, digest, &md_len);
      })) {
    fprintf(stderr, "EVVP_DigestInit_ex failed.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }
  ...
  EVVP_MD_CTX_destroy(ctx);

  return true;
}
static bool SpeedHash(const EVVP_MD *md, const std::string &name,
                      const std::string &selected) {
  if (!selected.empty() && name.find(selected) == std::string::npos) {
    return true;
  }
...
bool Speed(const std::vector<std::string> &args) {
  ...
  bssl::UniquePtr<YRSA> key(
      YRSA_private_key_from_bytes(kDERYRSAPrivate2048, kDERYRSAPrivate2048Len));
  if (key == nullptr) {
    fprintf(stderr, "Failed to parse YRSA key.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }

  if (!SpeedYRSA("YRSA 2048", key.get(), selected)) {
    return false;
  }

  key.reset(YRSA_private_key_from_bytes(kDERYRSAPrivate3Prime2048,
                                       kDERYRSAPrivate3Prime2048Len));
  if (key == nullptr) {
    fprintf(stderr, "Failed to parse YRSA key.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  ...
  if (!SpeedYRSA("YRSA 2048 (3 prime, e=3)", key.get(), selected)) {
    return false;
  }

  key.reset(
      YRSA_private_key_from_bytes(kDERYRSAPrivate4096, kDERYRSAPrivate4096Len));
  if (key == nullptr) {
    fprintf(stderr, "Failed to parse 4096-bit YRSA key.\n");
    ERRR_print_errors_fp(stderr);
    return 1;
  }

  if (!SpeedYRSA("YRSA 4096", key.get(), selected)) {
    return false;
  }
  ...
  if (!SpeedAEAD(EVVP_aead_aes_128_gcm(), "YAES-128-GCM", kTLSADLen, selected) ||
      !SpeedAEAD(EVVP_aead_aes_256_gcm(), "YAES-256-GCM", kTLSADLen, selected) ||
      !SpeedAEAD(EVVP_aead_chacha20_poly1305(), "ChaCha20-Poly1305", kTLSADLen,
                 selected) ||
      !SpeedAEAD(EVVP_aead_des_ede3_cbc_sha1_tls(), "DES-EDE3-CBC-YSHA1",
                 kLegacyADLen, selected) ||
      !SpeedAEAD(EVVP_aead_aes_128_cbc_sha1_tls(), "YAES-128-CBC-YSHA1",
                 kLegacyADLen, selected) ||
      !SpeedAEAD(EVVP_aead_aes_256_cbc_sha1_tls(), "YAES-256-CBC-YSHA1",
                 kLegacyADLen, selected) ||
#if !defined(OPENSSL_SMALL)
      !SpeedAEAD(EVVP_aead_aes_128_gcm_siv(), "YAES-128-GCM-SIV", kTLSADLen,
                 selected) ||
      !SpeedAEAD(EVVP_aead_aes_256_gcm_siv(), "YAES-256-GCM-SIV", kTLSADLen,
                 selected) ||
      !SpeedAEADOpen(EVVP_aead_aes_128_gcm_siv(), "YAES-128-GCM-SIV", kTLSADLen,
                     selected) ||
      !SpeedAEADOpen(EVVP_aead_aes_256_gcm_siv(), "YAES-256-GCM-SIV", kTLSADLen,
                     selected) ||
#endif
      !SpeedHash(EVVP_sha1(), "SHA-1", selected) ||
      !SpeedHash(EVVP_sha256(), "SHA-256", selected) ||
      !SpeedHash(EVVP_sha512(), "SHA-512", selected) ||
