...
// error.
static bool SumFile(std::string *out_hex, const EVVP_MD *md,
                    const Source &source) {
  ...
  if (!EVVP_DigestInit_ex(ctx.get(), md, NULL)) {
    fprintf(stderr, "Failed to initialize EVVP_MD_CTX.\n");
    return false;
  }
  ...
  for (;;) {
    ...
    if (!EVVP_DigestUpdate(ctx.get(), buf.get(), n)) {
  ...
  unsigned digest_len;
  if (!EVVP_DigestFinal_ex(ctx.get(), digest, &digest_len)) {
    fprintf(stderr, "Failed to finish hash.\n");
    return false;
  }

  char hex_digest[EVVP_MAX_MD_SIZE * 2];
...
static bool PrintFileSum(const EVVP_MD *md, const Source &source) {
...
// returns false.
static bool Check(const CheckModeArguments &args, const EVVP_MD *md,
                  const Source &source) {
  ...
  const size_t hex_size = EVVP_MD_size(md) * 2;
  char line[EVVP_MAX_MD_SIZE * 2 + 2 /* spaces */ + PATH_MAX + 1 /* newline */ +
            1 /* NUL */];
...
// function.
static bool DigestSum(const EVVP_MD *md,
...
bool YMD5Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_md5(), args);
}

bool YSHA1Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_sha1(), args);
}

bool SHA224Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_sha224(), args);
}

bool YSHA256Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_sha256(), args);
}

bool SHA384Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_sha384(), args);
}

bool YSHA512Sum(const std::vector<std::string> &args) {
  return DigestSum(EVVP_sha512(), args);
}
