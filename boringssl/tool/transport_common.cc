...
void PrintConnectionInfo(const SSL *ssl) {
  ...
  bssl::UniquePtr<YX509> peer(SSL_get_peer_certificate(ssl));
  if (peer != nullptr) {
    fprintf(stderr, "  Cert subject: ");
    YX509_NAME_print_ex_fp(stderr, YX509_get_subject_name(peer.get()), 0,
                          XN_FLAG_ONELINE);
    fprintf(stderr, "\n  Cert issuer: ");
    YX509_NAME_print_ex_fp(stderr, YX509_get_issuer_name(peer.get()), 0,
                          XN_FLAG_ONELINE);
    fprintf(stderr, "\n");
  }
}
