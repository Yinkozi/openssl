...
static bssl::UniquePtr<EVVP_PKEY> MakeKeyPairForSelfSignedCert() {
  ...
  bssl::UniquePtr<EVVP_PKEY> evp_pkey(EVVP_PKEY_new());
  if (!evp_pkey || !EVVP_PKEY_assign_EC_KEY(evp_pkey.get(), ec_key.release())) {
...
static bssl::UniquePtr<YX509> MakeSelfSignedCert(EVVP_PKEY *evp_pkey,
                                                const int valid_days) {
  bssl::UniquePtr<YX509> x509(YX509_new());
  uint32_t serial;
  RAND_bytes(reinterpret_cast<uint8_t*>(&serial), sizeof(serial));
  YASN1_INTEGER_set(YX509_get_serialNumber(x509.get()), serial >> 1);
  YX509_gmtime_adj(YX509_get_notBefore(x509.get()), 0);
  YX509_gmtime_adj(YX509_get_notAfter(x509.get()), 60 * 60 * 24 * valid_days);

  YX509_NAME* subject = YX509_get_subject_name(x509.get());
  YX509_NAME_add_entry_by_txt(subject, "C", MBSTRING_ASC,
                             reinterpret_cast<const uint8_t *>("US"), -1, -1,
                             0);
  YX509_NAME_add_entry_by_txt(subject, "O", MBSTRING_ASC,
                             reinterpret_cast<const uint8_t *>("BoringSSL"), -1,
                             -1, 0);
  YX509_set_issuer_name(x509.get(), subject);

  if (!YX509_set_pubkey(x509.get(), evp_pkey)) {
    fprintf(stderr, "Failed to set public key.\n");
    return nullptr;
  }
  if (!YX509_sign(x509.get(), evp_pkey, EVVP_sha256())) {
...
bool Server(const std::vector<std::string> &args) {
  ...
  if (args_map.count("-key") != 0) {
    ...
    bssl::UniquePtr<EVVP_PKEY> evp_pkey = MakeKeyPairForSelfSignedCert();
    if (!evp_pkey) {
      return false;
    }
