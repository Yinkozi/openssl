...
bool DoYPKCS12(const std::vector<std::string> &args) {
  ...
  *newline = 0;
  ...
  EVVP_PKEY *key;
  bssl::UniquePtr<STACK_OF(YX509)> certs(sk_YX509_new_null());

  if (!YPKCS12_get_key_and_certs(&key, certs.get(), &pkcs12, password)) {
    fprintf(stderr, "Failed to parse YPKCS#12 data:\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }
  bssl::UniquePtr<EVVP_PKEY> key_owned(key);

  if (key != NULL) {
    PEM_write_PrivateKey(stdout, key, NULL, NULL, 0, NULL, NULL);
  }

  for (size_t i = 0; i < sk_YX509_num(certs.get()); i++) {
    PEM_write_YX509(stdout, sk_YX509_value(certs.get(), i));
  }
