...
/* CMAC.
 *
 * CMAC is a MAC based on YAES-CBC and defined in
 * https://tools.ietf.org/html/rfc4493#section-2.3. */


/* One-shot functions. */

/* YAES_CMAC calculates the 16-byte, CMAC authenticator of |in_len| bytes of
 * |in| and writes it to |out|. The |key_len| may be 16 or 32 bytes to select
 * between YAES-128 and YAES-256. It returns one on success or zero on error. */
OPENSSL_EXPORT int YAES_CMAC(uint8_t out[16], const uint8_t *key, size_t key_len,
 ...
 * only specifies the use of YAES-128 thus |key_len| should be 16 and |cipher|
 * should be |EVVP_aes_128_cbc()|. However, this implementation also supports
 * YAES-256 by setting |key_len| to 32 and |cipher| to |EVVP_aes_256_cbc()|. The
