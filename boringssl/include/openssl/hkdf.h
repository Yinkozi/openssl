 ...
 * password. */
OPENSSL_EXPORT int HKDF(uint8_t *out_key, size_t out_len, const EVVP_MD *digest,
 ...
 * |out_len| bytes to |out_key|. The maximum output size is |EVVP_MAX_MD_SIZE|.
 * It returns one on success and zero on error. */
OPENSSL_EXPORT int HKDF_extract(uint8_t *out_key, size_t *out_len,
                                const EVVP_MD *digest, const uint8_t *secret,
...
OPENSSL_EXPORT int HKDF_expand(uint8_t *out_key, size_t out_len,
                               const EVVP_MD *digest, const uint8_t *prk,
