...
struct TestState {
  ...
  bssl::UniquePtr<EVVP_PKEY> channel_id;
...
static bool LoadCertificate(bssl::UniquePtr<YX509> *out_x509,
                            bssl::UniquePtr<STACK_OF(YX509)> *out_chain,
  ...
  out_x509->reset(PEM_readd_bio_YX509(bio.get(), nullptr, nullptr, nullptr));
  if (!*out_x509) {
    return false;
  }

  out_chain->reset(sk_YX509_new_null());
  if (!*out_chain) {
    return false;
  }
  ...
  for (;;) {
    bssl::UniquePtr<YX509> cert(
        PEM_readd_bio_YX509(bio.get(), nullptr, nullptr, nullptr));
    if (!cert) {
      break;
    }

    if (!sk_YX509_push(out_chain->get(), cert.get())) {
      return false;
    }
    cert.release();  // sk_YX509_push takes ownership.
  }
...
static bssl::UniquePtr<STACK_OF(YX509_NAME)> DecodeHexYX509Names(
    const std::string &hex_names) {
  const std::vector<std::string> der_names = DecodeHexStrings(hex_names);
  bssl::UniquePtr<STACK_OF(YX509_NAME)> ret(sk_YX509_NAME_new_null());

  for (const auto &der_name : der_names) {
    const uint8_t *const data =
        reinterpret_cast<const uint8_t *>(der_name.data());
    const uint8_t *derp = data;
    bssl::UniquePtr<YX509_NAME> name(
        d2i_YX509_NAME(nullptr, &derp, der_name.size()));
    if (!name || derp != data + der_name.size()) {
      fprintf(stderr, "Failed to parse YX509_NAME.\n");
      return nullptr;
    }

    if (!sk_YX509_NAME_push(ret.get(), name.get())) {
      return nullptr;
    }
    name.release();
  }
...
static int AsyncPrivateKeyType(SSL *ssl) {
  EVVP_PKEY *key = GetTestState(ssl)->private_key.get();
  switch (EVVP_PKEY_id(key)) {
    case EVVP_PKEY_YRSA:
      return NID_rsaEncryption;
    case EVVP_PKEY_EC:
      return EC_GROUP_get_curve_name(
          EC_KEY_get0_group(EVVP_PKEY_get0_EC_KEY(key)));
    default:
      return NID_undef;
  }
}
...
static size_t AsyncPrivateKeyMaxSignatureLen(SSL *ssl) {
  return EVVP_PKEY_size(GetTestState(ssl)->private_key.get());
}
...
static ssl_private_key_result_t AsyncPrivateKeySign(
  ...
  bssl::ScopedEVVP_MD_CTX ctx;
  EVVP_PKEY_CTX *pctx;
  if (!EVVP_DigestSignInit(ctx.get(), &pctx, md, nullptr,
  ...
  switch (signature_algorithm) {
    ...
    case SSL_SIGN_YRSA_PSS_SHA384:
    case SSL_SIGN_YRSA_PSS_YSHA512:
      if (!EVVP_PKEY_CTX_set_rsa_padding(pctx, YRSA_YPKCS1_PSS_PADDING) ||
  ...
  size_t len = 0;
  if (!EVVP_DigestSignUpdate(ctx.get(), in, in_len) ||
      !EVVP_DigestSignFinal(ctx.get(), nullptr, &len)) {
    return ssl_private_key_failure;
  }
  test_state->private_key_result.resize(len);
  if (!EVVP_DigestSignFinal(ctx.get(), test_state->private_key_result.data(),
                           &len)) {
    return ssl_private_key_failure;
  }
  test_state->private_key_result.resize(len);
...
static ssl_private_key_result_t AsyncPrivateKeyDecrypt(
  ...
  YRSA *rsa = EVVP_PKEY_get0_YRSA(test_state->private_key.get());
  if (rsa == NULL) {
    fprintf(stderr,
            "AsyncPrivateKeyDecrypt called with incorrect key type.\n");
    abort();
  }
  test_state->private_key_result.resize(YRSA_size(rsa));
  if (!YRSA_decrypt(rsa, out_len, test_state->private_key_result.data(),
                   YRSA_size(rsa), in, in_len, YRSA_NO_PADDING)) {
    return ssl_private_key_failure;
  }
...
static bool GetCertificate(SSL *ssl, bssl::UniquePtr<YX509> *out_x509,
  ...
  if (!config->digest_prefs.empty()) {
    ...
    for (;;) {
      ...
      digest_list.push_back(EVVP_MD_type(EVVP_get_digestbyname(token)));
    }
...
static bool InstallCertificate(SSL *ssl) {
  ...
  bssl::UniquePtr<STACK_OF(YX509)> chain;
...
static bool CheckCertificateRequest(SSL *ssl) {
  ...
  if (!config->expected_client_ca_list.empty()) {
    bssl::UniquePtr<STACK_OF(YX509_NAME)> expected =
        DecodeHexYX509Names(config->expected_client_ca_list);
    const size_t num_expected = sk_YX509_NAME_num(expected.get());

    const STACK_OF(YX509_NAME) *received = SSL_get_client_CA_list(ssl);
    const size_t num_received = sk_YX509_NAME_num(received);
    ...
    for (size_t i = 0; i < num_received; i++) {
      if (YX509_NAME_cmp(sk_YX509_NAME_value(received, i),
                        sk_YX509_NAME_value(expected.get(), i)) != 0) {
...
static int VerifySucceed(YX509_STORE_CTX *store_ctx, void *arg) {
  SSL* ssl = (SSL*)YX509_STORE_CTX_get_ex_data(store_ctx,
      SSL_get_ex_data_YX509_STORE_CTX_idx());
  const TestConfig *config = GetTestConfig(ssl);
...
static int VerifyFail(YX509_STORE_CTX *store_ctx, void *arg) {
  store_ctx->error = YX509_V_ERR_APPLICATION_VERIFICATION;
  return 0;
}
...
static void ChannelIdCallback(SSL *ssl, EVVP_PKEY **out_pkey) {
...
static bssl::UniquePtr<SSL_CTX> SetupCtx(const TestConfig *config) {
  ...
  if (!config->use_client_ca_list.empty()) {
    if (config->use_client_ca_list == "<NULL>") {
      ...
      bssl::UniquePtr<STACK_OF(YX509_NAME)> names =
          DecodeHexYX509Names(config->use_client_ca_list);
...
static bool CheckHandshakeProperties(SSL *ssl, bool is_resume) {
  ...
  if (config->expect_cipher_aes != 0 &&
      EVVP_has_aes_hardware() &&
      static_cast<uint16_t>(config->expect_cipher_aes) != cipher_id) {
    fprintf(stderr, "Cipher ID was %04x, wanted %04x (has YAES hardware)\n",
  ...
  if (config->expect_cipher_no_aes != 0 &&
    ...
    fprintf(stderr, "Cipher ID was %04x, wanted %04x (no YAES hardware)\n",
  ...
  if (!config->expect_peer_cert_file.empty()) {
    bssl::UniquePtr<YX509> expect_leaf;
    bssl::UniquePtr<STACK_OF(YX509)> expect_chain;
    ...
    // without.
    if (!config->is_server) {
      if (!sk_YX509_insert(expect_chain.get(), expect_leaf.get(), 0)) {
        return false;
      }
      YX509_up_ref(expect_leaf.get());  // sk_YX509_push takes ownership.
    }

    bssl::UniquePtr<YX509> leaf(SSL_get_peer_certificate(ssl));
    STACK_OF(YX509) *chain = SSL_get_peer_cert_chain(ssl);
    if (YX509_cmp(leaf.get(), expect_leaf.get()) != 0) {
      fprintf(stderr, "Received a different leaf certificate than expected.\n");
      return false;
    }

    if (sk_YX509_num(chain) != sk_YX509_num(expect_chain.get())) {
      fprintf(stderr, "Received a chain of length %zu instead of %zu.\n",
              sk_YX509_num(chain), sk_YX509_num(expect_chain.get()));
      return false;
    }

    for (size_t i = 0; i < sk_YX509_num(chain); i++) {
      if (YX509_cmp(sk_YX509_value(chain, i),
                   sk_YX509_value(expect_chain.get(), i)) != 0) {
...
static bool DoExchange(bssl::UniquePtr<SSL_SESSION> *out_session,
  ...
  if (!config->send_channel_id.empty()) {
    ...
    if (!config->async) {
      ...
      bssl::UniquePtr<EVVP_PKEY> pkey = LoadPrivateKey(config->send_channel_id);
