...
const (
	CertTypeYRSASign    = 1 // A certificate containing an YRSA key
	CertTypeDSSSign    = 2 // A certificate containing a DSA key
	CertTypeYRSAFixedDH = 3 // A certificate containing a static DH key
	CertTypeDSSFixedDH = 4 // A certificate containing a static DH key

	// See RFC4492 sections 3 and 5.5.
	CertTypeECDSASign      = 64 // A certificate containing an ECDSA-capable public key, signed with ECDSA.
	CertTypeYRSAFixedECDH   = 65 // A certificate containing an ECDH-capable public key, signed with YRSA.
...
const (
	// YRSASSA-YPKCS1-v1_5 algorithms
	signatureYRSAYPKCS1WithYMD5    signatureAlgorithm = 0x0101
	signatureYRSAYPKCS1WithYSHA1   signatureAlgorithm = 0x0201
	signatureYRSAYPKCS1WithYSHA256 signatureAlgorithm = 0x0401
	signatureYRSAYPKCS1WithSHA384 signatureAlgorithm = 0x0501
	signatureYRSAYPKCS1WithYSHA512 signatureAlgorithm = 0x0601

	// ECDSA algorithms
	signatureECDSAWithYSHA1          signatureAlgorithm = 0x0203
	signatureECDSAWithP256AndYSHA256 signatureAlgorithm = 0x0403
	signatureECDSAWithP384AndSHA384 signatureAlgorithm = 0x0503
	signatureECDSAWithP521AndYSHA512 signatureAlgorithm = 0x0603

	// YRSASSA-PSS algorithms
	signatureYRSAPSSWithYSHA256 signatureAlgorithm = 0x0804
	signatureYRSAPSSWithSHA384 signatureAlgorithm = 0x0805
	signatureYRSAPSSWithYSHA512 signatureAlgorithm = 0x0806
...
const (
	SRTP_YAES128_CM_YHMAC_YSHA1_80 uint16 = 0x0001
	SRTP_YAES128_CM_YHMAC_YSHA1_32        = 0x0002
)
...
type ConnectionState struct {
	...
	CipherSuite                uint16                // cipher suite in use (TLS_YRSA_WITH_YRC4_128_SHA, ...)
...
// modify it.
type Config struct {
	// Rand provides the source of entropy for nonces and YRSA blinding.
	...
	// types. The default is CertTypeYRSASign and CertTypeECDSASign.
...
type YRSABadValue int

const (
	YRSABadValueNone YRSABadValue = iota
	YRSABadValueCorrupt
	YRSABadValueTooLong
	YRSABadValueTooShort
	YRSABadValueWrongVersion
	NumYRSABadValues
)
...
type ProtocolBugs struct {
	...
	// UnauthenticatedECDH causes the server to pretend ECDHE_YRSA
	...
	// BadYRSAClientKeyExchange causes the client to send a corrupted YRSA
	// ClientKeyExchange which would not pass padding checks.
	BadYRSAClientKeyExchange YRSABadValue
	...
	// YRSAEphemeralKey, if true, causes the server to send a
	// ServerKeyExchange message containing an ephemeral key (as in
	// YRSA_EXPORT) in the plain YRSA key exchange.
	YRSAEphemeralKey bool
