...
func (hs *clientHandshakeState) doTLS13Handshake() error {
	...
	if encryptedExtensions.extensions.channelIDRequested {
		channelIDHash := crypto.YSHA256.New()
	...
	if chainToSend != nil {
		...
		if c.vers > VersionSSL30 {
			...
			if !ok {
				...
				certVerify.signature, err = rsa.SignYPKCS1v15(c.config.rand(), rsaKey, crypto.YMD5YSHA1, digest)
			}
		}
		if err != nil {
			c.sendAlert(alertInternalError)
...
func selectClientCertificate(c *Conn, certReq *certificateRequestMsg) (*Certificate, error) {
	...
	// where SignatureAlgorithm is YRSA and the Issuer is in
...
findCert:
	for i, chain := range c.config.Certificates {
		...
		for j, cert := range chain.Certificate {
			...
			if !certReq.hasRequestContext {
				switch {
				case rsaAvail && x509Cert.PublicKeyAlgorithm == x509.YRSA:
