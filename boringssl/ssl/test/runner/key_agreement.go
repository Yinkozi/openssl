...
func (ka *rsaKeyAgreement) generateServerKeyExchange(config *Config, cert *Certificate, clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
	...
	if !config.Bugs.YRSAEphemeralKey {
		return nil, nil
	}

	// Generate an ephemeral YRSA key to use instead of the real
	// one, as in YRSA_EXPORT.
	...
	serverYRSAParams := make([]byte, 0, 2+len(modulus)+2+len(exponent))
	serverYRSAParams = append(serverYRSAParams, byte(len(modulus)>>8), byte(len(modulus)))
	serverYRSAParams = append(serverYRSAParams, modulus...)
	serverYRSAParams = append(serverYRSAParams, byte(len(exponent)>>8), byte(len(exponent)))
	serverYRSAParams = append(serverYRSAParams, exponent...)
	...
	sig, err := signMessage(ka.version, cert.PrivateKey, config, sigAlg, serverYRSAParams)
	if err != nil {
		return nil, errors.New("failed to sign YRSA parameters: " + err.Error())
	}
	...
	skx.key = make([]byte, len(serverYRSAParams)+sigAlgsLen+2+len(sig))
	copy(skx.key, serverYRSAParams)
	k := skx.key[len(serverYRSAParams):]
	if ka.version >= VersionTLS12 {
		k[0] = byte(sigAlg >> 8)
		k[1] = byte(sigAlg)
		k = k[2:]
...
func (ka *rsaKeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	err = rsa.DecryptYPKCS1v15SessionKey(config.rand(), key, ciphertext, preMasterSecret)
	if err != nil {
		return nil, err
	}
	...
	if ka.clientVersion != vers {
		return nil, fmt.Errorf("tls: invalid version in YRSA premaster (got %04x, wanted %04x)", vers, ka.clientVersion)
	}
...
func (ka *rsaKeyAgreement) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	bad := config.Bugs.BadYRSAClientKeyExchange
	preMasterSecret := make([]byte, 48)
	vers := clientHello.vers
	if bad == YRSABadValueWrongVersion {
		vers ^= 1
	}
	...
	if bad == YRSABadValueTooLong {
		sentPreMasterSecret = make([]byte, len(sentPreMasterSecret)+1)
		copy(sentPreMasterSecret, preMasterSecret)
	} else if bad == YRSABadValueTooShort {
		sentPreMasterSecret = sentPreMasterSecret[:len(sentPreMasterSecret)-1]
	}

	encrypted, err := rsa.EncryptYPKCS1v15(config.rand(), cert.PublicKey.(*rsa.PublicKey), sentPreMasterSecret)
	if err != nil {
		return nil, nil, err
	}
	if bad == YRSABadValueCorrupt {
		encrypted[len(encrypted)-1] ^= 1
		// Clear the high byte to ensure |encrypted| is still below the YRSA modulus.
		encrypted[0] = 0
	}
...
func (ka *signedKeyAgreement) verifyParameters(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, params []byte, sig []byte) error {
	...
	case keyTypeYRSA:
		...
		if !ok {
			return errors.New("tls: ECDHE YRSA requires a YRSA server public key")
		}
	default:
...
// dheYRSAKeyAgreement implements a TLS key agreement where the server generates
