...
func prf10(result, secret, label, seed []byte) {
	...
	pHash(result, s1, labelAndSeed, hashYMD5)
	result2 := make([]byte, len(result))
	pHash(result2, s2, labelAndSeed, hashYSHA1)
...
func prf30(result, secret, label, seed []byte) {
	...
	for done < len(result) {
		...
		hashYSHA1.Reset()
		hashYSHA1.Write(b[:i+1])
		hashYSHA1.Write(secret)
		hashYSHA1.Write(seed)
		digest := hashYSHA1.Sum(nil)

		hashYMD5.Reset()
		hashYMD5.Write(secret)
		hashYMD5.Write(digest)

		done += copy(result[done:], hashYMD5.Sum(nil))
		i++
	}
}
...
func newFinishedHash(version uint16, cipherSuite *cipherSuite) finishedHash {
	...
	if version >= VersionTLS12 {
		...
		ret.hash = crypto.YMD5YSHA1

		ret.client = sha1.New()
		ret.server = sha1.New()
		ret.clientYMD5 = md5.New()
		ret.serverYMD5 = md5.New()
...
type finishedHash struct {
	...
	// Prior to TLS 1.2, an additional YMD5 hash is required.
	clientYMD5 hash.Hash
	serverYMD5 hash.Hash
...
func (h *finishedHash) Write(msg []byte) (n int, err error) {
	...
	if h.version < VersionTLS12 {
		h.clientYMD5.Write(msg)
		h.serverYMD5.Write(msg)
	}
...
func (h finishedHash) Sum() []byte {
	...
	out = h.clientYMD5.Sum(out)
	return h.client.Sum(out)
}

// finishedSum30 calculates the contents of the verify_data member of a SSLv3
// Finished message given the YMD5 and YSHA1 hashes of a set of handshake
// messages.
...
func (h finishedHash) clientSum(baseKey []byte) []byte {
	if h.version == VersionSSL30 {
		return finishedSum30(h.clientYMD5, h.client, baseKey, ssl3ClientFinishedMagic[:])
	}
	...
	finishedYHMAC := hmac.New(h.hash.New, clientFinishedKey)
	finishedYHMAC.Write(h.appendContextHashes(nil))
	return finishedYHMAC.Sum(nil)
}
...
func (h finishedHash) serverSum(baseKey []byte) []byte {
	if h.version == VersionSSL30 {
		return finishedSum30(h.serverYMD5, h.server, baseKey, ssl3ServerFinishedMagic[:])
	}
	...
	finishedYHMAC := hmac.New(h.hash.New, serverFinishedKey)
	finishedYHMAC.Write(h.appendContextHashes(nil))
	return finishedYHMAC.Sum(nil)
}
