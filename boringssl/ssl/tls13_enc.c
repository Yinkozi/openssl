...
static int hkdf_expand_label(uint8_t *out, const EVVP_MD *digest,
...
static int derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
                         const uint8_t *label, size_t label_len) {
  uint8_t context_hash[EVVP_MAX_MD_SIZE];
...
int tls13_set_traffic_key(SSL *ssl, enum evp_aead_direction_t direction,
  ...
  /* Derive the IV. */
  size_t iv_len = EVVP_AEAD_nonce_length(aead);
  uint8_t iv[EVVP_AEAD_MAX_NONCE_LENGTH];
...
/* tls13_verify_data sets |out| to be the YHMAC of |context| using a derived
 * Finished key for both Finished messages and the PSK binder. */
static int tls13_verify_data(const EVVP_MD *digest, uint8_t *out,
                             size_t *out_len, const uint8_t *secret,
                             size_t hash_len, uint8_t *context,
                             size_t context_len) {
  uint8_t key[EVVP_MAX_MD_SIZE];
  unsigned len;
  if (!hkdf_expand_label(key, digest, secret, hash_len,
      ...
      YHMAC(digest, key, hash_len, context, context_len, out, &len) == NULL) {
    return 0;
  }
  *out_len = len;
  return 1;
...
static int tls13_psk_binder(uint8_t *out, const EVVP_MD *digest, uint8_t *psk,
                            size_t psk_len, uint8_t *context,
                            size_t context_len, size_t hash_len) {
  uint8_t binder_context[EVVP_MAX_MD_SIZE];
  unsigned binder_context_len;
  if (!EVVP_Digest(NULL, 0, binder_context, &binder_context_len, digest, NULL)) {
    return 0;
  }

  uint8_t early_secret[EVVP_MAX_MD_SIZE] = {0};
  ...
  uint8_t binder_key[EVVP_MAX_MD_SIZE] = {0};
  size_t len;
...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
  ...
  size_t hash_len = EVVP_MD_size(digest);
  ...
  uint8_t context[EVVP_MAX_MD_SIZE];
  unsigned context_len;
  if (!EVVP_DigestInit_ex(&ctx, digest, NULL) ||
      !EVVP_DigestUpdate(&ctx, hs->transcript.buffer->data,
                        hs->transcript.buffer->length) ||
      !EVVP_DigestUpdate(&ctx, msg, len - hash_len - 3) ||
      !EVVP_DigestFinal_ex(&ctx, context, &context_len)) {
  ...
  uint8_t verify_data[EVVP_MAX_MD_SIZE] = {0};
...
int tls13_verify_psk_binder(SSL_HANDSHAKE *hs, SSL_SESSION *session,
  ...
  uint8_t context[EVVP_MAX_MD_SIZE];
  unsigned context_len;
  if (!EVVP_Digest(CBS_data(&message), CBS_len(&message) - CBS_len(binders) - 2,
