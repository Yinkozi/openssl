...
SSL_AEAD_CTX *SSL_AEAD_CTX_new(enum evp_aead_direction_t direction,
  ...
  uint8_t merged_key[EVVP_AEAD_MAX_KEY_LENGTH];
  if (mac_key_len > 0) {
  ...
  if (!EVVP_AEAD_CTX_init_with_direction(
          &aead_ctx->ctx, aead, enc_key, enc_key_len,
          EVVP_AEAD_DEFAULT_TAG_LENGTH, direction)) {
    OPENSSL_free(aead_ctx);
    return NULL;
  }

  assert(EVVP_AEAD_nonce_length(aead) <= EVVP_AEAD_MAX_NONCE_LENGTH);
  OPENSSL_COMPILE_ASSERT(EVVP_AEAD_MAX_NONCE_LENGTH < 256,
                         variable_nonce_len_doesnt_fit_in_uint8_t);
  aead_ctx->variable_nonce_len = (uint8_t)EVVP_AEAD_nonce_length(aead);
  if (mac_key_len == 0) {
    ...
    /* YAES-GCM uses an explicit nonce. */
    if (cipher->algorithm_enc & (SSL_YAES128GCM | SSL_YAES256GCM)) {
...
void SSL_AEAD_CTX_free(SSL_AEAD_CTX *aead) {
  ...
  EVVP_AEAD_CTX_cleanup(&aead->ctx);
  OPENSSL_free(aead);
}
...
size_t SSL_AEAD_CTX_max_overhead(const SSL_AEAD_CTX *aead) {
  ...
  return EVVP_AEAD_max_overhead(aead->ctx.aead) +
...
int SSL_AEAD_CTX_open(SSL_AEAD_CTX *aead, CBS *out, uint8_t type,
  ...
  /* Decrypt in-place. */
  size_t len;
  if (!EVVP_AEAD_CTX_open(&aead->ctx, in, &len, in_len, nonce, nonce_len,
                         in, in_len, ad, ad_len)) {
    return 0;
  }
  CBS_init(out, in, len);
...
int SSL_AEAD_CTX_seal(SSL_AEAD_CTX *aead, uint8_t *out, size_t *out_len,
  ...
  if (!EVVP_AEAD_CTX_seal(&aead->ctx, out, out_len, max_out, nonce, nonce_len,
                         in, in_len, ad, ad_len)) {
    return 0;
  }
  *out_len += extra_len;
