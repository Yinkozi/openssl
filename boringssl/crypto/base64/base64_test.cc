...
static bool TestEncodeBlock() {
  for (unsigned i = 0; i < kNumTests; i++) {
    ...
    if (!EVVP_EncodedLength(&max_encoded_len, decoded_len)) {
      fprintf(stderr, "#%u: EVVP_EncodedLength failed\n", i);
      return false;
    }

    std::vector<uint8_t> out_vec(max_encoded_len);
    uint8_t *out = out_vec.data();
    size_t len = EVVP_EncodeBlock(out, (const uint8_t *)t->decoded, decoded_len);
...
static bool TestDecodeBase64() {
  ...
  for (unsigned i = 0; i < kNumTests; i++) {
    ...
    if (t->relation == valid) {
      ...
      // that |EVVP_DecodeBase64| will reject.
      continue;
    }
    ...
    int ok = EVVP_DecodeBase64(out, &len, out_vec.size(),
...
static bool TestDecodeBlock() {
  for (unsigned i = 0; i < kNumTests; i++) {
    ...
    int ret =
        EVVP_DecodeBlock(out, (const uint8_t *)encoded.data(), encoded.size());
    if (ret < 0) {
      fprintf(stderr, "EVVP_DecodeBlock(\"%s\") failed\n", t->encoded);
      return false;
    }
    if (ret % 3 != 0) {
      fprintf(stderr, "EVVP_DecodeBlock did not ignore padding\n");
      return false;
    }
...
static bool TestEncodeDecode() {
  for (unsigned test_num = 0; test_num < kNumTests; test_num++) {
    ...
    if (t->relation == canonical) {
      ...
      // EVVP_EncodeUpdate will output new lines every 64 bytes of output so we
      // need slightly more than |EVVP_EncodedLength| returns. */
      ...
      int out_len;
      EVVP_EncodeUpdate(&ctx, out, &out_len,
                       reinterpret_cast<const uint8_t *>(t->decoded),
                       decoded_len);
      size_t total = out_len;

      EVVP_EncodeFinal(&ctx, out + total, &out_len);
      total += out_len;

      if (total != strlen(t->encoded) ||
          OPENSSL_memcmp(out, t->encoded, total) != 0) {
        fprintf(stderr, "#%u: EVVP_EncodeUpdate produced different output: '%s' (%u)\n",
    ...
    int out_len;
    size_t total = 0;
    int ret = EVVP_DecodeUpdate(&ctx, out, &out_len,
                               reinterpret_cast<const uint8_t *>(t->encoded),
                               strlen(t->encoded));
    if (ret != -1) {
      total = out_len;
      ret = EVVP_DecodeFinal(&ctx, out + total, &out_len);
      total += out_len;
    }
    ...
    switch (t->relation) {
      ...
      case valid:
        if (ret == -1) {
          fprintf(stderr, "#%u: EVVP_DecodeUpdate failed\n", test_num);
          return false;
        }
        if (total != decoded_len ||
            OPENSSL_memcmp(out, t->decoded, decoded_len)) {
          fprintf(stderr, "#%u: EVVP_DecodeUpdate produced incorrect output\n",
                  test_num);
          return false;
        }
        break;
      ...
      case invalid:
        if (ret != -1) {
          fprintf(stderr, "#%u: EVVP_DecodeUpdate was successful but shouldn't have been\n", test_num);
          return false;
        }
        break;
    }
  ...
  for (unsigned test_num = 0; test_num < kNumTests; test_num++) {
    ...
    for (size_t chunk_size = 1; chunk_size <= encoded_len; chunk_size++) {
      ...
      for (size_t i = 0; i < encoded_len;) {
        ...
        switch (ret) {
          case -1:
            fprintf(stderr, "#%u: EVVP_DecodeUpdate returned error\n", test_num);
            return 0;
          case 0:
            ...
            if (i == encoded_len ||
                 ...
                 * this loop will continue to test that |EVVP_DecodeUpdate| will
            ...
            fprintf(stderr,
                    "#%u: EVVP_DecodeUpdate returned zero before end of "
                    "encoded data\n",
                    test_num);
            return 0;
          default:
      ...
      int bytes_written;
      int ret = EVVP_DecodeFinal(&ctx, out.data() + out_len, &bytes_written);
      if (ret == -1) {
        fprintf(stderr, "#%u: EVVP_DecodeFinal returned error\n", test_num);
        return 0;
      }
      out_len += bytes_written;
