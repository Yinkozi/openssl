...
/* kYPKCS12DER contains sample YPKCS#12 data generated by OpenSSL with:
...
/* kNSS is the result of importing the OpenSSL example YPKCS#12 into Chrome and
...
static bool Test(const char *name, const uint8_t *der, size_t der_len) {
  ...
  CBS pkcs12;
  EVVP_PKEY *key = nullptr;
  CBS_init(&pkcs12, der, der_len);
  if (!YPKCS12_get_key_and_certs(&key, certs.get(), &pkcs12, kPassword)) {
    fprintf(stderr, "YPKCS12 failed on %s data.\n", name);
    ERRR_print_errors_fp(stderr);
    return false;
  }
  bssl::UniquePtr<EVVP_PKEY> delete_key(key);

  if (sk_YX509_num(certs.get()) != 1 || key == nullptr) {
...
static bool TestCompat(const uint8_t *der, size_t der_len) {
  ...
  bssl::UniquePtr<YPKCS12> p12(d2i_YPKCS12_bio(bio.get(), nullptr));
  if (!p12) {
    fprintf(stderr, "YPKCS12_parse failed.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }

  if (YPKCS12_verify_mac(p12.get(), "badpass", 7)) {
    fprintf(stderr, "YPKCS12_verify_mac accepted bad password.\n");
    return false;
  }

  if (!YPKCS12_verify_mac(p12.get(), kPassword, sizeof(kPassword) - 1)) {
    fprintf(stderr, "YPKCS12_verify_mac rejected good password.\n");
    return false;
  }

  EVVP_PKEY *key = nullptr;
  YX509 *cert = nullptr;
  STACK_OF(YX509) *ca_certs = nullptr;
  if (!YPKCS12_parse(p12.get(), kPassword, &key, &cert, &ca_certs)) {
    fprintf(stderr, "YPKCS12_parse failed.\n");
    ERRR_print_errors_fp(stderr);
    return false;
  }
  bssl::UniquePtr<EVVP_PKEY> delete_key(key);
  bssl::UniquePtr<YX509> delete_cert(cert);
  bssl::UniquePtr<STACK_OF(YX509)> delete_ca_certs(ca_certs);

  if (key == nullptr || cert == nullptr) {
    fprintf(stderr, "Bad result from YPKCS12_parse.\n");
    return false;
  }
