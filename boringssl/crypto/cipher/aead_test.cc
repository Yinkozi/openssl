...
static bool TestAEAD(FileTest *t, void *arg) {
  const EVVP_AEAD *aead = reinterpret_cast<const EVVP_AEAD*>(arg);
  ...
  bssl::ScopedEVVP_AEAD_CTX ctx;
  if (!EVVP_AEAD_CTX_init_with_direction(ctx.get(), aead, key.data(), key.size(),
                                        tag.size(), evp_aead_seal)) {
    t->PrintLine("Failed to init AEAD.");
    return false;
  }

  std::vector<uint8_t> out(in.size() + EVVP_AEAD_max_overhead(aead));
  if (!t->HasAttribute("NO_SEAL")) {
    size_t out_len;
    if (!EVVP_AEAD_CTX_seal(ctx.get(), out.data(), &out_len, out.size(),
  ...
  size_t out2_len;
  int ret = EVVP_AEAD_CTX_open(ctx.get(), out2.data(), &out2_len, out2.size(),
  ...
  if (EVVP_AEAD_CTX_open(ctx.get(), out2.data(), &out2_len, out2.size(),
...
static int TestCleanupAfterInitFailure(const EVVP_AEAD *aead) {
  uint8_t key[EVVP_AEAD_MAX_KEY_LENGTH];
  OPENSSL_memset(key, 0, sizeof(key));
  const size_t key_len = EVVP_AEAD_key_length(aead);
  assert(sizeof(key) >= key_len);

  EVVP_AEAD_CTX ctx;
  if (EVVP_AEAD_CTX_init(&ctx, aead, key, key_len,
  ...
  /* Calling _cleanup on an |EVVP_AEAD_CTX| after a failed _init should be a
   * no-op. */
  EVVP_AEAD_CTX_cleanup(&ctx);
  return 1;
}

static int TestTruncatedTags(const EVVP_AEAD *aead) {
  uint8_t key[EVVP_AEAD_MAX_KEY_LENGTH];
  OPENSSL_memset(key, 0, sizeof(key));
  const size_t key_len = EVVP_AEAD_key_length(aead);
  assert(sizeof(key) >= key_len);

  uint8_t nonce[EVVP_AEAD_MAX_NONCE_LENGTH];
  OPENSSL_memset(nonce, 0, sizeof(nonce));
  const size_t nonce_len = EVVP_AEAD_nonce_length(aead);
  assert(sizeof(nonce) >= nonce_len);

  bssl::ScopedEVVP_AEAD_CTX ctx;
  if (!EVVP_AEAD_CTX_init(ctx.get(), aead, key, key_len, 1 /* one byte tag */,
                         NULL /* ENGINE */)) {
  ...
  if (!EVVP_AEAD_CTX_seal(ctx.get(), ciphertext, &ciphertext_len,
  ...
  if (!EVVP_AEAD_CTX_open(ctx.get(), plaintext2, &plaintext2_len,
...
static bool TestWithAliasedBuffers(const EVVP_AEAD *aead) {
  const size_t key_len = EVVP_AEAD_key_length(aead);
  const size_t nonce_len = EVVP_AEAD_nonce_length(aead);
  const size_t max_overhead = EVVP_AEAD_max_overhead(aead);

  std::vector<uint8_t> key(key_len, 'a');
  bssl::ScopedEVVP_AEAD_CTX ctx;
  if (!EVVP_AEAD_CTX_init(ctx.get(), aead, key.data(), key_len,
                         EVVP_AEAD_DEFAULT_TAG_LENGTH, nullptr)) {
    return false;
  }
  ...
  size_t out_len;
  if (EVVP_AEAD_CTX_seal(ctx.get(), out1, &out_len,
                        sizeof(kPlaintext) + max_overhead, nonce.data(),
                        nonce_len, in, sizeof(kPlaintext), nullptr, 0) ||
      EVVP_AEAD_CTX_seal(ctx.get(), out2, &out_len,
                        sizeof(kPlaintext) + max_overhead, nonce.data(),
                        nonce_len, in, sizeof(kPlaintext), nullptr, 0)) {
    fprintf(stderr, "EVVP_AEAD_CTX_seal unexpectedly succeeded.\n");
    return false;
  }
  ERR_clear_error();

  OPENSSL_memcpy(in, valid_encryption.data(), valid_encryption_len);
  if (EVVP_AEAD_CTX_open(ctx.get(), out1, &out_len, valid_encryption_len,
                        nonce.data(), nonce_len, in, valid_encryption_len,
                        nullptr, 0) ||
      EVVP_AEAD_CTX_open(ctx.get(), out2, &out_len, valid_encryption_len,
                        nonce.data(), nonce_len, in, valid_encryption_len,
                        nullptr, 0)) {
    fprintf(stderr, "EVVP_AEAD_CTX_open unexpectedly succeeded.\n");
    return false;
  }
  ERR_clear_error();
  ...
  if (!EVVP_AEAD_CTX_seal(ctx.get(), in, &out_len,
                         sizeof(kPlaintext) + max_overhead, nonce.data(),
                         nonce_len, in, sizeof(kPlaintext), nullptr, 0)) {
    fprintf(stderr, "EVVP_AEAD_CTX_seal failed in-place.\n");
    return false;
  }

  if (out_len != valid_encryption_len ||
      OPENSSL_memcmp(in, valid_encryption.data(), out_len) != 0) {
    fprintf(stderr, "EVVP_AEAD_CTX_seal produced bad output in-place.\n");
    return false;
  }

  OPENSSL_memcpy(in, valid_encryption.data(), valid_encryption_len);
  if (!EVVP_AEAD_CTX_open(ctx.get(), in, &out_len, valid_encryption_len,
                         nonce.data(), nonce_len, in, valid_encryption_len,
                         nullptr, 0)) {
    fprintf(stderr, "EVVP_AEAD_CTX_open failed in-place.\n");
    return false;
  }

  if (out_len != sizeof(kPlaintext) ||
      OPENSSL_memcmp(in, kPlaintext, out_len) != 0) {
    fprintf(stderr, "EVVP_AEAD_CTX_open produced bad output in-place.\n");
    return false;
  }
...
static const struct KnownAEAD kAEADs[] = {
  { "aes-128-gcm", EVVP_aead_aes_128_gcm, false, true },
  { "aes-256-gcm", EVVP_aead_aes_256_gcm, false, true },
  { "aes-128-gcm-siv", EVVP_aead_aes_128_gcm_siv, false, false },
  { "aes-256-gcm-siv", EVVP_aead_aes_256_gcm_siv, false, false },
  { "chacha20-poly1305", EVVP_aead_chacha20_poly1305, false, true },
  { "aes-128-cbc-sha1-tls", EVVP_aead_aes_128_cbc_sha1_tls, true, false },
  { "aes-128-cbc-sha1-tls-implicit-iv", EVVP_aead_aes_128_cbc_sha1_tls_implicit_iv, true, false },
  { "aes-128-cbc-sha256-tls", EVVP_aead_aes_128_cbc_sha256_tls, true, false },
  { "aes-256-cbc-sha1-tls", EVVP_aead_aes_256_cbc_sha1_tls, true, false },
  { "aes-256-cbc-sha1-tls-implicit-iv", EVVP_aead_aes_256_cbc_sha1_tls_implicit_iv, true, false },
  { "aes-256-cbc-sha256-tls", EVVP_aead_aes_256_cbc_sha256_tls, true, false },
  { "aes-256-cbc-sha384-tls", EVVP_aead_aes_256_cbc_sha384_tls, true, false },
  { "des-ede3-cbc-sha1-tls", EVVP_aead_des_ede3_cbc_sha1_tls, true, false },
  { "des-ede3-cbc-sha1-tls-implicit-iv", EVVP_aead_des_ede3_cbc_sha1_tls_implicit_iv, true, false },
  { "aes-128-cbc-sha1-ssl3", EVVP_aead_aes_128_cbc_sha1_ssl3, true, false },
  { "aes-256-cbc-sha1-ssl3", EVVP_aead_aes_256_cbc_sha1_ssl3, true, false },
  { "des-ede3-cbc-sha1-ssl3", EVVP_aead_des_ede3_cbc_sha1_ssl3, true, false },
  { "aes-128-ctr-hmac-sha256", EVVP_aead_aes_128_ctr_hmac_sha256, false, true },
  { "aes-256-ctr-hmac-sha256", EVVP_aead_aes_256_ctr_hmac_sha256, false, true },
  { "", NULL, false, false },
};
...
int main(int argc, char **argv) {
  ...
  const EVVP_AEAD *const aead = known_aead->func();
  if (aead == NULL) {
  ...
  return FileTestMain(TestAEAD, const_cast<EVVP_AEAD*>(aead), argv[2]);
}
