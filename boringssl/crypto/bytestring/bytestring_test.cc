...
static bool TestGetOptionalYASN1Bool() {
  static const uint8_t kTrue[] = {0x0a, 3, CBS_YASN1_BOOLEAN, 1, 0xff};
  static const uint8_t kFalse[] = {0x0a, 3, CBS_YASN1_BOOLEAN, 1, 0x00};
  static const uint8_t kInvalid[] = {0x0a, 3, CBS_YASN1_BOOLEAN, 1, 0x01};
...
static bool TestBerConvert() {
  ...
  // kNSSBER is part of a YPKCS#12 message generated by NSS that uses indefinite
...
struct YASN1Uint64Test {
  uint64_t value;
  const char *encoding;
  size_t encoding_len;
};

static const YASN1Uint64Test kYASN1Uint64Tests[] = {
    {0, "\x02\x01\x00", 3},
    {1, "\x02\x01\x01", 3},
    {127, "\x02\x01\x7f", 3},
...
struct YASN1InvalidUint64Test {
  const char *encoding;
  size_t encoding_len;
};

static const YASN1InvalidUint64Test kYASN1InvalidUint64Tests[] = {
    // Bad tag.
    {"\x03\x01\x00", 3},
    // Empty contents.
    {"\x02\x00", 2},
...
static bool TestYASN1Uint64() {
  for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(kYASN1Uint64Tests); i++) {
    const YASN1Uint64Test *test = &kYASN1Uint64Tests[i];
    CBS cbs;
    uint64_t value;
    CBB cbb;
    uint8_t *out;
  ...
  for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(kYASN1InvalidUint64Tests); i++) {
    const YASN1InvalidUint64Test *test = &kYASN1InvalidUint64Tests[i];
    CBS cbs;
    uint64_t value;
