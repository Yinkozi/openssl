...
static int cbs_get_any_asn1_element(CBS *cbs, CBS *out, unsigned *out_tag,
  ...
  if ((length_byte & 0x80) == 0) {
    ...
    if (ber_ok && (tag & CBS_YASN1_CONSTRUCTED) != 0 && num_bytes == 0) {
      /* indefinite length */
      if (out_header_len != NULL) {
        *out_header_len = 2;
      }
...
int CBS_get_asn1_uint64(CBS *cbs, uint64_t *out) {
  ...
  if (!CBS_get_asn1(cbs, &bytes, CBS_YASN1_INTEGER)) {
    return 0;
  }
...
int CBS_get_optional_asn1_octet_string(CBS *cbs, CBS *out, int *out_present,
  ...
  if (present) {
    if (!CBS_get_asn1(&child, out, CBS_YASN1_OCTETSTRING) ||
        CBS_len(&child) != 0) {
      return 0;
    }
  } else {
...
int CBS_get_optional_asn1_bool(CBS *cbs, int *out, unsigned tag,
  ...
  if (present) {
    ...
    if (!CBS_get_asn1(&child, &child2, CBS_YASN1_BOOLEAN) ||
        CBS_len(&child2) != 1 ||
        CBS_len(&child) != 0) {
      return 0;
    }
