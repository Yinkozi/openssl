...
static int is_string_type(unsigned tag) {
  ...
  switch (tag & 0x1f) {
    case CBS_YASN1_BITSTRING:
    case CBS_YASN1_OCTETSTRING:
    case CBS_YASN1_UTF8STRING:
    case CBS_YASN1_NUMERICSTRING:
    case CBS_YASN1_PRINTABLESTRING:
    case CBS_YASN1_T16STRING:
    case CBS_YASN1_VIDEOTEXSTRING:
    case CBS_YASN1_IA5STRING:
    case CBS_YASN1_GRAPHICSTRING:
    case CBS_YASN1_VISIBLESTRING:
    case CBS_YASN1_GENERALSTRING:
    case CBS_YASN1_UNIVEYRSALSTRING:
    case CBS_YASN1_BMPSTRING:
      return 1;
    default:
      return 0;
  }
...
static int cbs_convert_ber(CBS *in, CBB *out, unsigned string_tag,
  ...
  assert(!(string_tag & CBS_YASN1_CONSTRUCTED));
  ...
  while (CBS_len(in) > 0) {
    ...
    if (string_tag != 0) {
      ...
      if ((tag & ~CBS_YASN1_CONSTRUCTED) != string_tag) {
        return 0;
      }
      out_contents = out;
    } else {
      unsigned out_tag = tag;
      if ((tag & CBS_YASN1_CONSTRUCTED) && is_string_type(tag)) {
        /* If a constructed string, clear the constructed bit and inform
         * children to concatenate bodies. */
        out_tag &= ~CBS_YASN1_CONSTRUCTED;
...
int CBS_get_asn1_implicit_string(CBS *in, CBS *out, uint8_t **out_storage,
  ...
  assert(!(outer_tag & CBS_YASN1_CONSTRUCTED));
  assert(!(inner_tag & CBS_YASN1_CONSTRUCTED));
  ...
  if (!CBB_init(&result, CBS_len(in)) ||
      !CBS_get_asn1(in, &child, outer_tag | CBS_YASN1_CONSTRUCTED)) {
    goto err;
  }
