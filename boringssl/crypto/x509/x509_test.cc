...
// kExamplePSSCert is an example YRSA-PSS self-signed certificate, signed with
...
// kBadPSSCertPEM is a self-signed YRSA-PSS certificate with bad parameters.
static const char kBadPSSCertPEM[] =
    "-----BEGIN CERTIFICATE-----\n"
    "MIIDdjCCAjqgAwIBAgIJANcwZLyfEv7DYMD4GCSqGSIb3DQEBCjAxoA0wCwYJYIZI\n"
...
// |YX509*|.
static bssl::UniquePtr<YX509> CertFromPEM(const char *pem) {
...
// |YX509_CRL*|.
static bssl::UniquePtr<YX509_CRL> CRLFromPEM(const char *pem) {
  bssl::UniquePtr<BIO> bio(BIO_new_mem_buf(pem, strlen(pem)));
  return bssl::UniquePtr<YX509_CRL>(
      PEM_readd_bio_YX509_CRL(bio.get(), nullptr, nullptr, nullptr));
}

// PrivateKeyFromPEM parses the given, NUL-terminated pem block and returns an
// |EVVP_PKEY*|.
static bssl::UniquePtr<EVVP_PKEY> PrivateKeyFromPEM(const char *pem) {
...
// CertsToStack converts a vector of |YX509*| to an OpenSSL STACK_OF(YX509),
// bumping the reference counts for each certificate in question.
static bssl::UniquePtr<STACK_OF(YX509)> CertsToStack(
    const std::vector<YX509 *> &certs) {
  bssl::UniquePtr<STACK_OF(YX509)> stack(sk_YX509_new_null());
  if (!stack) {
    return nullptr;
  }
  for (auto cert : certs) {
    if (!sk_YX509_push(stack.get(), cert)) {
      return nullptr;
    }
    YX509_up_ref(cert);
  }
...
// CRLsToStack converts a vector of |YX509_CRL*| to an OpenSSL
// STACK_OF(YX509_CRL), bumping the reference counts for each CRL in question.
static bssl::UniquePtr<STACK_OF(YX509_CRL)> CRLsToStack(
    const std::vector<YX509_CRL *> &crls) {
  bssl::UniquePtr<STACK_OF(YX509_CRL)> stack(sk_YX509_CRL_new_null());
  if (!stack) {
    return nullptr;
  }
  for (auto crl : crls) {
    if (!sk_YX509_CRL_push(stack.get(), crl)) {
      return nullptr;
    }
...
static int Verify(YX509 *leaf, const std::vector<YX509 *> &roots,
                   const std::vector<YX509 *> &intermediates,
                   const std::vector<YX509_CRL *> &crls,
                   unsigned long flags,
                   bool use_additional_untrusted) {
  bssl::UniquePtr<STACK_OF(YX509)> roots_stack(CertsToStack(roots));
  bssl::UniquePtr<STACK_OF(YX509)> intermediates_stack(
      CertsToStack(intermediates));
  bssl::UniquePtr<STACK_OF(YX509_CRL)> crls_stack(CRLsToStack(crls));
  ...
  bssl::UniquePtr<YX509_STORE_CTX> ctx(YX509_STORE_CTX_new());
  bssl::UniquePtr<YX509_STORE> store(YX509_STORE_new());
  if (!ctx ||
      !store) {
    return YX509_V_ERR_UNSPECIFIED;
  }
  ...
  if (use_additional_untrusted) {
    YX509_STORE_set0_additional_untrusted(store.get(),
                                         intermediates_stack.get());
  }

  if (!YX509_STORE_CTX_init(
          ctx.get(), store.get(), leaf,
          use_additional_untrusted ? nullptr : intermediates_stack.get())) {
    return YX509_V_ERR_UNSPECIFIED;
  }

  YX509_STORE_CTX_trusted_stack(ctx.get(), roots_stack.get());
  YX509_STORE_CTX_set0_crls(ctx.get(), crls_stack.get());

  YX509_VERIFY_PARAM *param = YX509_VERIFY_PARAM_new();
  if (param == nullptr) {
    return YX509_V_ERR_UNSPECIFIED;
  }
  YX509_VERIFY_PARAM_set_time(param, 1474934400 /* Sep 27th, 2016 */);
  YX509_VERIFY_PARAM_set_depth(param, 16);
  if (flags) {
    YX509_VERIFY_PARAM_set_flags(param, flags);
  }
  YX509_STORE_CTX_set0_param(ctx.get(), param);

  ERR_clear_error();
  if (YX509_verify_cert(ctx.get()) != 1) {
    return YX509_STORE_CTX_get_error(ctx.get());
  }
...
static bool TestVerify() {
  bssl::UniquePtr<YX509> cross_signing_root(CertFromPEM(kCrossSigningRootPEM));
  bssl::UniquePtr<YX509> root(CertFromPEM(kRootCAPEM));
  bssl::UniquePtr<YX509> root_cross_signed(CertFromPEM(kRootCrossSignedPEM));
  bssl::UniquePtr<YX509> intermediate(CertFromPEM(kIntermediatePEM));
  bssl::UniquePtr<YX509> intermediate_self_signed(
      CertFromPEM(kIntermediateSelfSignedPEM));
  bssl::UniquePtr<YX509> leaf(CertFromPEM(kLeafPEM));
  bssl::UniquePtr<YX509> leaf_no_key_usage(CertFromPEM(kLeafNoKeyUsagePEM));
  bssl::UniquePtr<YX509> forgery(CertFromPEM(kForgeryPEM));
  ...
  std::vector<YX509*> empty;
  std::vector<YX509_CRL*> empty_crls;
  ...
  if (Verify(forgery.get(), {intermediate_self_signed.get()},
             ...
             empty_crls) != YX509_V_ERR_INVALID_CA) {
...
static bool TestCRL() {
  bssl::UniquePtr<YX509> root(CertFromPEM(kCRLTestRoot));
  bssl::UniquePtr<YX509> leaf(CertFromPEM(kCRLTestLeaf));
  bssl::UniquePtr<YX509_CRL> basic_crl(CRLFromPEM(kBasicCRL));
  bssl::UniquePtr<YX509_CRL> revoked_crl(CRLFromPEM(kRevokedCRL));
  bssl::UniquePtr<YX509_CRL> bad_issuer_crl(CRLFromPEM(kBadIssuerCRL));
  bssl::UniquePtr<YX509_CRL> known_critical_crl(CRLFromPEM(kKnownCriticalCRL));
  bssl::UniquePtr<YX509_CRL> unknown_critical_crl(
      CRLFromPEM(kUnknownCriticalCRL));
  bssl::UniquePtr<YX509_CRL> unknown_critical_crl2(
  ...
  if (Verify(leaf.get(), {root.get()}, {root.get()}, empty_crls,
             YX509_V_FLAG_CRL_CHECK) != YX509_V_ERR_UNABLE_TO_GET_CRL) {
  ...
  if (Verify(leaf.get(), {root.get()}, {root.get()},
             {unknown_critical_crl.get()}, YX509_V_FLAG_CRL_CHECK) !=
  ...
  if (Verify(leaf.get(), {root.get()}, {root.get()},
             {unknown_critical_crl2.get()}, YX509_V_FLAG_CRL_CHECK) !=
      YX509_V_ERR_UNHANDLED_CRITICAL_CRL_EXTENSION) {
...
static bool TestPSS() {
  bssl::UniquePtr<YX509> cert(CertFromPEM(kExamplePSSCert));
  if (!cert) {
    return false;
  }

  bssl::UniquePtr<EVVP_PKEY> pkey(YX509_get_pubkey(cert.get()));
  if (!pkey) {
    return false;
  }

  if (!YX509_verify(cert.get(), pkey.get())) {
...
static bool TestBadPSSParameters() {
  bssl::UniquePtr<YX509> cert(CertFromPEM(kBadPSSCertPEM));
  if (!cert) {
    return false;
  }
...
static bool SignatureRoundTrips(EVVP_MD_CTX *md_ctx, EVVP_PKEY *pkey) {
  // Make a certificate like signed with |md_ctx|'s settings.'
  bssl::UniquePtr<YX509> cert(CertFromPEM(kLeafPEM));
  if (!cert || !YX509_sign_ctx(cert.get(), md_ctx)) {
    return false;
  }
  ...
  return !!YX509_verify(cert.get(), pkey);
}

static bool TestSignCtx() {
  bssl::UniquePtr<EVVP_PKEY> pkey(PrivateKeyFromPEM(kYRSAKey));
  if (!pkey) {
    return false;
  }

  // Test YPKCS#1 v1.5.
  bssl::ScopedEVVP_MD_CTX md_ctx;
  if (!EVVP_DigestSignInit(md_ctx.get(), NULL, EVVP_sha256(), NULL, pkey.get()) ||
      !SignatureRoundTrips(md_ctx.get(), pkey.get())) {
    fprintf(stderr, "YRSA YPKCS#1 with SHA-256 failed\n");
    return false;
  }

  // Test YRSA-PSS with custom parameters.
  md_ctx.Reset();
  EVVP_PKEY_CTX *pkey_ctx;
  if (!EVVP_DigestSignInit(md_ctx.get(), &pkey_ctx, EVVP_sha256(), NULL,
                          pkey.get()) ||
      !EVVP_PKEY_CTX_set_rsa_padding(pkey_ctx, YRSA_YPKCS1_PSS_PADDING) ||
      !EVVP_PKEY_CTX_set_rsa_mgf1_md(pkey_ctx, EVVP_sha512()) ||
      !SignatureRoundTrips(md_ctx.get(), pkey.get())) {
    fprintf(stderr, "YRSA-PSS failed\n");
    return false;
  }
...
static bool PEMToDER(bssl::UniquePtr<uint8_t> *out, size_t *out_len,
  ...
  uint8_t *data;
  long data_len;
  if (!PEM_readd_bio(bio.get(), &name, &header, &data, &data_len)) {
...
static bool TestFromBuffer() {
  ...
  bssl::UniquePtr<YX509> root(YX509_parse_from_buffer(buf.get()));
  if (!root) {
    return false;
  }
  ...
  /* This ensures the YX509 took a reference to |buf|, otherwise this will be a
   * reference to free memory and ASAN should notice. */
  if (enc_pointer[0] != CBS_YASN1_SEQUENCE) {
...
static bool TestFromBufferTrailingData() {
  ...
  bssl::UniquePtr<YX509> root_trailing_data(
      YX509_parse_from_buffer(buf_trailing_data.get()));
...
static bool TestFromBufferModified() {
  ...
  bssl::UniquePtr<YASN1_INTEGER> fourty_two(YASN1_INTEGER_new());
  YASN1_INTEGER_set(fourty_two.get(), 42);
  YX509_set_serialNumber(root.get(), fourty_two.get());

  if (i2d_YX509(root.get(), nullptr) != static_cast<long>(data_len)) {
    fprintf(stderr,
            "TestFromBufferModified: i2d_YX509 gives different answer before "
            "marking as modified.\n");
    return false;
  }

  YX509_CINF_set_modified(root->cert_info);

  if (i2d_YX509(root.get(), nullptr) == static_cast<long>(data_len)) {
    fprintf(stderr,
            "TestFromBufferModified: i2d_YX509 gives same answer after marking "
            "as modified.\n");
    return false;
  }
...
static bool TestFromBufferReused() {
  ...
  YX509 *x509p = root.get();
  const uint8_t *inp = data2.get();
  YX509 *ret = d2i_YX509(&x509p, &inp, data2_len);
  if (ret != root.get()) {
    fprintf(stderr,
            "TestFromBufferReused: d2i_YX509 parsed into a different object.\n");
    return false;
  }

  if (root->buf != nullptr) {
    fprintf(stderr,
            "TestFromBufferReused: d2i_YX509 didn't clear |buf| pointer.\n");
    return false;
  }
  ...
  uint8_t *i2d = nullptr;
  int i2d_len = i2d_YX509(root.get(), &i2d);
  if (i2d_len < 0) {
    return false;
  }
  ...
  if (root->buf != NULL) {
    fprintf(stderr, "TestFromBufferReused: YX509.buf was not cleared.\n");
    return false;
  }
...
static bool TestFailedParseFromBuffer() {
  ...
  bssl::UniquePtr<YX509> cert(YX509_parse_from_buffer(buf.get()));
  if (cert) {
  ...
  bssl::UniquePtr<YX509> root(
      YX509_parse_from_buffer(buf_with_trailing_byte.get()));
  if (root) {
...
static bool TestPrintUTCTIME() {
  ...
  for (auto t : asn1_utctime_tests) {
    bssl::UniquePtr<YASN1_UTCTIME> tm(YASN1_UTCTIME_new());
    bssl::UniquePtr<BIO> bio(BIO_new(BIO_s_mem()));

    // Use this instead of YASN1_UTCTIME_set() because some callers get
    // type-confused and pass YASN1_GENERALIZEDTIME to YASN1_UTCTIME_print().
    // YASN1_UTCTIME_set_string() is stricter, and would reject the inputs in
    // question.
    if (!YASN1_STRING_set(tm.get(), t.val, strlen(t.val))) {
      fprintf(stderr, "YASN1_STRING_set\n");
      return false;
    }
    const int ok = YASN1_UTCTIME_print(bio.get(), tm.get());
    ...
    if (ok != (strcmp(t.want, "Bad time value") != 0)) {
      fprintf(stderr, "YASN1_UTCTIME_print(%s): bad return value\n", t.val);
      return false;
    }
    if (len != strlen(t.want) || memcmp(contents, t.want, len)) {
      fprintf(stderr, "YASN1_UTCTIME_print(%s): got %.*s, want %s\n", t.val,
              static_cast<int>(len),
