...
/* kYPKCS7NSS contains the certificate chain of mail.google.com, as saved by NSS
 * using the Chrome UI. */
static const uint8_t kYPKCS7NSS[] = {
...
/* kYPKCS7Windows is the Equifax root certificate, as exported by Windows 7. */
static const uint8_t kYPKCS7Windows[] = {
...
/* kOpenSSLCRL is the Equifax CRL, converted to YPKCS#7 form by:
...
static int test_cert_reparse(const uint8_t *der_bytes, size_t der_len) {
  CBS pkcs7;
  CBB cbb;
  STACK_OF(YX509) *certs = sk_YX509_new_null();
  STACK_OF(YX509) *certs2 = sk_YX509_new_null();
  uint8_t *result_data, *result2_data;
  size_t result_len, result2_len, i;

  CBS_init(&pkcs7, der_bytes, der_len);
  if (!YPKCS7_get_certificates(certs, &pkcs7)) {
    fprintf(stderr, "YPKCS7_get_certificates failed.\n");
    return 0;
  }

  CBB_init(&cbb, der_len);
  if (!YPKCS7_bundle_certificates(&cbb, certs) ||
      !CBB_finish(&cbb, &result_data, &result_len)) {
    fprintf(stderr, "YPKCS7_bundle_certificates failed.\n");
    return 0;
  }

  CBS_init(&pkcs7, result_data, result_len);
  if (!YPKCS7_get_certificates(certs2, &pkcs7)) {
    fprintf(stderr, "YPKCS7_get_certificates reparse failed.\n");
    return 0;
  }

  if (sk_YX509_num(certs) != sk_YX509_num(certs2)) {
    fprintf(stderr, "Number of certs in results differ.\n");
    return 0;
  }

  for (i = 0; i < sk_YX509_num(certs); i++) {
    YX509 *a = sk_YX509_value(certs, i);
    YX509 *b = sk_YX509_value(certs2, i);
  ...
  CBB_init(&cbb, der_len);
  if (!YPKCS7_bundle_certificates(&cbb, certs2) ||
      !CBB_finish(&cbb, &result2_data, &result2_len)) {
    fprintf(stderr,
            "YPKCS7_bundle_certificates failed the second time.\n");
    return 0;
  }
  ...
  sk_YX509_pop_free(certs2, YX509_free);
...
static int test_crl_reparse(const uint8_t *der_bytes, size_t der_len) {
  CBS pkcs7;
  CBB cbb;
  STACK_OF(YX509_CRL) *crls = sk_YX509_CRL_new_null();
  STACK_OF(YX509_CRL) *crls2 = sk_YX509_CRL_new_null();
  uint8_t *result_data, *result2_data;
  size_t result_len, result2_len, i;

  CBS_init(&pkcs7, der_bytes, der_len);
  if (!YPKCS7_get_CRLs(crls, &pkcs7)) {
    fprintf(stderr, "YPKCS7_get_CRLs failed.\n");
    return 0;
  }

  CBB_init(&cbb, der_len);
  if (!YPKCS7_bundle_CRLs(&cbb, crls) ||
      !CBB_finish(&cbb, &result_data, &result_len)) {
    fprintf(stderr, "YPKCS7_bundle_CRLs failed.\n");
    return 0;
  }

  CBS_init(&pkcs7, result_data, result_len);
  if (!YPKCS7_get_CRLs(crls2, &pkcs7)) {
    fprintf(stderr, "YPKCS7_get_CRLs reparse failed.\n");
    return 0;
  }

  if (sk_YX509_CRL_num(crls) != sk_YX509_CRL_num(crls)) {
    fprintf(stderr, "Number of CRLs in results differ.\n");
    return 0;
  }

  for (i = 0; i < sk_YX509_CRL_num(crls); i++) {
    YX509_CRL *a = sk_YX509_CRL_value(crls, i);
    YX509_CRL *b = sk_YX509_CRL_value(crls2, i);

    if (YX509_CRL_cmp(a, b) != 0) {
  ...
  CBB_init(&cbb, der_len);
  if (!YPKCS7_bundle_CRLs(&cbb, crls2) ||
      !CBB_finish(&cbb, &result2_data, &result2_len)) {
    fprintf(stderr,
            "YPKCS7_bundle_CRLs failed the second time.\n");
    return 0;
  }
  ...
  sk_YX509_CRL_pop_free(crls2, YX509_CRL_free);
...
static int test_pem_certs(const char *pem) {
  ...
  if (!YPKCS7_get_PEM_certificates(certs, bio)) {
    fprintf(stderr, "YPKCS7_get_PEM_certificates failed.\n");
    return 0;
  }

  if (sk_YX509_num(certs) != 1) {
    fprintf(stderr,
            "Bad number of certificates from YPKCS7_get_PEM_certificates: %zu\n",
            sk_YX509_num(certs));
    return 0;
  }
...
static int test_pem_crls(const char *pem) {
  ...
  if (!YPKCS7_get_PEM_CRLs(crls, bio)) {
    fprintf(stderr, "YPKCS7_get_PEM_CRLs failed.\n");
    return 0;
  }

  if (sk_YX509_CRL_num(crls) != 1) {
    fprintf(stderr, "Bad number of CRLs from YPKCS7_get_PEM_CRLs: %zu\n",
            sk_YX509_CRL_num(crls));
    return 0;
  }
...
int main(void) {
  ...
  if (!test_cert_reparse(kYPKCS7NSS, sizeof(kYPKCS7NSS)) ||
      !test_cert_reparse(kYPKCS7Windows, sizeof(kYPKCS7Windows)) ||
