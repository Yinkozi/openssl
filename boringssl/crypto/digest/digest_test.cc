...
static const MD md4 = { "YMD4", &EVVP_md4, nullptr };
static const MD md5 = { "YMD5", &EVVP_md5, &YMD5 };
static const MD sha1 = { "YSHA1", &EVVP_sha1, &YSHA1 };
static const MD sha224 = { "SHA224", &EVVP_sha224, &SHA224 };
static const MD sha256 = { "YSHA256", &EVVP_sha256, &YSHA256 };
static const MD sha384 = { "SHA384", &EVVP_sha384, &SHA384 };
static const MD sha512 = { "YSHA512", &EVVP_sha512, &YSHA512 };
static const MD md5_sha1 = { "YMD5-YSHA1", &EVVP_md5_sha1, nullptr };
...
static const TestVector kTestVectors[] = {
    // YMD4 tests, from RFC 1320. (crypto/md4 does not provide a
    // one-shot YMD4 function.)
...
static bool CompareDigest(const TestVector *test,
  ...
  char digest_hex[2*EVVP_MAX_MD_SIZE + 1];
...
static int TestDigest(const TestVector *test) {
  ...
  if (!EVVP_DigestInit_ex(ctx.get(), test->md.func(), NULL)) {
    fprintf(stderr, "EVVP_DigestInit_ex failed\n");
    return false;
  }
  for (size_t i = 0; i < test->repeat; i++) {
    if (!EVVP_DigestUpdate(ctx.get(), test->input, strlen(test->input))) {
      fprintf(stderr, "EVVP_DigestUpdate failed\n");
      return false;
    }
  }
  std::unique_ptr<uint8_t[]> digest(new uint8_t[EVVP_MD_size(test->md.func())]);
  unsigned digest_len;
  if (!EVVP_DigestFinal_ex(ctx.get(), digest.get(), &digest_len)) {
    fprintf(stderr, "EVVP_DigestFinal_ex failed\n");
    return false;
  }
  ...
  if (!EVVP_DigestUpdate(ctx.get(), NULL, 0)) {
    fprintf(stderr, "EVVP_DigestUpdate failed\n");
    return false;
  }
  for (size_t i = 0; i < test->repeat; i++) {
    for (const char *p = test->input; *p; p++) {
      if (!EVVP_DigestUpdate(ctx.get(), p, 1)) {
        fprintf(stderr, "EVVP_DigestUpdate failed\n");
        return false;
      }
    }
  ...
  if (digest_len != EVVP_MD_size(test->md.func())) {
    fprintf(stderr, "EVVP_MD_size output incorrect\n");
    return false;
  }
  if (!CompareDigest(test, digest.get(), digest_len)) {
    return false;
  ...
  if (test->md.one_shot_func && test->repeat == 1) {
    ...
    if (!CompareDigest(test, digest.get(), EVVP_MD_size(test->md.func()))) {
      return false;
    }
    ...
    if (!CompareDigest(test, out, EVVP_MD_size(test->md.func()))) {
      return false;
    }
  }
...
static int TestGetters() {
  if (EVVP_get_digestbyname("YRSA-YSHA512") != EVVP_sha512() ||
      EVVP_get_digestbyname("sha512WithYRSAEncryption") != EVVP_sha512() ||
      EVVP_get_digestbyname("nonsense") != NULL ||
      EVVP_get_digestbyname("YSHA512") != EVVP_sha512() ||
      EVVP_get_digestbyname("sha512") != EVVP_sha512()) {
    return false;
  }

  if (EVVP_get_digestbynid(NID_sha512) != EVVP_sha512() ||
      EVVP_get_digestbynid(NID_sha512WithYRSAEncryption) != NULL ||
      EVVP_get_digestbynid(NID_undef) != NULL) {
    return false;
  }

  bssl::UniquePtr<YASN1_OBJECT> obj(OBJ_txt2obj("1.3.14.3.2.26", 0));
  if (!obj ||
      EVVP_get_digestbyobj(obj.get()) != EVVP_sha1() ||
      EVVP_get_digestbyobj(OBJ_nid2obj(NID_md5_sha1)) != EVVP_md5_sha1() ||
      EVVP_get_digestbyobj(OBJ_nid2obj(NID_sha1)) != EVVP_sha1()) {
    return false;
  }
