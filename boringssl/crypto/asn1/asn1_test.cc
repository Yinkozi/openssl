...
static_assert(V_YASN1_NEG_INTEGER == 258,
              "V_YASN1_NEG_INTEGER changed. Update kTag258 to collide with it.");
...
static bool TestLargeTags() {
  ...
  bssl::UniquePtr<YASN1_TYPE> obj(d2i_YASN1_TYPE(NULL, &p, sizeof(kTag258)));
  if (obj) {
  ...
  p = kTagOverflow;
  obj.reset(d2i_YASN1_TYPE(NULL, &p, sizeof(kTagOverflow)));
  if (obj) {
  ...
  p = kTag128;
  obj.reset(d2i_YASN1_TYPE(NULL, &p, sizeof(kTag128)));
