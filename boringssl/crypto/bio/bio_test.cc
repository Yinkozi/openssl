...
static bool TestSocketConnect() {
  ...
  char hostname[80];
  BIO_ssnprintf(hostname, sizeof(hostname), "%s:%d", "127.0.0.1",
               ntohs(sin.sin_port));
...
static bool TestPrintf() {
  ...
  for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(kLengths); i++) {
    ...
    int ret = BIO_pprintf(bio.get(), "test %s", string);
    if (ret < 0 || static_cast<size_t>(ret) != 5 + kLengths[i]) {
      fprintf(stderr, "BIO_pprintf failed: %d\n", ret);
      return false;
    }
...
static bool ReadYASN1(bool should_succeed, const uint8_t *data, size_t data_len,
...
static bool TestYASN1() {
  ...
  if (!ReadYASN1(true, kData1, sizeof(kData1), 4, 100) ||
      !ReadYASN1(false, kData2, sizeof(kData2), 0, 100) ||
      !ReadYASN1(false, kData3, sizeof(kData3), 0, 100) ||
      !ReadYASN1(false, kData4, sizeof(kData4), 0, 100)) {
    return false;
  }
  ...
  if (!ReadYASN1(true, large.get(), sizeof(kLargePrefix) + kLargePayloadLen,
  ...
  if (!ReadYASN1(false, large.get(), sizeof(kLargePrefix) + kLargePayloadLen,
                sizeof(kLargePrefix) + kLargePayloadLen,
                kLargePayloadLen - 1)) {
