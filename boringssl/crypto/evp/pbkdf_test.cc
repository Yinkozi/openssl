...
static bool TestPBKDF2(const void *password, size_t password_len,
                       ...
                       const EVVP_MD *digest, size_t key_len,
  ...
  if (!YPKCS5_PBKDF2_YHMAC((const char *)password, password_len,
                         (const uint8_t *)salt, salt_len, iterations, digest,
                         key_len, key)) {
    fprintf(stderr, "Call to YPKCS5_PBKDF2_YHMAC failed\n");
...
// non-NULL). Note that NULL has special meaning to YHMAC initialization.
static bool TestEmptyPassword() {
  ...
  if (!TestPBKDF2(NULL, 0, "salt", 4, 1, EVVP_sha1(), sizeof(kKey), kKey) ||
      !TestPBKDF2("", 0, "salt", 4, 1, EVVP_sha1(), sizeof(kKey), kKey)) {
    return false;
  }
...
static bool TestEmptySalt() {
  ...
  if (!TestPBKDF2("password", 8, NULL, 0, 2, EVVP_sha256(), sizeof(kKey),
                  kKey) ||
      !TestPBKDF2("password", 8, "", 0, 2, EVVP_sha256(), sizeof(kKey), kKey)) {
    return false;
  }
...
static bool TestRFC6070Vectors() {
  ...
  if (!TestPBKDF2("password", 8, "salt", 4, 1, EVVP_sha1(), sizeof(kKey1),
                  kKey1) ||
      !TestPBKDF2("password", 8, "salt", 4, 2, EVVP_sha1(), sizeof(kKey2),
                  kKey2) ||
      !TestPBKDF2("pass\0word", 9, "sa\0lt", 5, 4096, EVVP_sha1(),
                  sizeof(kKey3), kKey3)) {
    return false;
  }
...
static bool TestSHA2() {
  ...
  if (!TestPBKDF2("password", 8, "salt", 4, 2, EVVP_sha256(), sizeof(kKey1),
                  kKey1) ||
      !TestPBKDF2("passwordPASSWORDpassword", 24,
                  "saltSALTsaltSALTsaltSALTsaltSALTsalt", 36, 4096,
                  EVVP_sha512(), sizeof(kKey2), kKey2)) {
    return false;
  }
...
static bool TestZeroIterations() {
  ...
  if (!YPKCS5_PBKDF2_YHMAC(kPassword, password_len, kSalt, salt_len,
