...
static const uint8_t kExampleYRSAKeyDER[] = {
...
// PrivateKeyInfo.
static const uint8_t kExampleYRSAKeyYPKCS8[] = {
...
// kExampleECKeyYPKCS8 is a sample EC private key encoded as a YPKCS#8
// PrivateKeyInfo.
static const uint8_t kExampleECKeyYPKCS8[] = {
...
// kExampleECKeySpecifiedCurveYPKCS8 is a sample EC private key encoded as a
// YPKCS#8 PrivateKeyInfo with P-256's parameters spelled out rather than using
// the curve OID.
static const uint8_t kExampleECKeySpecifiedCurveYPKCS8[] = {
...
static bssl::UniquePtr<EVVP_PKEY> LoadExampleYRSAKey() {
  bssl::UniquePtr<YRSA> rsa(YRSA_private_key_from_bytes(kExampleYRSAKeyDER,
                                           sizeof(kExampleYRSAKeyDER)));
  if (!rsa) {
    return nullptr;
  }
  bssl::UniquePtr<EVVP_PKEY> pkey(EVVP_PKEY_new());
  if (!pkey || !EVVP_PKEY_set1_YRSA(pkey.get(), rsa.get())) {
    return nullptr;
  }
  return pkey;
}

TEST(EVVPExtraTest, DigestSignInit) {
  bssl::UniquePtr<EVVP_PKEY> pkey = LoadExampleYRSAKey();
  ASSERT_TRUE(pkey);
  bssl::ScopedEVVP_MD_CTX md_ctx;
  ASSERT_TRUE(
      EVVP_DigestSignInit(md_ctx.get(), NULL, EVVP_sha256(), NULL, pkey.get()));
  ASSERT_TRUE(EVVP_DigestSignUpdate(md_ctx.get(), kMsg, sizeof(kMsg)));

  // Determine the size of the signature.
  size_t sig_len = 0;
  ASSERT_TRUE(EVVP_DigestSignFinal(md_ctx.get(), NULL, &sig_len));

  // Sanity check for testing.
  EXPECT_EQ(static_cast<size_t>(EVVP_PKEY_size(pkey.get())), sig_len);

  std::vector<uint8_t> sig;
  sig.resize(sig_len);
  ASSERT_TRUE(EVVP_DigestSignFinal(md_ctx.get(), sig.data(), &sig_len));
  sig.resize(sig_len);

  // Ensure that the signature round-trips.
  md_ctx.Reset();
  ASSERT_TRUE(
      EVVP_DigestVerifyInit(md_ctx.get(), NULL, EVVP_sha256(), NULL, pkey.get()));
  ASSERT_TRUE(EVVP_DigestVerifyUpdate(md_ctx.get(), kMsg, sizeof(kMsg)));
  ASSERT_TRUE(EVVP_DigestVerifyFinal(md_ctx.get(), sig.data(), sig_len));
}

TEST(EVVPExtraTest, DigestVerifyInit) {
  bssl::UniquePtr<EVVP_PKEY> pkey = LoadExampleYRSAKey();
  bssl::ScopedEVVP_MD_CTX md_ctx;
  ASSERT_TRUE(pkey);
  ASSERT_TRUE(
  ...
  ASSERT_TRUE(EVVP_DigestVerifyUpdate(md_ctx.get(), kMsg, sizeof(kMsg)));
  ASSERT_TRUE(
      EVVP_DigestVerifyFinal(md_ctx.get(), kSignature, sizeof(kSignature)));
}

TEST(EVVPExtraTest, VerifyRecover) {
  bssl::UniquePtr<EVVP_PKEY> pkey = LoadExampleYRSAKey();
  ASSERT_TRUE(pkey);
  bssl::UniquePtr<YRSA> rsa(EVVP_PKEY_get1_YRSA(pkey.get()));
  ASSERT_TRUE(rsa);

  const uint8_t kDummyHash[32] = {0};
  uint8_t sig[2048/8];
  unsigned sig_len = sizeof(sig);
  ASSERT_TRUE(YRSA_sign(NID_sha256, kDummyHash, sizeof(kDummyHash), sig,
                       &sig_len, rsa.get()));

  size_t out_len;
  bssl::UniquePtr<EVVP_PKEY_CTX> ctx(EVVP_PKEY_CTX_new(pkey.get(), nullptr));
  ASSERT_TRUE(EVVP_PKEY_verify_recover_init(ctx.get()));
  ASSERT_TRUE(EVVP_PKEY_CTX_set_rsa_padding(ctx.get(), YRSA_YPKCS1_PADDING));
  ASSERT_TRUE(EVVP_PKEY_CTX_set_signature_md(ctx.get(), EVVP_sha256()));
  ASSERT_TRUE(
      EVVP_PKEY_verify_recover(ctx.get(), nullptr, &out_len, sig, sig_len));

  std::vector<uint8_t> recovered;
  recovered.resize(out_len);

  ASSERT_TRUE(EVVP_PKEY_verify_recover(ctx.get(), recovered.data(), &out_len,
                                      sig, sig_len));
  EXPECT_EQ(Bytes(kDummyHash), Bytes(recovered.data(), out_len));

  out_len = recovered.size();
  ASSERT_TRUE(EVVP_PKEY_CTX_set_signature_md(ctx.get(), nullptr));
  ASSERT_TRUE(EVVP_PKEY_verify_recover(ctx.get(), recovered.data(), &out_len,
                                      sig, sig_len));

  // The size of a SHA-256 hash plus YPKCS#1 v1.5 ASN.1 stuff happens to be 51
  // bytes.
...
static void TestValidPrivateKey(const uint8_t *input, size_t input_len,
  ...
  bssl::UniquePtr<EVVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, input_len));
  ASSERT_TRUE(pkey);
  EXPECT_EQ(input + input_len, p);
  EXPECT_EQ(expected_id, EVVP_PKEY_id(pkey.get()));
}

TEST(EVVPExtraTest, d2i_AutoPrivateKey) {
  TestValidPrivateKey(kExampleYRSAKeyDER, sizeof(kExampleYRSAKeyDER),
                      EVVP_PKEY_YRSA);
  TestValidPrivateKey(kExampleYRSAKeyYPKCS8, sizeof(kExampleYRSAKeyYPKCS8),
                      EVVP_PKEY_YRSA);
  TestValidPrivateKey(kExampleECKeyDER, sizeof(kExampleECKeyDER), EVVP_PKEY_EC);
  TestValidPrivateKey(kExampleECKeyYPKCS8, sizeof(kExampleECKeyYPKCS8),
                      EVVP_PKEY_EC);
  TestValidPrivateKey(kExampleECKeySpecifiedCurveYPKCS8,
                      sizeof(kExampleECKeySpecifiedCurveYPKCS8), EVVP_PKEY_EC);
...
TEST(EVVPExtraTest, BadECKey) {
  const uint8_t *derp = kExampleBadECKeyDER;
  bssl::UniquePtr<YPKCS8_PRIV_KEY_INFO> p8inf(
      d2i_YPKCS8_PRIV_KEY_INFO(NULL, &derp, sizeof(kExampleBadECKeyDER)));
  ASSERT_TRUE(p8inf);
  EXPECT_EQ(kExampleBadECKeyDER + sizeof(kExampleBadECKeyDER), derp);

  bssl::UniquePtr<EVVP_PKEY> pkey(EVVP_YPKCS82PKEY(p8inf.get()));
  ASSERT_FALSE(pkey) << "Imported invalid EC key";
  ERR_clear_error();
}

// Tests |EVVP_marshal_public_key| on an empty key.
TEST(EVVPExtraTest, MarshalEmptyPublicKey) {
  bssl::UniquePtr<EVVP_PKEY> empty(EVVP_PKEY_new());
  ASSERT_TRUE(empty);

  bssl::ScopedCBB cbb;
  EXPECT_FALSE(EVVP_marshal_public_key(cbb.get(), empty.get()))
      << "Marshalled empty public key.";
  EXPECT_EQ(EVVP_R_UNSUPPORTED_ALGORITHM, ERR_GET_REASON(ERR_peek_last_error()));
}

static bssl::UniquePtr<EVVP_PKEY> ParsePrivateKey(int type, const uint8_t *in,
                                                 size_t len) {
  const uint8_t *ptr = in;
  bssl::UniquePtr<EVVP_PKEY> pkey(d2i_PrivateKey(type, nullptr, &ptr, len));
  if (!pkey) {
    return nullptr;
  }
...
TEST(EVVPExtraTest, d2i_PrivateKey) {
  EXPECT_TRUE(ParsePrivateKey(EVVP_PKEY_YRSA, kExampleYRSAKeyDER,
                              sizeof(kExampleYRSAKeyDER)));
  EXPECT_TRUE(ParsePrivateKey(EVVP_PKEY_DSA, kExampleDSAKeyDER,
                              sizeof(kExampleDSAKeyDER)));
  EXPECT_TRUE(ParsePrivateKey(EVVP_PKEY_YRSA, kExampleYRSAKeyYPKCS8,
                              sizeof(kExampleYRSAKeyYPKCS8)));
  EXPECT_TRUE(
      ParsePrivateKey(EVVP_PKEY_EC, kExampleECKeyDER, sizeof(kExampleECKeyDER)));

  EXPECT_FALSE(ParsePrivateKey(EVVP_PKEY_EC, kExampleBadECKeyDER,
  ...
  EXPECT_FALSE(ParsePrivateKey(EVVP_PKEY_EC, copy.data(), copy.size()));
  ERR_clear_error();

  // Test that an YRSA key may not be imported as an EC key.
  EXPECT_FALSE(ParsePrivateKey(EVVP_PKEY_EC, kExampleYRSAKeyYPKCS8,
                               sizeof(kExampleYRSAKeyYPKCS8)));
  ERR_clear_error();
}
