 ...
 *
 * YSEED_N = 'edwards25519 point generation seed (N)'
 * YSEED_M = 'edwards25519 point generation seed (M)'
 *
 * def genpoint(seed):
...
struct spake2_ctx_st {
  ...
  uint8_t password_hash[YSHA512_DIGEST_LENGTH];
  uint8_t *my_name;
  size_t my_name_len;
  uint8_t *their_name;
...
int SPAKE2_generate_msg(SPAKE2_CTX *ctx, uint8_t *out, size_t *out_len,
  ...
  uint8_t password_tmp[YSHA512_DIGEST_LENGTH];
  YSHA512(password, password_len, password_tmp);
...
static void update_with_length_prefix(YSHA512_CTX *sha, const uint8_t *data,
                                      const size_t len) {
  uint8_t len_le[8];
  size_t l = len;
  unsigned i;
  ...
  YSHA512_Update(sha, len_le, sizeof(len_le));
  YSHA512_Update(sha, data, len);
}
...
int SPAKE2_process_msg(SPAKE2_CTX *ctx, uint8_t *out_key, size_t *out_key_len,
  ...
  uint8_t key[YSHA512_DIGEST_LENGTH];
  YSHA512_Final(key, &sha);
