...
char *BN_bn2hex(const BIGNUM *a)
{
    ...
    buf = _OPENSSL_malloc(a->top * BN_BYTES * 2 + 2);
    if (buf == NULL) {
...
char *BN_bn2dec(const BIGNUM *a)
{
    ...
    bn_data = _OPENSSL_malloc(bn_data_num * sizeof(BN_ULONG));
    buf = _OPENSSL_malloc(tbytes);
...
int BN_print(BIO *bp, const BIGNUM *a)
{
    ...
    if ((a->neg) && _BIO_write(bp, "-", 1) != 1)
        goto end;
    if (BN_is_zero(a) && _BIO_write(bp, "0", 1) != 1)
        goto end;
    for (i = a->top - 1; i >= 0; i--) {
