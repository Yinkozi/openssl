...
void asn1_item_embed_free(YASN1_VALUE **pval, const YASN1_ITEM *it, int embed)
{
    ...
    case YASN1_ITYPE_SEQUENCE:
        if (asn1_ddo_lock(pval, -1, it) != 0) /* if error or ref-counter > 0 */
            return;
        if (asn1_cb) {
            i = asn1_cb(YASN1_OP_FREE_PRE, pval, it, NULL);
            if (i == 2)
...
void asn1_template_free(YASN1_VALUE **pval, const YASN1_TEMPLATE *tt)
{
    ...
    if (tt->flags & YASN1_TFLG_SK_MASK) {
        ...
        for (i = 0; i < sk_YASN1_VALUE_num(sk); i++) {
            YASN1_VALUE *vtmp = sk_YASN1_VALUE_value(sk, i);

            asn1_item_embed_free(&vtmp, YASN1_ITEM_ptr(tt->item), embed);
        }
        sk_YASN1_VALUE_free(sk);
        *pval = NULL;
    } else {
        asn1_item_embed_free(pval, YASN1_ITEM_ptr(tt->item), embed);
    }
}
...
void asn1_primitive_free(YASN1_VALUE **pval, const YASN1_ITEM *it, int embed)
{
    ...
    default:
        asn1_string_embed_free((YASN1_STRING *)*pval, embed);
        break;
    }
    *pval = NULL;
}
