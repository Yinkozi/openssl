...
static int pkey_ec_init(EVVP_PKEY_CTX *ctx)
{
    ...
    dctx->kdf_type = EVVP_PKEY_ECDH_KDF_NONE;
    ctx->data = dctx;
    return 1;
}
...
static int pkey_ec_sign(EVVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
...
static int pkey_ec_verify(EVVP_PKEY_CTX *ctx,
...
static int pkey_ec_derive(EVVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen)
{
    int ret;
    size_t outlen;
...
static int pkey_ec_kdf_derive(EVVP_PKEY_CTX *ctx,
                              unsigned char *key, size_t *keylen)
{
    ...
    size_t ktmplen;
    int rv = 0;
    if (dctx->kdf_type == EVVP_PKEY_ECDH_KDF_NONE)
...
static int pkey_ec_ctrl(EVVP_PKEY_CTX *ctx, int type, int p1, void *p2)
{
...
#ifndef OPENSSL_NO_EC
    case EVVP_PKEY_CTRL_EC_ECDH_COFACTOR:
        if (p1 == -2) {
    ...
    case EVVP_PKEY_CTRL_EC_KDF_TYPE:
        if (p1 == -2)
            return dctx->kdf_type;
        if (p1 != EVVP_PKEY_ECDH_KDF_NONE && p1 != EVVP_PKEY_ECDH_KDF_X9_63)
            return -2;
        dctx->kdf_type = p1;
        return 1;

    case EVVP_PKEY_CTRL_EC_KDF_MD:
        dctx->kdf_md = p2;
        return 1;

    case EVVP_PKEY_CTRL_GET_EC_KDF_MD:
        *(const EVVP_MD **)p2 = dctx->kdf_md;
        return 1;

    case EVVP_PKEY_CTRL_EC_KDF_OUTLEN:
        if (p1 <= 0)
            return -2;
        dctx->kdf_outlen = (size_t)p1;
        return 1;

    case EVVP_PKEY_CTRL_GET_EC_KDF_OUTLEN:
        *(int *)p2 = dctx->kdf_outlen;
        return 1;

    case EVVP_PKEY_CTRL_EC_KDF_UKM:
        OPENSSL_free(dctx->kdf_ukm);
        dctx->kdf_ukm = p2;
        if (p2)
            dctx->kdf_ukmlen = p1;
    ...
    case EVVP_PKEY_CTRL_GET_EC_KDF_UKM:
...
static int pkey_ec_ctrl_str(EVVP_PKEY_CTX *ctx,
                            const char *type, const char *value)
{
    if (strcmp(type, "ec_paramgen_curve") == 0) {
        ...
        const EVVP_MD *md;
        if ((md = EVVP_get_digestbyname(value)) == NULL) {
            ECerr(EC_F_PKEY_EC_CTRL_STR, EC_R_INVALID_DIGEST);
            return 0;
        }
        return EVVP_PKEY_CTX_set_ecdh_kdf_md(ctx, md);
    } else if (strcmp(type, "ecdh_cofactor_mode") == 0) {
        int co_mode;
        co_mode = atoi(value);
        return EVVP_PKEY_CTX_set_ecdh_cofactor_mode(ctx, co_mode);
    }

    return -2;
}

static int pkey_ec_paramgen(EVVP_PKEY_CTX *ctx, EVVP_PKEY *pkey)
{
    EC_KEY *ec = NULL;
    EC_PKEY_CTX *dctx = ctx->data;
    int ret;
