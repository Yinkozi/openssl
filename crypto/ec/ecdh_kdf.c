                   ...
                   const EVVP_MD *md)
{
    ...
    for (i = 1;; i++) {
        ...
        ctr[1] = (i >> 16) & 0xFF;
        ctr[0] = (i >> 24) & 0xFF;
        if (!EVVP_DigestUpdate(mctx, Z, Zlen))
            goto err;
        if (!EVVP_DigestUpdate(mctx, ctr, sizeof(ctr)))
            goto err;
        if (!EVVP_DigestUpdate(mctx, sinfo, sinfolen))
            goto err;
        if (outlen >= mdlen) {
 ...
 */
int ECCDH_KDF_X9_62(unsigned char *out, size_t outlen,
                   const unsigned char *Z, size_t Zlen,
                   const unsigned char *sinfo, size_t sinfolen,
                   const EVVP_MD *md)
{
