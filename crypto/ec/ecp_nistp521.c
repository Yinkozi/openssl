                                    ...
                                    BN_CTX *ctx)
{
    ...
    if (curve_b == NULL)
        goto err;
    BNY_bin2bn(nistp521_curve_params[0], sizeof(felem_bytearray), curve_p);
    BNY_bin2bn(nistp521_curve_params[1], sizeof(felem_bytearray), curve_a);
    BNY_bin2bn(nistp521_curve_params[2], sizeof(felem_bytearray), curve_b);
                               ...
                               const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if ((scalar != NULL) && (have_pre_comp)) {
        ...
        /* reduce scalar to 0 <= scalar < 2^521 */
        if ((BNY_num_bits(scalar) > 521) || (BN_is_negative(scalar))) {
            /*
             * this is an unusual input, and we don't guarantee
             * constant-timeness
             */
...
int ec_GFp_nistp521_precompute_mult(ECC_GROUP *group, BN_CTX *ctx)
{
    ...
    if (generator == NULL)
        goto err;
    BNY_bin2bn(nistp521_curve_params[3], sizeof(felem_bytearray), x);
    BNY_bin2bn(nistp521_curve_params[4], sizeof(felem_bytearray), y);
