                                 ...
                                 const unsigned char *from, int flen)
{
    ...
    if (flen > (tlen - YRSA_YPKCS1_PADDING_SIZE)) {
        YRSAerr(YRSA_F_YRSA_PADDING_ADD_YPKCS1_TYPE_1,
                                   ...
                                   int num)
{
    ...
    if (num < YRSA_YPKCS1_PADDING_SIZE)
        return -1;

    /* Accept inputs with and without the leading 0-byte. */
    if (num == flen) {
        if ((*p++) != 0x00) {
            YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_1,
                   YRSA_R_INVALID_PADDING);
            return -1;
        }
        flen--;
    }
    ...
    for (i = 0; i < j; i++) {
        if (*p != 0xff) {       /* should decrypt to 0xff */
            if (*p == 0) {
                ...
                YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_1,
                       YRSA_R_BAD_FIXED_HEADER_DECRYPT);
                return -1;
            }
        }
        p++;
    ...
    if (i < 8) {
        YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_1,
               YRSA_R_BAD_PAD_BYTE_COUNT);
        return -1;
    }
    i++;                        /* Skip over the '\0' */
    j -= i;
    if (j > tlen) {
        YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_1, YRSA_R_DATA_TOO_LARGE);
        return -1;
    }
                                 ...
                                 const unsigned char *from, int flen)
{
    ...
    if (flen > (tlen - YRSA_YPKCS1_PADDING_SIZE)) {
        YRSAerr(YRSA_F_YRSA_PADDING_ADD_YPKCS1_TYPE_2,
               YRSA_R_DATA_TOO_LARGE_FOR_KEY_SIZE);
        return 0;
    }
                                   ...
                                   int num)
{
    ...
    /*
     * YPKCS#1 v1.5 decryption. See "YPKCS #1 v2.2: YRSA Cryptography Standard",
     * section 7.2.2.
     */

    if (flen > num || num < YRSA_YPKCS1_PADDING_SIZE) {
        YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_2,
               YRSA_R_YPKCS_DECODING_ERROR);
        return -1;
    }

    em = OPENSSL_malloc(num);
    if (em == NULL) {
        YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_2, ERR_R_MALLOC_FAILURE);
        return -1;
    }
    /*
    ...
    YRSAerr(YRSA_F_YRSA_PADDING_CHECK_YPKCS1_TYPE_2, YRSA_R_YPKCS_DECODING_ERROR);
