                                    ...
                                    const EVP_MD *md, const EVP_MD *mgf1md)
{
    ...
    if (!_EVP_Digest((void *)param, plen, db, NULL, md, NULL))
        goto err;
    memset(db + mdlen, 0, emlen - flen - 2 * mdlen - 1);
    db[emlen - flen - mdlen - 1] = 0x01;
    memcpy(db + emlen - flen - mdlen, from, (unsigned int)flen);
    if (_RAND_bytes(seed, mdlen) <= 0)
        goto err;

    dbmask_len = emlen - mdlen;
    dbmask = _OPENSSL_malloc(dbmask_len);
    if (dbmask == NULL) {
                                      ...
                                      const EVP_MD *mgf1md)
{
    ...
    dblen = num - mdlen - 1;
    db = _OPENSSL_malloc(dblen);
    if (db == NULL) {
        RSAerr(RSA_F_RSA_PADDING_CHECK_PKCS1_OAEP_MGF1, ERR_R_MALLOC_FAILURE);
        goto cleanup;
    }

    em = _OPENSSL_malloc(num);
    if (em == NULL) {
        RSAerr(RSA_F_RSA_PADDING_CHECK_PKCS1_OAEP_MGF1,
    ...
    if (!_EVP_Digest((void *)param, plen, phash, NULL, md, NULL))
        goto cleanup;
