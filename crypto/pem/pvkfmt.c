...
static int do_i2b(unsigned char **out, EVP_PKEY *pk, int ispub)
{
    ...
    if (*out)
        ...
        if ((p = _OPENSSL_malloc(outlen)) == NULL) {
                          ...
                          const unsigned char *pass, int passlen)
{
    ...
    int rv = 1;
    if (mctx == NULL
        || !EVP_DigestInit_ex(mctx, _EVP_sha1(), NULL)
                             ...
                             pem_password_cb *cb, void *u)
{
    ...
    if (saltlen) {
        ...
        enctmp = _OPENSSL_malloc(keylen + 8);
        if (enctmp == NULL) {
        ...
        if (!_EVP_DecryptUpdate(cctx, q, &enctmplen, p, inlen))
            goto err;
        if (!_EVP_DecryptFinal_ex(cctx, q + enctmplen, &enctmplen))
            goto err;
                   ...
                   pem_password_cb *cb, void *u)
{
    ...
    if (*out != NULL) {
        ...
        start = p = _OPENSSL_malloc(outlen);
        if (p == NULL) {
    ...
    if (enclevel) {
        if (_RAND_bytes(p, PVK_SALTLEN) <= 0)
            goto error;
        salt = p;
        p += PVK_SALTLEN;
    }
