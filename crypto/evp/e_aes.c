...
static int s390x_aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_AEAD_SET_IVLEN:
        ...
        if (arg != 12) {
            ...
            if (gctx->ivlen == 12 || len > S390X_gcm_ivpadlen(gctx->ivlen)) {
                ...
                if ((gctx->iv = _OPENSSL_malloc(len)) == NULL) {
    ...
    case EVP_CTRL_COPY:
        ...
        if (gctx->iv == iv) {
            ...
            if ((gctx_out->iv = _OPENSSL_malloc(len)) == NULL) {
                EVPerr(EVP_F_S390X_AES_GCM_CTRL, ERR_R_MALLOC_FAILURE);
                return 0;
            }
...
static int aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_AEAD_SET_IVLEN:
        ...
        if ((arg > EVP_MAX_IV_LENGTH) && (arg > gctx->ivlen)) {
            if (gctx->iv != c->iv)
                _OPENSSL_free(gctx->iv);
            if ((gctx->iv = _OPENSSL_malloc(arg)) == NULL) {
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (gctx->iv == c->iv)
                ...
                if ((gctx_out->iv = _OPENSSL_malloc(gctx->ivlen)) == NULL) {
                    EVPerr(EVP_F_AES_GCM_CTRL, ERR_R_MALLOC_FAILURE);
                    return 0;
                }
                          ...
                          const unsigned char *in, size_t len)
{
    ...
    if (in != NULL) {
         ...
         * routines. We do it here rather than in _EVP_EncryptUpdate/
         * _EVP_DecryptUpdate because we need to pass full blocks of AAD too
