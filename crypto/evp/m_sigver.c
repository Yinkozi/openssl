...
int _EVP_DigestSignInit(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
                       const EVP_MD *type, ENGINE *e, EVP_PKEY *pkey)
{
    return do_sigver_init(ctx, pctx, type, e, pkey, 0);
}

int _EVP_DigestVerifyInit(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
                         const EVP_MD *type, ENGINE *e, EVP_PKEY *pkey)
{
                     ...
                     size_t siglen, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (_EVP_DigestVerifyUpdate(ctx, tbs, tbslen) <= 0)
        return -1;
    return _EVP_DigestVerifyFinal(ctx, sigret, siglen);
}
