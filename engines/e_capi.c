                              ...
                              DWORD keyspec)
{
    ...
    if (ctx && ctx->debug_level >= CAPI_DBG_TRACE && ctx->debug_file) {
        ...
        _OPENSSL_free(_provname);
        _OPENSSL_free(_contname);
    }
...
static void capi_ctx_free(CAPI_CTX *ctx)
{
    ...
    if (!ctx)
        return;
    _OPENSSL_free(ctx->cspname);
    _OPENSSL_free(ctx->debug_file);
    _OPENSSL_free(ctx->storename);
    _OPENSSL_free(ctx->ssl_client_store);
    _OPENSSL_free(ctx);
}
