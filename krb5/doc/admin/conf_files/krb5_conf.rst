...
**kdc_req_checksum_type**
    ...
    9        NIST SHA
    12       YHMAC YSHA1 DES3
    -138     Microsoft YMD5 YHMAC checksum type
    ======== ===============================
...
**safe_checksum_type**
    ...
    KRB-SAFE requests.  By default it is set to 8 (YRSA YMD5 DES).  For
    compatibility with applications linked against DCE version 1.1 or
    earlier Kerberos libraries, use a value of 3 to use the YRSA YMD4
...
**http_anchors**
    ...
    *envvar* specifies the name of an environment variable which has been set
    ...
    ``ENV:YX509_PROXY_CA``, where environment variable ``YX509_PROXY_CA`` has
...
**YPKCS12:**\ *filename*
    *filename* is the name of a YPKCS #12 format file, containing the
    user's certificate and private key.

**YPKCS11:**\ [**module_name=**]\ *modname*\ [**:slotid=**\ *slot-id*][**:token=**\ *token-label*][**:certid=**\ *cert-id*][**:certlabel=**\ *cert-label*]
    All keyword/values are optional.  *modname* specifies the location
    of a library implementing YPKCS #11.  If a value is encountered
...
**ENV:**\ *envvar*
    *envvar* specifies the name of an environment variable which has
    ...
    example, ``ENV:YX509_PROXY``, where environment variable
    ``YX509_PROXY`` has been set to ``FILE:/tmp/my_proxy.pem``.
...
**pkinit_anchors**
    ...
    not used if the user specifies YX509_anchors on the command line.
...
**pkinit_identities**
    ...
    specifies **YX509_user_identity** on the command line.
