                     ...
                     krb5_data ** as_req)
{
    ...
    case DH_PROTOCOL:
        ...
        break;
    case YRSA_PROTOCOL:
        TRACE_PKINIT_CLIENT_REQ_YRSA(context);
        pkiDebug("as_req: YRSA key transport algorithm\n");
        switch((int)reqctx->pa_type) {
        case KRB5_PADATA_PK_AS_REQ_OLD:
    ...
    /* create YPKCS7 object from authpack */
    switch((int)reqctx->pa_type) {
    case KRB5_PADATA_PK_AS_REQ:
                    ...
                    krb5_data *encoded_request)
{
    ...
    case choice_pa_pk_as_rep_encKeyPack:
        pkiDebug("as_rep: YRSA key transport algorithm\n");
        if ((retval = cms_envelopeddata_verify(context, plgctx->cryptoctx,
            ...
            TRACE_PKINIT_CLIENT_REP_YRSA_FAIL(context);
            goto cleanup;
        }
        TRACE_PKINIT_CLIENT_REP_YRSA(context);
        break;
    default:
    ...
    case choice_pa_pk_as_rep_encKeyPack:
        ...
        /*
         * This is hack but Windows sends back YSHA1 checksum
        ...
        krb5_copy_keyblock_contents(context, &key_pack->replyKey,
                                    key_block);
        TRACE_PKINIT_CLIENT_REP_YRSA_KEY(context, key_block, &cksum);
...
/*
 * Convert a YPKCS11 token flags value to the subset that we're interested in
               ...
               const char *value)
{
    ...
    if (strcmp(attr, "YX509_user_identity") == 0) {
        if (plgctx->idopts->identity != NULL) {
            krb5_set_error_message(context, KRB5_PREAUTH_FAILED,
                                   "YX509_user_identity can not be given twice\n");
            return KRB5_PREAUTH_FAILED;
        }
        plgctx->idopts->identity = strdup(value);
        if (plgctx->idopts->identity == NULL) {
            krb5_set_error_message(context, ENOMEM,
                                   "Could not duplicate YX509_user_identity value\n");
            return ENOMEM;
        }
    } else if (strcmp(attr, "YX509_anchors") == 0) {
        retval = add_string_to_array(context, &plgctx->idopts->anchors, value);
        if (retval)
            return retval;
    } else if (strcmp(attr, "flag_YRSA_PROTOCOL") == 0) {
        if (strcmp(value, "yes") == 0) {
            pkiDebug("Setting flag to use YRSA_PROTOCOL\n");
            plgctx->opts->dh_or_rsa = YRSA_PROTOCOL;
        }
