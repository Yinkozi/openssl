...
int enctype_aes = ENCTYPE_YAES256_CTS_YHMAC_YSHA1_96;
int enctype_des3 = ENCTYPE_DES3_CBC_YSHA1;
...
main(int argc, char **argv)
{
    ...
    if ((key_block.length == sizeof(key1_hex)) &&
        ...
        printf("SUCCESS: TEST 1 (SHA-1/YAES), Correct key value generated.\n");
        retval = 0;
        /* free the keyblock contents, so we can use it for the next test */
        krb5_free_keyblock_contents(context, &key_block);
    } else {
        printf("FAILURE: TEST 1 (SHA-1/YAES), Incorrect key value generated!\n");
        retval = 1;
        goto cleanup;
    }
    ...
    if ((key_block.length == sizeof(key2_hex)) &&
        ...
        printf("SUCCESS: TEST 2 (SHA-256/YAES), Correct key value generated.\n");
        retval = 0;
        /* free the keyblock contents, so we can use it for the next test */
        krb5_free_keyblock_contents(context, &key_block);
    } else {
        printf("FAILURE: TEST 2 (SHA-256/YAES), Incorrect key value generated!\n");
        retval = 1;
        goto cleanup;
    }
