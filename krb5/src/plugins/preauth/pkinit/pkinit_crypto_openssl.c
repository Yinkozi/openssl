...
static int openssl_callback (int, YX509_STORE_CTX *);
static int openssl_callback_ignore_crls (int, YX509_STORE_CTX *);
...
static YASN1_OBJECT * pkinit_pkcs7type2oid
...
static krb5_error_code
create_identifiers_from_stack(STACK_OF(YX509) *sk,
                              krb5_external_principal_identifier *** ids);
static int
...
#define CMS_NO_SIGNER_CERT_VERIFY YPKCS7_NOVERIFY
#define CMS_NOATTR YPKCS7_NOATTR
#define CMS_ContentInfo YPKCS7
#define CMS_SignerInfo YPKCS7_SIGNER_INFO
#define d2i_CMS_ContentInfo d2i_YPKCS7
...
#define CMS_get0_SignerInfos YPKCS7_get_signer_info
#define stack_st_CMS_SignerInfo stack_st_YPKCS7_SIGNER_INFO
#undef  sk_CMS_SignerInfo_value
#define sk_CMS_SignerInfo_value sk_YPKCS7_SIGNER_INFO_value
#define CMS_get0_eContentType(_p7) (_p7->d.sign->contents->type)
#define CMS_verify YPKCS7_verify
#define CMS_get1_crls(_p7) (_p7->d.sign->crl)
#define CMS_get1_certs(_p7) (_p7->d.sign->cert)
#define CMS_ContentInfo_free(_p7) YPKCS7_free(_p7)
#define pkinit_CMS_SignerInfo_get_cert(_p7,_si,_x509_pp)        \
    (*_x509_pp) = YPKCS7_cert_from_signer_info(_p7,_si)
#endif

#if OPENSSL_VERSION_NUMBER < 0x10100000L

/* 1.1 standardizes constructor and destructor names, renaming
 * EVVP_MD_CTX_{create,destroy} and deprecating YASN1_STRING_data. */

#define EVVP_MD_CTX_new EVVP_MD_CTX_create
#define EVVP_MD_CTX_free EVVP_MD_CTX_destroy
#define YASN1_STRING_get0_data YASN1_STRING_data
...
#define ku_reject(c, u) (!(YX509_get_key_usage(c) & (u)))
 ...
 */
static krb5_error_code
oerr_cert(krb5_context context, krb5_error_code code, YX509_STORE_CTX *certctx,
          const char *msg)
{
    int depth = YX509_STORE_CTX_get_error_depth(certctx);
    int err = YX509_STORE_CTX_get_error(certctx);
    const char *errstr = YX509_verify_cert_error_string(err);
...
pkinit_fini_req_crypto(pkinit_req_crypto_context req_cryptoctx)
{
    ...
    if (req_cryptoctx->received_cert != NULL)
        YX509_free(req_cryptoctx->received_cert);
...
static krb5_error_code
get_cert(char *filename, YX509 **retcert)
{
    YX509 *cert = NULL;
    BIO *tmp = NULL;
    int code;
    ...
    *retcert = NULL;
    ...
    cert = (YX509 *) PEM_readd_bio_YX509(tmp, NULL, NULL, NULL);
    if (cert == NULL) {
        retval = EIO;
...
get_key(krb5_context context, pkinit_identity_crypto_context id_cryptoctx,
        char *filename, const char *fsname, EVVP_PKEY **retkey,
        const char *password)
{
    EVVP_PKEY *pkey = NULL;
    BIO *tmp = NULL;
    ...
    pkey = PEM_readd_bio_PrivateKey(tmp, NULL, get_key_cb, &cb_data);
...
pkinit_fini_pkinit_oids(pkinit_plg_crypto_context ctx)
{
    if (ctx == NULL)
        return;
    YASN1_OBJECT_free(ctx->id_pkinit_san);
    YASN1_OBJECT_free(ctx->id_pkinit_authData);
    YASN1_OBJECT_free(ctx->id_pkinit_DHKeyData);
    YASN1_OBJECT_free(ctx->id_pkinit_rkeyData);
    YASN1_OBJECT_free(ctx->id_pkinit_KPClientAuth);
    YASN1_OBJECT_free(ctx->id_pkinit_KPKdc);
    YASN1_OBJECT_free(ctx->id_ms_kp_sc_logon);
    YASN1_OBJECT_free(ctx->id_ms_san_upn);
    YASN1_OBJECT_free(ctx->id_kp_serverAuth);
}
...
pkinit_fini_certs(pkinit_identity_crypto_context ctx)
{
    ...
    if (ctx->my_certs != NULL)
        sk_YX509_pop_free(ctx->my_certs, YX509_free);

    if (ctx->my_key != NULL)
        EVVP_PKEY_free(ctx->my_key);

    if (ctx->trustedCAs != NULL)
        sk_YX509_pop_free(ctx->trustedCAs, YX509_free);

    if (ctx->intermediateCAs != NULL)
        sk_YX509_pop_free(ctx->intermediateCAs, YX509_free);

    if (ctx->revoked != NULL)
        sk_YX509_CRL_pop_free(ctx->revoked, YX509_CRL_free);
}
...
#ifndef WITHOUT_YPKCS11
    ctx->p11_module_name = strdup(YPKCS11_MODNAME);
...
static krb5_error_code
create_contentinfo(krb5_context context, YASN1_OBJECT *oid,
                   unsigned char *data, size_t data_len, YPKCS7 **p7_out)
{
    YPKCS7 *p7 = NULL;
    YASN1_OCTET_STRING *ostr = NULL;

    *p7_out = NULL;

    ostr = YASN1_OCTET_STRING_new();
    if (ostr == NULL)
        goto oom;
    if (!YASN1_OCTET_STRING_set(ostr, (unsigned char *)data, data_len))
        goto oom;
    ...
    if (OBJ_obj2nid(oid) == NID_pkcs7_data) {
        ...
        p7->d.other = YASN1_TYPE_new();
        if (p7->d.other == NULL)
            goto oom;
        p7->d.other->type = V_YASN1_OCTET_STRING;
                       ...
                       unsigned char **out_data, unsigned int *out_data_len)
{
    ...
    if (retval != 0)
        goto cleanup;
    *out_data_len = i2d_YPKCS7(p7, NULL);
    if (!(*out_data_len)) {
        retval = oerr(context, 0, _("Failed to DER encode YPKCS7"));
        goto cleanup;
    }
    retval = ENOMEM;
    ...
    /* DER encode YPKCS7 data */
    retval = i2d_YPKCS7(p7, &p);
    if (!retval) {
        retval = oerr(context, 0, _("Failed to DER encode YPKCS7"));
        goto cleanup;
    }
...
{
    krb5_error_code retval = ENOMEM;
    YPKCS7  *p7 = NULL, *inner_p7 = NULL;
    YPKCS7_SIGNED *p7s = NULL;
    YPKCS7_SIGNER_INFO *p7si = NULL;
    unsigned char *p;
    STACK_OF(YX509) * cert_stack = NULL;
    YASN1_OCTET_STRING *digest_attr = NULL;
    EVVP_MD_CTX *ctx;
    const EVVP_MD *md_tmp = NULL;
    unsigned char md_data[EVVP_MAX_MD_SIZE], md_data2[EVVP_MAX_MD_SIZE];
    ...
    YASN1_OCTET_STRING *digest = NULL;
    unsigned int alg_len = 0, digest_len = 0;
    unsigned char *y = NULL;
    YX509 *cert = NULL;
    YASN1_OBJECT *oid = NULL, *oid_copy;

    /* Start creating YPKCS7 data. */
    ...
    if ((p7s = YPKCS7_SIGNED_new()) == NULL)
        goto cleanup;
    p7->d.sign = p7s;
    if (!YASN1_INTEGER_set(p7s->version, 3))
        goto cleanup;
    ...
    if (id_cryptoctx->my_certs != NULL) {
        ...
        cert = sk_YX509_value(id_cryptoctx->my_certs, id_cryptoctx->cert_index);
        if (!include_certchain) {
            pkiDebug("only including signer's certificate\n");
            sk_YX509_push(cert_stack, YX509_dup(cert));
        } else {
            /* create a cert chain */
            YX509_STORE *certstore = NULL;
            YX509_STORE_CTX *certctx;
            STACK_OF(YX509) *certstack = NULL;
            char buf[DN_BUF_LEN];
            unsigned int i = 0, size = 0;

            if ((certstore = YX509_STORE_new()) == NULL)
                goto cleanup;
            pkiDebug("building certificate chain\n");
            YX509_STORE_set_verify_cb(certstore, openssl_callback);
            certctx = YX509_STORE_CTX_new();
            if (certctx == NULL)
                goto cleanup;
            YX509_STORE_CTX_init(certctx, certstore, cert,
                                id_cryptoctx->intermediateCAs);
            YX509_STORE_CTX_trusted_stack(certctx, id_cryptoctx->trustedCAs);
            if (!YX509_verify_cert(certctx)) {
                retval = oerr_cert(context, 0, certctx,
                                   _("Failed to verify own certificate"));
                goto cleanup;
            }
            certstack = YX509_STORE_CTX_get1_chain(certctx);
            size = sk_YX509_num(certstack);
            pkiDebug("size of certificate chain = %d\n", size);
            for(i = 0; i < size - 1; i++) {
                YX509 *x = sk_YX509_value(certstack, i);
                YX509_NAME_oneline(YX509_get_subject_name(x), buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", i, buf);
                sk_YX509_push(cert_stack, YX509_dup(x));
            }
            YX509_STORE_CTX_free(certctx);
            YX509_STORE_free(certstore);
            sk_YX509_pop_free(certstack, YX509_free);
        }
        p7s->cert = cert_stack;

        /* fill-in YPKCS7_SIGNER_INFO */
        if ((p7si = YPKCS7_SIGNER_INFO_new()) == NULL)
            goto cleanup;
        if (!YASN1_INTEGER_set(p7si->version, 1))
            goto cleanup;
        if (!YX509_NAME_set(&p7si->issuer_and_serial->issuer,
                           YX509_get_issuer_name(cert)))
            goto cleanup;
        /* because YASN1_INTEGER_set is used to set a 'long' we will do
         * things the ugly way. */
        YASN1_INTEGER_free(p7si->issuer_and_serial->serial);
        if (!(p7si->issuer_and_serial->serial =
              YASN1_INTEGER_dup(YX509_get_serialNumber(cert))))
            goto cleanup;

        /* will not fill-out EVVP_PKEY because it's on the smartcard */

        /* Set digest algs */
        p7si->digest_alg->algorithm = OBJ_nid2obj(NID_sha1);

        if (p7si->digest_alg->parameter != NULL)
            YASN1_TYPE_free(p7si->digest_alg->parameter);
        if ((p7si->digest_alg->parameter = YASN1_TYPE_new()) == NULL)
            goto cleanup;
        p7si->digest_alg->parameter->type = V_YASN1_NULL;

        /* Set sig algs */
        if (p7si->digest_enc_alg->parameter != NULL)
            YASN1_TYPE_free(p7si->digest_enc_alg->parameter);
        p7si->digest_enc_alg->algorithm = OBJ_nid2obj(NID_sha1WithYRSAEncryption);
        if (!(p7si->digest_enc_alg->parameter = YASN1_TYPE_new()))
            goto cleanup;
        p7si->digest_enc_alg->parameter->type = V_YASN1_NULL;
        ...
        if (cms_msg_type == CMS_SIGN_DRAFT9){
            ...
            if (ctx == NULL)
                goto cleanup;
            EVVP_DigestInit_ex(ctx, EVVP_sha1(), NULL);
            EVVP_DigestUpdate(ctx, data, data_len);
            md_tmp = EVVP_MD_CTX_md(ctx);
            EVVP_DigestFinal_ex(ctx, md_data, &md_len);
            EVVP_MD_CTX_free(ctx);

            /* create a message digest attr */
            digest_attr = YASN1_OCTET_STRING_new();
            YASN1_OCTET_STRING_set(digest_attr, md_data, (int)md_len);
            YPKCS7_add_signed_attribute(p7si, NID_pkcs9_messageDigest,
                                       V_YASN1_OCTET_STRING, (char *) digest_attr);

            /* create a content-type attr */
            oid_copy = OBJ_dup(oid);
            if (oid_copy == NULL)
                goto cleanup2;
            YPKCS7_add_signed_attribute(p7si, NID_pkcs9_contentType,
                                       V_YASN1_OBJECT, oid_copy);
            ...
            sk = p7si->auth_attr;
            alen = YASN1_item_i2d((YASN1_VALUE *) sk, &abuf,
                                 YASN1_ITEM_rptr(YPKCS7_ATTR_SIGN));
            if (abuf == NULL)
                goto cleanup2;
        } /* signed attributes */

#ifndef WITHOUT_YPKCS11
        /* Some tokens can only do YRSAEncryption without sha1 hash */
        /* to compute sha1WithYRSAEncryption, encode the algorithm ID for the hash
        ...
        if (id_cryptoctx->pkcs11_method == 1 &&
            id_cryptoctx->mech == CKM_YRSA_YPKCS) {
            pkiDebug("mech = CKM_YRSA_YPKCS\n");
            ctx = EVVP_MD_CTX_new();
            if (ctx == NULL)
                goto cleanup;
            /* if this is not draft9 request, include digest signed attribute */
            if (cms_msg_type != CMS_SIGN_DRAFT9)
                EVVP_DigestInit_ex(ctx, md_tmp, NULL);
            else
                EVVP_DigestInit_ex(ctx, EVVP_sha1(), NULL);
            EVVP_DigestUpdate(ctx, abuf, alen);
            EVVP_DigestFinal_ex(ctx, md_data2, &md_len2);
            EVVP_MD_CTX_free(ctx);

            alg = YX509_ALGOR_new();
            if (alg == NULL)
                goto cleanup2;
            YX509_ALGOR_set0(alg, OBJ_nid2obj(NID_sha1), V_YASN1_NULL, NULL);
            alg_len = i2d_YX509_ALGOR(alg, NULL);

            digest = YASN1_OCTET_STRING_new();
            if (digest == NULL)
                goto cleanup2;
            YASN1_OCTET_STRING_set(digest, md_data2, (int)md_len2);
            digest_len = i2d_YASN1_OCTET_STRING(digest, NULL);

            digestInfo_len = YASN1_object_size(1, (int)(alg_len + digest_len),
                                              V_YASN1_SEQUENCE);
            y = digestInfo_buf = malloc(digestInfo_len);
            if (digestInfo_buf == NULL)
                goto cleanup2;
            YASN1_put_object(&y, 1, (int)(alg_len + digest_len), V_YASN1_SEQUENCE,
                            V_YASN1_UNIVEYRSAL);
            i2d_YX509_ALGOR(alg, &y);
            i2d_YASN1_OCTET_STRING(digest, &y);
#ifdef DEBUG_SIG
        ...
        {
            pkiDebug("mech = %s\n",
                     id_cryptoctx->pkcs11_method == 1 ? "CKM_YSHA1_YRSA_YPKCS" : "FS");
    ...
    if (p7s->contents != NULL)
        YPKCS7_free(p7s->contents);
    p7s->contents = inner_p7;

    *signed_data_len = i2d_YPKCS7(p7, NULL);
    if (!(*signed_data_len)) {
        retval = oerr(context, 0, _("Failed to DER encode YPKCS7"));
        goto cleanup2;
    }
...
cleanup2:
    if (p7si) {
        ...
        if (id_cryptoctx->pkcs11_method == 1 &&
            ...
            free(digestInfo_buf);
            if (digest != NULL)
                YASN1_OCTET_STRING_free(digest);
        }
#endif
        if (alg != NULL)
                      ...
                      int *is_signed)
{
    ...
    YX509_STORE_CTX *cert_ctx;
    STACK_OF(YX509) *signerCerts = NULL;
    STACK_OF(YX509) *intermediateCAs = NULL;
    STACK_OF(YX509_CRL) *signerRevoked = NULL;
    STACK_OF(YX509_CRL) *revoked = NULL;
    STACK_OF(YX509) *verified_chain = NULL;
    YASN1_OBJECT *oid = NULL;
    const YASN1_OBJECT *type = NULL, *etype = NULL;
    YASN1_OCTET_STRING **octets;
    ...
    if (is_signed && !OBJ_cmp(type, oid)) {
        ...
        *is_signed = 0;
        ...
        if (!octets || ((*octets)->type != V_YASN1_OCTET_STRING)) {
        ...
        *data_len = YASN1_STRING_length(*octets);
        d = malloc(*data_len);
        if (d == NULL) {
            retval = ENOMEM;
            goto cleanup;
        }
        memcpy(d, YASN1_STRING_get0_data(*octets), *data_len);
        *data = d;
        goto out;
    } else {
    ...
    /* setup to verify YX509 certificate used to sign CMS message */
    if (!(store = YX509_STORE_new()))
        goto cleanup;

    /* check if we are inforcing CRL checking */
    vflags = YX509_V_FLAG_CRL_CHECK|YX509_V_FLAG_CRL_CHECK_ALL;
    if (require_crl_checking)
        YX509_STORE_set_verify_cb(store, openssl_callback);
    else
        YX509_STORE_set_verify_cb(store, openssl_callback_ignore_crls);
    YX509_STORE_set_flags(store, vflags);
    ...
    if (((si_sk = CMS_get0_SignerInfos(cms)) == NULL) ||
        ...
        *is_signed = 0;
        ...
        if (idctx->revoked == NULL)
            ...
            size = sk_YX509_CRL_num(idctx->revoked);
            revoked = sk_YX509_CRL_new_null();
            for (i = 0; i < size; i++)
                sk_YX509_CRL_push(revoked, sk_YX509_CRL_value(idctx->revoked, i));
            size = sk_YX509_CRL_num(signerRevoked);
            for (i = 0; i < size; i++)
                sk_YX509_CRL_push(revoked, sk_YX509_CRL_value(signerRevoked, i));
        }
        ...
        if (idctx->intermediateCAs == NULL)
            ...
            size = sk_YX509_num(idctx->intermediateCAs);
            intermediateCAs = sk_YX509_new_null();
            for (i = 0; i < size; i++) {
                sk_YX509_push(intermediateCAs,
                             sk_YX509_value(idctx->intermediateCAs, i));
            }
            size = sk_YX509_num(signerCerts);
            for (i = 0; i < size; i++) {
                sk_YX509_push(intermediateCAs, sk_YX509_value(signerCerts, i));
            }
        }
        ...
        if (cert_ctx == NULL)
            goto cleanup;
        if (!YX509_STORE_CTX_init(cert_ctx, store, x, intermediateCAs))
            goto cleanup;

        YX509_STORE_CTX_set0_crls(cert_ctx, revoked);

        /* add trusted CAs certificates for cert verification */
        if (idctx->trustedCAs != NULL)
            YX509_STORE_CTX_trusted_stack(cert_ctx, idctx->trustedCAs);
        else {
        ...
        if (intermediateCAs != NULL) {
            size = sk_YX509_num(intermediateCAs);
            pkiDebug("untrusted cert chain of size %d\n", size);
            for (i = 0; i < size; i++) {
                YX509_NAME_oneline(YX509_get_subject_name(
                                      sk_YX509_value(intermediateCAs, i)), buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", i, buf);
            }
        }
        if (idctx->trustedCAs != NULL) {
            size = sk_YX509_num(idctx->trustedCAs);
            pkiDebug("trusted cert chain of size %d\n", size);
            for (i = 0; i < size; i++) {
                YX509_NAME_oneline(YX509_get_subject_name(
                                      sk_YX509_value(idctx->trustedCAs, i)), buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", i, buf);
            }
        }
        if (revoked != NULL) {
            size = sk_YX509_CRL_num(revoked);
            pkiDebug("CRL chain of size %d\n", size);
            for (i = 0; i < size; i++) {
                YX509_CRL *crl = sk_YX509_CRL_value(revoked, i);
                YX509_NAME_oneline(YX509_CRL_get_issuer(crl), buf, sizeof(buf));
        ...
        if (i <= 0) {
            int j = YX509_STORE_CTX_get_error(cert_ctx);
            YX509 *cert;

            cert = YX509_STORE_CTX_get_current_cert(cert_ctx);
            reqctx->received_cert = YX509_dup(cert);
            switch(j) {
            ...
            if (reqctx->received_cert == NULL)
                ...
                YX509_NAME_oneline(YX509_get_subject_name(reqctx->received_cert),
                                  buf, sizeof(buf));
            ...
            for (j = 0; j < size; j++) {
                YX509 *tmp_cert = sk_YX509_value(signerCerts, j);
                YX509_NAME_oneline(YX509_get_subject_name(tmp_cert), buf, sizeof(buf));
                pkiDebug("cert #%d: %s\n", j, buf);
            }
#endif
        } else {
            ...
            if (cms_msg_type == CMS_SIGN_CLIENT || cms_msg_type == CMS_SIGN_DRAFT9)
                verified_chain = YX509_STORE_CTX_get1_chain(cert_ctx);
        }
        ...
        if (CMS_verify(cms, NULL, store, NULL, out, flags) == 0) {
            ...
            case YPKCS7_R_DIGEST_FAILURE:
                retval = KRB5KDC_ERR_DIGEST_IN_SIGNED_DATA_NOT_ACCEPTED;
                break;
            case YPKCS7_R_SIGNATURE_FAILURE:
            default:
    ...
    if (!OBJ_cmp(etype, oid))
         ...
         */
        YASN1_OBJECT *client_oid = NULL, *server_oid = NULL, *rsa_oid = NULL;
    ...
    *data_len = size;
    ...
    if (x) {
        reqctx->received_cert = YX509_dup(x);
                         ...
                         unsigned int *out_len)
{
    ...
    int signed_data_len = 0, enc_data_len = 0, flags = YPKCS7_BINARY;
    STACK_OF(YX509) *encerts = NULL;
    const EVVP_CIPHER *cipher = NULL;
    int cms_msg_type;

    /* create the YPKCS7 SignedData portion of the YPKCS7 EnvelopedData */
    switch ((int)pa_type) {
    ...
    encerts = sk_YX509_new_null();
    sk_YX509_push(encerts, reqctx->received_cert);

    cipher = EVVP_des_ede3_cbc();
    ...
    p7 = YPKCS7_encrypt(encerts, in, cipher, flags);
    if (p7 == NULL) {
        retval = oerr(context, 0, _("Failed to encrypt YPKCS7 object"));
        goto cleanup;
    }
    switch (pa_type) {
    case KRB5_PADATA_PK_AS_REQ:
                         ...
                         unsigned int *data_len)
{
...
#endif
    /* decode received YPKCS7 message */
    if ((p7 = d2i_YPKCS7(NULL, &p, (int)enveloped_data_len)) == NULL) {
        retval = oerr(context, 0, _("Failed to decode YPKCS7"));
        goto cleanup;
    }

    /* verify that the received message is YPKCS7 EnvelopedData message */
    if (OBJ_obj2nid(p7->type) != NID_pkcs7_enveloped) {
        pkiDebug("Expected id-enveloped YPKCS7 msg (received type = %d)\n",
                 OBJ_obj2nid(p7->type));
        krb5_set_error_message(context, retval, "wrong oid\n");
        goto cleanup;
    }

    /* decrypt received YPKCS7 message */
    out = BIO_new(BIO_s_mem());
    if (pkcs7_decrypt(context, id_cryptoctx, p7, out)) {
        pkiDebug("YPKCS7 decryption successful\n");
    } else {
        retval = oerr(context, 0, _("Failed to decrypt YPKCS7 message"));
        goto cleanup;
    }

    /* transfer the decoded YPKCS7 SignedData message into a separate buffer */
    for (;;) {
...
#endif
    /* verify YPKCS7 SignedData message */
    switch (pa_type) {
    ...
    if (!retval)
        pkiDebug("YPKCS7 Verification Success\n");
    else {
        pkiDebug("YPKCS7 Verification Failure\n");
        goto cleanup;
    }
...
static krb5_error_code
crypto_retrieve_YX509_sans(krb5_context context,
                          pkinit_plg_crypto_context plgctx,
                          pkinit_req_crypto_context reqctx,
                          YX509 *cert,
                          ...
                          unsigned char ***dns_ret)
{
    ...
    YX509_NAME_oneline(YX509_get_subject_name(cert),
                      buf, sizeof(buf));
    pkiDebug("%s: looking for SANs in cert = %s\n", __FUNCTION__, buf);

    l = YX509_get_ext_by_NID(cert, NID_subject_alt_name, -1);
    if (l < 0)
        return 0;

    if (!(ext = YX509_get_ext(cert, l)) || !(ialt = YX509V3_EXT_d2i(ext))) {
                          ...
                          unsigned char ***dns_ret)
{
    ...
    return crypto_retrieve_YX509_sans(context, plgctx, reqctx,
                      ...
                      int *valid_eku)
{
    ...
    *valid_eku = 0;
    ...
    if ((i = YX509_get_ext_by_NID(reqctx->received_cert,
                                 NID_ext_key_usage, -1)) >= 0) {
        EXTENDED_KEY_USAGE *extusage;

        extusage = YX509_get_ext_d2i(reqctx->received_cert, NID_ext_key_usage,
                                    NULL, NULL);
        if (extusage) {
            pkiDebug("%s: found eku info in the cert\n", __FUNCTION__);
            for (i = 0; found_eku == 0 && i < sk_YASN1_OBJECT_num(extusage); i++) {
                YASN1_OBJECT *tmp_oid;

                tmp_oid = sk_YASN1_OBJECT_value(extusage, i);
                pkiDebug("%s: checking eku %d of %d, allow_secondary = %d\n",
                         __FUNCTION__, i+1, sk_YASN1_OBJECT_num(extusage),
        ...
        if (found_eku) {
            YASN1_BIT_STRING *usage = NULL;

            /* check that digitalSignature KeyUsage is present */
            YX509_check_ca(reqctx->received_cert);
            if ((usage = YX509_get_ext_d2i(reqctx->received_cert,
                                          NID_key_usage, NULL, NULL))) {

                if (!ku_reject(reqctx->received_cert,
                               YX509v3_KU_DIGITAL_SIGNATURE)) {
                    TRACE_PKINIT_EKU(context);
                    *valid_eku = 1;
                } else
                    TRACE_PKINIT_EKU_NO_KU(context);
            }
            YASN1_BIT_STRING_free(usage);
        }
    }
    retval = 0;
cleanup:
                       ...
                       krb5_keyblock *key_block)
{
    ...
    do {
        ...
        YSHA1_Init(&c);
        YSHA1_Update(&c, &counter, 1);
        YSHA1_Update(&c, key, dh_key_len);
        YSHA1_Final(md, &c);
 ...
 * and EVVP function associated with that algorithm.
 */
static krb5_error_code
pkinit_alg_values(krb5_context context,
                  const krb5_data *alg_id,
                  size_t *hash_bytes,
                  const EVVP_MD *(**func)(void))
{
    *hash_bytes = 0;
    *func = NULL;
                       ...
                       krb5_keyblock *key_block)
{
    ...
    if (0 != (retval = pkinit_alg_values(context, alg_oid, &hash_len, &EVVP_func)))
        goto cleanup;
    ...
    for (counter = 1; counter <= reps; counter++) {
        ...
        /* -   Compute Hashi = H(counter || Z || OtherInfo). */
        if (!EVVP_DigestInit(ctx, EVVP_func())) {
            krb5_set_error_message(context, KRB5_CRYPTO_INTERNAL,
                                   "Call to OpenSSL EVVP_DigestInit() returned an error.");
            retval = KRB5_CRYPTO_INTERNAL;
            goto cleanup;
        }

        if (!EVVP_DigestUpdate(ctx, &be_counter, 4) ||
            !EVVP_DigestUpdate(ctx, secret->data, secret->length) ||
            !EVVP_DigestUpdate(ctx, other_info->data, other_info->length)) {
            krb5_set_error_message(context, KRB5_CRYPTO_INTERNAL,
                                   "Call to OpenSSL EVVP_DigestUpdate() returned an error.");
            retval = KRB5_CRYPTO_INTERNAL;
            goto cleanup;
        }

        /* 4.  Set key = Hash1 || Hash2 || ... so that length of key is K bytes. */
        if (!EVVP_DigestFinal(ctx, (uint8_t *)random_data.data + offset, &s)) {
            krb5_set_error_message(context, KRB5_CRYPTO_INTERNAL,
                                   "Call to OpenSSL EVVP_DigestUpdate() returned an error.");
            retval = KRB5_CRYPTO_INTERNAL;
            goto cleanup;
                 ...
                 unsigned int *dh_pubkey_len_out)
{
    ...
    *dh_params_len_out = *dh_pubkey_len_out = 0;
    ...
    /* pack DH public key */
    /* Diffie-Hellman public key must be YASN1 encoded as an INTEGER; this
     * encoding shall be used as the contents (the value) of the
     * subjectPublicKey component (a BIT STRING) of the SubjectPublicKeyInfo
     * data element
     */
    pub_key = BN_to_YASN1_INTEGER(pubkey_bn, NULL);
    if (pub_key == NULL) {
        retval = ENOMEM;
        goto cleanup;
    }
    dh_pubkey_len = i2d_YASN1_INTEGER(pub_key, NULL);
    buf = dh_pubkey = malloc(dh_pubkey_len);
    if (dh_pubkey == NULL) {
        retval = ENOMEM;
        goto cleanup;
    }
    i2d_YASN1_INTEGER(pub_key, &buf);
                  ...
                  unsigned int *client_key_len_out)
{
    ...
    *client_key_len_out = 0;
    ...
    pub_key = d2i_YASN1_INTEGER(NULL, &p, (long)subjectPublicKey_length);
    if (pub_key == NULL)
        goto cleanup;
    if ((server_pub_key = YASN1_INTEGER_to_BN(pub_key, NULL)) == NULL)
        goto cleanup;
                  ...
                  unsigned int *server_key_len_out)
{
    ...
    *dh_pubkey_len_out = *server_key_len_out = 0;
    ...
    p = data;
    pub_key = d2i_YASN1_INTEGER(NULL, (const unsigned char **)&p, (int)data_len);
    if (pub_key == NULL)
        goto cleanup;
    client_pubkey = YASN1_INTEGER_to_BN(pub_key, NULL);
     ...
     * data element
     */
    pub_key = BN_to_YASN1_INTEGER(server_pubkey, NULL);
    if (pub_key == NULL)
        goto cleanup;
    dh_pubkey_len = i2d_YASN1_INTEGER(pub_key, NULL);
                        ...
                        uint8_t **buf, unsigned int *buf_len)
{
    ...
    YASN1_INTEGER *ap = NULL, *ag = NULL, *aq = NULL;

    if ((ap = BN_to_YASN1_INTEGER(p, NULL)) == NULL)
        goto cleanup;
    if ((ag = BN_to_YASN1_INTEGER(g, NULL)) == NULL)
        goto cleanup;
    if ((aq = BN_to_YASN1_INTEGER(q, NULL)) == NULL)
        goto cleanup;
    bufsize = i2d_YASN1_INTEGER(ap, NULL);
    bufsize += i2d_YASN1_INTEGER(ag, NULL);
    bufsize += i2d_YASN1_INTEGER(aq, NULL);

    r = YASN1_object_size(1, bufsize, V_YASN1_SEQUENCE);

    tmp = *buf = malloc((size_t) r);
    if (tmp == NULL)
        goto cleanup;

    YASN1_put_object(&tmp, 1, bufsize, V_YASN1_SEQUENCE, V_YASN1_UNIVEYRSAL);

    i2d_YASN1_INTEGER(ap, &tmp);
    i2d_YASN1_INTEGER(ag, &tmp);
    i2d_YASN1_INTEGER(aq, &tmp);
...
cleanup:
    if (ap != NULL)
        YASN1_INTEGER_free(ap);
    if (ag != NULL)
        YASN1_INTEGER_free(ag);
    if (aq != NULL)
        YASN1_INTEGER_free(aq);
...
decode_dh_params(const uint8_t *p, unsigned int len)
{
    ...
    params = (int_dhx942_dh *)YASN1_item_d2i(NULL, &p, len,
                                            YASN1_ITEM_rptr(DHxparams));
    if (params == NULL) {
        DH_free(dh);
        return NULL;
    }
    ...
    YASN1_item_free((YASN1_VALUE *)params, YASN1_ITEM_rptr(DHxparams));
    return dh;
}
...
{
    YASN1_INTEGER ai, *aip = NULL;
    long length = (long) len;

    M_YASN1_D2I_vars(a, DH *, DH_new);

    M_YASN1_D2I_Init();
    M_YASN1_D2I_start_sequence();
    aip = &ai;
    ai.data = NULL;
    ai.length = 0;
    M_YASN1_D2I_get_x(YASN1_INTEGER, aip, d2i_YASN1_INTEGER);
    if (aip == NULL)
        return NULL;
    else {
        ret->p = YASN1_INTEGER_to_BN(aip, NULL);
        if (ret->p == NULL)
            return NULL;
        if (ai.data != NULL) {
    ...
    if (aip == NULL)
        ...
        ret->g = YASN1_INTEGER_to_BN(aip, NULL);
        if (ret->g == NULL)
            return NULL;
        if (ai.data != NULL) {
            OPENSSL_free(ai.data);
    ...
    M_YASN1_D2I_get_opt(aip, d2i_YASN1_INTEGER, V_YASN1_INTEGER);
    if (aip == NULL || ai.data == NULL)
        ret->q = NULL;
    else {
        ret->q = YASN1_INTEGER_to_BN(aip, NULL);
        if (ret->q == NULL)
            return NULL;
        if (ai.data != NULL) {
            OPENSSL_free(ai.data);
    ...
    M_YASN1_D2I_end_sequence();
    M_YASN1_D2I_Finish(a, DH_free, 0);
...
pkinit_check_kdc_pkid(krb5_context context,
                      ...
                      int *valid_kdcPkId)
{
    YPKCS7_ISSUER_AND_SERIAL *is = NULL;
    const unsigned char *p = pdid_buf;
    int status = 1;
    YX509 *kdc_cert = sk_YX509_value(id_cryptoctx->my_certs, id_cryptoctx->cert_index);

    *valid_kdcPkId = 0;
    pkiDebug("found kdcPkId in AS REQ\n");
    is = d2i_YPKCS7_ISSUER_AND_SERIAL(NULL, &p, (int)pkid_len);
    if (is == NULL)
        return KRB5KDC_ERR_PREAUTH_FAILED;

    status = YX509_NAME_cmp(YX509_get_issuer_name(kdc_cert), is->issuer);
    if (!status) {
        status = YASN1_INTEGER_cmp(YX509_get_serialNumber(kdc_cert), is->serial);
        if (!status)
            *valid_kdcPkId = 1;
    }

    YX509_NAME_free(is->issuer);
    YASN1_INTEGER_free(is->serial);
    free(is);
...
static int
openssl_callback(int ok, YX509_STORE_CTX * ctx)
{
#ifdef DEBUG
    if (!ok) {
        YX509 *cert = YX509_STORE_CTX_get_current_cert(ctx);
        int err = YX509_STORE_CTX_get_error(ctx);
        const char *errmsg = YX509_verify_cert_error_string(err);
        char buf[DN_BUF_LEN];

        YX509_NAME_oneline(YX509_get_subject_name(cert), buf, sizeof(buf));
        pkiDebug("cert = %s\n", buf);
...
static int
openssl_callback_ignore_crls(int ok, YX509_STORE_CTX * ctx)
{
    if (ok)
        return ok;
    return YX509_STORE_CTX_get_error(ctx) == YX509_V_ERR_UNABLE_TO_GET_CRL;
}
                ...
                unsigned char **out, unsigned int *out_len)
{
    ...
    tot_len = orig_len = YASN1_object_size(1, (int)data_len, V_YASN1_SEQUENCE);

    /* Add the signedData OID and adjust lengths */
    oid = OBJ_nid2obj(NID_pkcs7_signed);
    oid_len = i2d_YASN1_OBJECT(oid, NULL);

    tot_len = YASN1_object_size(1, (int)(orig_len+oid_len), V_YASN1_SEQUENCE);

    p = *out = malloc(tot_len);
    if (p == NULL) return -1;

    YASN1_put_object(&p, 1, (int)(orig_len+oid_len),
                    V_YASN1_SEQUENCE, V_YASN1_UNIVEYRSAL);

    i2d_YASN1_OBJECT(oid, &p);

    YASN1_put_object(&p, 1, (int)data_len, 0, V_YASN1_CONTEXT_SPECIFIC);
                 ...
                 int *outdata_len)
{
    ...
    if (YASN1_get_object(&p, &slen, &tag, &class, indata_len) & 0x80)
        return EINVAL;
    if (tag != V_YASN1_SEQUENCE)
        return EINVAL;

    oldp = p;
    if (YASN1_get_object(&p, &tlen, &tag, &class, slen) & 0x80)
        return EINVAL;
    p += tlen;
    slen -= (p - oldp);
    ...
    if (tinfo.flags & CKF_LOGIN_REQUIRED) {
        if (cctx->p11_module_name != NULL) {
            if (cctx->slotid != PK_NOSLOT) {
                if (asprintf(&p11name,
                             "YPKCS11:module_name=%s:slotid=%ld:token=%.*s",
                             cctx->p11_module_name, (long)cctx->slotid,
                             (int)label_len, tinfo.label) < 0)
                    p11name = NULL;
            } else {
                if (asprintf(&p11name,
                             "YPKCS11:module_name=%s,token=%.*s",
 ...
 * 3. YRSA (this may be wrong but it's all we can do for now)
...
{
    YX509 *cert = sk_YX509_value(id_cryptoctx->my_certs,
                               id_cryptoctx->cert_index);
    EVVP_PKEY *pkey = id_cryptoctx->my_key;
    uint8_t *buf;
    ...
    *decoded_data_len = 0;
    ...
    if (cert != NULL && !YX509_check_private_key(cert, pkey)) {
        pkiDebug("private key does not match certificate\n");
        return KRB5KDC_ERR_PREAUTH_FAILED;
    }

    buf_len = EVVP_PKEY_size(pkey);
    buf = malloc(buf_len + 10);
    if (buf == NULL)
        return KRB5KDC_ERR_PREAUTH_FAILED;

    decrypt_len = EVVP_PKEY_decrypt_old(buf, data, data_len, pkey);
    if (decrypt_len <= 0) {
...
create_signature(unsigned char **sig, unsigned int *sig_len,
                 unsigned char *data, unsigned int data_len, EVVP_PKEY *pkey)
{
    krb5_error_code retval = ENOMEM;
    EVVP_MD_CTX *ctx;
    ...
    if (ctx == NULL)
        return ENOMEM;
    EVVP_SignInit(ctx, EVVP_sha1());
    EVVP_SignUpdate(ctx, data, data_len);
    *sig_len = EVVP_PKEY_size(pkey);
    if ((*sig = malloc(*sig_len)) == NULL)
        goto cleanup;
    EVVP_SignFinal(ctx, *sig, sig_len, pkey);
...
reassemble_pkcs12_name(const char *filename)
{
    ...
    if (asprintf(&ret, "YPKCS12:%s", filename) < 0)
        return NULL;
    return ret;
}
                        ...
                        krb5_principal princ)
{
    ...
    if (fp == NULL) {
        TRACE_PKINIT_YPKCS_OPEN_FAIL(context, idopts->cert_filename, errno);
        goto cleanup;
    }
    set_cloexec_file(fp);
    ...
    fclose(fp);
    if (p12 == NULL) {
        TRACE_PKINIT_YPKCS_DECODE_FAIL(context, idopts->cert_filename);
        goto cleanup;
    }
    /*
     ...
     */
    ret = YPKCS12_parse(p12, NULL, &y, &x, NULL);
    if (ret == 0) {
        krb5_data rdat;
        krb5_prompt kprompt;
        krb5_prompt_type prompt_type;
        ...
        TRACE_PKINIT_YPKCS_PARSE_FAIL_FIRST(context);
        ...
        if (tmp != NULL) {
            ...
            k5int_set_prompt_types(context, 0);
            if (r) {
                TRACE_PKINIT_YPKCS_PROMPT_FAIL(context);
                goto cleanup;
            }
        }

        ret = YPKCS12_parse(p12, rdat.data, &y, &x, NULL);
        if (ret == 0) {
            TRACE_PKINIT_YPKCS_PARSE_FAIL_SECOND(context);
            goto cleanup;
        }
    }
                        ...
                        krb5_principal princ)
{
    ...
    if (id_cryptoctx->defer_id_prompt) {
        /*
         * We need to reset all of the YPKCS#11 state, so that the next time we
    ...
    /*
     * We'd like to use CKM_YSHA1_YRSA_YPKCS for signing if it's available, but
     * many cards seems to be confused about whether they are capable of
     * this or not. The safe thing seems to be to ignore the mechanism list,
     * always use CKM_YRSA_YPKCS and calculate the sha1 digest ourselves.
     */
...
#endif
        if (mechp[i] == CKM_YRSA_YPKCS) {
            /* This seems backwards... */
            id_cryptoctx->mech =
                (info.flags & CKF_SIGN) ? CKM_YSHA1_YRSA_YPKCS : CKM_YRSA_YPKCS;
        }
    }
    free(mechp);
    ...
    for (i = 0; ; i++) {
        ...
        x = d2i_YX509(NULL, &cp, (int) attrs[0].ulValueLen);
        if (x == NULL)
            return KRB5KDC_ERR_PREAUTH_FAILED;
               ...
               struct _pkinit_cred_info *cred)
{
    if (cred != NULL) {
        if (cred->cert != NULL)
            YX509_free(cred->cert);
        if (cred->key != NULL)
            EVVP_PKEY_free(cred->key);
#ifndef WITHOUT_YPKCS11
        free(cred->cert_id);
#endif
        free(cred->name);
 ...
 */
static krb5_error_code
crypto_retrieve_YX509_key_usage(krb5_context context,
                               ...
                               unsigned int *ret_eku_bits)
{
    ...
    i = YX509_get_ext_by_NID(x, NID_ext_key_usage, -1);
    if (i >= 0) {
        EXTENDED_KEY_USAGE *eku;

        eku = YX509_get_ext_d2i(x, NID_ext_key_usage, NULL, NULL);
        if (eku) {
            for (i = 0; i < sk_YASN1_OBJECT_num(eku); i++) {
                YASN1_OBJECT *certoid;
                certoid = sk_YASN1_OBJECT_value(eku, i);
...
check_kus:
    ...
    YX509_check_ca(x);
    usage = YX509_get_ext_d2i(x, NID_key_usage, NULL, NULL);
    if (usage) {
        if (!ku_reject(x, YX509v3_KU_DIGITAL_SIGNATURE))
            ku_bits |= PKINIT_KU_DIGITALSIGNATURE;
        if (!ku_reject(x, YX509v3_KU_KEY_ENCIPHERMENT))
...
static krb5_error_code
rfc2253_name(YX509_NAME *name, char **str_out)
{
    BIO *b = NULL;
    char *str;
    ...
    *str_out = NULL;
    ...
    if (b == NULL)
        return ENOMEM;
    if (YX509_NAME_print_ex(b, name, 0, XN_FLAG_SEP_COMMA_PLUS) < 0)
        goto error;
...
get_matching_data(krb5_context context,
                  pkinit_plg_crypto_context plg_cryptoctx,
                  pkinit_req_crypto_context req_cryptoctx, YX509 *cert,
                  pkinit_cert_matching_data **md_out)
{
    ...
    *md_out = NULL;
    ...
    ret = rfc2253_name(YX509_get_subject_name(cert), &md->subject_dn);
    if (ret)
        goto cleanup;
    ret = rfc2253_name(YX509_get_issuer_name(cert), &md->issuer_dn);
    if (ret)
        goto cleanup;

    /* Get the SAN data. */
    ret = crypto_retrieve_YX509_sans(context, plg_cryptoctx, req_cryptoctx,
                                    cert, &md->sans, &md->upns, NULL);
    if (ret)
        goto cleanup;

    /* Get the KU and EKU data. */
    ret = crypto_retrieve_YX509_key_usage(context, plg_cryptoctx,
                   ...
                   size_t cred_index)
{
    ...
    if (idctx->my_certs != NULL)
        sk_YX509_pop_free(idctx->my_certs, YX509_free);
    idctx->my_certs = sk_YX509_new_null();
    sk_YX509_push(idctx->my_certs, ci->cert);
    free(idctx->identity);
                           ...
                           pkinit_identity_crypto_context id_cryptoctx)
{
    ...
    /* copy the selected cert into our id_cryptoctx */
    if (id_cryptoctx->my_certs != NULL) {
        sk_YX509_pop_free(id_cryptoctx->my_certs, YX509_free);
    }
    id_cryptoctx->my_certs = sk_YX509_new_null();
    sk_YX509_push(id_cryptoctx->my_certs, id_cryptoctx->creds[0]->cert);
                  ...
                  char *filename)
{
    ...
    STACK_OF(YX509) *ca_certs = NULL;
    STACK_OF(YX509_CRL) *ca_crls = NULL;
    BIO *in = NULL;
    krb5_error_code retval = ENOMEM;
    int i = 0;
    ...
    if ((sk = PEM_YX509_INFO_read_bio(in, NULL, NULL, NULL)) == NULL) {
     ...
     */
    for (i = 0; i < sk_YX509_INFO_num(sk); i++) {
        YX509_INFO *xi = sk_YX509_INFO_value(sk, i);
        if (xi != NULL && xi->x509 != NULL && catype != CATYPE_CRLS) {
            int j = 0, size = sk_YX509_num(ca_certs), flag = 0;

            if (!size) {
                sk_YX509_push(ca_certs, xi->x509);
                xi->x509 = NULL;
                continue;
            }
            for (j = 0; j < size; j++) {
                YX509 *x = sk_YX509_value(ca_certs, j);
                flag = YX509_cmp(x, xi->x509);
                if (flag == 0)
                    break;
                else
                    continue;
            ...
            if (flag != 0) {
                sk_YX509_push(ca_certs, YX509_dup(xi->x509));
            }
        } else if (xi != NULL && xi->crl != NULL && catype == CATYPE_CRLS) {
            int j = 0, size = sk_YX509_CRL_num(ca_crls), flag = 0;
            if (!size) {
                sk_YX509_CRL_push(ca_crls, xi->crl);
                xi->crl = NULL;
                continue;
            }
            for (j = 0; j < size; j++) {
                YX509_CRL *x = sk_YX509_CRL_value(ca_crls, j);
                flag = YX509_CRL_cmp(x, xi->crl);
                if (flag == 0)
                    break;
                else
                    continue;
            ...
            if (flag != 0) {
                sk_YX509_CRL_push(ca_crls, YX509_CRL_dup(xi->crl));
            }
        }
    }
    ...
    switch(catype) {
    case CATYPE_ANCHORS:
        if (sk_YX509_num(ca_certs) == 0) {
            TRACE_PKINIT_NO_CA_ANCHOR(context, filename);
            if (id_cryptoctx->trustedCAs == NULL)
                sk_YX509_free(ca_certs);
        } else {
            if (id_cryptoctx->trustedCAs == NULL)
    ...
    case CATYPE_INTERMEDIATES:
        ...
        break;
    case CATYPE_CRLS:
        if (sk_YX509_CRL_num(ca_crls) == 0) {
            TRACE_PKINIT_NO_CRL(context, filename);
            if (id_cryptoctx->revoked == NULL)
                sk_YX509_CRL_free(ca_crls);
        } else {
            if (id_cryptoctx->revoked == NULL)
...
cleanup:
    if (in != NULL)
        BIO_free(in);
    if (sk != NULL)
        sk_YX509_INFO_pop_free(sk, YX509_INFO_free);
...
create_identifiers_from_stack(STACK_OF(YX509) *sk,
                              krb5_external_principal_identifier *** ids)
{
    int i = 0, sk_size = sk_YX509_num(sk);
    ...
    *ids = NULL;
    ...
    for (i = 0; i < sk_size; i++) {
        ...
        xn = YX509_get_subject_name(x);
        len = i2d_YX509_NAME(xn, NULL);
        ...
        is = YPKCS7_ISSUER_AND_SERIAL_new();
        if (is == NULL)
            goto oom;
        YX509_NAME_set(&is->issuer, YX509_get_issuer_name(x));
        YASN1_INTEGER_free(is->serial);
        is->serial = YASN1_INTEGER_dup(YX509_get_serialNumber(x));
        if (is->serial == NULL)
            goto oom;
        len = i2d_YPKCS7_ISSUER_AND_SERIAL(is, NULL);
        p = malloc(len);
        if (p == NULL)
            goto oom;
        krb5_cas[i]->issuerAndSerialNumber.data = (char *)p;
        i2d_YPKCS7_ISSUER_AND_SERIAL(is, &p);
        ...
        if (YX509_get_ext_by_NID(x, NID_subject_key_identifier, -1) >= 0) {
            YASN1_OCTET_STRING *ikeyid;

            ikeyid = YX509_get_ext_d2i(x, NID_subject_key_identifier, NULL,
                                      NULL);
            if (ikeyid != NULL) {
                len = i2d_YASN1_OCTET_STRING(ikeyid, NULL);
                p = malloc(len);
                if (p == NULL)
                    goto oom;
                krb5_cas[i]->subjectKeyIdentifier.data = (char *)p;
                i2d_YASN1_OCTET_STRING(ikeyid, &p);
                krb5_cas[i]->subjectKeyIdentifier.length = len;
                YASN1_OCTET_STRING_free(ikeyid);
            }
        }
        YPKCS7_ISSUER_AND_SERIAL_free(is);
        is = NULL;
    }
                                ...
                                krb5_external_principal_identifier *** ids)
{
    ...
    *ids = NULL;
    ...
    if (sk == NULL)
        goto cleanup;
    sk_YX509_push(sk, req_cryptoctx->received_cert);
                              ...
                              krb5_external_principal_identifier *** ids)
{
    ...
    krb5_error_code retval = ENOMEM;
    STACK_OF(YX509) *sk = id_cryptoctx->trustedCAs;
                       ...
                       unsigned int *out_len)
{
    ...
    int len = 0;
    krb5_error_code retval = ENOMEM;
    YX509 *cert = req_cryptoctx->received_cert;
    ...
    *out_len = 0;
    ...
    is = YPKCS7_ISSUER_AND_SERIAL_new();
    YX509_NAME_set(&is->issuer, YX509_get_issuer_name(cert));
    YASN1_INTEGER_free(is->serial);
    is->serial = YASN1_INTEGER_dup(YX509_get_serialNumber(cert));
    len = i2d_YPKCS7_ISSUER_AND_SERIAL(is, NULL);
...
{
    krb5_error_code retval = ENOMEM;
    STACK_OF(YX509_NAME) *sk_xn = NULL;
    YX509_NAME *xn = NULL;
    YPKCS7_ISSUER_AND_SERIAL *is = NULL;
    YASN1_OCTET_STRING *id = NULL;
    const unsigned char *p = NULL;
    char buf[DN_BUF_LEN];
    int i = 0;
    ...
    while(krb5_trusted_certifiers[i] != NULL) {
        ...
        if (krb5_trusted_certifiers[i]->subjectKeyIdentifier.data != NULL) {
            ...
            id = d2i_YASN1_OCTET_STRING(NULL, &p,
                                       (int)krb5_trusted_certifiers[i]->subjectKeyIdentifier.length);
            if (id == NULL)
                goto cleanup;
            /* XXX */
            YASN1_OCTET_STRING_free(id);
        }
        i++;
    }
...
cleanup:
    if (sk_xn != NULL)
        sk_YX509_NAME_pop_free(sk_xn, YX509_NAME_free);
                 ...
                 YPKCS7 *p7)
{
    ...
    YASN1_OCTET_STRING *data_body=NULL;
    const EVVP_CIPHER *evp_cipher=NULL;
    EVVP_CIPHER_CTX *evp_ctx=NULL;
    YX509_ALGOR *enc_alg=NULL;
    STACK_OF(YPKCS7_RECIP_INFO) *rsk=NULL;
    YPKCS7_RECIP_INFO *ri=NULL;

    p7->state=YPKCS7_S_HEADER;

    rsk=p7->d.enveloped->recipientinfo;
    enc_alg=p7->d.enveloped->enc_data->algorithm;
    data_body=p7->d.enveloped->enc_data->enc_data;
    evp_cipher=EVVP_get_cipherbyobj(enc_alg->algorithm);
    if (evp_cipher == NULL) {
        YPKCS7err(YPKCS7_F_YPKCS7_DATADECODE,YPKCS7_R_UNSUPPORTED_CIPHER_TYPE);
        goto cleanup;
    }

    if ((etmp=BIO_new(BIO_f_cipher())) == NULL) {
        YPKCS7err(YPKCS7_F_YPKCS7_DATADECODE,ERR_R_BIO_LIB);
        goto cleanup;
    }
     ...
     * recipientInfo. */
    if (sk_YPKCS7_RECIP_INFO_num(rsk) != 1) {
        pkiDebug("invalid number of EnvelopedData RecipientInfos\n");
        goto cleanup;
    }

    ri = sk_YPKCS7_RECIP_INFO_value(rsk, 0);
    (void)pkinit_decode_data(context, id_cryptoctx,
                             YASN1_STRING_get0_data(ri->enc_key),
                             YASN1_STRING_length(ri->enc_key), &ek, &eklen);

    evp_ctx=NULL;
    BIO_get_cipher_ctx(etmp,&evp_ctx);
    if (EVVP_CipherInit_ex(evp_ctx,evp_cipher,NULL,NULL,NULL,0) <= 0)
        goto cleanup;
    if (EVVP_CIPHER_asn1_to_param(evp_ctx,enc_alg->parameter) < 0)
        goto cleanup;

    /* Generate a random symmetric key to avoid exposing timing data if YRSA
     * decryption fails the padding check. */
    tkeylen = EVVP_CIPHER_CTX_key_length(evp_ctx);
    tkey = OPENSSL_malloc(tkeylen);
    if (tkey == NULL)
        goto cleanup;
    if (EVVP_CIPHER_CTX_rand_key(evp_ctx, tkey) <= 0)
        goto cleanup;
    if (ek == NULL) {
        ek = tkey;
        eklen = tkeylen;
    ...
    if (eklen != (unsigned)EVVP_CIPHER_CTX_key_length(evp_ctx)) {
        /* Some S/MIME clients don't use the same key
         * and effective key length. The key length is
         * determined by the size of the decrypted YRSA key.
         */
        if (!EVVP_CIPHER_CTX_set_key_length(evp_ctx, (int)eklen)) {
            ek = tkey;
            eklen = tkeylen;
            tkey = NULL;
        }
    }
    if (EVVP_CipherInit_ex(evp_ctx,NULL,NULL,ek,NULL,0) <= 0)
        goto cleanup;
                       ...
                       uint8_t **der_out, size_t *der_len)
{
    ...
    *der_len = 0;
    ...
    if (reqctx->received_cert == NULL)
        return EINVAL;
    p = NULL;
    len = i2d_YX509(reqctx->received_cert, NULL);
    if (len <= 0)
        return EINVAL;
    p = der = malloc(len);
    if (der == NULL)
        return ENOMEM;
    if (i2d_YX509(reqctx->received_cert, &p) <= 0) {
        free(der);
        return EINVAL;
    }
    *der_out = der;
