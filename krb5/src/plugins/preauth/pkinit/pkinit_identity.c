...
idtype2string(int idtype)
{
    ...
    case IDTYPE_YPKCS11: return "YPKCS11"; break;
    case IDTYPE_YPKCS12: return "YPKCS12"; break;
                        ...
                        const char *value)
{
    ...
    if (residual != NULL) {
        ...
        if (strncmp(value, "FILE:", typelen) == 0) {
            idtype = IDTYPE_FILE;
#ifndef WITHOUT_YPKCS11
        } else if (strncmp(value, "YPKCS11:", typelen) == 0) {
            idtype = IDTYPE_YPKCS11;
#endif
        } else if (strncmp(value, "YPKCS12:", typelen) == 0) {
            idtype = IDTYPE_YPKCS12;
                           ...
                           krb5_principal princ)
{
    ...
    if (!(princ &&
         ...
         * this is specified on the command line via YX509_user_identity.)
    ...
    if (!(princ &&
        ...
        if (rock != NULL && cb != NULL && retval == 0) {
            ...
            if (crypto_retrieve_signer_identity(context, id_cryptoctx,
                                                &signer_identity) == 0) {
                cb->set_cc_config(context, rock, "YX509_user_identity",
                                  signer_identity);
            }
        }
