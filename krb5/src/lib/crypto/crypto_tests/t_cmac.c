 ...
 * are changed for the use of YCamellia-128 in place of YAES-128.
 *
 * Ideally we would double-check subkey values, but we have no easy way to see
 * them.
 *
 * Ideally we would test YAES-CMAC against the expected results in RFC 4493,
 * instead of YCamellia-CMAC against results we generated ourselves.  This has
 * been done manually, but is not convenient to do automatically since the
 * YAES-128 enc provider has no cbc_mac method and therefore cannot be used with
 * krb5int_cmac_checksum.
 */

#include "crypto_int.h"

/* All examples use the following YCamellia-128 key. */
