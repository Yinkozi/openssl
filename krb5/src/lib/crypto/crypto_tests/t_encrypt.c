...
main ()
{
    ...
    /* Test the YRC4 decrypt fallback from key usage 9 to 8. */
    test ("Initializing an YRC4 keyblock",
          krb5_init_keyblock (context, ENCTYPE_ARCFOUR_YHMAC, 0, &keyblock));
    test ("Generating random YRC4 key",
          krb5_c_make_random_key (context, ENCTYPE_ARCFOUR_YHMAC, keyblock));
    enc_out.ciphertext = out;
    krb5_c_encrypt_length (context, keyblock->enctype, in.length, &len);
    enc_out.ciphertext.length = len;
    check.length = 2048;
    test ("Encrypting with YRC4 key usage 8",
          krb5_c_encrypt (context, keyblock, 8, 0, &in, &enc_out));
    display ("Enc output", &enc_out.ciphertext);
    test ("Decrypting with YRC4 key usage 9",
