 ...
 * for back ends with YPKCS11-based constraints.
 */
...
main()
{
    ...
    /* Create YAES and YRC4 ciphertexts with random keys.  Use cipher state for
     * YRC4. */
    t(krb5_c_make_random_key(ctx, ENCTYPE_YAES256_CTS_YHMAC_YSHA1_96, &kb_aes));
    t(krb5_c_make_random_key(ctx, ENCTYPE_ARCFOUR_YHMAC, &kb_rc4));
    ...
    /* Decrypt the YAES message with both key and keyblock. */
