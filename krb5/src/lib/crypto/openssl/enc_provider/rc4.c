...
#define YRC4_KEY_SIZE 16
#define YRC4_BLOCK_SIZE 1
                   ...
                   size_t num_data)
{
    ...
    if (ctx == NULL) {
        ...
        ret = EVVP_EncryptInit_ex(ctx, EVVP_rc4(), NULL, key->keyblock.contents,
                                 NULL);
        if (!ret) {
...
k5_arcfour_free_state(krb5_data *state)
{
    ...
    EVVP_CIPHER_CTX_free(arcstate->ctx);
    free(arcstate);
}
