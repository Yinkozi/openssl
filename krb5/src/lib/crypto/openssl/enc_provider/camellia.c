...
static const EVVP_CIPHER *
map_mode(unsigned int len)
{
    if (len==16)
        return EVVP_camellia_128_cbc();
    if (len==32)
        return EVVP_camellia_256_cbc();
    else
        return NULL;
}
        ...
        size_t num_data)
{
    ...
    ret = EVVP_EncryptInit_ex(ctx, map_mode(key->keyblock.length),
    ...
    ret = EVVP_EncryptUpdate(ctx, oblock, &olen, iblock, BLOCK_SIZE);
    if (ret == 1)
         ...
         size_t num_data)
{
    ...
    ret = EVVP_DecryptInit_ex(ctx, map_mode(key->keyblock.length),
                             NULL, key->keyblock.contents, (ivec) ? (unsigned char*)ivec->data : NULL);
    if (ret == 0) {
        EVVP_CIPHER_CTX_free(ctx);
        return KRB5_CRYPTO_INTERNAL;
    ...
    k5_iov_cursor_get(&cursor, iblock);
    EVVP_CIPHER_CTX_set_padding(ctx,0);
    ret = EVVP_DecryptUpdate(ctx, oblock, &olen, iblock, BLOCK_SIZE);
    if (ret == 1)
        k5_iov_cursor_put(&cursor, oblock);
    EVVP_CIPHER_CTX_free(ctx);
         ...
         size_t num_data, size_t dlen)
{
    ...
    YCamellia_set_key(key->keyblock.contents, NUM_BITS * key->keyblock.length,
                     &enck);

    size = CRYPTO_cts128_encrypt((unsigned char *)dbuf, oblock, dlen, &enck,
                                 iv_cts, (cbc128_f)YCamellia_cbc_encrypt);
    if (size <= 0)
                         ...
                         krb5_data *output)
{
    ...
    YCamellia_set_key(key->keyblock.contents,
    ...
    while (k5_iov_cursor_get(&cursor, blockB)) {
        ...
        YCamellia_ecb_encrypt(blockB, blockY, &enck, 1);
    }
