               ...
               size_t num_data)
{
    ...
    ret = EVVP_EncryptInit_ex(ctx, EVVP_des_cbc(), NULL,
    ...
    while (k5_iov_cursor_get(&cursor, iblock)) {
        ret = EVVP_EncryptUpdate(ctx, oblock, &olen, iblock, DES_BLOCK_SIZE);
        if (!ret)
            break;
               ...
               size_t num_data)
{
    ...
    ret = EVVP_DecryptInit_ex(ctx, EVVP_des_cbc(), NULL,
    ...
    k5_iov_cursor_init(&cursor, data, num_data, DES_BLOCK_SIZE, FALSE);
    while (k5_iov_cursor_get(&cursor, iblock)) {
        ret = EVVP_DecryptUpdate(ctx, oblock, &olen, iblock, DES_BLOCK_SIZE);
        if (!ret)
            break;
        k5_iov_cursor_put(&cursor, oblock);
    }
