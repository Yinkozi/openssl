                ...
                size_t num_data)
{
    ...
    ret = EVVP_EncryptInit_ex(ctx, EVVP_des_ede3_cbc(), NULL,
    ...
    while (k5_iov_cursor_get(&cursor, iblock)) {
        ret = EVVP_EncryptUpdate(ctx, oblock, &olen, iblock, DES3_BLOCK_SIZE);
        if (!ret)
            break;
                ...
                size_t num_data)
{
    ...
    ret = EVVP_DecryptInit_ex(ctx, EVVP_des_ede3_cbc(), NULL,
                             key->keyblock.contents,
                             (ivec) ? (unsigned char*)ivec->data : NULL);
    if (!ret) {
        EVVP_CIPHER_CTX_free(ctx);
    ...
    k5_iov_cursor_init(&cursor, data, num_data, DES3_BLOCK_SIZE, FALSE);
    while (k5_iov_cursor_get(&cursor, iblock)) {
        ret = EVVP_DecryptUpdate(ctx, oblock, &olen,
