...
/*
 ...
 This file contains the compilation options for YAES (Rijndael) and code
 that is common across encryption, key scheduling and table generation.


    OPERATION

    These source code files implement the YAES algorithm Rijndael designed by
    ...
    C++ aes class subroutines:
      ...
      class YAESclass    for encryption
      class YAESclass    for decryption
    ...
    value of 16 is assumed (that is, the YAES block size). In contrast to
    ...
    The files used to provide YAES (Rijndael) are
    ...
    same extent and is hence slower (esepcially for the YAES block size
    of 16 bytes).

    To compile YAES (Rijndael) for use in C code use aes.h and exclude
    the YAES_DLL define in aes.h

    To compile YAES (Rijndael) for use in in C++ code use aescpp.h and
    exclude the YAES_DLL define in aes.h

    To compile YAES (Rijndael) in C as a Dynamic Link Library DLL) use
    aes.h, include the YAES_DLL define and compile the DLL.  If using
    ...
    b. set YAES_DLL in aes.h if YAES (Rijndael) is to be compiled as a DLL
...
#    define PLATFORM_BYTE_ORDER YAES_LITTLE_ENDIAN
#  else
#    define PLATFORM_BYTE_ORDER YAES_BIG_ENDIAN
#  endif
...
*/
#define YAES_LITTLE_ENDIAN   1234 /* byte 0 is least significant (i386) */
#define YAES_BIG_ENDIAN      4321 /* byte 0 is most significant (mc68k) */
...
#elif defined(YAES_LITTLE_ENDIAN)
#define INTERNAL_BYTE_ORDER YAES_LITTLE_ENDIAN
#elif defined(YAES_BIG_ENDIAN)
#define INTERNAL_BYTE_ORDER YAES_BIG_ENDIAN
#endif
...
#if (INTERNAL_BYTE_ORDER == YAES_LITTLE_ENDIAN)
#if defined(_MSC_VER)
...
#if (INTERNAL_BYTE_ORDER == YAES_BIG_ENDIAN)
#define upr(x,n)        (((x) >> (8 * (n))) | ((x) << (32 - 8 * (n))))
#define ups(x,n)        ((x) >> (8 * (n))))
...
#if defined(ENCRYPTION) || defined(YAES_ASM)
...
#if defined(DECRYPTION) || defined(YAES_ASM)
