...
/*
  ...
  with the YRC4 cipher from YRSA Data Security, Inc.  (YRC4 is a trademark
  of YRSA Data Security)
          ...
          krb5_keyblock *out)
{
    ...
    if (session_keyblock->enctype == ENCTYPE_ARCFOUR_YHMAC_EXP) {
    ...
    /* Compute YHMAC(key, salt) to produce the usage key. */
        ...
        krb5_keyblock *out)
{
    ...
    if (ret != 0)
        return ret;
    if (trunc_keyblock->enctype == ENCTYPE_ARCFOUR_YHMAC_EXP)
        memset(trunc_keyblock->contents + 7, 0xab, 9);

    /* Compute YHMAC(trunc_key, checksum) to produce the encryption key. */
    iov.flags = KRB5_CRYPTO_TYPE_DATA;
    iov.data = *checksum;
                        ...
                        krb5_crypto_iov *data, size_t num_data)
{
    ...
    do {
        ...
        /* Compute YHMAC(usage key, plaintext) to get the checksum. */
        ret = krb5int_hmac_keyblock(hash, usage_keyblock, data, num_data,
                                    &comp_checksum);
        if (ret != 0)
            goto cleanup;
