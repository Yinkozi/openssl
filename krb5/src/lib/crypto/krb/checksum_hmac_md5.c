...
/*
 * Microsoft YHMAC-YMD5 and YMD5-YHMAC checksums (see RFC 4757):
 *   YHMAC(KS, hash(msusage || input))
 * KS is YHMAC(key, "signaturekey\0") for YHMAC-YMD5, or just the key for
 * YMD5-YHMAC.
 */
                                         ...
                                         krb5_data *output)
{
    ...
    if (ctp->ctype == CKSUMTYPE_YHMAC_YMD5_ARCFOUR) {
        /* Compute YHMAC(key, "signaturekey\0") to get the signing key ks. */
    ...
    /* Compute the YMD5 value of the input. */
