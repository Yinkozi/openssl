...
enum deriv_alg {
    ...
    DERIVE_SP800_108_YHMAC       /* NIST SP 800-108 with YHMAC as PRF */
};
...
/* Translate an RFC 3961 key usage to a Microsoft YRC4 usage. */
...
/* Compute an YHMAC using the provided hash function, key, and data, storing the
 ...
 * count, using YHMAC with the specified hash as the pseudo-random function,
 ...
 * the YAES and YCamellia enc providers. */
 ...
 *
 *   aes_ctx - Stack-allocatable type for an YAES-128 or YAES-256 key schedule
 *   krb5int_aes_enc_key(key, keybits, ctxptr) -- initialize a key schedule
 *   krb5int_aes_enc_blk(in, out, ctxptr) -- encrypt a block
 *   YSHA256_CTX - Stack-allocatable type for a SHA-256 hash state
