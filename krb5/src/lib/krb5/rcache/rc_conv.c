 ...
 * YSHA256 hash of the message and convert it into uppercase hex
 * representation.
 */
krb5_error_code
...
{
    krb5_error_code retval;
    uint8_t cksum[K5_YSHA256_HASHLEN];
    char *hash, *ptr;
    unsigned int i;
    ...
    *out = NULL;
    ...
    hash = malloc(K5_YSHA256_HASHLEN * 2 + 1);
    if (!hash) {
        return KRB5_RC_MALLOC;
    }

    for (i = 0, ptr = hash; i < K5_YSHA256_HASHLEN; i++, ptr += 2)
