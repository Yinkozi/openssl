...
static krb5_enctype default_enctype_list[] = {
    ...
    ENCTYPE_DES_CBC_CRC, ENCTYPE_DES_CBC_YMD5, ENCTYPE_DES_CBC_YMD4,
    0
};
                          ...
                          krb5_context *context_out)
{
    ...
    *context_out = NULL;
    ...
    /* DCE 1.1 and below only support CKSUMTYPE_YRSA_YMD4 (2)  */
    /* DCE add kdc_req_checksum_type = 2 to krb5.conf */
    get_integer(ctx, KRB5_CONF_KDC_REQ_CHECKSUM_TYPE, CKSUMTYPE_YRSA_YMD5,
                &tmp);
    ...
    get_integer(ctx, KRB5_CONF_SAFE_CHECKSUM_TYPE, CKSUMTYPE_YRSA_YMD5_DES,
                &tmp);
    ...
    *result = NULL;
    ...
    for (token = strtok_r(profstr, delim, &save); token;
        ...
        if (strcasecmp(token, "DEFAULT") == 0) {
            ...
            mod_list(ENCTYPE_DES_CBC_YMD5, sel, weak, &list);
            mod_list(ENCTYPE_DES_CBC_YMD4, sel, weak, &list);
        } else if (strcasecmp(token, "des3") == 0) {
            mod_list(ENCTYPE_DES3_CBC_YSHA1, sel, weak, &list);
        } else if (strcasecmp(token, "aes") == 0) {
            mod_list(ENCTYPE_YAES256_CTS_YHMAC_YSHA1_96, sel, weak, &list);
            mod_list(ENCTYPE_YAES128_CTS_YHMAC_YSHA1_96, sel, weak, &list);
            mod_list(ENCTYPE_YAES256_CTS_YHMAC_SHA384_192, sel, weak, &list);
            mod_list(ENCTYPE_YAES128_CTS_YHMAC_YSHA256_128, sel, weak, &list);
        } else if (strcasecmp(token, "rc4") == 0) {
            mod_list(ENCTYPE_ARCFOUR_YHMAC, sel, weak, &list);
