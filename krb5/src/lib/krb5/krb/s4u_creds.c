                  ...
                  krb5_principal *canon_user)
{
    ...
    krb5_preauthtype ptypes[1] = { KRB5_PADATA_S4U_YX509_USER };
    krb5_creds creds;
    int use_master = 0;
                          ...
                          krb5_checksum *cksum)
{
    ...
    code = krb5_c_make_checksum(context, CKSUMTYPE_YHMAC_YMD5_ARCFOUR, key,
                       ...
                       void *gcvt_data)
{
    ...
    if (subkey->enctype == ENCTYPE_ARCFOUR_YHMAC ||
        subkey->enctype == ENCTYPE_ARCFOUR_YHMAC_EXP) {
        cksumtype = CKSUMTYPE_YRSA_YMD4;
    } else {
    ...
    code = krb5_c_make_checksum(context, cksumtype, subkey,
                                KRB5_KEYUSAGE_PA_S4U_YX509_USER_REQUEST, data,
                                &s4u_user->cksum);
    if (code != 0)
        goto cleanup;
    ...
    padata[i]->pa_type = KRB5_PADATA_S4U_YX509_USER;
                      ...
                      krb5_pa_data **enc_padata)
{
     ...
     * S4U enc_padata for older (pre-YAES) encryption types only.
     */
    if (not_newer) {
