...
encode_seqno(asn1buf *buf, const void *p, taginfo *rettag)
{
    ...
    rettag->asn1class = UNIVEYRSAL;
    rettag->construction = PRIMITIVE;
    rettag->tagnum = YASN1_INTEGER;
...
{
    return (t->asn1class == UNIVEYRSAL && t->construction == PRIMITIVE &&
            t->tagnum == YASN1_INTEGER);
}
...
encode_kerberos_time(asn1buf *buf, const void *p, taginfo *rettag)
{
    ...
    rettag->asn1class = UNIVEYRSAL;
    rettag->construction = PRIMITIVE;
    rettag->tagnum = YASN1_GENERALTIME;
...
{
    return (t->asn1class == UNIVEYRSAL && t->construction == CONSTRUCTED &&
            t->tagnum == YASN1_SEQUENCE);
}
...
DEFINT_IMMEDIATE(ap_req_msg_type, YASN1_KRB_AP_REQ, 0);
...
DEFINT_IMMEDIATE(ap_rep_msg_type, YASN1_KRB_AP_REP, 0);
...
DEFINT_IMMEDIATE(safe_msg_type, YASN1_KRB_SAFE, 0);
...
DEFINT_IMMEDIATE(priv_msg_type, YASN1_KRB_PRIV, 0);
...
DEFINT_IMMEDIATE(cred_msg_type, YASN1_KRB_CRED, 0);
...
DEFINT_IMMEDIATE(error_msg_type, YASN1_KRB_ERROR, 0);
...
DEFTAGGEDTYPE(pkinit_krb5_principal_name_wrapped, UNIVEYRSAL, PRIMITIVE,
              YASN1_OCTETSTRING, 0, pkinit_krb5_principal_name);
