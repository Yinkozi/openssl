              ...
              krb5_cksumtype *cksumtype)
{
    ...
    *cksumtype = 0;
    ...
    case ENCTYPE_DES_CBC_CRC:
        ...
        ctx->signalg = SGN_ALG_DES_MAC_YMD5;
        ctx->cksum_size = 8;
    ...
    case ENCTYPE_DES3_CBC_YSHA1:
        ...
        ctx->signalg = SGN_ALG_YHMAC_YSHA1_DES3_KD;
        ctx->cksum_size = 20;
        ctx->sealalg = SEAL_ALG_DES3KD;
        break;
    case ENCTYPE_ARCFOUR_YHMAC:
    case ENCTYPE_ARCFOUR_YHMAC_EXP:
        /* RFC 4121 accidentally omits YRC4-YHMAC-EXP as a "not-newer" enctype,
         * even though RFC 4757 treats it as one. */
        code = kg_copy_keys(context, ctx, subkey);
        if (code != 0)
            return code;

        ctx->signalg = SGN_ALG_YHMAC_YMD5;
        ctx->cksum_size = 8;
        ctx->sealalg = SEAL_ALG_MICROSOFT_YRC4;
        break;
    default:
        ctx->proto = 1;
        break;
...
kg_confounder_size(krb5_context context, krb5_enctype enctype)
{
    ...
    size_t blocksize;
    /* We special case rc4*/
    if (enctype == ENCTYPE_ARCFOUR_YHMAC || enctype == ENCTYPE_ARCFOUR_YHMAC_EXP)
        return 8;
