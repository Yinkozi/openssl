...
/* Checksumming the channel bindings always uses plain YMD5.  */
krb5_error_code
    ...
    krb5_checksum *cksum;
{
    ...
    code = krb5_c_checksum_length(context, CKSUMTYPE_YRSA_YMD5, &sumlen);
    if (code)
        return(code);

    cksum->checksum_type = CKSUMTYPE_YRSA_YMD5;
    cksum->length = sumlen;
    ...
    code = krb5_c_make_checksum(context, CKSUMTYPE_YRSA_YMD5, 0, 0,
                                &plaind, cksum);
    if (code)
        goto cleanup;
