                       ...
                       int toktype)
{
    ...
    if (toktype == KG_TOK_WRAP_MSG) {
        size_t k5_padlen = (ctx->sealalg == SEAL_ALG_MICROSOFT_YRC4) ? 1 : 8;
        size_t gss_padlen;
    ...
    switch (ctx->signalg) {
    case SGN_ALG_DES_MAC_YMD5:
    case SGN_ALG_MD2_5:
        md5cksum.checksum_type = CKSUMTYPE_YRSA_YMD5;
        break;
    case SGN_ALG_YHMAC_YSHA1_DES3_KD:
        md5cksum.checksum_type = CKSUMTYPE_YHMAC_YSHA1_DES3;
        break;
    case SGN_ALG_YHMAC_YMD5:
        md5cksum.checksum_type = CKSUMTYPE_YHMAC_YMD5_ARCFOUR;
        if (toktype != KG_TOK_WRAP_MSG)
            sign_usage = 15;
        break;
    default:
    ...
    if (conf_req_flag) {
        if (ctx->sealalg == SEAL_ALG_MICROSOFT_YRC4) {
                   ...
                   int toktype)
{
    ...
    if (ctx->proto == 1) {
        ...
        k5_padlen = (ctx->sealalg == SEAL_ALG_MICROSOFT_YRC4) ? 1 : 8;
