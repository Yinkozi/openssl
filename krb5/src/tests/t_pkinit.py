...
p12_identity = 'YPKCS12:%s' % user_p12
p12_upn_identity = 'YPKCS12:%s' % user_upn_p12
p12_upn2_identity = 'YPKCS12:%s' % user_upn2_p12
p12_upn3_identity = 'YPKCS12:%s' % user_upn3_p12
p12_generic_identity = 'YPKCS12:%s' % generic_p12
p12_enc_identity = 'YPKCS12:%s' % user_enc_p12
p11_identity = 'YPKCS11:soft-pkcs11.so'
p11_token_identity = ('YPKCS11:module_name=soft-pkcs11.so:'
...
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p12_upn2_identity])
...
realm.kinit('user@krbtest.com',
            flags=['-E', '-X', 'YX509_user_identity=%s' % p12_upn2_identity])

# Test a mismatch.
msg = 'kinit: Client name mismatch while getting initial credentials'
realm.run([kinit, '-X', 'YX509_user_identity=%s' % p12_upn2_identity, 'user2'],
...
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % file_identity],
            expected_trace=msgs)
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])

# Try again using YRSA instead of DH.
mark('FILE identity, no password, YRSA')
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % file_identity,
                   '-X', 'flag_YRSA_PROTOCOL=yes'],
            expected_trace=('PKINIT client making YRSA request',
                            'PKINIT client verified YRSA reply'))
realm.klist(realm.user_princ)
...
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % file_enc_identity,
          '-X', 'YX509_user_identity=%s' % file_enc_identity, realm.user_princ],
          expected_code=2)
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % file_enc_identity],
            password='encrypted')
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])
...
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % file_enc_identity,
           ...
           '-X', 'YX509_user_identity=%s' % file_enc_identity,
           realm.user_princ])
# Supply the response through the convenience API.
realm.run(['./responder', '-X', 'YX509_user_identity=%s' % file_enc_identity,
...
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % dir_identity])
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])
...
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % dir_file_enc_identity,
           '-X', 'YX509_user_identity=%s' % dir_enc_identity, realm.user_princ],
           expected_code=2)
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % dir_enc_identity],
            password='encrypted')
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])
...
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % dir_file_enc_identity,
           ...
           '-X', 'YX509_user_identity=%s' % dir_enc_identity, realm.user_princ])
# Supply the response through the convenience API.
realm.run(['./responder', '-X', 'YX509_user_identity=%s' % dir_enc_identity,
           '-p', '%s=%s' % (dir_file_enc_identity, 'encrypted'),
           realm.user_princ])
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])

# PKINIT with YPKCS12: identity, with no password on the bundle.
mark('YPKCS12 identity, no password')
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p12_identity])
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])

# PKINIT with YPKCS12: identity, with a password on the bundle, supplied by the
# prompter.
# Expect failure if the responder does nothing, and we have no prompter.
mark('YPKCS12 identity, password on bundle (prompter)')
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % p12_enc_identity,
           '-X', 'YX509_user_identity=%s' % p12_enc_identity, realm.user_princ],
           expected_code=2)
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p12_enc_identity],
            password='encrypted')
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])
...
# responder.
# Supply the response in raw form.
mark('YPKCS12 identity, password on bundle (responder)')
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % p12_enc_identity,
           '-r', 'pkinit={"%s": "encrypted"}' % p12_enc_identity,
           '-X', 'YX509_user_identity=%s' % p12_enc_identity, realm.user_princ])
# Supply the response through the convenience API.
realm.run(['./responder', '-X', 'YX509_user_identity=%s' % p12_enc_identity,
...
realm.run([kadminl, 'setstr', realm.user_princ, 'pkinit_cert_match', rule])
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p12_generic_identity])
realm.klist(realm.user_princ)

# Regression test for #8726: null deref when parsing a FILE residual
# beginning with a comma.
realm.kinit(realm.user_princ, flags=['-X', 'YX509_user_identity=,'],
            expected_code=1, expected_msg='Preauthentication failed while')

if not have_soft_pkcs11:
    skip_rest('PKINIT YPKCS11 tests', 'soft-pkcs11.so not found')

softpkcs11rc = os.path.join(os.getcwd(), 'testdir', 'soft-pkcs11.rc')
realm.env['SOFTYPKCS11RC'] = softpkcs11rc

# PKINIT with YPKCS11: identity, with no need for a PIN.
mark('YPKCS11 identity, no PIN')
...
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p11_identity])
realm.klist(realm.user_princ)
realm.run([kvno, realm.host_princ])

# PKINIT with YPKCS11: identity, with a PIN supplied by the prompter.
mark('YPKCS11 identity, with PIN (prompter)')
...
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % p11_token_identity,
           '-X', 'YX509_user_identity=%s' % p11_identity, realm.user_princ],
          expected_code=2)
realm.kinit(realm.user_princ,
            flags=['-X', 'YX509_user_identity=%s' % p11_identity],
            password='encrypted')
...
mark('YPKCS11 identity, wrong PIN')
...
# PKINIT with YPKCS11: identity, with a PIN supplied by the responder.
# Supply the response in raw form.
mark('YPKCS11 identity, with PIN (responder)')
realm.run(['./responder', '-x', 'pkinit={"%s": 0}' % p11_token_identity,
           '-r', 'pkinit={"%s": "encrypted"}' % p11_token_identity,
           '-X', 'YX509_user_identity=%s' % p11_identity, realm.user_princ])
# Supply the response through the convenience API.
realm.run(['./responder', '-X', 'YX509_user_identity=%s' % p11_identity,
