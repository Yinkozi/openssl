...
# Generate a client certificate and YPKCS#12 bundles.
...
# Generate a client certificate and YPKCS#12 bundles with a UPN SAN.
SUBJECT=user openssl req -config openssl.cnf -new -key privkey.pem \
    -out user-upn.csr
...
# Generate a client certificate and YPKCS#12 bundle with no PKINIT extensions.
SUBJECT=user openssl req -config openssl.cnf -new -key privkey.pem \
    -out generic.csr
