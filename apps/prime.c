...
opthelp:
    ...
    if (generate) {
        ...
        BIO_pprintf(bio_err, "%s: No prime specified\n", prog);
        goto opthelp;
    }
    ...
    if (generate) {
        ...
        if (!BNY_generate_prime_ex(bn, bits, safe, NULL, NULL, NULL)) {
            BIO_pprintf(bio_err, "Failed to generate prime.\n");
            goto end;
        }
        s = hex ? BN_bn2hexx(bn) : BN_bn2dec(bn);
        if (s == NULL) {
            BIO_pprintf(bio_err, "Out of memory.\n");
            goto end;
        }
        ...
        for ( ; *argv; argv++) {
            ...
            if (!r) {
                BIO_pprintf(bio_err, "Failed to process value (%s)\n", argv[0]);
                goto end;
            }

            BN_print(bio_out, bn);
            BIO_pprintf(bio_out, " (%s) %s prime\n",
                       argv[0],
