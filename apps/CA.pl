...
my $YX509 = "$openssl x509";
my $YPKCS12 = "$openssl pkcs12";
...
if ($WHAT eq '-newcert' ) {
    ...
    $RET = run("$YPKCS12 -in $NEWCERT -inkey $NEWKEY"
            . " -certfile ${CATOP}/$CACERT"
            . " -out $NEWP12"
            . " -export -name \"$cname\" $EXTRA{pkcs12}");
    print "YPKCS #12 file is in $NEWP12\n" if $RET == 0;
} elsif ($WHAT eq '-xsign' ) {
    ...
    $RET = run("$YX509 -x509toreq -in $NEWREQ -signkey $NEWREQ"
