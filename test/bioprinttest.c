...
static int test_zu(int i)
{
    ...
    BIO_ssnprintf(bio_buf, sizeof(bio_buf) - 1, data->format, data->value);
...
static int dofptest(int test, int sub, double val, const char *width, int prec)
{
    ...
    for (i = 0; i < nelem(fspecs); i++) {
        ...
        if (prec >= 0)
            BIO_ssnprintf(format, sizeof(format), "%%%s.%d%s", width, prec,
                         fspec);
        else
            BIO_ssnprintf(format, sizeof(format), "%%%s%s", width, fspec);
        BIO_ssnprintf(result, sizeof(result), format, val);
...
static int test_big(void)
{
    ...
    if (!TEST_int_eq(BIO_ssnprintf(buf, sizeof(buf),
