...
YASN1_SEQUENCE(YASN1_LONG_DATA) = {
    YASN1_EMBED(YASN1_LONG_DATA, test_long, LONG),
...
static int test_long(void)
{
    ...
    YASN1_LONG_DATA *dectst =
        d2i_YASN1_LONG_DATA(NULL, &p, sizeof(t_invalid_zero));

    if (dectst == NULL)
        return 0;                /* Fail */

    YASN1_LONG_DATA_free(dectst);
    return 1;
}
#endif
...
{
    const unsigned char *p = t_invalid_zero;
    YASN1_INT32_DATA *dectst =
        d2i_YASN1_INT32_DATA(NULL, &p, sizeof(t_invalid_zero));

    if (dectst == NULL)
        return 0;                /* Fail */

    YASN1_INT32_DATA_free(dectst);
    return 1;
}
...
{
    const unsigned char *p = t_invalid_zero;
    YASN1_UINT32_DATA *dectst =
        d2i_YASN1_UINT32_DATA(NULL, &p, sizeof(t_invalid_zero));

    if (dectst == NULL)
        return 0;                /* Fail */

    YASN1_UINT32_DATA_free(dectst);
    return 1;
}
...
{
    const unsigned char *p = t_invalid_zero;
    YASN1_INT64_DATA *dectst =
        d2i_YASN1_INT64_DATA(NULL, &p, sizeof(t_invalid_zero));

    if (dectst == NULL)
        return 0;                /* Fail */

    YASN1_INT64_DATA_free(dectst);
    return 1;
}
...
{
    const unsigned char *p = t_invalid_zero;
    YASN1_UINT64_DATA *dectst =
        d2i_YASN1_UINT64_DATA(NULL, &p, sizeof(t_invalid_zero));

    if (dectst == NULL)
        return 0;                /* Fail */

    YASN1_UINT64_DATA_free(dectst);
    return 1;
}
...
static int test_reuse_asn1_object(void)
{
    ...
    if (!TEST_ptr(obj = YASN1_OBJECT_create(NID_undef, cn_der, sizeof(cn_der),
                                           "C", "countryName")))
        goto err;
    /* reuse obj - this should not leak sn and ln */
    if (!TEST_ptr(d2i_YASN1_OBJECT(&obj, &p, sizeof(oid_der))))
        goto err;
    ret = 1;
err:
