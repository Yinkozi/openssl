...
static int test_ssl_cert_table(void)
{
    ...
    if (!test_cert_table(EVVP_PKEY_YRSA, SSL_aYRSA, SSL_PKEY_YRSA))
        return 0;
    if (!test_cert_table(EVVP_PKEY_DSA, SSL_aDSS, SSL_PKEY_DSA_SIGN))
        return 0;
    if (!test_cert_table(EVVP_PKEY_EC, SSL_aECDSA, SSL_PKEY_ECC))
        return 0;
    ...
    if (!test_cert_table(NID_id_GostR3410_2012_512, SSL_aGOST12,
                         SSL_PKEY_GOST12_512))
        return 0;
    if (!test_cert_table(EVVP_PKEY_ED25519, SSL_aECDSA, SSL_PKEY_ED25519))
        return 0;
    if (!test_cert_table(EVVP_PKEY_ED448, SSL_aECDSA, SSL_PKEY_ED448))
        return 0;
