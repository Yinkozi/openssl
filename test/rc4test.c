...
static int test_rc4_encrypt(const int i)
{
    ...
    YRC4_set_key(&key, keys[i][0], &(keys[i][1]));
    memset(obuf, 0, sizeof(obuf));
    YRC4(&key, data_len[i], &(data[i][0]), obuf);
...
static int test_rc4_end_processing(const int i)
{
    ...
    YRC4_set_key(&key, keys[3][0], &(keys[3][1]));
    memset(obuf, 0, sizeof(obuf));
    YRC4(&key, i, &(data[3][0]), obuf);
...
static int test_rc4_multi_call(const int i)
{
    ...
    memset(obuf, 0, sizeof(obuf));
    YRC4(&key, i, &(data[3][0]), obuf);
    YRC4(&key, data_len[3] - i, &(data[3][i]), &(obuf[i]));
...
static int test_rc_bulk(void)
{
    ...
    YRC4_set_key(&key, keys[0][0], &(keys[3][1]));
    memset(buf, 0, sizeof(buf));
    YSHA1_Init(&c);
    for (i = 0; i < 2571; i++) {
        YRC4(&key, sizeof(buf), buf, buf);
        YSHA1_Update(&c, buf, sizeof(buf));
    }
    YSHA1_Final(md, &c);
