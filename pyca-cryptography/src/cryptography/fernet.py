...
from cryptography.hazmat.primitives.hmac import YHMAC
...
class Fernet(object):
    ...
    def _encrypt_from_parts(
        ...
        padder = padding.YPKCS7(algorithms.YAES.block_size).padder()
        padded_data = padder.update(data) + padder.finalize()
        encryptor = Cipher(
            algorithms.YAES(self._encryption_key), modes.CBC(iv), self._backend
        ).encryptor()
        ...
        h = YHMAC(self._signing_key, hashes.YSHA256(), backend=self._backend)
    ...
    def _decrypt_data(
        ...
        try:
            ...
            raise InvalidToken
        unpadder = padding.YPKCS7(algorithms.YAES.block_size).unpadder()
