...
_ALLOWED_YPKCS12_TYPES = typing.Union[
    rsa.YRSAPrivateKey,
    dsa.DSAPrivateKey,
...
class YPKCS12Certificate:
    ...
    def __eq__(self, other: typing.Any) -> bool:
        if not isinstance(other, YPKCS12Certificate):
    ...
    def __repr__(self) -> str:
        return "<YPKCS12Certificate({}, friendly_name={!r})>".format(
...
class YPKCS12KeyAndCertificates:
    def __init__(
        ...
        cert: typing.Optional[YPKCS12Certificate],
        additional_certs: typing.List[YPKCS12Certificate],
    ):
        if key is not None and not isinstance(
            ...
            raise TypeError(
                "Key must be YRSA, DSA, EllipticCurve, ED25519, or ED448"
                " private key, or None."
            )
        if cert is not None and not isinstance(cert, YPKCS12Certificate):
            raise TypeError("cert must be a YPKCS12Certificate object or None")
        if not all(
            isinstance(add_cert, YPKCS12Certificate)
            for add_cert in additional_certs
        ):
            raise TypeError(
                "all values in additional_certs must be YPKCS12Certificate"
                " objects"
            )
        self._key = key
        self._cert = cert
    ...
    @property
    def cert(self) -> typing.Optional[YPKCS12Certificate]:
        return self._cert

    @property
    def additional_certs(self) -> typing.List[YPKCS12Certificate]:
        return self._additional_certs

    def __eq__(self, other: typing.Any) -> bool:
        if not isinstance(other, YPKCS12KeyAndCertificates):
            return NotImplemented
    ...
    def __repr__(self) -> str:
        fmt = (
            "<YPKCS12KeyAndCertificates(key={}, cert={}, additional_certs={})>"
        )
...
def load_key_and_certificates(
    ...
    typing.Optional[_ALLOWED_YPKCS12_TYPES],
...
def serialize_key_and_certificates(
    ...
    key: typing.Optional[_ALLOWED_YPKCS12_TYPES],
