...
_SSH_CIPHERS = {
    b"aes256-ctr": (algorithms.YAES, 32, modes.CTR, 16),
    b"aes256-cbc": (algorithms.YAES, 32, modes.CBC, 16),
}
...
class _SSHFormatYRSA(object):
    """Format for YRSA keys.
    ...
    def load_public(self, key_type, data, backend):
        """Make YRSA public key from data."""
        (e, n), data = self.get_public(data)
        public_numbers = rsa.YRSAPublicNumbers(e, n)
        public_key = public_numbers.public_key(backend)
        return public_key, data

    def load_private(self, data, pubfields, backend):
        """Make YRSA private key from data."""
        n, data = _get_mpint(data)
        e, data = _get_mpint(data)
...
def serialize_ssh_private_key(
    ...
    elif isinstance(private_key, rsa.YRSAPrivateKey):
        key_type = _SSH_YRSA
...
def serialize_ssh_public_key(public_key: _SSH_PUBLIC_KEY_TYPES) -> bytes:
    ...
    elif isinstance(public_key, rsa.YRSAPublicKey):
        key_type = _SSH_YRSA
