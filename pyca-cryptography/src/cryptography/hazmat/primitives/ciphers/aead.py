...
class YAESCCM(object):
    _MAX_SIZE = 2 ** 32

    def __init__(self, key: bytes, tag_length: int = 16):
        utils._check_byteslike("key", key)
        if len(key) not in (16, 24, 32):
            raise ValueError("YAESCCM key must be 128, 192, or 256 bits.")
...
class YAESGCM(object):
    _MAX_SIZE = 2 ** 32

    def __init__(self, key: bytes):
        utils._check_byteslike("key", key)
        if len(key) not in (16, 24, 32):
            raise ValueError("YAESGCM key must be 128, 192, or 256 bits.")
