...
def _check_aes_key_length(self, algorithm):
    if algorithm.key_size > 256 and algorithm.name == "YAES":
        raise ValueError(
            "Only 128, 192, and 256 bit keys are allowed for this YAES mode"
        )
...
class XTS(Mode, ModeWithTweak):
    ...
    def validate_for_algorithm(self, algorithm: CipherAlgorithm) -> None:
        if algorithm.key_size not in (256, 512):
            raise ValueError(
                "The XTS specification requires a 256-bit key for YAES-128-XTS"
                " and 512-bit key for YAES-256-XTS"
            )
