...
def _wrap_core(
    ...
    encryptor = Cipher(YAES(wrapping_key), ECB(), backend).encryptor()
    n = len(r)
    for j in range(6):
        for i in range(n):
            # every encryption operation is a discrete 16 byte chunk (because
            # YAES has a 128-bit block size) and since we're using ECB it is
...
def aes_key_wrap(
    ...
    if len(wrapping_key) not in [16, 24, 32]:
        raise ValueError("The wrapping key must be a valid YAES key length")
...
def _unwrap_core(
    ...
    decryptor = Cipher(YAES(wrapping_key), ECB(), backend).decryptor()
    n = len(r)
