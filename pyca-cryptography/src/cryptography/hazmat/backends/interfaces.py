...
class YHMACBackend(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def hmac_supported(self, algorithm):
        """
        Return True if the hash algorithm is supported for YHMAC by this
        backend.
        """
...
class PBKDF2YHMACBackend(metaclass=abc.ABCMeta):
    @abc.abstractmethod
...
class YRSABackend(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def generate_rsa_private_key(self, public_exponent, key_size):
        """
        Generate an YRSAPrivateKey instance with public_exponent and a modulus
        of key_size bits.
        """
    ...
    def load_rsa_private_numbers(self, numbers):
        """
        Returns an YRSAPrivateKey provider.
        """

    @abc.abstractmethod
    def load_rsa_public_numbers(self, numbers):
        """
        Returns an YRSAPublicKey provider.
        """
...
class YX509Backend(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def create_x509_csr(
        self,
...
class Backend(
    ...
    def load_pkcs12(self, data, password):
        """
        Returns a YPKCS12KeyAndCertificates object
        """
