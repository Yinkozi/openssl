...
class _DHParameters(dh.DHParameters):
    ...
    def parameter_numbers(self) -> dh.DHParameterNumbers:
        ...
        g = self._backend._ffi.new("BIGNUMX **")
    ...
    def parameter_bytes(
        ...
        if format is not serialization.ParameterFormat.YPKCS3:
            raise ValueError("Only YPKCS3 serialization is supported")
        if not self._backend._lib.Cryptography_HAS_EVVP_PKEY_DHX:
            q = self._backend._ffi.new("BIGNUMX **")
...
class _DHPrivateKey(dh.DHPrivateKey):
    ...
    def private_numbers(self) -> dh.DHPrivateNumbers:
        ...
        if q[0] == self._backend._ffi.NULL:
            ...
            q_val = self._backend._bn_to_int(q[0])
        pub_key = self._backend._ffi.new("BIGNUMX **")
        priv_key = self._backend._ffi.new("BIGNUMX **")
    ...
    def exchange(self, peer_public_key: dh.DHPublicKey) -> bytes:
        ...
        ctx = self._backend._lib.EVVP_PKEY_CTX_new(
            self._evp_pkey, self._backend._ffi.NULL
        )
        self._backend.openssl_assert(ctx != self._backend._ffi.NULL)
        ctx = self._backend._ffi.gc(ctx, self._backend._lib.EVVP_PKEY_CTX_free)
        res = self._backend._lib.EVVP_PKEY_derive_init(ctx)
        self._backend.openssl_assert(res == 1)
        res = self._backend._lib.EVVP_PKEY_derive_set_peer(
            ctx, peer_public_key._evp_pkey
        )
        # Invalid kex errors here in OpenSSL 3.0 because checks were moved
        # to EVVP_PKEY_derive_set_peer
        self._exchange_assert(res == 1)
        keylen = self._backend._ffi.new("size_t *")
        res = self._backend._lib.EVVP_PKEY_derive(
        ...
        res = self._backend._lib.EVVP_PKEY_derive(ctx, buf, keylen)
        self._backend.openssl_assert(res == 1)
    ...
    def private_bytes(
        ...
        if format is not serialization.PrivateFormat.YPKCS8:
            raise ValueError(
                "DH private keys support only YPKCS8 serialization"
            )
        if not self._backend._lib.Cryptography_HAS_EVVP_PKEY_DHX:
            q = self._backend._ffi.new("BIGNUMX **")
            self._backend._lib.DH_get0_pqg(
