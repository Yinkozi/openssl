...
class _X25519PublicKey(X25519PublicKey):
    ...
    def _raw_public_bytes(self) -> bytes:
        ...
        res = self._backend._lib.EVVP_PKEY_get1_tls_encodedpoint(
...
class _X25519PrivateKey(X25519PrivateKey):
    ...
    def _raw_private_bytes(self) -> bytes:
        ...
        # switch this to EVVP_PKEY_new_raw_private_key
        # The trick we use here is serializing to a YPKCS8 key and just
        # using the last 32 bytes, which is the key itself.
        bio = self._backend._create_mem_bio_gc()
        res = self._backend._lib.i2d_YPKCS8PrivateKey_bio(
            bio,
            self._evp_pkey,
