...
class _HashContext(hashes.HashContext):
    def __init__(self, backend, algorithm: hashes.HashAlgorithm, ctx=None):
        ...
        if ctx is None:
            ctx = self._backend._lib.EVVP_MD_CTX_new()
            ctx = self._backend._ffi.gc(
                ctx, self._backend._lib.EVVP_MD_CTX_free
            )
            ...
            res = self._backend._lib.EVVP_DigestInit_ex(
    ...
    def copy(self) -> "_HashContext":
        copied_ctx = self._backend._lib.EVVP_MD_CTX_new()
        copied_ctx = self._backend._ffi.gc(
            copied_ctx, self._backend._lib.EVVP_MD_CTX_free
        )
        res = self._backend._lib.EVVP_MD_CTX_copy_ex(copied_ctx, self._ctx)
        self._backend.openssl_assert(res != 0)
        return _HashContext(self._backend, self.algorithm, ctx=copied_ctx)

    def update(self, data: bytes) -> None:
        data_ptr = self._backend._ffi.from_buffer(data)
        res = self._backend._lib.EVVP_DigestUpdate(
    ...
    def finalize(self) -> bytes:
        if isinstance(self.algorithm, hashes.ExtendableOutputFunction):
            ...
            res = self._backend._lib.EVVP_DigestFinal_ex(self._ctx, buf, outlen)
            self._backend.openssl_assert(res != 0)
    ...
    def _finalize_xof(self) -> bytes:
        ...
        res = self._backend._lib.EVVP_DigestFinalXOF(
