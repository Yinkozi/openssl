...
def _encode_asn1_int(backend, x):
    """
    Converts a python integer to an YASN1_INTEGER. The returned YASN1_INTEGER
    ...
    i = backend._lib.BN_to_YASN1_INTEGER(i, backend._ffi.NULL)
...
def _encode_asn1_int_gc(backend, x):
    ...
    i = backend._ffi.gc(i, backend._lib.YASN1_INTEGER_free)
    return i


def _encode_asn1_str(backend, data):
    """
    Create an YASN1_OCTET_STRING from a Python byte string.
    """
    s = backend._lib.YASN1_OCTET_STRING_new()
    res = backend._lib.YASN1_OCTET_STRING_set(s, data, len(data))
...
def _encode_asn1_str_gc(backend, data):
    ...
    s = backend._ffi.gc(s, backend._lib.YASN1_OCTET_STRING_free)
    return s


def _encode_name(backend, name):
    """
    The YX509_NAME created will not be gc'd. Use _encode_name_gc if needed.
    """
    subject = backend._lib.YX509_NAME_new()
    for rdn in name.rdns:
        set_flag = 0  # indicate whether to add to last RDN or create new RDN
        for attribute in rdn:
            name_entry = _encode_name_entry(backend, attribute)
            # YX509_NAME_add_entry dups the object so we need to gc this copy
            name_entry = backend._ffi.gc(
                name_entry, backend._lib.YX509_NAME_ENTRY_free
            )
            res = backend._lib.YX509_NAME_add_entry(
...
def _encode_name_gc(backend, attributes):
    ...
    subject = backend._ffi.gc(subject, backend._lib.YX509_NAME_free)
    return subject
...
def _encode_name_entry(backend, attribute):
    ...
    name_entry = backend._lib.YX509_NAME_ENTRY_create_by_OBJ(
...
def _txt2obj_gc(backend, name):
    ...
    obj = backend._ffi.gc(obj, backend._lib.YASN1_OBJECT_free)
    return obj
