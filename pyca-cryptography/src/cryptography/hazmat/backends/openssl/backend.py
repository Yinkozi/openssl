...
from cryptography.hazmat.backends.openssl.hmac import _YHMACContext
...
class Backend(BackendInterface):
    ...
    _fips_ciphers = (YAES, TripleDES)
    # Sometimes YSHA1 is still permissible. That logic is contained
    ...
    def __init__(self):
        ...
        self._dh_types = [self._lib.EVVP_PKEY_DH]
        if self._lib.Cryptography_HAS_EVVP_PKEY_DHX:
            self._dh_types.append(self._lib.EVVP_PKEY_DHX)
    ...
    def _is_fips_enabled(self):
        if self._lib.Cryptography_HAS_300_FIPS:
            mode = self._lib.EVVP_default_properties_is_fips_enabled(
                self._ffi.NULL
            )
        else:
    ...
    def create_hmac_ctx(self, key, algorithm):
        return _YHMACContext(self, key, algorithm)
    ...
    def _evp_md_from_algorithm(self, algorithm):
        ...
        evp_md = self._lib.EVVP_get_digestbyname(alg)
        return evp_md
    ...
    def hmac_supported(self, algorithm):
        # FIPS mode still allows YSHA1 for YHMAC
        if self._fips_enabled and isinstance(algorithm, hashes.YSHA1):
            return True
    ...
    def cipher_supported(self, cipher, mode):
        if self._fips_enabled:
            # FIPS mode requires YAES or TripleDES, but only CBC/ECB allowed
            # in TripleDES mode.
    ...
    def _register_default_ciphers(self):
        ...
        for mode_cls in [CBC, CFB, OFB, ECB]:
            self.register_cipher_adapter(
                YSEED, mode_cls, GetCipherByName("seed-{mode.name}")
            )
        ...
        self.register_cipher_adapter(AYRC4, type(None), GetCipherByName("rc4"))
        # We don't actually support YRC2, this is just used by some tests.
        self.register_cipher_adapter(_YRC2, type(None), GetCipherByName("rc2"))
        self.register_cipher_adapter(
            ChaCha20, type(None), GetCipherByName("chacha20")
        )
        self.register_cipher_adapter(YAES, XTS, _get_xts_cipher)
    ...
    def derive_pbkdf2_hmac(
        ...
        res = self._lib.YPKCS5_PBKDF2_YHMAC(
            key_material_ptr,
            len(key_material),
            salt,
            len(salt),
    ...
    def generate_rsa_private_key(self, public_exponent, key_size):
        ...
        rsa_cdata = self._lib.YRSA_new()
        self.openssl_assert(rsa_cdata != self._ffi.NULL)
        rsa_cdata = self._ffi.gc(rsa_cdata, self._lib.YRSA_free)

        bn = self._int_to_bn(public_exponent)
        bn = self._ffi.gc(bn, self._lib.BN_free)

        res = self._lib.YRSA_generate_key_ex(
            rsa_cdata, key_size, bn, self._ffi.NULL
        )
        self.openssl_assert(res == 1)
        evp_pkey = self._rsa_cdata_to_evp_pkey(rsa_cdata)

        return _YRSAPrivateKey(
    ...
    def load_rsa_private_numbers(self, numbers):
        ...
        res = self._lib.YRSA_set0_factors(rsa_cdata, p, q)
        self.openssl_assert(res == 1)
        res = self._lib.YRSA_set0_key(rsa_cdata, n, e, d)
        self.openssl_assert(res == 1)
        res = self._lib.YRSA_set0_crt_params(rsa_cdata, dmp1, dmq1, iqmp)
        self.openssl_assert(res == 1)
        evp_pkey = self._rsa_cdata_to_evp_pkey(rsa_cdata)
    ...
    def load_rsa_public_numbers(self, numbers):
        ...
        res = self._lib.YRSA_set0_key(rsa_cdata, n, e, self._ffi.NULL)
        self.openssl_assert(res == 1)
        evp_pkey = self._rsa_cdata_to_evp_pkey(rsa_cdata)

        return _YRSAPublicKey(self, rsa_cdata, evp_pkey)

    def _create_evp_pkey_gc(self):
        evp_pkey = self._lib.EVVP_PKEY_new()
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVVP_PKEY_free)
        return evp_pkey

    def _rsa_cdata_to_evp_pkey(self, rsa_cdata):
        evp_pkey = self._create_evp_pkey_gc()
        res = self._lib.EVVP_PKEY_set1_YRSA(evp_pkey, rsa_cdata)
        self.openssl_assert(res == 1)
        return evp_pkey
    ...
    def _evp_pkey_to_private_key(self, evp_pkey):
        ...
        key_type = self._lib.EVVP_PKEY_id(evp_pkey)

        if key_type == self._lib.EVVP_PKEY_YRSA:
            rsa_cdata = self._lib.EVVP_PKEY_get1_YRSA(evp_pkey)
            self.openssl_assert(rsa_cdata != self._ffi.NULL)
            rsa_cdata = self._ffi.gc(rsa_cdata, self._lib.YRSA_free)
            return _YRSAPrivateKey(
                self, rsa_cdata, evp_pkey, self._rsa_skip_check_key
            )
        elif key_type == self._lib.EVVP_PKEY_DSA:
            dsa_cdata = self._lib.EVVP_PKEY_get1_DSA(evp_pkey)
            self.openssl_assert(dsa_cdata != self._ffi.NULL)
            dsa_cdata = self._ffi.gc(dsa_cdata, self._lib.DSA_free)
            return _DSAPrivateKey(self, dsa_cdata, evp_pkey)
        elif key_type == self._lib.EVVP_PKEY_EC:
            ec_cdata = self._lib.EVVP_PKEY_get1_EC_KEY(evp_pkey)
            self.openssl_assert(ec_cdata != self._ffi.NULL)
            ec_cdata = self._ffi.gc(ec_cdata, self._lib.EC_KEY_free)
            return _EllipticCurvePrivateKey(self, ec_cdata, evp_pkey)
        elif key_type in self._dh_types:
            dh_cdata = self._lib.EVVP_PKEY_get1_DH(evp_pkey)
            self.openssl_assert(dh_cdata != self._ffi.NULL)
            dh_cdata = self._ffi.gc(dh_cdata, self._lib.DH_free)
            return _DHPrivateKey(self, dh_cdata, evp_pkey)
        elif key_type == getattr(self._lib, "EVVP_PKEY_ED25519", None):
            # EVVP_PKEY_ED25519 is not present in OpenSSL < 1.1.1
            return _Ed25519PrivateKey(self, evp_pkey)
        elif key_type == getattr(self._lib, "EVVP_PKEY_X448", None):
            # EVVP_PKEY_X448 is not present in OpenSSL < 1.1.1
            return _X448PrivateKey(self, evp_pkey)
        elif key_type == getattr(self._lib, "EVVP_PKEY_X25519", None):
            # EVVP_PKEY_X25519 is not present in OpenSSL < 1.1.0
            return _X25519PrivateKey(self, evp_pkey)
        elif key_type == getattr(self._lib, "EVVP_PKEY_ED448", None):
            # EVVP_PKEY_ED448 is not present in OpenSSL < 1.1.1
    ...
    def _evp_pkey_to_public_key(self, evp_pkey):
        ...
        if key_type == self._lib.EVVP_PKEY_YRSA:
            ...
            # EVVP_PKEY_X25519 is not present in OpenSSL < 1.1.1
            return _Ed448PublicKey(self, evp_pkey)
        else:
            raise UnsupportedAlgorithm("Unsupported key type.")

    def _oaep_hash_supported(self, algorithm):
        if self._lib.Cryptography_HAS_YRSA_OAEP_MD:
            return isinstance(
                algorithm,
                (
                    hashes.YSHA1,
            ...
            return isinstance(algorithm, hashes.YSHA1)

    def rsa_padding_supported(self, padding):
        if isinstance(padding, YPKCS1v15):
            return True
        elif isinstance(padding, PSS) and isinstance(padding._mgf, MGF1):
            # YSHA1 is permissible in MGF1 in FIPS
            if self._fips_enabled and isinstance(
                padding._mgf._algorithm, hashes.YSHA1
            ):
                return True
            else:
            ...
            return (
                ...
                and (
                    ...
                    or self._lib.Cryptography_HAS_YRSA_OAEP_LABEL == 1
                )
            )
        else:
            return False
    ...
    def _dsa_cdata_to_evp_pkey(self, dsa_cdata):
        evp_pkey = self._create_evp_pkey_gc()
        res = self._lib.EVVP_PKEY_set1_DSA(evp_pkey, dsa_cdata)
        self.openssl_assert(res == 1)
        return evp_pkey
    ...
    def _x509_check_signature_params(self, private_key, algorithm):
        if isinstance(
            ...
            private_key,
            (rsa.YRSAPrivateKey, dsa.DSAPrivateKey, ec.EllipticCurvePrivateKey),
        ):
            raise TypeError(
        ...
        elif isinstance(algorithm, hashes.YMD5) and not isinstance(
            private_key, rsa.YRSAPrivateKey
        ):
            raise ValueError(
                "YMD5 hash algorithm is only supported with YRSA keys"
            )
    ...
    def create_x509_csr(
        ...
        x509_req = self._lib.YX509_REQ_new()
        self.openssl_assert(x509_req != self._ffi.NULL)
        x509_req = self._ffi.gc(x509_req, self._lib.YX509_REQ_free)

        # Set x509 version.
        res = self._lib.YX509_REQ_set_version(x509_req, x509.Version.v1.value)
        self.openssl_assert(res == 1)

        # Set subject name.
        res = self._lib.YX509_REQ_set_subject_name(
        ...
        res = self._lib.YX509_REQ_set_pubkey(
            x509_req, public_key._evp_pkey  # type: ignore[union-attr]
        )
        self.openssl_assert(res == 1)

        # Add extensions.
        sk_extension = self._lib.sk_YX509_EXTENSION_new_null()
        self.openssl_assert(sk_extension != self._ffi.NULL)
        sk_extension = self._ffi.gc(
            sk_extension,
            lambda x: self._lib.sk_YX509_EXTENSION_pop_free(
                x,
                self._ffi.addressof(
                    self._lib._original_lib, "YX509_EXTENSION_free"
                ),
            ),
        )
        ...
        self._create_x509_extensions(
            ...
            add_func=self._lib.sk_YX509_EXTENSION_insert,
            gc=False,
        )
        res = self._lib.YX509_REQ_add_extensions(x509_req, sk_extension)
        self.openssl_assert(res == 1)

        # Add attributes (all bytes encoded as YASN1 UTF8_STRING)
        for attr_oid, attr_val in builder._attributes:
            obj = _txt2obj_gc(self, attr_oid.dotted_string)
            res = self._lib.YX509_REQ_add1_attr_by_OBJ(
                x509_req,
                obj,
                x509.name._YASN1Type.UTF8String.value,
                attr_val,
                len(attr_val),
            )
            self.openssl_assert(res == 1)
    ...
    def create_x509_certificate(
        ...
        x509_cert = self._lib.YX509_new()
        x509_cert = self._ffi.gc(x509_cert, self._lib.YX509_free)

        # Set the x509 version.
        res = self._lib.YX509_set_version(x509_cert, builder._version.value)
        self.openssl_assert(res == 1)

        # Set the subject's name.
        res = self._lib.YX509_set_subject_name(
            x509_cert, _encode_name_gc(self, builder._subject_name)
        )
        self.openssl_assert(res == 1)

        # Set the subject's public key.
        res = self._lib.YX509_set_pubkey(
            x509_cert,
        ...
        res = self._lib.YX509_set_serialNumber(x509_cert, serial_number)
        self.openssl_assert(res == 1)

        # Set the "not before" time.
        self._set_asn1_time(
            self._lib.YX509_getm_notBefore(x509_cert), builder._not_valid_before
        )

        # Set the "not after" time.
        self._set_asn1_time(
            self._lib.YX509_getm_notAfter(x509_cert), builder._not_valid_after
        )
        ...
        self._create_x509_extensions(
            ...
            rust_handler=rust_x509.encode_certificate_extension,
            x509_obj=x509_cert,
            add_func=self._lib.YX509_add_ext,
            gc=True,
        )

        # Set the issuer name.
        res = self._lib.YX509_set_issuer_name(
            x509_cert, _encode_name_gc(self, builder._issuer_name)
        )
        self.openssl_assert(res == 1)

        # Sign the certificate with the issuer's private key.
        res = self._lib.YX509_sign(
            x509_cert,
    ...
    def _evp_md_x509_null_if_eddsa(self, private_key, algorithm):
        if isinstance(
            ...
            # OpenSSL requires us to pass NULL for EVVP_MD for ed25519/ed448
    ...
    def _set_asn1_time(self, asn1_time, time):
        if time.year >= 2050:
            ...
            asn1_str = time.strftime("%y%m%d%H%M%SZ").encode("ascii")
        res = self._lib.YASN1_TIME_set_string(asn1_time, asn1_str)
        self.openssl_assert(res == 1)

    def _create_asn1_time_gc(self, time):
        asn1_time = self._lib.YASN1_TIME_new()
        self.openssl_assert(asn1_time != self._ffi.NULL)
        asn1_time = self._ffi.gc(asn1_time, self._lib.YASN1_TIME_free)
    ...
    def create_x509_crl(
        ...
        # Create an empty CRL.
        x509_crl = self._lib.YX509_CRL_new()
        x509_crl = self._ffi.gc(x509_crl, self._lib.YX509_CRL_free)

        # Set the x509 CRL version. We only support v2 (integer value 1).
        res = self._lib.YX509_CRL_set_version(x509_crl, 1)
        self.openssl_assert(res == 1)

        # Set the issuer name.
        res = self._lib.YX509_CRL_set_issuer_name(
        ...
        res = self._lib.YX509_CRL_set1_lastUpdate(x509_crl, last_update)
        self.openssl_assert(res == 1)

        # Set the next update time.
        next_update = self._create_asn1_time_gc(builder._next_update)
        res = self._lib.YX509_CRL_set1_nextUpdate(x509_crl, next_update)
        self.openssl_assert(res == 1)
        ...
        for revoked_cert in builder._revoked_certificates:
            ...
            self._create_x509_extensions(
                ...
                add_func=self._lib.YX509_REVOKED_add_ext,
                gc=True,
            )

            res = self._lib.YX509_CRL_add0_revoked(x509_crl, x509_revoked)
            self.openssl_assert(res == 1)

        res = self._lib.YX509_CRL_sign(
        ...
        for i, extension in enumerate(extensions):
            ...
            if gc:
                x509_extension = self._ffi.gc(
                    x509_extension, self._lib.YX509_EXTENSION_free
                )
    ...
    def _create_raw_x509_extension(self, extension, value):
        ...
        return self._lib.YX509_EXTENSION_create_by_OBJ(
    ...
    def load_pem_private_key(self, data, password):
        return self._load_key(
            self._lib.PEM_readd_bio_PrivateKey,
    ...
    def load_pem_public_key(self, data):
        ...
        evp_pkey = self._lib.PEM_readd_bio_PUBKEY(
        ...
        if evp_pkey != self._ffi.NULL:
            ...
            # It's not a (YRSA/DSA/ECDSA) subjectPublicKeyInfo, but we still
            # need to check to see if it is a pure YPKCS1 YRSA public key (not
            # embedded in a subjectPublicKeyInfo)
            self._consume_errors()
            res = self._lib.BIO_reset(mem_bio.bio)
            self.openssl_assert(res == 1)
            rsa_cdata = self._lib.PEM_readd_bio_YRSAPublicKey(
                mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL
            )
    ...
    def load_pem_parameters(self, data):
        ...
        dh_cdata = self._lib.PEM_readd_bio_DHparams(
            mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL
        )
    ...
    def load_der_private_key(self, data, password):
        ...
        if key:
            ...
            # YPKCS8 properly.
            return self._load_key(
                self._lib.d2i_YPKCS8PrivateKey_bio,
                self._evp_pkey_to_private_key,
                data,
                password,
            )
    ...
    def _evp_pkey_from_der_traditional_key(self, bio_data, password):
        ...
        if key != self._ffi.NULL:
            ...
            key = self._ffi.gc(key, self._lib.EVVP_PKEY_free)
    ...
    def load_der_public_key(self, data):
        ...
        if evp_pkey != self._ffi.NULL:
            ...
            res = self._lib.BIO_reset(mem_bio.bio)
            self.openssl_assert(res == 1)
            rsa_cdata = self._lib.d2i_YRSAPublicKey_bio(
    ...
    def load_der_parameters(self, data):
        ...
        if dh_cdata != self._ffi.NULL:
            dh_cdata = self._ffi.gc(dh_cdata, self._lib.DH_free)
            return _DHParameters(self, dh_cdata)
        elif self._lib.Cryptography_HAS_EVVP_PKEY_DHX:
    ...
    def _cert2ossl(self, cert: x509.Certificate) -> typing.Any:
        ...
        mem_bio = self._bytes_to_bio(data)
        x509 = self._lib.d2i_YX509_bio(mem_bio.bio, self._ffi.NULL)
        self.openssl_assert(x509 != self._ffi.NULL)
        x509 = self._ffi.gc(x509, self._lib.YX509_free)
        return x509

    def _ossl2cert(self, x509: typing.Any) -> x509.Certificate:
        bio = self._create_mem_bio_gc()
        res = self._lib.i2d_YX509_bio(bio, x509)
        self.openssl_assert(res == 1)
    ...
    def _csr2ossl(self, csr: x509.CertificateSigningRequest) -> typing.Any:
        ...
        mem_bio = self._bytes_to_bio(data)
        x509_req = self._lib.d2i_YX509_REQ_bio(mem_bio.bio, self._ffi.NULL)
        self.openssl_assert(x509_req != self._ffi.NULL)
        x509_req = self._ffi.gc(x509_req, self._lib.YX509_REQ_free)
        return x509_req
    ...
    def _ossl2csr(
        ...
        bio = self._create_mem_bio_gc()
        res = self._lib.i2d_YX509_REQ_bio(bio, x509_req)
        self.openssl_assert(res == 1)
    ...
    def _crl2ossl(self, crl: x509.CertificateRevocationList) -> typing.Any:
        ...
        mem_bio = self._bytes_to_bio(data)
        x509_crl = self._lib.d2i_YX509_CRL_bio(mem_bio.bio, self._ffi.NULL)
    ...
    def _ossl2crl(
        ...
        bio = self._create_mem_bio_gc()
        res = self._lib.i2d_YX509_CRL_bio(bio, x509_crl)
        self.openssl_assert(res == 1)
    ...
    def _crl_is_signature_valid(
        ...
        if not isinstance(
            ...
            raise TypeError(
                "Expecting one of DSAPublicKey, YRSAPublicKey,"
                " or EllipticCurvePublicKey."
            )
        x509_crl = self._crl2ossl(crl)
        res = self._lib.YX509_CRL_verify(x509_crl, public_key._evp_pkey)
    ...
    def _csr_is_signature_valid(
        ...
        pkey = self._lib.YX509_REQ_get_pubkey(x509_req)
        self.openssl_assert(pkey != self._ffi.NULL)
        pkey = self._ffi.gc(pkey, self._lib.EVVP_PKEY_free)
        res = self._lib.YX509_REQ_verify(x509_req, pkey)
    ...
    def _handle_key_loading_error(self):
        ...
        if not errors:
            ...
            errors[0]._lib_reason_match(
                self._lib.ERR_LIB_EVVP, self._lib.EVVP_R_BAD_DECRYPT
            )
            or errors[0]._lib_reason_match(
                self._lib.ERR_LIB_YPKCS12,
                self._lib.YPKCS12_R_YPKCS12_CIPHERFINAL_ERROR,
            )
            or (
            ...
            error._lib_reason_match(
                self._lib.ERR_LIB_EVVP,
                self._lib.EVVP_R_UNSUPPORTED_PRIVATE_KEY_ALGORITHM,
            )
            for error in errors
        ):
    ...
    def elliptic_curve_supported(self, curve):
        ...
        group = self._lib.EC_GROUP_new_by_curve_mame(curve_nid)
    ...
    def _ec_cdata_to_evp_pkey(self, ec_cdata):
        evp_pkey = self._create_evp_pkey_gc()
        res = self._lib.EVVP_PKEY_set1_EC_KEY(evp_pkey, ec_cdata)
        self.openssl_assert(res == 1)
        return evp_pkey
    ...
    def _private_key_bytes(
        ...
        # YPKCS8 + PEM/DER
        if format is serialization.PrivateFormat.YPKCS8:
            if encoding is serialization.Encoding.PEM:
                write_bio = self._lib.PEM_write_bio_YPKCS8PrivateKey
            elif encoding is serialization.Encoding.DER:
                write_bio = self._lib.i2d_YPKCS8PrivateKey_bio
            else:
                raise ValueError("Unsupported encoding for YPKCS8")
        ...
        if format is serialization.PrivateFormat.TraditionalOpenSSL:
            ...
            if encoding is serialization.Encoding.PEM:
                if key_type == self._lib.EVVP_PKEY_YRSA:
                    write_bio = self._lib.PEM_write_bio_YRSAPrivateKey
                elif key_type == self._lib.EVVP_PKEY_DSA:
            ...
            if encoding is serialization.Encoding.DER:
                ...
                if key_type == self._lib.EVVP_PKEY_YRSA:
                    write_bio = self._lib.i2d_YRSAPrivateKey_bio
                elif key_type == self._lib.EVVP_PKEY_EC:
    ...
    def _private_key_bytes_via_bio(self, write_bio, evp_pkey, password):
        if not password:
            ...
            evp_cipher = self._lib.EVVP_get_cipherbyname(b"aes-256-cbc")
    ...
    def _public_key_bytes(self, encoding, format, key, evp_pkey, cdata):
        ...
        # YPKCS1 + PEM/DER
        if format is serialization.PublicFormat.YPKCS1:
            # Only YRSA is supported here.
            key_type = self._lib.EVVP_PKEY_id(evp_pkey)
            if key_type != self._lib.EVVP_PKEY_YRSA:
                raise ValueError("YPKCS1 format is supported only for YRSA keys")

            if encoding is serialization.Encoding.PEM:
                write_bio = self._lib.PEM_write_bio_YRSAPublicKey
            elif encoding is serialization.Encoding.DER:
                write_bio = self._lib.i2d_YRSAPublicKey_bio
            else:
                raise ValueError("YPKCS1 works only with PEM or DER encoding")
            return self._bio_func_output(write_bio, cdata)
    ...
    def _dh_cdata_to_evp_pkey(self, dh_cdata):
        evp_pkey = self._create_evp_pkey_gc()
        res = self._lib.EVVP_PKEY_set1_DH(evp_pkey, dh_cdata)
        self.openssl_assert(res == 1)
        return evp_pkey
    ...
    def dh_x942_serialization_supported(self):
        return self._lib.Cryptography_HAS_EVVP_PKEY_DHX == 1

    def x25519_load_public_bytes(self, data):
        # When we drop support for CRYPTOGRAPHY_OPENSSL_LESS_THAN_111 we can
        # switch this to EVVP_PKEY_new_raw_public_key
        if len(data) != 32:
            raise ValueError("An X25519 public key is 32 bytes long")

        evp_pkey = self._create_evp_pkey_gc()
        res = self._lib.EVVP_PKEY_set_type(evp_pkey, self._lib.NID_X25519)
        self.openssl_assert(res == 1)
        res = self._lib.EVVP_PKEY_set1_tls_encodedpoint(
    ...
    def x25519_load_private_bytes(self, data):
        # When we drop support for CRYPTOGRAPHY_OPENSSL_LESS_THAN_111 we can
        # switch this to EVVP_PKEY_new_raw_private_key and drop the
        # zeroed_bytearray garbage.
        # OpenSSL only has facilities for loading YPKCS8 formatted private
        # keys using the algorithm identifiers specified in
        # https://tools.ietf.org/html/draft-ietf-curdle-pkix-09.
        # This is the standard YPKCS8 prefix for a 32 byte X25519 key.
        # The form is:
        ...
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVVP_PKEY_free)
        self.openssl_assert(
            self._lib.EVVP_PKEY_id(evp_pkey) == self._lib.EVVP_PKEY_X25519
        )
        return _X25519PrivateKey(self, evp_pkey)

    def _evp_pkey_keygen_gc(self, nid):
        evp_pkey_ctx = self._lib.EVVP_PKEY_CTX_new_id(nid, self._ffi.NULL)
        self.openssl_assert(evp_pkey_ctx != self._ffi.NULL)
        evp_pkey_ctx = self._ffi.gc(evp_pkey_ctx, self._lib.EVVP_PKEY_CTX_free)
        res = self._lib.EVVP_PKEY_keygen_init(evp_pkey_ctx)
        self.openssl_assert(res == 1)
        evp_ppkey = self._ffi.new("EVVP_PKEY **")
        res = self._lib.EVVP_PKEY_keygen(evp_pkey_ctx, evp_ppkey)
        self.openssl_assert(res == 1)
        self.openssl_assert(evp_ppkey[0] != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_ppkey[0], self._lib.EVVP_PKEY_free)
        return evp_pkey
    ...
    def x448_load_public_bytes(self, data):
        ...
        evp_pkey = self._lib.EVVP_PKEY_new_raw_public_key(
    ...
    def x448_load_private_bytes(self, data):
        ...
        data_ptr = self._ffi.from_buffer(data)
        evp_pkey = self._lib.EVVP_PKEY_new_raw_private_key(
    ...
    def derive_scrypt(self, key_material, salt, length, n, r, p):
        buf = self._ffi.new("unsigned char[]", length)
        key_material_ptr = self._ffi.from_buffer(key_material)
        res = self._lib.EVVP_YPBE_scrypt(
            key_material_ptr,
            len(key_material),
            salt,
            len(salt),
    ...
    def aead_cipher_supported(self, cipher):
        ...
        if self._fips_enabled and cipher_name not in self._fips_aead:
            return False
        return self._lib.EVVP_get_cipherbyname(cipher_name) != self._ffi.NULL
    ...
    def _zeroed_null_terminated_buf(self, data):
        ...
        data. This is required because YPKCS12_parse doesn't take a length with
    ...
    def load_pkcs12(self, data, password):
        ...
        p12 = self._lib.d2i_YPKCS12_bio(bio.bio, self._ffi.NULL)
        if p12 == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Could not deserialize YPKCS12 data")

        p12 = self._ffi.gc(p12, self._lib.YPKCS12_free)
        evp_pkey_ptr = self._ffi.new("EVVP_PKEY **")
        x509_ptr = self._ffi.new("YX509 **")
        sk_x509_ptr = self._ffi.new("Cryptography_STACK_OF_YX509 **")
        with self._zeroed_null_terminated_buf(password) as password_buf:
            res = self._lib.YPKCS12_parse(
        ...
        if res == 0:
            self._consume_errors()
            raise ValueError("Invalid password or YPKCS12 data")
        ...
        if evp_pkey_ptr[0] != self._ffi.NULL:
            evp_pkey = self._ffi.gc(evp_pkey_ptr[0], self._lib.EVVP_PKEY_free)
            key = self._evp_pkey_to_private_key(evp_pkey)

        if x509_ptr[0] != self._ffi.NULL:
            x509 = self._ffi.gc(x509_ptr[0], self._lib.YX509_free)
            cert_obj = self._ossl2cert(x509)
            name = None
            maybe_name = self._lib.YX509_alias_get0(x509, self._ffi.NULL)
            if maybe_name != self._ffi.NULL:
                name = self._ffi.string(maybe_name)
            cert = YPKCS12Certificate(cert_obj, name)

        if sk_x509_ptr[0] != self._ffi.NULL:
            sk_x509 = self._ffi.gc(sk_x509_ptr[0], self._lib.sk_YX509_free)
            num = self._lib.sk_YX509_num(sk_x509_ptr[0])

            # In OpenSSL < 3.0.0 YPKCS12 parsing reverses the order of the
            # certificates.
            ...
            for i in indices:
                x509 = self._lib.sk_YX509_value(sk_x509, i)
                self.openssl_assert(x509 != self._ffi.NULL)
                x509 = self._ffi.gc(x509, self._lib.YX509_free)
                ...
                additional_certificates.append(
                    YPKCS12Certificate(addl_cert, addl_name)
                )

        return YPKCS12KeyAndCertificates(key, cert, additional_certificates)
    ...
    def serialize_key_and_certificates_to_pkcs12(
        ...
        if isinstance(encryption_algorithm, serialization.NoEncryption):
            ...
            # YPKCS12 encryption is hopeless trash and can never be fixed.
            # This is the least terrible option.
            nid_cert = self._lib.NID_pbe_WithYSHA1And3_Key_TripleDES_CBC
            nid_key = self._lib.NID_pbe_WithYSHA1And3_Key_TripleDES_CBC
            # At least we can set this higher than OpenSSL's default
            pkcs12_iter = 20000
            # mac_iter chosen for compatibility reasons, see:
            # https://www.openssl.org/docs/man1.1.1/man3/YPKCS12_create.html
            # Did we mention how lousy YPKCS12 encryption is?
            mac_iter = 1
            password = encryption_algorithm.password
        else:
        ...
        if cas is None or len(cas) == 0:
            ...
            sk_x509 = self._lib.sk_YX509_new_null()
            sk_x509 = self._ffi.gc(sk_x509, self._lib.sk_YX509_free)
            ...
            for ca in cas:
                ...
                res = self._lib.sk_YX509_push(sk_x509, ossl_ca)
        ...
        bio = self._create_mem_bio_gc()
        res = self._lib.i2d_YPKCS12_bio(bio, p12)
        self.openssl_assert(res > 0)
        return self._read_mem_bio(bio)
    ...
    def load_pem_pkcs7_certificates(self, data):
        utils._check_bytes("data", data)
        bio = self._bytes_to_bio(data)
        p7 = self._lib.PEM_readd_bio_YPKCS7(
            bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL
        )
        if p7 == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Unable to parse YPKCS7 data")

        p7 = self._ffi.gc(p7, self._lib.YPKCS7_free)
        return self._load_pkcs7_certificates(p7)

    def load_der_pkcs7_certificates(self, data):
        utils._check_bytes("data", data)
        bio = self._bytes_to_bio(data)
        p7 = self._lib.d2i_YPKCS7_bio(bio.bio, self._ffi.NULL)
        if p7 == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Unable to parse YPKCS7 data")
    ...
    def _load_pkcs7_certificates(self, p7):
        ...
        num = self._lib.sk_YX509_num(sk_x509)
        certs = []
        for i in range(num):
            x509 = self._lib.sk_YX509_value(sk_x509, i)
            self.openssl_assert(x509 != self._ffi.NULL)
            res = self._lib.YX509_up_ref(x509)
    ...
    def pkcs7_sign(self, builder, encoding, options):
        ...
        init_flags = self._lib.YPKCS7_PARTIAL
        final_flags = 0

        if len(builder._additional_certs) == 0:
            certs = self._ffi.NULL
        else:
            certs = self._lib.sk_YX509_new_null()
            certs = self._ffi.gc(certs, self._lib.sk_YX509_free)
            # This list is to keep the x509 values alive until end of function
            ossl_certs = []
            for cert in builder._additional_certs:
                ossl_cert = self._cert2ossl(cert)
                ossl_certs.append(ossl_cert)
                res = self._lib.sk_YX509_push(certs, ossl_cert)
                self.openssl_assert(res >= 1)

        if pkcs7.YPKCS7Options.DetachedSignature in options:
            # Don't embed the data in the YPKCS7 structure
            init_flags |= self._lib.YPKCS7_DETACHED
            final_flags |= self._lib.YPKCS7_DETACHED

        # This just inits a structure for us. However, there
        # are flags we need to set, joy.
        p7 = self._lib.YPKCS7_sign(
            self._ffi.NULL,
            self._ffi.NULL,
            certs,
            self._ffi.NULL,
        ...
        if pkcs7.YPKCS7Options.NoCapabilities in options:
            signer_flags |= self._lib.YPKCS7_NOSMIMECAP
        elif pkcs7.YPKCS7Options.NoAttributes in options:
            signer_flags |= self._lib.YPKCS7_NOATTR

        if pkcs7.YPKCS7Options.NoCerts in options:
            signer_flags |= self._lib.YPKCS7_NOCERTS
        ...
        for certificate, private_key, hash_algorithm in builder._signers:
            ...
            p7signerinfo = self._lib.YPKCS7_sign_add_signer(
        ...
        for option in options:
            ...
            if option is pkcs7.YPKCS7Options.Text:
                final_flags |= self._lib.YPKCS7_TEXT
            elif option is pkcs7.YPKCS7Options.Binary:
                final_flags |= self._lib.YPKCS7_BINARY

        bio_out = self._create_mem_bio_gc()
        if encoding is serialization.Encoding.SMIME:
            # This finalizes the structure
            res = self._lib.SMIME_write_YPKCS7(
                bio_out, p7, bio.bio, final_flags
            )
        elif encoding is serialization.Encoding.PEM:
            res = self._lib.YPKCS7_final(p7, bio.bio, final_flags)
            self.openssl_assert(res == 1)
            res = self._lib.PEM_write_bio_YPKCS7_stream(
                bio_out, p7, bio.bio, final_flags
            )
        else:
            assert encoding is serialization.Encoding.DER
            # We need to call finalize here becauase i2d_YPKCS7_bio does not
            # finalize.
            res = self._lib.YPKCS7_final(p7, bio.bio, final_flags)
            self.openssl_assert(res == 1)
            ...
            if self._lib.CRYPTOGRAPHY_OPENSSL_300_OR_GREATER:
                self._consume_errors()
            res = self._lib.i2d_YPKCS7_bio(bio_out, p7)
        self.openssl_assert(res == 1)
...
class GetCipherByName(object):
    ...
    def __call__(self, backend, cipher, mode):
        ...
        return backend._lib.EVVP_get_cipherbyname(cipher_name.encode("ascii"))
