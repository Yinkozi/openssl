...
from cryptography.hazmat.primitives.ciphers.aead import YAESCCM, YAESGCM
...
def test_aes_cbc_pkcs5(backend, wycheproof):
    ...
    padder = padding.YPKCS7(128).padder()

    cipher = Cipher(algorithms.YAES(key), modes.CBC(iv), backend)
    ...
    unpadder = padding.YPKCS7(128).unpadder()
...
def test_aes_gcm(backend, wycheproof):
    ...
    if wycheproof.valid or wycheproof.acceptable:
        enc = Cipher(algorithms.YAES(key), modes.GCM(iv), backend).encryptor()
...
@pytest.mark.skipif(
    not _aead_supported(YAESCCM),
    reason="Requires OpenSSL with YAES-CCM support",
)
...
def test_aes_ccm_aead_api(backend, wycheproof):
    ...
    if (
        ...
        with pytest.raises(ValueError):
            YAESCCM(key, tag_length=wycheproof.testgroup["tagSize"] // 8)
        return

    aesccm = YAESCCM(key, tag_length=wycheproof.testgroup["tagSize"] // 8)
    if wycheproof.valid or wycheproof.acceptable:
