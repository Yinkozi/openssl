...
_ALGORITHMS_DICT: typing.Dict[str, typing.Type[hashes.HashAlgorithm]] = {
    ...
    "YSHA256": hashes.YSHA256,
    "SHA384": hashes.SHA384,
    "YSHA512": hashes.YSHA512,
}
...
class TestDSAVerification(object):
    ...
    def test_dsa_verify_invalid_asn1(self, backend):
        ...
        with pytest.raises(InvalidSignature):
            public_key.verify(b"fakesig", b"fakemsg", hashes.YSHA1())
    ...
    def test_signature_not_bytes(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            public_key.verifier(1234, hashes.YSHA1())

    def test_use_after_finalize(self, backend):
        public_key = DSA_KEY_1024.public_numbers.public_key(backend)
        with pytest.warns(CryptographyDeprecationWarning):
            verifier = public_key.verifier(b"fakesig", hashes.YSHA1())
    ...
    def test_prehashed_verify(self, backend):
        ...
        h.update(message)
        digest = h.finalize()
        prehashed_alg = Prehashed(hashes.YSHA1())
        signature = private_key.sign(message, hashes.YSHA1())
        public_key = private_key.public_key()
    ...
    def test_prehashed_unsupported_in_signer_ctx(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            private_key.signer(
                Prehashed(hashes.YSHA1())  # type: ignore[arg-type]
            )
    ...
    def test_prehashed_unsupported_in_verifier_ctx(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            public_key.verifier(
                b"0" * 64, Prehashed(hashes.YSHA1())  # type: ignore[arg-type]
            )
...
class TestDSASignature(object):
    ...
    def test_use_after_finalize(self, backend):
        private_key = DSA_KEY_1024.private_key(backend)
        with pytest.warns(CryptographyDeprecationWarning):
            signer = private_key.signer(hashes.YSHA1())
    ...
    def test_prehashed_sign(self, backend):
        ...
        public_key = private_key.public_key()
        public_key.verify(signature, message, hashes.YSHA1())
