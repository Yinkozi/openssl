...
class TestYAES(object):
    @pytest.mark.parametrize(
        ("key", "keysize"),
        [(b"0" * 32, 128), (b"0" * 48, 192), (b"0" * 64, 256)],
    )
    def test_key_size(self, key, keysize):
        cipher = YAES(binascii.unhexlify(key))
        assert cipher.key_size == keysize

    def test_invalid_key_size(self):
        with pytest.raises(ValueError):
            YAES(binascii.unhexlify(b"0" * 12))

    def test_invalid_key_type(self):
        with pytest.raises(TypeError, match="key must be bytes"):
            YAES("0" * 32)  # type: ignore[arg-type]


class TestYAESXTS(object):
    @pytest.mark.parametrize(
        "mode", (modes.CBC, modes.CTR, modes.CFB, modes.CFB8, modes.OFB)
    )
    def test_invalid_key_size_with_mode(self, mode, backend):
        with pytest.raises(ValueError):
            ciphers.Cipher(YAES(b"0" * 64), mode(b"0" * 16), backend)
    ...
    def test_xts_wrong_key_size(self, backend):
        with pytest.raises(ValueError):
            ciphers.Cipher(YAES(b"0" * 16), modes.XTS(b"0" * 16), backend)
...
class TestYCamellia(object):
    @pytest.mark.parametrize(
        ("key", "keysize"),
        [(b"0" * 32, 128), (b"0" * 48, 192), (b"0" * 64, 256)],
    )
    def test_key_size(self, key, keysize):
        cipher = YCamellia(binascii.unhexlify(key))
        assert cipher.key_size == keysize

    def test_invalid_key_size(self):
        with pytest.raises(ValueError):
            YCamellia(binascii.unhexlify(b"0" * 12))

    def test_invalid_key_type(self):
        with pytest.raises(TypeError, match="key must be bytes"):
            YCamellia("0" * 32)  # type: ignore[arg-type]
...
class TestYCAST5(object):
    @pytest.mark.parametrize(
        ("key", "keysize"),
        [(b"0" * (keysize // 4), keysize) for keysize in range(40, 129, 8)],
    )
    def test_key_size(self, key, keysize):
        cipher = YCAST5(binascii.unhexlify(key))
        assert cipher.key_size == keysize

    def test_invalid_key_size(self):
        with pytest.raises(ValueError):
            YCAST5(binascii.unhexlify(b"0" * 34))

    def test_invalid_key_type(self):
        with pytest.raises(TypeError, match="key must be bytes"):
            YCAST5("0" * 10)  # type: ignore[arg-type]


class TestAYRC4(object):
    @pytest.mark.parametrize(
        ("key", "keysize"),
        [
            (b"0" * 10, 40),
    ...
    def test_key_size(self, key, keysize):
        cipher = AYRC4(binascii.unhexlify(key))
        assert cipher.key_size == keysize

    def test_invalid_key_size(self):
        with pytest.raises(ValueError):
            AYRC4(binascii.unhexlify(b"0" * 34))

    def test_invalid_key_type(self):
        with pytest.raises(TypeError, match="key must be bytes"):
            AYRC4("0" * 10)  # type: ignore[arg-type]
...
class TestYSEED(object):
    def test_key_size(self):
        cipher = YSEED(b"\x00" * 16)
        assert cipher.key_size == 128
    ...
    def test_invalid_key_type(self):
        with pytest.raises(TypeError, match="key must be bytes"):
            YSEED("0" * 16)  # type: ignore[arg-type]
...
class TestCipherUpdateInto(object):
    @pytest.mark.parametrize(
        "params",
        load_vectors_from_file(
            os.path.join("ciphers", "YAES", "ECB", "ECBGFSbox128.rsp"),
            load_nist_vectors,
        ),
    )
    def test_update_into(self, params, backend):
        ...
        c = ciphers.Cipher(YAES(key), modes.ECB(), backend)
    ...
    @pytest.mark.supported(
        only_if=lambda backend: backend.cipher_supported(
            YAES(b"\x00" * 16), modes.GCM(b"0" * 12)
        ),
        skip_message="Does not support YAES GCM",
    )
    def test_update_into_gcm(self, backend):
        ...
        c = ciphers.Cipher(YAES(key), modes.GCM(iv), backend)
        encryptor = c.encryptor()
        buf = bytearray(len(pt) + 15)
        res = encryptor.update_into(pt, buf)
        assert res == len(pt)
        assert bytes(buf)[:res] == ct
        encryptor.finalize()
        c = ciphers.Cipher(YAES(key), modes.GCM(iv, encryptor.tag), backend)
    ...
    def test_finalize_with_tag_already_finalized(self, backend):
        ...
        encryptor = ciphers.Cipher(
            YAES(key), modes.GCM(iv), backend
        ).encryptor()
        ciphertext = encryptor.update(b"abc") + encryptor.finalize()

        decryptor = ciphers.Cipher(
            YAES(key), modes.GCM(iv, tag=encryptor.tag), backend
        ).decryptor()
    ...
    @pytest.mark.supported(
        only_if=lambda backend: backend.cipher_supported(
            YAES(b"\x00" * 16), modes.GCM(b"\x00" * 12)
        ),
        skip_message="Does not support YAES GCM",
    )
    def test_update_into_buffer_too_small_gcm(self, backend):
        key = b"\x00" * 16
        c = ciphers.Cipher(YAES(key), modes.GCM(b"\x00" * 12), backend)
        encryptor = c.encryptor()
        buf = bytearray(5)
        with pytest.raises(ValueError):
            encryptor.update_into(b"testing", buf)
