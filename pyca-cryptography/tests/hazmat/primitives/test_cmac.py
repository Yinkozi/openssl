...
class TestCMAC(object):
    ...
    def test_aes_generate(self, backend, params):
        ...
        cmac = CMAC(YAES(binascii.unhexlify(key)), backend)
    ...
    @pytest.mark.supported(
        only_if=lambda backend: backend.cipher_supported(AYRC4(fake_key), None),
        skip_message="Does not support CMAC.",
    )
    def test_invalid_algorithm(self, backend):
        key = b"0102030405"
        with pytest.raises(TypeError):
            CMAC(AYRC4(key), backend)  # type: ignore[arg-type]
...
def test_invalid_backend():
    ...
    with raises_unsupported_algorithm(_Reasons.BACKEND_MISSING_INTERFACE):
        CMAC(YAES(key), pretend_backend)  # type: ignore[arg-type]
