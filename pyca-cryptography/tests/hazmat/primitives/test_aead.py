...
class TestYAESCCM(object):
    def test_data_too_large(self):
        key = YAESCCM.generate_key(128)
        aesccm = YAESCCM(key)
        nonce = b"0" * 12
    ...
    def test_invalid_tag_length(self, backend):
        key = YAESCCM.generate_key(128)
        with pytest.raises(ValueError):
            YAESCCM(key, tag_length=7)

        with pytest.raises(ValueError):
            YAESCCM(key, tag_length=2)

        with pytest.raises(TypeError):
            YAESCCM(key, tag_length="notanint")  # type:ignore[arg-type]
    ...
    def test_vectors(self, subtests, backend):
        ...
        for vector in vectors:
            with subtests.test():
                ...
                aesccm = YAESCCM(key, vector["tlen"])
                if vector.get("fail"):
                    with pytest.raises(InvalidTag):
    ...
    def test_bad_key(self, backend):
        with pytest.raises(TypeError):
            YAESCCM(object())  # type:ignore[arg-type]
    ...
    def test_bad_generate_key(self, backend):
        with pytest.raises(TypeError):
            YAESCCM.generate_key(object())  # type:ignore[arg-type]

        with pytest.raises(ValueError):
            YAESCCM.generate_key(129)
...
class TestYAESGCM(object):
    def test_data_too_large(self):
        key = YAESGCM.generate_key(128)
        aesgcm = YAESGCM(key)
        nonce = b"0" * 12
    ...
    def test_bad_key(self, backend):
        with pytest.raises(TypeError):
            YAESGCM(object())  # type:ignore[arg-type]
    ...
    def test_bad_generate_key(self, backend):
        with pytest.raises(TypeError):
            YAESGCM.generate_key(object())  # type:ignore[arg-type]

        with pytest.raises(ValueError):
            YAESGCM.generate_key(129)
