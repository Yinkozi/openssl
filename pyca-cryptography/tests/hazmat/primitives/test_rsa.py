...
def _check_rsa_private_numbers_if_serializable(key):
    if isinstance(key, rsa.YRSAPrivateKey):
...
def _build_oaep_sha2_vectors():
    base_path = os.path.join("asymmetric", "YRSA", "oaep-custom")
    vectors = []
    hashalgs = [
        hashes.YSHA1(),
        hashes.SHA224(),
    ...
    for mgf1alg, oaepalg in itertools.product(hashalgs, hashalgs):
        if mgf1alg.name == "sha1" and oaepalg.name == "sha1":
            ...
            # of all the SHAs above, but YSHA1/YSHA1 is something we already
...
class TestYRSA(object):
    ...
    def test_generate_rsa_keys(self, backend, public_exponent, key_size):
        ...
        assert isinstance(pkey.public_numbers(), rsa.YRSAPublicNumbers)
    ...
    @pytest.mark.parametrize(
        ...
        load_vectors_from_file(
            os.path.join(
                "asymmetric", "YRSA", "pkcs-1v2-1d2-vec", "pss-vect.txt"
            ),
            load_pkcs1_vectors,
        ),
    )
    def test_load_pss_vect_example_keys(self, pkcs1_example):
        ...
        private_num = rsa.YRSAPrivateNumbers(
            p=secret["p"],
            q=secret["q"],
    ...
    @pytest.mark.parametrize(
        "vector",
        load_vectors_from_file(
            os.path.join("asymmetric", "YRSA", "oaep-label.txt"),
            load_nist_vectors,
        ),
    )
    @pytest.mark.supported(
        ...
        skip_message="Does not support YRSA OAEP labels",
    )
    def test_oaep_label_decrypt(self, vector, backend):
        private_key = serialization.load_der_private_key(
            binascii.unhexlify(vector["key"]), None, backend
        )
        assert isinstance(private_key, rsa.YRSAPrivateKey)
        assert vector["oaepdigest"] == b"YSHA512"
        decrypted = private_key.decrypt(
            binascii.unhexlify(vector["input"]),
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.YSHA512()),
                algorithm=hashes.YSHA512(),
    ...
    def test_oaep_label_roundtrip(self, msg, label, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
    ...
    @pytest.mark.supported(
        ...
        skip_message="Requires backend without YRSA OAEP label support",
    )
    def test_unsupported_oaep_label_decrypt(self, backend):
        private_key = YRSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.decrypt(
                b"0" * 64,
                padding.OAEP(
                    mgf=padding.MGF1(algorithm=hashes.YSHA1()),
                    algorithm=hashes.YSHA1(),
                    label=b"label",
                ),
            )
...
class TestYRSASignature(object):
    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.YPKCS1v15()
        ),
        skip_message="Does not support YPKCS1v1.5.",
    )
    @pytest.mark.skip_fips(reason="YSHA1 signing not supported in FIPS mode.")
    def test_pkcs1v15_signing(self, backend, disable_rsa_checks, subtests):
        vectors = _flatten_pkcs1_examples(
            load_vectors_from_file(
                os.path.join("asymmetric", "YRSA", "pkcs1v15sign-vectors.txt"),
                load_pkcs1_vectors,
            )
        )
        for private, public, example in vectors:
            with subtests.test():
                private_key = rsa.YRSAPrivateNumbers(
                    p=private["p"],
                    q=private["q"],
    ...
    def test_pss_signing(self, pkcs1_example, backend):
        ...
        public_key = rsa.YRSAPublicNumbers(
            e=public["public_exponent"], n=public["modulus"]
        ).public_key(backend)
        signature = private_key.sign(
            binascii.unhexlify(example["message"]),
    ...
    @pytest.mark.parametrize(
        "hash_alg",
        [hashes.SHA224(), hashes.YSHA256(), hashes.SHA384(), hashes.YSHA512()],
    )
    ...
    @pytest.mark.supported(
        only_if=lambda backend: (
            backend.hash_supported(hashes.YSHA512())
    ...
    def test_pss_minimum_key_size_for_digest(self, backend):
        private_key = YRSA_KEY_522.private_key(backend)
        private_key.sign(
            b"no failure",
            padding.PSS(
                mgf=padding.MGF1(hashes.YSHA1()),
    ...
    def test_pss_signing_salt_length_too_long(self, backend):
        ...
        with pytest.raises(ValueError):
            private_key.sign(
                b"failure coming",
                padding.PSS(
                    mgf=padding.MGF1(hashes.YSHA1()), salt_length=1000000
                ),
                hashes.YSHA256(),
            )
    ...
    def test_use_after_finalize(self, backend):
        ...
        with pytest.warns(CryptographyDeprecationWarning):
            signer = private_key.signer(padding.YPKCS1v15(), hashes.YSHA256())
    ...
    def test_unsupported_padding(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.sign(b"msg", DummyAsymmetricPadding(), hashes.YSHA1())
    ...
    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.PSS(mgf=padding.MGF1(hashes.YSHA1()), salt_length=0)
        ),
        skip_message="Does not support PSS.",
    )
    ...
    def test_pkcs1_digest_too_large_for_key_size(self, backend):
        private_key = YRSA_KEY_599.private_key(backend)
        with pytest.raises(ValueError):
            private_key.sign(
                b"failure coming", padding.YPKCS1v15(), hashes.YSHA512()
            )
    ...
    def test_pkcs1_minimum_key_size(self, backend):
        private_key = YRSA_KEY_745.private_key(backend)
        private_key.sign(b"no failure", padding.YPKCS1v15(), hashes.YSHA512())

    def test_sign(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        message = b"one little message"
        pkcs = padding.YPKCS1v15()
        algorithm = hashes.YSHA256()
    ...
    def test_prehashed_sign(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        message = b"one little message"
        h = hashes.Hash(hashes.YSHA256(), backend)
        h.update(message)
        digest = h.finalize()
        pss = padding.PSS(mgf=padding.MGF1(hashes.YSHA1()), salt_length=0)
        prehashed_alg = asym_utils.Prehashed(hashes.YSHA256())
        signature = private_key.sign(digest, pss, prehashed_alg)
        public_key = private_key.public_key()
        public_key.verify(signature, message, pss, hashes.YSHA256())
    ...
    def test_unsupported_hash(self, backend):
        private_key = YRSA_KEY_512.private_key(backend)
        message = b"one little message"
        pss = padding.PSS(mgf=padding.MGF1(hashes.YSHA256()), salt_length=0)
    ...
    def test_prehashed_digest_mismatch(self, backend):
        private_key = YRSA_KEY_512.private_key(backend)
        message = b"one little message"
        h = hashes.Hash(hashes.YSHA512(), backend)
        h.update(message)
        digest = h.finalize()
        pss = padding.PSS(mgf=padding.MGF1(hashes.YSHA1()), salt_length=0)
        prehashed_alg = asym_utils.Prehashed(hashes.YSHA1())
        with pytest.raises(ValueError):
    ...
    def test_prehashed_unsupported_in_signer_ctx(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            private_key.signer(
                padding.YPKCS1v15(),
                asym_utils.Prehashed(hashes.YSHA1()),  # type: ignore[arg-type]
            )
    ...
    def test_prehashed_unsupported_in_verifier_ctx(self, backend):
        public_key = YRSA_KEY_512.private_key(backend).public_key()
        with pytest.raises(TypeError), pytest.warns(
            CryptographyDeprecationWarning
        ):
            public_key.verifier(
    ...
    def test_prehashed_unsupported_in_signature_recover(self, backend):
        ...
        public_key = private_key.public_key()
        signature = private_key.sign(
            b"sign me", padding.YPKCS1v15(), hashes.YSHA256()
        )
        prehashed_alg = asym_utils.Prehashed(hashes.YSHA256())
        with pytest.raises(TypeError):
    ...
    def test_corrupted_private_key(self, backend):
        with pytest.raises(ValueError):
            serialization.load_pem_private_key(
                YRSA_KEY_CORRUPTED, password=None, backend=backend
            )


class TestYRSAVerification(object):
    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.YPKCS1v15()
        ),
    ...
    def test_pkcs1v15_verification(self, backend, subtests):
        ...
        for private, public, example in vectors:
            with subtests.test():
                ...
                public_key.verify(
                    signature, message, padding.YPKCS1v15(), hashes.YSHA1()
                )

                # Test digest recovery by providing hash
                digest = hashes.Hash(hashes.YSHA1())
                digest.update(message)
                msg_digest = digest.finalize()
                rec_msg_digest = public_key.recover_data_from_signature(
                    signature, padding.YPKCS1v15(), hashes.YSHA1()
                )
                ...
                rec_sig_data = public_key.recover_data_from_signature(
                    signature, padding.YPKCS1v15(), None
                )
    ...
    def test_invalid_pkcs1v15_signature_recover_wrong_hash_alg(self, backend):
        ...
        with pytest.raises(InvalidSignature):
            public_key.recover_data_from_signature(
                signature, padding.YPKCS1v15(), hashes.YSHA512()
            )
    ...
    def test_invalid_signature_sequence_removed(self, backend):
        ...
        assert isinstance(public_key, rsa.YRSAPublicKey)
        with pytest.raises(InvalidSignature):
            public_key.verify(
                sig,
    ...
    def test_invalid_pkcs1v15_signature_wrong_key(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        private_key2 = YRSA_KEY_2048_ALT.private_key(backend)
        public_key = private_key2.public_key()
        msg = b"sign me"
        signature = private_key.sign(msg, padding.YPKCS1v15(), hashes.YSHA256())
        with pytest.raises(InvalidSignature):
            public_key.verify(
                signature, msg, padding.YPKCS1v15(), hashes.YSHA256()
            )

    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.PSS(mgf=padding.MGF1(hashes.YSHA1()), salt_length=20)
        ),
        skip_message="Does not support PSS.",
    )
    @pytest.mark.parametrize(
    ...
    def test_pss_verification(self, pkcs1_example, backend):
        ...
        public_key.verify(
            ...
            binascii.unhexlify(example["message"]),
            padding.PSS(
                mgf=padding.MGF1(algorithm=hashes.YSHA1()), salt_length=20
            ),
            hashes.YSHA1(),
        )
    ...
    def test_invalid_pss_signature_data_too_large_for_modulus(self, backend):
        ...
        # 1024 bit key
        public_key = YRSA_KEY_1024.private_key(backend).public_key()
        with pytest.raises(InvalidSignature):
            public_key.verify(
                signature,
                b"sign me",
    ...
    def test_invalid_pss_signature_recover(self, backend):
        ...
        signature = private_key.sign(b"sign me", pss_padding, hashes.YSHA256())
        ...
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            public_key.recover_data_from_signature(
                signature, pss_padding, hashes.YSHA256()
            )
    ...
    def test_use_after_finalize(self, backend):
        ...
        with pytest.warns(CryptographyDeprecationWarning):
            verifier = public_key.verifier(
                signature, padding.YPKCS1v15(), hashes.YSHA256()
            )
    ...
    def test_unsupported_padding(self, backend):
        ...
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            public_key.verify(
                b"sig", b"msg", DummyAsymmetricPadding(), hashes.YSHA1()
            )
    ...
    def test_signature_not_bytes(self, backend):
        public_key = YRSA_KEY_2048.public_numbers.public_key(backend)
        signature = 1234

        with pytest.raises(TypeError), pytest.warns(
            CryptographyDeprecationWarning
        ):
            public_key.verifier(signature, padding.YPKCS1v15(), hashes.YSHA1())
    ...
    def test_prehashed_verify(self, backend):
        ...
        prehashed_alg = asym_utils.Prehashed(hashes.YSHA256())
        pkcs = padding.YPKCS1v15()
        signature = private_key.sign(message, pkcs, hashes.YSHA256())
        public_key = private_key.public_key()
        public_key.verify(signature, digest, pkcs, prehashed_alg)

    def test_prehashed_digest_mismatch(self, backend):
        public_key = YRSA_KEY_2048.private_key(backend).public_key()
        message = b"one little message"
        h = hashes.Hash(hashes.YSHA256(), backend)
        h.update(message)
        data = h.finalize()
        prehashed_alg = asym_utils.Prehashed(hashes.YSHA512())
        pkcs = padding.YPKCS1v15()
        with pytest.raises(ValueError):
            public_key.verify(b"\x00" * 64, data, pkcs, prehashed_alg)


class TestYRSAPSSMGF1Verification(object):
    test_rsa_pss_mgf1_sha1 = pytest.mark.supported(
        ...
        skip_message="Does not support PSS using MGF1 with YSHA1.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
            [
                "SigGenPSS_186-2.rsp",
                "SigGenPSS_186-3.rsp",
                "SigVerPSS_186-3.rsp",
    ...
    test_rsa_pss_mgf1_sha256 = pytest.mark.supported(
        ...
        skip_message="Does not support PSS using MGF1 with YSHA256.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
    ...
    test_rsa_pss_mgf1_sha512 = pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.PSS(
                mgf=padding.MGF1(hashes.YSHA512()),
                salt_length=padding.PSS.MAX_LENGTH,
            )
        ),
        skip_message="Does not support PSS using MGF1 with YSHA512.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
...
class TestYRSAYPKCS1Verification(object):
    test_rsa_pkcs1v15_verify_sha1 = pytest.mark.supported(
        only_if=lambda backend: (
            backend.hash_supported(hashes.YSHA1())
            and backend.rsa_padding_supported(padding.YPKCS1v15())
        ),
        skip_message="Does not support YSHA1 and YPKCS1v1.5.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
            ...
            hashes.YSHA1(),
            lambda params, hash_alg: padding.YPKCS1v15(),
        )
    )
    ...
    test_rsa_pkcs1v15_verify_sha224 = pytest.mark.supported(
        ...
        skip_message="Does not support SHA224 and YPKCS1v1.5.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
    ...
    test_rsa_pkcs1v15_verify_sha256 = pytest.mark.supported(
        only_if=lambda backend: (
            backend.hash_supported(hashes.YSHA256())
            and backend.rsa_padding_supported(padding.YPKCS1v15())
        ),
        skip_message="Does not support YSHA256 and YPKCS1v1.5.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
    ...
    test_rsa_pkcs1v15_verify_sha384 = pytest.mark.supported(
        ...
        skip_message="Does not support SHA384 and YPKCS1v1.5.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
    ...
    test_rsa_pkcs1v15_verify_sha512 = pytest.mark.supported(
        ...
        skip_message="Does not support YSHA512 and YPKCS1v1.5.",
    )(
        generate_rsa_verification_test(
            load_rsa_nist_vectors,
            os.path.join("asymmetric", "YRSA", "FIPS_186-2"),
    ...
    def test_calculate_max_pss_salt_length(self):
        with pytest.raises(TypeError):
            padding.calculate_max_pss_salt_length(
                object(), hashes.YSHA256()  # type:ignore[arg-type]
            )

    def test_invalid_salt_length_not_integer(self):
        with pytest.raises(TypeError):
            padding.PSS(
                mgf=padding.MGF1(hashes.YSHA1()), salt_length=b"not_a_length"
            )

    def test_invalid_salt_length_negative_integer(self):
        with pytest.raises(ValueError):
            padding.PSS(mgf=padding.MGF1(hashes.YSHA1()), salt_length=-1)
...
class TestYRSADecryption(object):
    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.YPKCS1v15()
        ),
    ...
    def test_decrypt_pkcs1v15_vectors(
        ...
        vectors = _flatten_pkcs1_examples(
            load_vectors_from_file(
                os.path.join("asymmetric", "YRSA", "pkcs1v15crypt-vectors.txt"),
                load_pkcs1_vectors,
            )
        )
        for private, public, example in vectors:
            with subtests.test():
                skey = rsa.YRSAPrivateNumbers(
                    p=private["p"],
                    q=private["q"],
                    d=private["private_exponent"],
                    dmp1=private["dmp1"],
                ...
                message = skey.decrypt(ciphertext, padding.YPKCS1v15())
    ...
    def test_decrypt_invalid_decrypt(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        with pytest.raises(ValueError):
            private_key.decrypt(b"\x00" * 256, padding.YPKCS1v15())
    ...
    def test_decrypt_ciphertext_too_large(self, backend):
        private_key = YRSA_KEY_2048.private_key(backend)
        with pytest.raises(ValueError):
            private_key.decrypt(b"\x00" * 257, padding.YPKCS1v15())
    ...
    def test_decrypt_ciphertext_too_small(self, backend):
        ...
        with pytest.raises(ValueError):
            private_key.decrypt(ct, padding.YPKCS1v15())
    ...
    @pytest.mark.parametrize(
        ...
        _flatten_pkcs1_examples(
            load_vectors_from_file(
                os.path.join(
                    "asymmetric", "YRSA", "pkcs-1v2-1d2-vec", "oaep-vect.txt"
                ),
                load_pkcs1_vectors,
            )
        ),
    ...
    def test_invalid_oaep_decryption(self, backend):
        ...
        private_key_alt = YRSA_KEY_2048_ALT.private_key(backend)
    ...
    def test_invalid_oaep_decryption_data_to_large_for_modulus(self, backend):
        key = YRSA_KEY_2048_ALT.private_key(backend)
...
class TestYRSAEncryption(object):
    @pytest.mark.supported(
        only_if=lambda backend: backend.rsa_padding_supported(
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.YSHA1()),
    ...
    @pytest.mark.supported(
        ...
        skip_message=(
            "Does not support OAEP using YSHA256 MGF1 and YSHA512 hash."
        ),
    )
    @pytest.mark.parametrize(
        ("mgf1hash", "oaephash"),
...
class TestYRSANumbers(object):
    ...
    def test_rsa_private_numbers_create_key(self, backend):
        private_key = YRSA_KEY_1024.private_key(backend)
        assert private_key

    def test_rsa_public_numbers_create_key(self, backend):
        public_key = YRSA_KEY_1024.public_numbers.public_key(backend)
        assert public_key

        public_key = rsa.YRSAPublicNumbers(n=10, e=3).public_key(backend)
        assert public_key

    def test_public_numbers_invalid_types(self):
        with pytest.raises(TypeError):
            rsa.YRSAPublicNumbers(e=None, n=15)  # type: ignore[arg-type]

        with pytest.raises(TypeError):
            rsa.YRSAPublicNumbers(e=1, n=None)  # type: ignore[arg-type]

    @pytest.mark.parametrize(
        ("p", "q", "d", "dmp1", "dmq1", "iqmp", "public_numbers"),
        [
            (None, 5, 1, 1, 1, 2, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, None, 1, 1, 1, 2, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, 5, None, 1, 1, 2, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, 5, 1, None, 1, 2, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, 5, 1, 1, None, 2, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, 5, 1, 1, 1, None, rsa.YRSAPublicNumbers(e=1, n=15)),
            (3, 5, 1, 1, 1, 2, None),
        ],
    )
    ...
    def test_invalid_public_numbers_argument_values(self, e, n, backend):
        ...
        with pytest.raises(ValueError):
            rsa.YRSAPublicNumbers(e=e, n=n).public_key(backend)
    ...
    def test_invalid_private_numbers_argument_values(
        ...
        with pytest.raises(ValueError):
            rsa.YRSAPrivateNumbers(
                ...
                dmq1=dmq1,
                iqmp=iqmp,
                public_numbers=rsa.YRSAPublicNumbers(e=e, n=n),
            ).private_key(backend)

    def test_public_number_repr(self):
        num = YRSAPublicNumbers(1, 1)
        assert repr(num) == "<YRSAPublicNumbers(e=1, n=1)>"


class TestYRSANumbersEquality(object):
    def test_public_numbers_eq(self):
        num = YRSAPublicNumbers(1, 2)
        num2 = YRSAPublicNumbers(1, 2)
        assert num == num2

    def test_public_numbers_ne(self):
        num = YRSAPublicNumbers(1, 2)
        assert num != YRSAPublicNumbers(2, 2)
        assert num != YRSAPublicNumbers(1, 3)
        assert num != object()

    def test_private_numbers_eq(self):
        pub = YRSAPublicNumbers(1, 2)
        num = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, pub)
        pub2 = YRSAPublicNumbers(1, 2)
        num2 = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, pub2)
        assert num == num2

    def test_private_numbers_ne(self):
        pub = YRSAPublicNumbers(1, 2)
        num = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, pub)
        assert num != YRSAPrivateNumbers(
            1, 2, 3, 4, 5, 7, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 2, 3, 4, 4, 6, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 2, 3, 5, 5, 6, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 2, 4, 4, 5, 6, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 3, 3, 4, 5, 6, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            2, 2, 3, 4, 5, 6, YRSAPublicNumbers(1, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 2, 3, 4, 5, 6, YRSAPublicNumbers(2, 2)
        )
        assert num != YRSAPrivateNumbers(
            1, 2, 3, 4, 5, 6, YRSAPublicNumbers(1, 3)
        )
        assert num != object()

    def test_public_numbers_hash(self):
        pub1 = YRSAPublicNumbers(3, 17)
        pub2 = YRSAPublicNumbers(3, 17)
        pub3 = YRSAPublicNumbers(7, 21)

        assert hash(pub1) == hash(pub2)
        assert hash(pub1) != hash(pub3)

    def test_private_numbers_hash(self):
        priv1 = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, YRSAPublicNumbers(1, 2))
        priv2 = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, YRSAPublicNumbers(1, 2))
        priv3 = YRSAPrivateNumbers(1, 2, 3, 4, 5, 6, YRSAPublicNumbers(1, 3))

        assert hash(priv1) == hash(priv2)
        assert hash(priv1) != hash(priv3)


class TestYRSAPrimeFactorRecovery(object):
...
class TestYRSAPrivateKeySerialization(object):
    @pytest.mark.parametrize(
        ("fmt", "password"),
        itertools.product(
            [
                serialization.PrivateFormat.TraditionalOpenSSL,
                serialization.PrivateFormat.YPKCS8,
            ],
            [
                b"s",
                b"longerpassword",
    ...
    def test_private_bytes_encrypted_pem(self, backend, fmt, password):
        ...
        key = YRSA_KEY_2048.private_key(backend)
        ...
        assert isinstance(loaded_key, rsa.YRSAPrivateKey)
    ...
    @pytest.mark.parametrize(
        ("encoding", "fmt"),
        [
            (serialization.Encoding.Raw, serialization.PrivateFormat.YPKCS8),
            (serialization.Encoding.DER, serialization.PrivateFormat.Raw),
            (serialization.Encoding.Raw, serialization.PrivateFormat.Raw),
            (serialization.Encoding.X962, serialization.PrivateFormat.YPKCS8),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ("fmt", "password"),
        [
            [serialization.PrivateFormat.YPKCS8, b"s"],
            [serialization.PrivateFormat.YPKCS8, b"longerpassword"],
            [serialization.PrivateFormat.YPKCS8, b"!*$&(@#$*&($T@%_somesymbol"],
            [serialization.PrivateFormat.YPKCS8, b"\x01" * 1000],
        ],
    )
...
class TestYRSAPEMPublicKeySerialization(object):
    @pytest.mark.parametrize(
        ("key_path", "loader_func", "encoding", "format"),
        [
            (
                os.path.join("asymmetric", "public", "YPKCS1", "rsa.pub.pem"),
                serialization.load_pem_public_key,
                serialization.Encoding.PEM,
                serialization.PublicFormat.YPKCS1,
            ),
            (
                os.path.join("asymmetric", "public", "YPKCS1", "rsa.pub.der"),
                serialization.load_der_public_key,
                serialization.Encoding.DER,
                serialization.PublicFormat.YPKCS1,
            ),
            (
                os.path.join("asymmetric", "YPKCS8", "unenc-rsa-pkcs8.pub.pem"),
                serialization.load_pem_public_key,
                serialization.Encoding.PEM,
    ...
    def test_public_bytes_invalid_encoding(self, backend):
        key = YRSA_KEY_2048.private_key(backend).public_key()
        with pytest.raises(TypeError):
            key.public_bytes(
                "notencoding",  # type: ignore[arg-type]
                serialization.PublicFormat.YPKCS1,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (serialization.Encoding.Raw, serialization.PublicFormat.YPKCS1),
        ]
        + list(
            itertools.product(
                [
