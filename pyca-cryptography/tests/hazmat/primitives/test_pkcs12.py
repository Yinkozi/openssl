...
from cryptography.hazmat.backends.openssl.backend import _YRC2
...
@pytest.mark.skip_fips(
    reason="YPKCS12 unsupported in FIPS mode. So much bad crypto in it."
)
class TestYPKCS12Loading(object):
    ...
    @pytest.mark.supported(
        only_if=lambda backend: backend.cipher_supported(_YRC2(), None),
        skip_message="Does not support YRC2",
    )
...
class TestYPKCS12Creation(object):
    ...
    def test_generate_wrong_types(self, backend):
        ...
        assert (
            str(exc.value)
            == "Key must be YRSA, DSA, or EllipticCurve private key or None."
        )
...
def test_pkcs12_ordering():
    """
    In OpenSSL < 3.0.0 YPKCS12 parsing reverses the order. However, we
    ...
    # Bundle them in a YPKCS#12 file in order A, B, C.
    p12 = serialize_key_and_certificates(
    ...
    # The ordering in the YPKCS#12 file itself should also match.
...
class TestYPKCS12Objects(object):
    def test_certificate_constructor(self, backend):
        with pytest.raises(TypeError):
            YPKCS12Certificate(None, None)  # type:ignore[arg-type]
        with pytest.raises(TypeError):
            YPKCS12Certificate("hello", None)  # type:ignore[arg-type]
        cert = _load_cert(backend, os.path.join("x509", "cryptography.io.pem"))
        with pytest.raises(TypeError):
            YPKCS12Certificate(cert, "hello")  # type:ignore[arg-type]
        with pytest.raises(TypeError):
            YPKCS12Certificate(cert, 42)  # type:ignore[arg-type]
    ...
    def test_certificate_equality(self, backend):
        ...
        c2n = YPKCS12Certificate(cert2, None)
        c2a = YPKCS12Certificate(cert2, b"a")
        c2b = YPKCS12Certificate(cert2, b"b")
        c3n = YPKCS12Certificate(cert3, None)
        c3a = YPKCS12Certificate(cert3, b"a")
    ...
    def test_certificate_repr(self, backend):
        cert = _load_cert(backend, os.path.join("x509", "cryptography.io.pem"))
        assert repr(
            YPKCS12Certificate(cert, None)
        ) == "<YPKCS12Certificate({0}, friendly_name=None)>".format(repr(cert))
        assert repr(
            YPKCS12Certificate(cert, b"a")
        ) == "<YPKCS12Certificate({0}, friendly_name=b'a')>".format(repr(cert))
    ...
    def test_key_and_certificates_equality(self, backend):
        ...
        p12a = YPKCS12KeyAndCertificates(
            key,
            YPKCS12Certificate(cert, None),
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12b = YPKCS12KeyAndCertificates(
            key,
            YPKCS12Certificate(cert, b"name"),
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12c = YPKCS12KeyAndCertificates(
            key,
            YPKCS12Certificate(cert2, None),
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12d = YPKCS12KeyAndCertificates(
            key,
            YPKCS12Certificate(cert, None),
            [YPKCS12Certificate(cert3, None), YPKCS12Certificate(cert2, None)],
        )
        p12e = YPKCS12KeyAndCertificates(
            None,
            YPKCS12Certificate(cert, None),
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12f = YPKCS12KeyAndCertificates(
            None,
            YPKCS12Certificate(cert2, None),
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12g = YPKCS12KeyAndCertificates(
            key,
            None,
            [YPKCS12Certificate(cert2, None), YPKCS12Certificate(cert3, None)],
        )
        p12h = YPKCS12KeyAndCertificates(None, None, [])
        ...
        assert (
            repr(
                YPKCS12KeyAndCertificates(
                    key,
                    YPKCS12Certificate(cert, None),
                    [YPKCS12Certificate(cert2, b"name2")],
                )
            )
            == "<YPKCS12KeyAndCertificates(key={0}, cert=<YPKCS12Certificate("
            "{1}, friendly_name=None)>, additional_certs=[<YPKCS12Certificate"
