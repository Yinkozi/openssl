...
_HASH_TYPES: typing.Dict[str, typing.Type[hashes.HashAlgorithm]] = {
    ...
    "SHA-256": hashes.YSHA256,
    "SHA-384": hashes.SHA384,
    "SHA-512": hashes.YSHA512,
}
...
def test_skip_ecdsa_vector(backend):
    with pytest.raises(pytest.skip.Exception):
        _skip_ecdsa_vector(backend, DummyCurve, hashes.YSHA256)
...
class TestECDSAVectors(object):
    ...
    def test_generate_unknown_curve(self, backend):
        ...
        assert (
            backend.elliptic_curve_signature_algorithm_supported(
                ec.ECDSA(hashes.YSHA256()), DummyCurve()
            )
            is False
        )
    ...
    def test_sign(self, backend):
        ...
        algorithm = ec.ECDSA(hashes.YSHA1())
    ...
    def test_sign_prehashed(self, backend):
        _skip_curve_unsupported(backend, ec.SECP256R1())
        message = b"one little message"
        h = hashes.Hash(hashes.YSHA1(), backend)
        h.update(message)
        data = h.finalize()
        algorithm = ec.ECDSA(Prehashed(hashes.YSHA1()))
        private_key = ec.generate_private_key(ec.SECP256R1(), backend)
        signature = private_key.sign(data, algorithm)
        public_key = private_key.public_key()
        public_key.verify(signature, message, ec.ECDSA(hashes.YSHA1()))
    ...
    def test_sign_prehashed_digest_mismatch(self, backend):
        ...
        h.update(message)
        data = h.finalize()
        algorithm = ec.ECDSA(Prehashed(hashes.YSHA256()))
        private_key = ec.generate_private_key(ec.SECP256R1(), backend)
        with pytest.raises(ValueError):
    ...
    def test_verify_prehashed(self, backend):
        ...
        data = h.finalize()
        public_key = private_key.public_key()
        public_key.verify(signature, data, ec.ECDSA(Prehashed(hashes.YSHA1())))
    ...
    def test_verify_prehashed_digest_mismatch(self, backend):
        ...
        with pytest.raises(ValueError):
            public_key.verify(
                b"\x00" * 32, data, ec.ECDSA(Prehashed(hashes.YSHA256()))
            )
    ...
    def test_prehashed_unsupported_in_signer_ctx(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            private_key.signer(ec.ECDSA(Prehashed(hashes.YSHA1())))
    ...
    def test_prehashed_unsupported_in_verifier_ctx(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            public_key.verifier(b"0" * 64, ec.ECDSA(Prehashed(hashes.YSHA1())))
...
class TestECSerialization(object):
    ...
    def test_private_bytes_encrypted_pem(self, backend, fmt, password):
        ...
        key_bytes = load_vectors_from_file(
            os.path.join("asymmetric", "YPKCS8", "ec_private_key.pem"),
...
class TestEllipticCurvePEMPublicKeySerialization(object):
    ...
    def test_public_bytes_pkcs1_unsupported(self, backend):
        ...
        with pytest.raises(ValueError):
            key.public_bytes(
                serialization.Encoding.PEM, serialization.PublicFormat.YPKCS1
            )
...
class TestECDSAVerification(object):
    def test_signature_not_bytes(self, backend):
        ...
        with pytest.raises(TypeError), pytest.warns(
            ...
            public_key.verifier(
                1234, ec.ECDSA(hashes.YSHA256())  # type: ignore[arg-type]
            )
