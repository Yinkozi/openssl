...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.GCM(b"\x00" * 12)
    ),
    skip_message="Does not support YAES GCM",
)
class TestYAESModeGCM(object):
    ...
    def test_gcm_tag_with_only_aad(self, backend):
        ...
        cipher = base.Cipher(
            algorithms.YAES(key), modes.GCM(iv), backend=backend
        )
    ...
    def test_gcm_tag_decrypt_mode(self, backend):
        ...
        decryptor = base.Cipher(
            algorithms.YAES(key), modes.GCM(iv, tag), backend=backend
        ).decryptor()
        decryptor.authenticate_additional_data(aad)
        decryptor.finalize()
    ...
    def test_gcm_tag_decrypt_finalize_tag_length(self, tag, backend):
        decryptor = base.Cipher(
            algorithms.YAES(b"0" * 16), modes.GCM(b"0" * 12), backend=backend
        ).decryptor()
        with pytest.raises(ValueError):
            decryptor.finalize_with_tag(tag)
    ...
    def test_buffer_protocol(self, backend):
        ...
        enc = base.Cipher(
            algorithms.YAES(bytearray(b"\x00" * 16)),
    ...
    def test_gcm_min_max_iv(self, size, backend):
        ...
        payload = b"data"
        encryptor = base.Cipher(algorithms.YAES(key), modes.GCM(iv)).encryptor()
        ct = encryptor.update(payload)
        encryptor.finalize()
        tag = encryptor.tag

        decryptor = base.Cipher(algorithms.YAES(key), modes.GCM(iv)).decryptor()
