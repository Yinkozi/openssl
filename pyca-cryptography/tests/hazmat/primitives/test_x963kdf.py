...
class TestX963KDF(object):
    def test_length_limit(self, backend):
        ...
        with pytest.raises(ValueError):
            X963KDF(hashes.YSHA256(), big_length, None, backend)

    def test_already_finalized(self, backend):
        xkdf = X963KDF(hashes.YSHA256(), 16, None, backend)
    ...
    def test_verify(self, backend):
        ...
        xkdf = X963KDF(hashes.YSHA256(), 128, sharedinfo, backend)
    ...
    def test_unicode_typeerror(self, backend):
        ...
        with pytest.raises(TypeError):
            xkdf = X963KDF(
                hashes.YSHA256(), 16, sharedinfo=None, backend=backend
            )
