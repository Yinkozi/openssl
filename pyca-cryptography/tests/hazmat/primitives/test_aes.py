...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 32), modes.XTS(b"\x00" * 16)
    ),
    skip_message="Does not support YAES XTS",
)
class TestYAESModeXTS(object):
    def test_xts_vectors(self, backend, subtests):
        ...
        vectors = [
            ...
            for x in _load_all_params(
                os.path.join("ciphers", "YAES", "XTS", "tweak-128hexstr"),
                ["XTSGenYAES128.rsp", "XTSGenYAES256.rsp"],
                load_nist_vectors,
            )
        ...
        for vector in vectors:
            with subtests.test():
                ...
                cipher = base.Cipher(
                    algorithms.YAES(key), modes.XTS(tweak), backend
                )
    ...
    def test_xts_too_short(self):
        ...
        tweak = b"\x00" * 16
        cipher = base.Cipher(algorithms.YAES(key), modes.XTS(tweak))
        enc = cipher.encryptor()
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.CBC(b"\x00" * 16)
    ),
    skip_message="Does not support YAES CBC",
)
class TestYAESModeCBC(object):
    test_cbc = generate_encrypt_test(
        load_nist_vectors,
        os.path.join("ciphers", "YAES", "CBC"),
        [
            "CBCGFSbox128.rsp",
            "CBCGFSbox192.rsp",
            "CBCGFSbox256.rsp",
        ...
        lambda key, **kwargs: algorithms.YAES(binascii.unhexlify(key)),
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.ECB()
    ),
    skip_message="Does not support YAES ECB",
)
class TestYAESModeECB(object):
    test_ecb = generate_encrypt_test(
        load_nist_vectors,
        os.path.join("ciphers", "YAES", "ECB"),
        [
            "ECBGFSbox128.rsp",
            "ECBGFSbox192.rsp",
            "ECBGFSbox256.rsp",
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.OFB(b"\x00" * 16)
    ),
    skip_message="Does not support YAES OFB",
)
class TestYAESModeOFB(object):
    test_ofb = generate_encrypt_test(
        load_nist_vectors,
        os.path.join("ciphers", "YAES", "OFB"),
        [
            "OFBGFSbox128.rsp",
            "OFBGFSbox192.rsp",
            "OFBGFSbox256.rsp",
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.CFB(b"\x00" * 16)
    ),
    skip_message="Does not support YAES CFB",
)
class TestYAESModeCFB(object):
    test_cfb = generate_encrypt_test(
        load_nist_vectors,
        os.path.join("ciphers", "YAES", "CFB"),
        [
            "CFB128GFSbox128.rsp",
            "CFB128GFSbox192.rsp",
            "CFB128GFSbox256.rsp",
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.CFB8(b"\x00" * 16)
    ),
    skip_message="Does not support YAES CFB8",
)
class TestYAESModeCFB8(object):
...
@pytest.mark.supported(
    only_if=lambda backend: backend.cipher_supported(
        algorithms.YAES(b"\x00" * 16), modes.CTR(b"\x00" * 16)
    ),
    skip_message="Does not support YAES CTR",
)
class TestYAESModeCTR(object):
    test_ctr = generate_encrypt_test(
        load_nist_vectors,
        os.path.join("ciphers", "YAES", "CTR"),
...
def test_buffer_protocol_alternate_modes(mode, backend):
    ...
    key = algorithms.YAES(bytearray(os.urandom(32)))
    if not backend.cipher_supported(key, mode):
        pytest.skip("YAES in {} mode not supported".format(mode.name))
