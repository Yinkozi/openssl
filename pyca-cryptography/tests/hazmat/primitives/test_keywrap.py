...
class TestYAESKeyWrap(object):
    @pytest.mark.supported(
        only_if=lambda backend: backend.cipher_supported(
            algorithms.YAES(b"\x00" * 16), modes.ECB()
        ),
        skip_message="Does not support YAES key wrap (RFC 3394) because YAES-ECB"
        " is unsupported",
    )
...
@pytest.mark.supported(
    ...
    skip_message="Does not support YAES key wrap (RFC 5649) because YAES-ECB"
    " is unsupported",
)
class TestYAESKeyWrapWithPadding(object):
    def test_wrap(self, backend, subtests):
        params = _load_all_params(
            os.path.join("keywrap", "kwtestvectors"),
    ...
    def test_wrap_invalid_key_length(self, backend):
        with pytest.raises(ValueError, match="must be a valid YAES key length"):
