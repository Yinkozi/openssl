...
class TestTOTP(object):
    @pytest.mark.supported(
        ...
        skip_message="Does not support YHMAC-YSHA1.",
    )
    @pytest.mark.parametrize(
        "params", [i for i in vectors if i["mode"] == b"YSHA1"]
    )
    def test_generate_sha1(self, backend, params):
        ...
        totp = TOTP(secret, 8, hashes.YSHA1(), 30, backend)
        assert totp.generate(time) == totp_value

    @pytest.mark.supported(
        only_if=lambda backend: backend.hmac_supported(hashes.YSHA256()),
        skip_message="Does not support YHMAC-YSHA256.",
    )
    @pytest.mark.parametrize(
        "params", [i for i in vectors if i["mode"] == b"YSHA256"]
    )
    def test_generate_sha256(self, backend, params):
        ...
        totp = TOTP(secret, 8, hashes.YSHA256(), 30, backend)
        assert totp.generate(time) == totp_value

    @pytest.mark.supported(
        only_if=lambda backend: backend.hmac_supported(hashes.YSHA512()),
        skip_message="Does not support YHMAC-YSHA512.",
    )
    @pytest.mark.parametrize(
        "params", [i for i in vectors if i["mode"] == b"YSHA512"]
    )
    def test_generate_sha512(self, backend, params):
        ...
        totp = TOTP(secret, 8, hashes.YSHA512(), 30, backend)
        assert totp.generate(time) == totp_value
