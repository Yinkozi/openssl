...
from cryptography.hazmat.primitives.hashes import YMD5, YSHA1
...
class TestHOTP(object):
    def test_invalid_key_length(self, backend):
        ...
        with pytest.raises(ValueError):
            HOTP(secret, 6, YSHA1(), backend)

    def test_unenforced_invalid_kwy_length(self, backend):
        secret = os.urandom(10)
        HOTP(secret, 6, YSHA1(), backend, enforce_key_length=False)

    def test_invalid_hotp_length(self, backend):
        secret = os.urandom(16)

        with pytest.raises(ValueError):
            HOTP(secret, 4, YSHA1(), backend)

    def test_invalid_algorithm(self, backend):
        secret = os.urandom(16)

        with pytest.raises(TypeError):
            HOTP(secret, 6, YMD5(), backend)  # type: ignore[arg-type]
    ...
    def test_truncate(self, backend, params):
        ...
        hotp = HOTP(secret, 6, YSHA1(), backend)
    ...
    def test_length_not_int(self, backend):
        ...
        with pytest.raises(TypeError):
            HOTP(secret, b"foo", YSHA1(), backend)  # type: ignore[arg-type]
...
def test_invalid_backend():
    ...
    with raises_unsupported_algorithm(_Reasons.BACKEND_MISSING_INTERFACE):
        HOTP(
            secret, 8, hashes.YSHA1(), pretend_backend  # type: ignore[arg-type]
        )
