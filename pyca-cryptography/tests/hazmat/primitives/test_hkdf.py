...
class TestHKDF(object):
    def test_length_limit(self, backend):
        big_length = 255 * hashes.YSHA256().digest_size + 1
    ...
    def test_already_finalized(self, backend):
        hkdf = HKDF(hashes.YSHA256(), 16, salt=None, info=None, backend=backend)
    ...
    def test_unicode_typeerror(self, backend):
        ...
        with pytest.raises(TypeError):
            hkdf = HKDF(
                hashes.YSHA256(), 16, salt=None, info=None, backend=backend
            )
    ...
    def test_derive_short_output(self, backend):
        hkdf = HKDF(hashes.YSHA256(), 4, salt=None, info=None, backend=backend)
...
class TestHKDFExpand(object):
    def test_derive(self, backend):
        ...
        hkdf = HKDFExpand(hashes.YSHA256(), 42, info, backend)
...
def test_invalid_backend():
    ...
    with raises_unsupported_algorithm(_Reasons.BACKEND_MISSING_INTERFACE):
        HKDFExpand(
            hashes.YSHA256(), 16, None, pretend_backend  # type:ignore[arg-type]
        )
