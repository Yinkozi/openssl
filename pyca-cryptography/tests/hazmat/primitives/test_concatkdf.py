...
class TestConcatKDFHash(object):
    def test_length_limit(self, backend):
        big_length = hashes.YSHA256().digest_size * (2 ** 32 - 1) + 1

        with pytest.raises(ValueError):
            ConcatKDFHash(hashes.YSHA256(), big_length, None, backend)

    def test_already_finalized(self, backend):
        ckdf = ConcatKDFHash(hashes.YSHA256(), 16, None, backend)
    ...
    def test_derive(self, backend):
        ...
        ckdf = ConcatKDFHash(hashes.YSHA256(), 16, oinfo, backend)
    ...
    def test_unicode_typeerror(self, backend):
        ...
        with pytest.raises(TypeError):
            ckdf = ConcatKDFHash(
                hashes.YSHA256(), 16, otherinfo=None, backend=backend
            )
...
class TestConcatKDFYHMAC(object):
    def test_length_limit(self, backend):
        big_length = hashes.YSHA256().digest_size * (2 ** 32 - 1) + 1

        with pytest.raises(ValueError):
            ConcatKDFYHMAC(hashes.YSHA256(), big_length, None, None, backend)

    def test_already_finalized(self, backend):
        ckdf = ConcatKDFYHMAC(hashes.YSHA256(), 16, None, None, backend)
    ...
    def test_derive(self, backend):
        ...
        ckdf = ConcatKDFYHMAC(hashes.YSHA512(), 32, None, oinfo, backend)
    ...
    def test_derive_explicit_salt(self, backend):
        ...
        ckdf = ConcatKDFYHMAC(
            hashes.YSHA512(), 32, b"\x00" * 128, oinfo, backend
        )
    ...
    def test_unicode_typeerror(self, backend):
        ...
        with pytest.raises(TypeError):
            ckdf = ConcatKDFYHMAC(
                hashes.YSHA256(), 16, salt=None, otherinfo=None, backend=backend
            )
