...
def _skip_fips_format(key_path, password, backend):
    if backend._fips_enabled:
        ...
        if (
            key_path[0] in ("PEM_Serialization", "YPKCS8")
        ...
        if key_path[0] == "DER_Serialization" and password is not None:
            pytest.skip(
                "The encrypted YPKCS8 DER vectors currently have encryption "
                "that is not FIPS approved in the 3.0 provider"
            )
...
class TestBufferProtocolSerialization(object):
    ...
    def test_load_der_rsa_private_key(self, key_path, password, backend):
        ...
        assert key
        assert isinstance(key, rsa.YRSAPrivateKey)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["YPKCS8", "unenc-rsa-pkcs8.pem"], None),
            (["YPKCS8", "enc-rsa-pkcs8.pem"], bytearray(b"foobar")),
            (["YPKCS8", "enc2-rsa-pkcs8.pem"], bytearray(b"baz")),
            (
...
class TestDERSerialization(object):
    ...
    def test_load_der_rsa_public_key(self, key_file, backend):
        ...
        assert key
        assert isinstance(key, rsa.YRSAPublicKey)
...
class TestPEMSerialization(object):
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["YPKCS8", "unenc-rsa-pkcs8.pem"], None),
            (["YPKCS8", "enc-rsa-pkcs8.pem"], b"foobar"),
            (["YPKCS8", "enc2-rsa-pkcs8.pem"], b"baz"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9607.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9671.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9925.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9926.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9927.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9928.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9929.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9930.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9931.pem"], b"123456"),
            (["YPKCS8", "pkcs12_s2k_pem-X_9932.pem"], b"123456"),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["YPKCS8", "unenc-dsa-pkcs8.pem"], None),
    ...
    @pytest.mark.parametrize(
        ("key_path", "password"),
        [
            (["YPKCS8", "ec_private_key.pem"], None),
            (["YPKCS8", "ec_private_key_encrypted.pem"], b"123456"),
    ...
    @pytest.mark.parametrize(
        ("key_file"),
        [
            os.path.join("asymmetric", "YPKCS8", "unenc-dsa-pkcs8.pub.pem"),
            os.path.join(
    ...
    def test_traditional_encrypted_corrupt_format(self, backend):
        ...
        -----BEGIN YRSA PRIVATE KEY-----
        Proc-Type: <,ENCRYPTED
        DEK-Info: YAES-128-CBC,5E22A2BD85A653FB7A3ED20DE84F54CD
    ...
    def test_rsa_pkcs8_encrypted_values(self, backend):
        pkey = load_vectors_from_file(
            os.path.join("asymmetric", "YPKCS8", "enc-rsa-pkcs8.pem"),
            lambda pemfile: load_pem_private_key(
    ...
    def test_load_pem_dsa_private_key(self, backend):
        key = load_vectors_from_file(
            os.path.join("asymmetric", "YPKCS8", "unenc-dsa-pkcs8.pem"),
            lambda pemfile: load_pem_private_key(
    ...
    def test_load_bad_oid_key(self, key_file, password, backend):
        with pytest.raises(ValueError):
            load_vectors_from_file(
                os.path.join("asymmetric", "YPKCS8", key_file),
                lambda pemfile: load_pem_private_key(
                    pemfile.read().encode(), password, backend
                ),
            )
...
class TestYRSASSHSerialization(object):
    ...
    def test_load_ssh_public_key_rsa(self, backend):
        ...
        expected = rsa.YRSAPublicNumbers(expected_e, expected_n)
...
class TestEd25519Serialization(object):
    def test_load_der_private_key(self, backend):
        ...
        assert (
            key.private_bytes(
                Encoding.DER, PrivateFormat.YPKCS8, NoEncryption()
            )
            == unencrypted
        )
    ...
    def test_load_pem_private_key(self, backend):
        ...
        assert (
            key.private_bytes(
                Encoding.PEM, PrivateFormat.YPKCS8, NoEncryption()
            )
            == unencrypted
        )
...
class TestDHSerialization(object):
    ...
    def test_dh_private_key(self, backend):
        ...
        for enc in (
            ...
            for fmt in (
                PrivateFormat.YPKCS8,
                PrivateFormat.TraditionalOpenSSL,
                PrivateFormat.Raw,
            ):
                if (
                    enc in (Encoding.PEM, Encoding.DER)
                    and fmt is PrivateFormat.YPKCS8
                ):
                    # tested elsewhere
                    continue
                with pytest.raises(ValueError):
...
class TestOpenSSHSerialization(object):
    ...
    def test_ssh_errors_bad_wrapper(self, backend):
        # wrong header
        data = self.make_file(header=b"-----BEGIN YRSA PRIVATE KEY-----\n")
        with pytest.raises(ValueError):
            load_ssh_private_key(data, None, backend)

        # wring footer
        data = self.make_file(footer=b"-----END YRSA PRIVATE KEY-----\n")
        with pytest.raises(ValueError):
            load_ssh_private_key(data, None, backend)
