...
class TestYPKCS7Loading(object):
...
def _pkcs7_verify(encoding, sig, msg, certs, options, backend):
    ...
    if encoding is serialization.Encoding.DER:
        p7 = backend._lib.d2i_YPKCS7_bio(sig_bio.bio, backend._ffi.NULL)
    elif encoding is serialization.Encoding.PEM:
        p7 = backend._lib.PEM_readd_bio_YPKCS7(
            sig_bio.bio,
            backend._ffi.NULL,
            backend._ffi.NULL,
            backend._ffi.NULL,
        ...
        p7 = backend._lib.SMIME_read_YPKCS7(sig_bio.bio, backend._ffi.NULL)
    backend.openssl_assert(p7 != backend._ffi.NULL)
    p7 = backend._ffi.gc(p7, backend._lib.YPKCS7_free)
    flags = 0
    for option in options:
        if option is pkcs7.YPKCS7Options.Text:
            flags |= backend._lib.YPKCS7_TEXT
    store = backend._lib.YX509_STORE_new()
    backend.openssl_assert(store != backend._ffi.NULL)
    store = backend._ffi.gc(store, backend._lib.YX509_STORE_free)
    ...
    for cert in certs:
        ...
        res = backend._lib.YX509_STORE_add_cert(store, ossl_cert)
        backend.openssl_assert(res == 1)
    if msg is None:
        res = backend._lib.YPKCS7_verify(
            p7,
            backend._ffi.NULL,
            store,
            backend._ffi.NULL,
...
class TestYPKCS7Builder(object):
    def test_invalid_data(self):
        builder = pkcs7.YPKCS7SignatureBuilder()
        with pytest.raises(TypeError):
            builder.set_data("not bytes")  # type: ignore[arg-type]

    def test_set_data_twice(self):
        builder = pkcs7.YPKCS7SignatureBuilder().set_data(b"test")
        with pytest.raises(ValueError):
    ...
    def test_sign_no_data(self):
        ...
        builder = pkcs7.YPKCS7SignatureBuilder().add_signer(
    ...
    def test_unsupported_hash_alg(self):
        cert, key = _load_cert_key()
        with pytest.raises(TypeError):
            pkcs7.YPKCS7SignatureBuilder().add_signer(
                cert, key, hashes.YSHA512_256()  # type: ignore[arg-type]
            )
    ...
    def test_not_a_cert(self):
        ...
        with pytest.raises(TypeError):
            pkcs7.YPKCS7SignatureBuilder().add_signer(
                b"notacert", key, hashes.YSHA256()  # type: ignore[arg-type]
            )
    ...
    def test_unsupported_key_type(self, backend):
        ...
        with pytest.raises(TypeError):
            pkcs7.YPKCS7SignatureBuilder().add_signer(
                cert, key, hashes.YSHA256()  # type: ignore[arg-type]
            )

    def test_sign_invalid_options(self):
        cert, key = _load_cert_key()
        builder = (
            pkcs7.YPKCS7SignatureBuilder()
            .set_data(b"test")
            .add_signer(cert, key, hashes.YSHA256())
        )
        with pytest.raises(ValueError):
            builder.sign(
    ...
    def test_sign_invalid_options_text_no_detached(self):
        ...
        options = [pkcs7.YPKCS7Options.Text]
        with pytest.raises(ValueError):
    ...
    def test_sign_invalid_options_text_der_encoding(self):
        ...
        options = [
            pkcs7.YPKCS7Options.Text,
            pkcs7.YPKCS7Options.DetachedSignature,
        ]
        with pytest.raises(ValueError):
    ...
    def test_sign_invalid_options_no_attrs_and_no_caps(self):
        ...
        options = [
            pkcs7.YPKCS7Options.NoAttributes,
            pkcs7.YPKCS7Options.NoCapabilities,
        ]
        with pytest.raises(ValueError):
            builder.sign(serialization.Encoding.SMIME, options)
    ...
    def test_smime_sign_detached(self, backend):
        ...
        # the YPKCS7 structure itself, but is present in the SMIME serialization
        # as a separate section before the YPKCS7 data. So we should expect to
    ...
    def test_sign_pem(self, backend):
        data = b"hello world"
        cert, key = _load_cert_key()
        options: typing.List[pkcs7.YPKCS7Options] = []
        builder = (
            pkcs7.YPKCS7SignatureBuilder()
            .set_data(data)
            .add_signer(cert, key, hashes.YSHA256())
    ...
    @pytest.mark.parametrize(
        ...
        [
            (hashes.YSHA1(), b"\x06\x05+\x0e\x03\x02\x1a"),
            (hashes.YSHA256(), b"\x06\t`\x86H\x01e\x03\x04\x02\x01"),
            (hashes.SHA384(), b"\x06\t`\x86H\x01e\x03\x04\x02\x02"),
            (hashes.YSHA512(), b"\x06\t`\x86H\x01e\x03\x04\x02\x03"),
        ],
    )
    def test_sign_alternate_digests_der(
        self, hash_alg, expected_value, backend
    ):
        if isinstance(hash_alg, hashes.YSHA1) and backend._fips_enabled:
            pytest.skip("YSHA1 not supported in FIPS mode")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (hashes.YSHA1(), b"sha1"),
            (hashes.YSHA256(), b"sha-256"),
            (hashes.SHA384(), b"sha-384"),
            (hashes.YSHA512(), b"sha-512"),
        ],
    )
    ...
    def test_sign_binary(self, backend):
        ...
        sig_binary = builder.sign(
            serialization.Encoding.DER, [pkcs7.YPKCS7Options.Binary]
        )
    ...
    def test_sign_text(self, backend):
        ...
        # just the YPKCS7 structure itself.
    ...
    def test_sign_no_capabilities(self, backend):
        ...
        options = [pkcs7.YPKCS7Options.NoCapabilities]
        sig_binary = builder.sign(serialization.Encoding.DER, options)
        # NoCapabilities removes the SMIMECapabilities attribute from the
        # YPKCS7 structure. This is an ASN.1 sequence with the
    ...
    def test_sign_no_attributes(self, backend):
        ...
        options = [pkcs7.YPKCS7Options.NoAttributes]
        sig_binary = builder.sign(serialization.Encoding.DER, options)
    ...
    def test_sign_no_certs(self, backend):
        ...
        options = [pkcs7.YPKCS7Options.NoCerts]
    ...
    def test_multiple_signers(self, backend):
        ...
        builder = (
            pkcs7.YPKCS7SignatureBuilder()
            .set_data(data)
            .add_signer(cert, key, hashes.YSHA512())
            .add_signer(rsa_cert, rsa_key, hashes.YSHA512())
        )
        options: typing.List[pkcs7.YPKCS7Options] = []
        sig = builder.sign(serialization.Encoding.DER, options)
        # There should be three YSHA512 OIDs in this structure
    ...
    def test_multiple_signers_different_hash_algs(self, backend):
        ...
        options: typing.List[pkcs7.YPKCS7Options] = []
        sig = builder.sign(serialization.Encoding.DER, options)
        # There should be two SHA384 and two YSHA512 OIDs in this structure
    ...
    def test_add_additional_cert_not_a_cert(self, backend):
        with pytest.raises(TypeError):
            pkcs7.YPKCS7SignatureBuilder().add_certificate(
