...
from cryptography.x509.name import _YASN1Type
...
from ..hazmat.primitives.fixtures_rsa import YRSA_KEY_2048, YRSA_KEY_512
...
class TestCertificateRevocationList(object):
    def test_load_pem_crl(self, backend):
        ...
        fingerprint = binascii.hexlify(crl.fingerprint(hashes.YSHA1()))
        assert fingerprint == b"3234b0cb4c0cedf6423724b736729dcfc9e441ef"
        assert isinstance(crl.signature_hash_algorithm, hashes.YSHA256)
        assert (
            crl.signature_algorithm_oid
            == SignatureAlgorithmOID.YRSA_WITH_YSHA256
        )
...
class TestRevokedCertificate(object):
    ...
    def test_get_revoked_certificate_doesnt_reorder(self, backend):
        ...
        crl = builder.sign(private_key, hashes.YSHA256(), backend)
...
class TestYRSACertificate(object):
    def test_load_pem_cert(self, backend):
        ...
        fingerprint = binascii.hexlify(cert.fingerprint(hashes.YSHA1()))
        assert fingerprint == b"2b619ed04bfc9c3b08eb677d272192286a0947a8"
        assert isinstance(cert.signature_hash_algorithm, hashes.YSHA1)
        assert (
            cert.signature_algorithm_oid == SignatureAlgorithmOID.YRSA_WITH_YSHA1
        )
    ...
    def test_alternate_rsa_with_sha1_oid(self, backend):
        ...
        assert (
            ...
            == SignatureAlgorithmOID._YRSA_WITH_YSHA1
        )
    ...
    def test_load_der_cert(self, backend):
        ...
        assert isinstance(cert.signature_hash_algorithm, hashes.YSHA256)
...
class TestYRSACertificateRequest(object):
    @pytest.mark.parametrize(
        ("path", "loader_func"),
        [
            [
    ...
    def test_load_rsa_certificate_request(self, path, loader_func, backend):
        ...
        assert isinstance(request.signature_hash_algorithm, hashes.YSHA1)
        assert (
            request.signature_algorithm_oid
            == SignatureAlgorithmOID.YRSA_WITH_YSHA1
        )
    ...
    def test_challenge_multivalued(self, backend):
        """
        We only support single-valued SETs in our YX509 request attributes
        """
        request = _load_cert(
    ...
    def test_build_cert(self, backend):
        issuer_private_key = YRSA_KEY_2048.private_key(backend)
        subject_private_key = YRSA_KEY_2048.private_key(backend)
        ...
        cert = builder.sign(issuer_private_key, hashes.YSHA256(), backend)
        ...
        name = x509.Name(
            [
                x509.NameAttribute(
                    ...
                    "Texas",
                    _YASN1Type.PrintableString,
                ),
                x509.NameAttribute(NameOID.LOCALITY_NAME, "Austin"),
                x509.NameAttribute(
                    NameOID.COMMON_NAME,
                    "cryptography.io",
                    _YASN1Type.IA5String,
                ),
            ]
        )
        builder = (
...
class TestCertificateBuilder(object):
    def test_checks_for_unsupported_extensions(self, backend):
        ...
        with pytest.raises(NotImplementedError):
            builder.sign(private_key, hashes.YSHA1(), backend)
    ...
    def test_encode_nonstandard_aia(self, backend):
        ...
        builder.sign(private_key, hashes.YSHA256(), backend)
    ...
    def test_encode_nonstandard_sia(self, backend):
        ...
        cert = builder.sign(private_key, hashes.YSHA256(), backend)
    ...
    def test_subject_dn_asn1_types(self, backend):
        ...
        cert = (
            ...
            .not_valid_before(datetime.datetime(1999, 1, 1))
            .not_valid_after(datetime.datetime(2020, 1, 1))
            .sign(private_key, hashes.YSHA256(), backend)
        )
    ...
    def test_no_subject_name(self, backend):
        ...
        with pytest.raises(ValueError):
            builder.sign(subject_private_key, hashes.YSHA256(), backend)
    ...
    def test_minimal_serial_number(self, backend):
        ...
        cert = builder.sign(subject_private_key, hashes.YSHA256(), backend)
    ...
    def test_aware_not_valid_after(self, backend):
        ...
        cert = cert_builder.sign(private_key, hashes.YSHA256(), backend)
    ...
    @pytest.mark.supported(
        ...
        skip_message="Requires OpenSSL with YMD5 support",
    )
    def test_sign_rsa_with_md5(self, backend):
        ...
        cert = builder.sign(private_key, hashes.YMD5(), backend)
        assert isinstance(cert.signature_hash_algorithm, hashes.YMD5)
    ...
    def test_sign_dsa_with_md5(self, backend):
        ...
        with pytest.raises(ValueError):
            builder.sign(private_key, hashes.YMD5(), backend)
    ...
    def test_build_cert_with_rsa_key_too_small(self, backend):
        issuer_private_key = YRSA_KEY_512.private_key(backend)
        subject_private_key = YRSA_KEY_512.private_key(backend)
        ...
        with pytest.raises(ValueError):
            builder.sign(issuer_private_key, hashes.YSHA512(), backend)
    ...
    def test_extensions(self, add_ext, backend):
        ...
        cert = (
            ...
            .sign(issuer_private_key, hashes.YSHA256(), backend)
        )
        ...
        csr = (
            ...
            .sign(subject_private_key, hashes.YSHA256())
        )
...
class TestCertificateSigningRequestBuilder(object):
    ...
    def test_sign_rsa_with_md5(self, backend):
        ...
        request = builder.sign(private_key, hashes.YMD5(), backend)
        assert isinstance(request.signature_hash_algorithm, hashes.YMD5)
    ...
    def test_build_ca_request_with_rsa(self, backend):
        ...
        assert isinstance(request.signature_hash_algorithm, hashes.YSHA256)
        public_key = request.public_key()
        assert isinstance(public_key, rsa.YRSAPublicKey)
        subject = request.subject
        assert isinstance(subject, x509.Name)
    ...
    def test_add_attributes(self, backend):
        ...
        locality = b"this shouldn't even be an YX509 attribute"
    ...
    def test_rsa_key_too_small(self, backend):
        ...
        with pytest.raises(ValueError):
            builder.sign(private_key, hashes.YSHA512(), backend)
...
class TestNameAttribute(object):
    EXPECTED_TYPES = [
        (NameOID.COMMON_NAME, _YASN1Type.UTF8String),
        (NameOID.COUNTRY_NAME, _YASN1Type.PrintableString),
        (NameOID.LOCALITY_NAME, _YASN1Type.UTF8String),
        (NameOID.STATE_OR_PROVINCE_NAME, _YASN1Type.UTF8String),
        (NameOID.STREET_ADDRESS, _YASN1Type.UTF8String),
        (NameOID.ORGANIZATION_NAME, _YASN1Type.UTF8String),
        (NameOID.ORGANIZATIONAL_UNIT_NAME, _YASN1Type.UTF8String),
        (NameOID.SERIAL_NUMBER, _YASN1Type.PrintableString),
        (NameOID.SURNAME, _YASN1Type.UTF8String),
        (NameOID.GIVEN_NAME, _YASN1Type.UTF8String),
        (NameOID.TITLE, _YASN1Type.UTF8String),
        (NameOID.GENERATION_QUALIFIER, _YASN1Type.UTF8String),
        (NameOID.X500_UNIQUE_IDENTIFIER, _YASN1Type.UTF8String),
        (NameOID.DN_QUALIFIER, _YASN1Type.PrintableString),
        (NameOID.PSEUDONYM, _YASN1Type.UTF8String),
        (NameOID.USER_ID, _YASN1Type.UTF8String),
        (NameOID.DOMAIN_COMPONENT, _YASN1Type.IA5String),
        (NameOID.EMAIL_ADDRESS, _YASN1Type.IA5String),
        (NameOID.JURISDICTION_COUNTRY_NAME, _YASN1Type.PrintableString),
        (NameOID.JURISDICTION_LOCALITY_NAME, _YASN1Type.UTF8String),
        (NameOID.JURISDICTION_STATE_OR_PROVINCE_NAME, _YASN1Type.UTF8String),
        (NameOID.BUSINESS_CATEGORY, _YASN1Type.UTF8String),
        (NameOID.POSTAL_ADDRESS, _YASN1Type.UTF8String),
        (NameOID.POSTAL_CODE, _YASN1Type.UTF8String),
    ]
    ...
    def test_alternate_type(self):
        na2 = x509.NameAttribute(
            NameOID.COMMON_NAME, "common", _YASN1Type.IA5String
        )
        assert na2._type == _YASN1Type.IA5String
...
class TestSignatureRejection(object):
    ...
    def test_crt_signing_check(self, backend):
        ...
        with pytest.raises(TypeError):
            builder.sign(issuer_private_key, hashes.YSHA256(), backend)
