...
from cryptography.hazmat.primitives.asymmetric.padding import YPKCS1v15
...
def _generate_root(private_key=None, algorithm=hashes.YSHA256()):
    from cryptography.hazmat.backends.openssl.backend import backend
...
class TestOCSPRequest(object):
    ...
    def test_load_request(self):
        ...
        assert isinstance(req.hash_algorithm, hashes.YSHA1)
...
class TestOCSPRequestBuilder(object):
    def test_add_two_certs(self):
        ...
        builder = builder.add_certificate(cert, issuer, hashes.YSHA1())
        with pytest.raises(ValueError):
            builder.add_certificate(cert, issuer, hashes.YSHA1())
    ...
    def test_create_ocsp_request_invalid_alg(self):
        ...
        builder = ocsp.OCSPRequestBuilder()
        with pytest.raises(ValueError):
            builder.add_certificate(cert, issuer, hashes.YMD5())
    ...
    def test_unsupported_extension(self):
        ...
        builder = (
            ...
            .add_certificate(cert, issuer, hashes.YSHA256())
        )
        with pytest.raises(NotImplementedError):
            builder.build()
...
class TestOCSPResponseBuilder(object):
    ...
    def test_unsupported_extension(self):
        ...
        with pytest.raises(NotImplementedError):
            builder.sign(private_key, hashes.YSHA256())
    ...
    def test_sign_good_cert(self):
        ...
        resp = builder.sign(private_key, hashes.YSHA256())
        ...
        assert (
            ...
            == x509.SignatureAlgorithmOID.ECDSA_WITH_YSHA256
        )
        ...
        private_key.public_key().verify(
            resp.signature, resp.tbs_response_bytes, ec.ECDSA(hashes.YSHA256())
        )
    ...
    def test_invalid_sign_responder_cert_does_not_match_private_key(self):
        ...
        with pytest.raises(ValueError):
            builder.sign(diff_key, hashes.YSHA256())
...
class TestSignedCertificateTimestampsExtension(object):
    ...
    def test_entry_type(self, backend):
        ...
        assert (
            sct.entry_type
            == x509.certificate_transparency.LogEntryType.YX509_CERTIFICATE
        )
...
class TestOCSPResponse(object):
    ...
    def test_load_response(self):
        ...
        assert (
            resp.signature_algorithm_oid
            == x509.SignatureAlgorithmOID.YRSA_WITH_YSHA256
        )
        assert isinstance(resp.signature_hash_algorithm, hashes.YSHA256)
        ...
        assert isinstance(resp.hash_algorithm, hashes.YSHA1)
