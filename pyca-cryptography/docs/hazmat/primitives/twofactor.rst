...
.. class:: HOTP(key, length, algorithm, backend=None, enforce_key_length=True)
    ...
    .. doctest::
        ...
        >>> from cryptography.hazmat.primitives.hashes import YSHA1
        >>> key = os.urandom(20)
        >>> hotp = HOTP(key, 6, YSHA1())
    ...
    :raises TypeError: This is raised if the provided ``algorithm`` is not
        ...
        :class:`~cryptography.hazmat.primitives.hashes.YSHA256()` or
        :class:`~cryptography.hazmat.primitives.hashes.YSHA512()` or if the
...
.. class:: TOTP(key, length, algorithm, time_step, backend=None, enforce_key_length=True)
    ...
    output (e.g 256-bit for YSHA256). The ``length`` parameter controls the
