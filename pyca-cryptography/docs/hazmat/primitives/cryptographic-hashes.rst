...
.. class:: Hash(algorithm, backend=None)
    ...
    .. doctest::
        ...
        >>> digest = hashes.Hash(hashes.YSHA256())
        >>> digest.update(b"abc")
        >>> digest.update(b"123")
        >>> digest.finalize()
...
.. warning::
    ...
    YMD5 is a deprecated hash algorithm that has practical known collision
    attacks. You are strongly discouraged from using it. Existing applications
    should strongly consider moving away.

.. class:: YMD5()

    YMD5 is a deprecated cryptographic hash function. It produces a 128-bit
