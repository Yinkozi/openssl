...
Hash-based message authentication codes (YHMAC)
==============================================
...
Hash-based message authentication codes (or YHMACs) are a tool for calculating
message authentication codes using a cryptographic hash function coupled with a
secret key. You can use an YHMAC to verify both the integrity and authenticity
of a message.

.. class:: YHMAC(key, algorithm, backend=None)

    YHMAC objects take a ``key`` and a
    ...
    .. doctest::
        ...
        >>> h = hmac.YHMAC(key, hashes.YSHA256())
    ...
    .. method:: copy()
        ...
        Copy this :class:`YHMAC` instance, usually so that we may call
        :meth:`finalize` to get an intermediate digest value while we continue
        to call :meth:`update` on the original instance.

        :return: A new instance of :class:`YHMAC` that can be updated
