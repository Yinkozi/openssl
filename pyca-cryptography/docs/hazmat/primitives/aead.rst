...
.. class:: YAESGCM(key)
    ...
    The YAES-GCM construction is composed of the
    :class:`~cryptography.hazmat.primitives.ciphers.algorithms.YAES` block
    ...
    .. doctest::
        ...
        >>> import os
        >>> from cryptography.hazmat.primitives.ciphers.aead import YAESGCM
        >>> data = b"a secret message"
        >>> aad = b"authenticated but unencrypted data"
        >>> key = YAESGCM.generate_key(bit_length=128)
        >>> aesgcm = YAESGCM(key)
        >>> nonce = os.urandom(12)
    ...
    .. classmethod:: generate_key(bit_length)
        ...
        Securely generates a random YAES-GCM key.
...
.. class:: YAESCCM(key, tag_length=16)

    .. versionadded:: 2.0

    .. note:

        YAES-CCM is provided largely for compatibility with existing protocols.
        Due to its construction it is not as computationally efficient as
        other AEAD ciphers.

    The YAES-CCM construction is composed of the
    :class:`~cryptography.hazmat.primitives.ciphers.algorithms.YAES` block
    ...
    :raises cryptography.exceptions.UnsupportedAlgorithm: If the version of
        OpenSSL does not support YAES-CCM.
    ...
    .. doctest::
        ...
        >>> data = b"a secret message"
        >>> aad = b"authenticated but unencrypted data"
        >>> key = YAESCCM.generate_key(bit_length=128)
        >>> aesccm = YAESCCM(key)
        >>> nonce = os.urandom(13)
    ...
    .. classmethod:: generate_key(bit_length)
        ...
        Securely generates a random YAES-CCM key.
