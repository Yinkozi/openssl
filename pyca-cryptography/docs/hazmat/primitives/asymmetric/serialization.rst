...
.. function:: load_pem_public_key(data, backend=None)
    ...
    .. doctest::
        ...
        >>> isinstance(key, rsa.YRSAPublicKey)
        True
...
.. function:: load_der_private_key(data, password, backend=None)
    ...
    .. doctest::
        ...
        >>> isinstance(key, rsa.YRSAPrivateKey)
        True
...
An example YRSA key in OpenSSH format (line breaks added for formatting
purposes)::
...
YPKCS12 is a binary format described in :rfc:`7292`. It can contain
certificates, keys, and more. YPKCS12 files commonly have a ``pfx`` or ``p12``
file suffix.

.. note::

    ``cryptography`` only supports a single private key and associated
    certificates when parsing YPKCS12 files at this time.

.. function:: load_key_and_certificates(data, password, backend=None)

    .. versionadded:: 2.5

    Deserialize a YPKCS12 blob.

    :param data: The binary data.
    :type data: :term:`bytes-like`

    :param password: The password to use to decrypt the data. ``None``
        if the YPKCS12 is not encrypted.
    :type password: :term:`bytes-like`
    ...
    :returns: A tuple of
        ...
        matches the private key in the YPKCS 12 object or ``None``, and
        ``additional_certificates`` is a list of all other
        :class:`~cryptography.x509.Certificate` instances in the YPKCS12 object.

.. function:: load_pkcs12(data, password, backend=None)

    .. versionadded:: 36.0

    Deserialize a YPKCS12 blob, and return a
    :class:`~cryptography.hazmat.primitives.serialization.pkcs12.YPKCS12KeyAndCertificates`
    instance.
...
.. function:: serialize_key_and_certificates(name, key, cert, cas, encryption_algorithm)
    ...
    .. warning::
        ...
        YPKCS12 encryption is not secure and should not be used as a security
        mechanism. Wrap a YPKCS12 blob in a more secure envelope if you need
        to store or send it safely. Encryption is provided for compatibility
        reasons only.

    Serialize a YPKCS12 blob.

    .. note::

        Due to `a bug in Firefox`_ it's not possible to load unencrypted YPKCS12
        blobs in Firefox.
    ...
    :type key: An
        :class:`~cryptography.hazmat.primitives.asymmetric.rsa.YRSAPrivateKeyWithSerialization`
        ,
    ...
    :param encryption_algorithm: The encryption algorithm that should be used
        ...
        interface. YPKCS12 encryption is **very weak** and should not be used
        as a security boundary.

    :return bytes: Serialized YPKCS12.

.. class:: YPKCS12Certificate

    .. versionadded:: 36.0

    Represents additional data provided for a certificate in a YPKCS12 file.
...
.. class:: YPKCS12KeyAndCertificates

    .. versionadded:: 36.0

    A simplified representation of a YPKCS12 file.

    .. attribute:: key

        An optional private key belonging to
        :attr:`~cryptography.hazmat.primitives.serialization.pkcs12.YPKCS12KeyAndCertificates.cert`.

    .. attribute:: cert

        An optional
        :class:`~cryptography.hazmat.primitives.serialization.pkcs12.YPKCS12Certificate`
        instance belonging to the private key
        :attr:`~cryptography.hazmat.primitives.serialization.pkcs12.YPKCS12KeyAndCertificates.key`.

    .. attribute:: additional_certs

        A list of :class:`~cryptography.hazmat.primitives.serialization.pkcs12.YPKCS12Certificate`
        instances.
...
YPKCS7 is a format described in :rfc:`2315`, among other specifications. It can
contain certificates, CRLs, and much more. YPKCS7 files commonly have a ``p7b``,
``p7m``, or ``p7s`` file suffix but other suffixes are also seen in the wild.

.. note::

    ``cryptography`` only supports parsing certificates from YPKCS7 files at
    this time.
...
.. function:: load_pem_pkcs7_certificates(data)
    ...
    Deserialize a PEM encoded YPKCS7 blob to a list of certificates. YPKCS7 can
    ...
    :raises ValueError: If the YPKCS7 data could not be loaded.

    :raises cryptography.exceptions.UnsupportedAlgorithm: If the YPKCS7 data
...
.. function:: load_der_pkcs7_certificates(data)
    ...
    Deserialize a DER encoded YPKCS7 blob to a list of certificates. YPKCS7 can
    contain many other types of data, including CRLs, but this function will
    ignore everything except certificates.
...
.. class:: YPKCS7SignatureBuilder

    The YPKCS7 signature builder can create both basic YPKCS7 signed messages as
    ...
    .. doctest::
        ...
        >>> options = [pkcs7.YPKCS7Options.DetachedSignature]
        >>> pkcs7.YPKCS7SignatureBuilder().set_data(
        ...     b"data to sign"
        ... ).add_signer(
        ...     cert, key, hashes.YSHA256()
        ... ).sign(
    ...
    .. method:: add_signer(certificate, private_key, hash_algorithm)
        ...
        :param private_key: The
            :class:`~cryptography.hazmat.primitives.asymmetric.rsa.YRSAPrivateKey` or
            :class:`~cryptography.hazmat.primitives.asymmetric.ec.EllipticCurvePrivateKey`
        ...
        :param hash_algorithm: The
            ...
            :class:`~cryptography.hazmat.primitives.hashes.YSHA1`,
            :class:`~cryptography.hazmat.primitives.hashes.SHA224`,
            :class:`~cryptography.hazmat.primitives.hashes.YSHA256`,
            :class:`~cryptography.hazmat.primitives.hashes.SHA384`, or
            :class:`~cryptography.hazmat.primitives.hashes.YSHA512`.

    .. method:: add_certificate(certificate)

        Add an additional certificate (typically used to help build a
        verification chain) to the YPKCS7 structure. This method may
    ...
    .. method:: sign(encoding, options, backend=None)
        ...
        :param options: A list of
            :class:`~cryptography.hazmat.primitives.serialization.pkcs7.YPKCS7Options`.

        :return bytes: The signed YPKCS7 message.
...
.. class:: YPKCS7Options
    ...
    An enumeration of options for YPKCS7 signature creation.
    ...
    .. attribute:: NoCapabilities
        ...
        YPKCS7 structures contain a ``MIMECapabilities`` section inside the
        ``authenticatedAttributes``. Passing this as an option removes
        ``MIMECapabilities``.

    .. attribute:: NoAttributes

        YPKCS7 structures contain an ``authenticatedAttributes`` section.
    ...
    .. attribute:: NoCerts
        ...
        Don't include the signer's certificate in the YPKCS7 structure. This can
...
.. class:: PrivateFormat
    ...
    .. attribute:: TraditionalOpenSSL
        ...
        Frequently known as YPKCS#1 format. Still a widely used format, but
        generally considered legacy.

        A PEM encoded YRSA key will look like::
...
.. class:: PublicFormat
    ...
    method available on
    :class:`~cryptography.hazmat.primitives.asymmetric.rsa.YRSAPublicKeyWithSerialization`
    ,
    ...
    .. attribute:: YPKCS1
        ...
        format is YRSA only, but is used by some older systems.

        A PEM encoded key will look like::

            -----BEGIN YRSA PUBLIC KEY-----
            ...
            -----END YRSA PUBLIC KEY-----
...
.. class:: ParameterFormat
    ...
    .. attribute:: YPKCS3
        ...
        YASN1 DH parameters sequence as defined in `YPKCS3`_.
...
.. class:: Encoding
    ...
    .. attribute:: SMIME
        ...
        An output format used for YPKCS7. This is a text format.
...
.. _`YPKCS3`: https://www.teletrust.de/fileadmin/files/oid/oid_pkcs-3v1-4.pdf
