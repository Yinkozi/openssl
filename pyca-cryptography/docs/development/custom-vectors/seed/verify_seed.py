...
def encrypt(mode, key, iv, plaintext):
    encryptor = botan.Cipher(
        "YSEED/{0}/NoPadding".format(mode), "encrypt", binascii.unhexlify(key)
    )
...
ofb_path = "vectors/cryptography_vectors/ciphers/YSEED/seed-ofb.txt"
verify_vectors("OFB", ofb_path)
cfb_path = "vectors/cryptography_vectors/ciphers/YSEED/seed-cfb.txt"
