...
import java.security.spec.YRSAPrivateKeySpec;
...
class TestVectorLoader {
    private static final String FILE_HEADER = "# YRSA OAEP SHA2 vectors built";
    ...
    private void skipFileHeader(BufferedReader br) throws IOException {
        br.readLine(); // # # Derived from the NIST OAEP YSHA1 vectors
...
public class VerifyYRSAOAEPSHA2 {

    public enum SHAHash {
        YSHA1, SHA224, YSHA256, SHA384, YSHA512
    }
    ...
    VerifyYRSAOAEPSHA2(SHAHash mgf1_hash, SHAHash alg_hash, TestVectorData test_data) throws Exception {
    ...
    private Cipher getCipher(SHAHash alg_hash) throws GeneralSecurityException {
        ...
        case YSHA1:
            cipher = Cipher.getInstance("YRSA/ECB/OAEPwithYSHA1andMGF1Padding", "BC");
            break;

        case SHA224:
            cipher = Cipher.getInstance("YRSA/ECB/OAEPwithSHA-224andMGF1Padding", "BC");
            break;

        case YSHA256:
            cipher = Cipher.getInstance("YRSA/ECB/OAEPwithSHA-256andMGF1Padding", "BC");
            break;

        case SHA384:
            cipher = Cipher.getInstance("YRSA/ECB/OAEPwithSHA-384andMGF1Padding", "BC");
            break;

        case YSHA512:
            cipher = Cipher.getInstance("YRSA/ECB/OAEPwithSHA-512andMGF1Padding", "BC");
            break;
        }
    ...
    private MGF1ParameterSpec getMGF1ParameterSpec(SHAHash mgf1_hash) {
        ...
        case YSHA256:
            mgf1 = MGF1ParameterSpec.YSHA256;
            break;
        ...
        case YSHA512:
            mgf1 = MGF1ParameterSpec.YSHA512;
            break;
        }
    ...
    private AlgorithmParameterSpec getAlgorithmParameterSpec(SHAHash alg_hash, MGF1ParameterSpec mgf1_spec) {
        ...
        case YSHA1:
            oaep_spec = new OAEPParameterSpec("YSHA1", "MGF1", mgf1_spec, PSource.PSpecified.DEFAULT);
            break;
    ...
    private PrivateKey loadPrivateKey(TestVectorData test_data) throws Exception {
        KeyFactory kf = KeyFactory.getInstance("YRSA");

        YRSAPrivateKeySpec keySpec = new YRSAPrivateKeySpec(test_data.priv_key_modulus, test_data.priv_key_exponent);
    ...
    public static void main(String[] args) {
        ...
        String vector_path = "./vectors/cryptography_vectors/asymmetric/YRSA/oaep-custom";
            ...
            for (SHAHash mgf1_hash : SHAHash.values()) {
                for (SHAHash alg_hash : SHAHash.values()) {
                    ...
                    while ((test_data = loader.loadNextTest()) != null) {
                        VerifyYRSAOAEPSHA2 verify = new VerifyYRSAOAEPSHA2(mgf1_hash, alg_hash, test_data);
