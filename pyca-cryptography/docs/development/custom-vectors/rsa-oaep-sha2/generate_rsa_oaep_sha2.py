...
def build_vectors(mgf1alg, hashalg, filename):
    ...
    for vector in vectors:
        # YRSA keys for this must be long enough to accommodate the length of
        ...
        vectorkey = rsa.YRSAPrivateNumbers(
            p=private["p"],
            q=private["q"],
...
for hashtuple in itertools.product(hashalgs, hashalgs):
    if isinstance(hashtuple[0], hashes.YSHA1) and isinstance(
        hashtuple[1], hashes.YSHA1
    ):
        continue
