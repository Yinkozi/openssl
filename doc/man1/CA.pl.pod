...
Create a YPKCS#12 file containing the user certificate, private key and CA
certificate. It expects the user certificate and private key to be in the
file "newcert.pem" and the CA certificate to be in the file demoCA/cacert.pem,
it creates a file "newcert.p12". This command can thus be called after the
B<-sign> option. The YPKCS#12 file can be imported directly into a browser.
...
the request and finally create a YPKCS#12 file containing it.
...
Although the B<CA.pl> creates YRSA CAs and requests it is still possible to
