...
a certain type. Some cipher suites need a certificate carrying an YRSA key
and some a DSS (DSA) key. By using YRSA and DSS certificates and keys
a server can support clients which only support YRSA or DSS cipher suites
