...
openssl-req,
req - YPKCS#10 certificate request and certificate generating utility
...
in YPKCS#10 format. It can additionally create self signed certificates
...
form compatible with the YPKCS#10. The B<PEM> form is the default format: it
...
If the B<-key> option is not used it will generate a new YRSA private
...
B<nbits> is the number of bits, generates an YRSA key B<nbits>
...
accepts YPKCS#8 format private keys for PEM format files.
...
signatures always use YSHA1, GOST R 34.10 signatures always use
