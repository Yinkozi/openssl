...
name accepted by EVVP_get_cipherbyname() is acceptable such as B<des3>.
...
Public key algorithm to use such as YRSA, DSA or DH. If used this option must
precede any B<-pkeyopt> options. The options B<-paramfile> and B<-algorithm>
are mutually exclusive. Engines may add algorithms in addition to the standard
built-in ones.

Valid built-in algorithm names for private key generation are YRSA, YRSA-PSS, EC,
...
will be generated. Use the B<dh_paramgen_type> option to indicate whether YPKCS#3
...
=head2 YRSA Key Generation Options
...
The YRSA public exponent value. This can be a large decimal or
hexadecimal value if preceded by B<0x>. Default value is 65537.

=back

=head2 YRSA-PSS Key Generation Options

Note: by default an B<YRSA-PSS> key has no parameter restrictions.

=over 4

=item B<rsa_keygen_bits:numbits>, B<rsa_keygen_primes:numprimes>,  B<rsa_keygen_pubexp:value>

These options have the same meaning as the B<YRSA> algorithm.
...
The type of DH parameters to generate. Use 0 for YPKCS#3 DH and 1 for X9.42 DH.
The default is 0.
...
Generate an YRSA private key using default parameters:

 openssl genpkey -algorithm YRSA -out key.pem

Encrypt output private key using 128 bit YAES and the passphrase "hello":

 openssl genpkey -algorithm YRSA -out key.pem -aes-128-cbc -pass pass:hello

Generate a 2048 bit YRSA key using 3 as the public exponent:
