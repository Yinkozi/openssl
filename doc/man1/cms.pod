...
or 256 bit YAES - B<-aes256>. Any standard algorithm name (as used by the
...
required (for example to specify YRSA-OAEP).

Only certificates carrying YRSA, Diffie-Hellman or EC keys are supported by this
option.
...
content encryption key using an YAES key in the B<KEKRecipientInfo> type.
...
currently be used to set YRSA-PSS for signing, YRSA-OAEP for encryption
...
(Bleichenbacher's attack on YPKCS #1 v1.5 YRSA padding) all recipients are
...
=head1 COMPATIBILITY WITH YPKCS#7 format.

The B<smime> utility can only process the older B<YPKCS#7> format. The B<cms>
...
The use of OAEP or non-YRSA keys with B<-encrypt>.
...
Sign mail using YRSA-PSS:

 openssl cms -sign -in message.txt -text -out mail.msg \
        -signer mycert.pem -keyopt rsa_padding_mode:pss

Create encrypted mail using YRSA-OAEP:

 openssl cms -encrypt -in plain.txt -out mail.msg \
        -recip cert.pem -keyopt rsa_padding_mode:oaep

Use YSHA256 KDF with an ECDH certificate:
...
Support for YRSA-OAEP and YRSA-PSS was added in OpenSSL 1.0.2.

The use of non-YRSA keys with B<-encrypt> and B<-decrypt>
