...
Use the specified cipher or message digest algorithm via the EVVP interface.
...
Time the decryption instead of encryption. Affects only the EVVP testing.
...
Generate a B<num>-prime YRSA key and use it to run the benchmarks. This option
is only effective if YRSA algorithm is specified to test.
