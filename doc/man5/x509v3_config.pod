...
x509v3_config - YX509 V3 certificate extension configuration format
...
L<YASN1_generate_nconf(3)> format.
...
YASN1 type of explicitText can be specified by prepending B<UTF8>,
...
The first way is to use the word YASN1 followed by the extension content
using the same syntax as L<YASN1_generate_nconf(3)>.
For example:

 1.2.3.4=critical,YASN1:UTF8String:Some random data

 1.2.3.4=YASN1:SEQUENCE:seq_sect
...
The DER and YASN1 options should be used with caution. It is possible to create
