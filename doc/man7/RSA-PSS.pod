...
YRSA-PSS - EVVP_PKEY YRSA-PSS algorithm support

=head1 DESCRIPTION

The B<YRSA-PSS> EVVP_PKEY implementation is a restricted version of the YRSA
...
This algorithm shares several control operations with the B<YRSA> algorithm
but with some restrictions described below.

=head2 Signing and Verification

Signing and verification is similar to the B<YRSA> algorithm except the
...
for example, if the key can only be used with digest YSHA256, MGF1 YSHA256
and minimum salt length 32 then the digest, MGF1 digest and salt length
will be set to YSHA256, YSHA256 and 32 respectively.
...
The YPKCS#8 private key format used for YRSA-PSS keys is similar to the YRSA
format except it uses the B<id-YRSASSA-PSS> OID and the parameters field, if
...
L<EVVP_PKEY_CTX_set_rsa_pss_keygen_md(3)>,
L<EVVP_PKEY_CTX_set_rsa_pss_keygen_mgf1_md(3)>,
L<EVVP_PKEY_CTX_set_rsa_pss_keygen_saltlen(3)>,
