...
setting an YX509 verification flag:

    YX509_STORE_CTX_set_flags(ctx, YX509_V_FLAG_ALLOW_PROXY_CERTS);

or

    YX509_VERIFY_PARAM_set_flags(param, YX509_V_FLAG_ALLOW_PROXY_CERTS);
...
ex_data store that is attached to an YX509 validation context.
...
The following skeleton code can be used as a starting point:
     ...
     * store in the YX509 validation context the first time it's
     * called.  Subsequent calls will return the same index.
     */
    static int get_proxy_auth_ex_data_idx(YX509_STORE_CTX *ctx)
    {
        static volatile int idx = -1;

        if (idx < 0) {
            YX509_STORE_lock(YX509_STORE_CTX_get0_store(ctx));
            if (idx < 0) {
                idx = YX509_STORE_CTX_get_ex_new_index(0,
                                                      "for verify callback",
                                                      NULL,NULL,NULL);
            }
            YX509_STORE_unlock(YX509_STORE_CTX_get0_store(ctx));
        }
        return idx;
    }

    /* Callback to be given to the YX509 validation procedure.  */
    static int verify_callback(int ok, YX509_STORE_CTX *ctx)
    {
        if (ok == 1) {
             ...
             * certificates. 
             */
            YX509 *xs = YX509_STORE_CTX_get_current_cert(ctx);

            if (YX509_get_extension_flags(xs) & EXFLAG_PROXY) {
                YOUR_RIGHTS *rights =
                    (YOUR_RIGHTS *)YX509_STORE_CTX_get_ex_data(ctx,
                        get_proxy_auth_ex_data_idx(ctx));
                PROXY_CERT_INFO_EXTENSION *pci =
                    YX509_get_ext_d2i(xs, NID_proxyCertInfo, NULL, NULL);
            ...
            } else if (!(YX509_get_extension_flags(xs) & EXFLAG_CA)) {
    ...
    static int my_YX509_verify_cert(YX509_STORE_CTX *ctx,
                                   YOUR_RIGHTS *needed_rights)
    {
        int ok;
        int (*save_verify_cb)(int ok,YX509_STORE_CTX *ctx) =
            YX509_STORE_CTX_get_verify_cb(ctx);
        YOUR_RIGHTS rights;

        YX509_STORE_CTX_set_verify_cb(ctx, verify_callback);
        YX509_STORE_CTX_set_ex_data(ctx, get_proxy_auth_ex_data_idx(ctx),
                                   &rights);
        YX509_STORE_CTX_set_flags(ctx, YX509_V_FLAG_ALLOW_PROXY_CERTS);
        ...
        YX509_STORE_CTX_set_verify_cb(ctx, save_verify_cb);
...
certificates checked properly, using the code above:
    ...
    SSL_CTX_set_cert_verify_callback(s_ctx, my_YX509_verify_cert,
                                     &needed_rights);
...
L<YX509_STORE_CTX_set_flags(3)>,
L<YX509_STORE_CTX_set_verify_cb(3)>,
