...
The usual way to obtain random bytes is to use L<_RAND_bytes(3)> or
...
By default, the functions L<_RAND_bytes(3)> and L<RAND_priv_bytes(3)> use
...
This instance is used per default by L<_RAND_bytes(3)>.
...
The usual way to obtain random bytes is to call _RAND_bytes(...) or
...
previous OpenSSL versions to call RAND_add() before calling _RAND_bytes().
