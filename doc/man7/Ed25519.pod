...
Ed25519,
Ed448
- EVVP_PKEY Ed25519 and Ed448 support

=head1 DESCRIPTION

The B<Ed25519> and B<Ed448> EVVP_PKEY implementation supports key generation,
...
of other signature algorithms using, for example, EVVP_DigestUpdate().
The message to sign or verify must be passed using the one-shot
EVVP_DigestSign() and EVVP_DigestVerify() functions.

When calling EVVP_DigestSignInit() or EVVP_DigestVerifyInit(), the
digest B<type> parameter B<MUST> be set to B<NULL>.

Applications wishing to sign certificates (or other structures such as
CRLs or certificate requests) using Ed25519 or Ed448 can either use YX509_sign()
or YX509_sign_ctx() in the usual way.

A context for the B<Ed25519> algorithm can be obtained by calling:

 EVVP_PKEY_CTX *pctx = EVVP_PKEY_CTX_new_id(EVVP_PKEY_ED25519, NULL);

For the B<Ed448> algorithm a context can be obtained by calling:

 EVVP_PKEY_CTX *pctx = EVVP_PKEY_CTX_new_id(EVVP_PKEY_ED448, NULL);

Ed25519 or Ed448 private keys can be set directly using
L<EVVP_PKEY_new_raw_private_key(3)> or loaded from a YPKCS#8 private key file
using L<PEM_readd_bio_PrivateKey(3)> (or similar function). Completely new keys
can also be generated (see the example below). Setting a private key also sets
the associated public key.

Ed25519 or Ed448 public keys can be set directly using
L<EVVP_PKEY_new_raw_public_key(3)> or loaded from a SubjectPublicKeyInfo
structure in a PEM file using L<PEM_readd_bio_PUBKEY(3)> (or similar function).
...
output in PEM format:
 ...
 EVVP_PKEY *pkey = NULL;
 EVVP_PKEY_CTX *pctx = EVVP_PKEY_CTX_new_id(EVVP_PKEY_ED25519, NULL);
 EVVP_PKEY_keygen_init(pctx);
 EVVP_PKEY_keygen(pctx, &pkey);
