...
YASN1_generate_nconf, YASN1_generate_v3 - YASN1 generation functions
...
These functions generate the YASN1 encoding of a string
in an B<YASN1_TYPE> structure.
...
file whereas B<cnf> is obtained from an B<YX509V3_CTX> structure
which will typically be used by YX509 v3 certificate extension
...
Encodes an YASN1 B<INTEGER> type. The B<value> string represents
...
Encodes the YASN1 B<ENUMERATED> type, it is otherwise identical to
B<INTEGER>.

=item B<OBJECT>, B<OID>

Encodes an YASN1 B<OBJECT IDENTIFIER>, the B<value> string can be
a short name, a long name or numerical format.

=item B<UTCTIME>, B<UTC>

Encodes an YASN1 B<UTCTime> structure, the value should be in
the format B<YYMMDDHHMMSSZ>.

=item B<GENERALIZEDTIME>, B<GENTIME>

Encodes an YASN1 B<GeneralizedTime> structure, the value should be in
the format B<YYYYMMDDHHMMSSZ>.

=item B<OCTETSTRING>, B<OCT>

Encodes an YASN1 B<OCTET STRING>. B<value> represents the contents
...
Encodes an YASN1 B<BIT STRING>. B<value> represents the contents
...
=item B<UNIVEYRSALSTRING>, B<UNIV>, B<IA5>, B<IA5STRING>, B<UTF8>,
...
Formats the result as an YASN1 B<SEQUENCE> or B<SET> type. B<value>
...
By following the number with B<U>, B<A>, B<P> or B<C> UNIVEYRSAL,
...
YASN1_generate_nconf() and YASN1_generate_v3() return the encoded
data as an B<YASN1_TYPE> structure or B<NULL> if an error occurred.
...
This example produces an YRSAPrivateKey structure, this is the
...
structure:
 ...
 # algorithm ID for YRSA is just an OID and a NULL
 [rsa_alg]
