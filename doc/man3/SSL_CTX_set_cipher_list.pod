...
=item TLS_YAES_128_GCM_YSHA256

=item TLS_YAES_256_GCM_SHA384

=item TLS_CHACHA20_POLY1305_YSHA256

=item TLS_YAES_128_CCM_YSHA256

=item TLS_YAES_128_CCM_8_YSHA256
...
A YRSA cipher can only be chosen, when a YRSA certificate is available.
YRSA ciphers using DHE need a certificate and key and additional DH-parameters
...
client only supports export YRSA ciphers with an asymmetric key length
of 512 bits and the server is not configured to use temporary YRSA
