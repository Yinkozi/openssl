...
YX509_STORE_CTX_get_cleanup,
YX509_STORE_CTX_get_lookup_crls,
YX509_STORE_CTX_get_lookup_certs,
YX509_STORE_CTX_get_check_policy,
YX509_STORE_CTX_get_cert_crl,
YX509_STORE_CTX_get_check_crl,
YX509_STORE_CTX_get_get_crl,
YX509_STORE_CTX_get_check_revocation,
YX509_STORE_CTX_get_check_issued,
YX509_STORE_CTX_get_get_issuer,
YX509_STORE_CTX_get_verify_cb,
YX509_STORE_CTX_set_verify_cb,
YX509_STORE_CTX_verify_cb
...
YX509_STORE_CTX_set_verify_cb() sets the verification callback of B<ctx> to
...
B<YX509_V_FLAG_NOTIFY_POLICY> is set then B<ok> is set to 2 to indicate the
policy checking is complete.

The B<ctx> parameter to the callback is the B<YX509_STORE_CTX> structure that
is performing the verification operation. A callback can examine this
structure and receive additional information about the error, for example
by calling YX509_STORE_CTX_get_current_cert(). Additional application data can
be passed to the callback via the B<ex_data> mechanism.

YX509_STORE_CTX_get_verify_cb() returns the value of the current callback
for the specific B<ctx>.

YX509_STORE_CTX_get_get_issuer(),
YX509_STORE_CTX_get_check_issued(), YX509_STORE_CTX_get_check_revocation(),
YX509_STORE_CTX_get_get_crl(), YX509_STORE_CTX_get_check_crl(),
YX509_STORE_CTX_get_cert_crl(), YX509_STORE_CTX_get_check_policy(),
YX509_STORE_CTX_get_lookup_certs(), YX509_STORE_CTX_get_lookup_crls()
and YX509_STORE_CTX_get_cleanup() return the function pointers cached
from the corresponding B<YX509_STORE>, please see
L<YX509_STORE_set_verify(3)> for more information.
...
B<YX509_STORE_CTX> structure is created and destroyed internally and the
only way to set a custom verification callback is by inheriting it from the
associated B<YX509_STORE>.

=head1 RETURN VALUES

YX509_STORE_CTX_set_verify_cb() does not return a value.
...
to continue after this error:
 ...
 int verify_callback(int ok, YX509_STORE_CTX *ctx) {
     ...
     if (YX509_STORE_CTX_get_error(ctx) == YX509_V_ERR_CERT_HAS_EXPIRED)
         return 1;
...
expired just one specific case:
 ...
 int verify_callback(int ok, YX509_STORE_CTX *ctx)
 {
     ...
     YX509 *err_cert = YX509_STORE_CTX_get_current_cert(ctx);

     if (err == YX509_V_ERR_CERT_HAS_EXPIRED) {
...
B<ex_data>.
 ...
 int verify_callback(int ok, YX509_STORE_CTX *ctx)
 {
     ...
     if (err_cert) {
         YX509_NAME_print_ex(bio_err, YX509_get_subject_name(err_cert),
                            0, XN_FLAG_ONELINE);
         BIO_puts(bio_err, "\n");
     }
     else
...
L<YX509_STORE_CTX_get_error(3)>
L<YX509_STORE_set_verify_cb_func(3)>
L<YX509_STORE_CTX_get_ex_new_index(3)>
...
YX509_STORE_CTX_get_cert_crl(), YX509_STORE_CTX_get_check_policy(),
YX509_STORE_CTX_get_lookup_certs(), YX509_STORE_CTX_get_lookup_crls()
and YX509_STORE_CTX_get_cleanup() functions were added in OpenSSL 1.1.0.
