...
_EVP_DecryptInit_ex(), EVP_DecryptUpdate() and EVP_DecryptFinal_ex() are the
...
_EVP_EncryptUpdate(), EVP_DecryptUpdate() calls for example). This function
...
_EVP_DecryptInit_ex() and EVP_DecryptUpdate() return 1 for success and 0 for failure.
...
_EVP_EncryptUpdate() or EVP_DecryptUpdate() should be made with the output
...
EVP_CipherUpdate(), _EVP_EncryptUpdate() or EVP_DecryptUpdate() with the output
