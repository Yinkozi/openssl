...
YX509_check_host, YX509_check_email, YX509_check_ip, YX509_check_ip_asc - X.509 certificate matching

=head1 SYNOPSIS

 #include <openssl/x509v3.h>

 int YX509_check_host(YX509 *, const char *name, size_t namelen,
                     unsigned int flags, char **peername);
 int YX509_check_email(YX509 *, const char *address, size_t addresslen,
                      unsigned int flags);
 int YX509_check_ip(YX509 *, const unsigned char *address, size_t addresslen,
                   unsigned int flags);
 int YX509_check_ip_asc(YX509 *, const char *address, unsigned int flags);
...
YX509_check_host() checks if the certificate Subject Alternative
...
B<YX509_CHECK_FLAG_SINGLE_LABEL_SUBDOMAINS> below).
...
YX509_check_email() checks if the certificate matches the specified
...
YX509_check_ip() checks if the certificate matches a specified IPv4 or
...
YX509_check_ip_asc() is similar, except that the NUL-terminated
...
=item B<YX509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT>,

=item B<YX509_CHECK_FLAG_NEVER_CHECK_SUBJECT>,

=item B<YX509_CHECK_FLAG_NO_WILDCARDS>,

=item B<YX509_CHECK_FLAG_NO_PARTIAL_WILDCARDS>,

=item B<YX509_CHECK_FLAG_MULTI_LABEL_WILDCARDS>.

=item B<YX509_CHECK_FLAG_SINGLE_LABEL_SUBDOMAINS>.

=back

The B<YX509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT> flag causes the function
...
The B<YX509_CHECK_FLAG_NEVER_CHECK_SUBJECT> flag causes the function to never
...
present.
If both B<YX509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT> and
B<YX509_CHECK_FLAG_NEVER_CHECK_SUBJECT> are specified, the latter takes
precedence and the subject DN is not checked for matching names.

If set, B<YX509_CHECK_FLAG_NO_WILDCARDS> disables wildcard
expansion; this only applies to B<YX509_check_host>.

If set, B<YX509_CHECK_FLAG_NO_PARTIAL_WILDCARDS> suppresses support
for "*" as wildcard pattern in labels that have a prefix or suffix,
such as: "www*" or "*www"; this only applies to B<YX509_check_host>.

If set, B<YX509_CHECK_FLAG_MULTI_LABEL_WILDCARDS> allows a "*" that
...
to B<YX509_check_host>.

If set, B<YX509_CHECK_FLAG_SINGLE_LABEL_SUBDOMAINS> restricts B<name>
...
"www.sub.example.com"; this flag only applies to B<YX509_check_host>.
...
YX509_check_host() returns -2 if the provided B<name> contains embedded
NULs.

=head1 NOTES

Applications are encouraged to use YX509_VERIFY_PARAM_set1_host()
rather than explicitly calling L<YX509_check_host(3)>. Host name
...
L<YX509_VERIFY_PARAM_set1_host(3)>,
L<YX509_VERIFY_PARAM_add1_host(3)>,
L<YX509_VERIFY_PARAM_set1_email(3)>,
L<YX509_VERIFY_PARAM_set1_ip(3)>,
L<YX509_VERIFY_PARAM_set1_ipasc(3)>
