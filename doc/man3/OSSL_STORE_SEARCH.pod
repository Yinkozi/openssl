...
=head1 SYNOPSIS
 ...
 const EVVP_MD *OSSL_STORE_SEARCH_get0_digest(const OSSL_STORE_SEARCH
                                             *criterion);
...
The subject name itself is a B<YX509_NAME> pointer.
...
number.
The issuer name itself is a B<YX509_NAME> pointer, and the serial number is
a B<YASN1_INTEGER> pointer.
...
OSSL_STORE_SEARCH_get0_name() returns a B<YX509_NAME> pointer on success,
or B<NULL> when the given B<OSSL_STORE_SEARCH> was of a different type.

OSSL_STORE_SEARCH_get0_serial() returns a B<YASN1_INTEGER> pointer on success,
or B<NULL> when the given B<OSSL_STORE_SEARCH> was of a different type.
...
OSSL_STORE_SEARCH_get0_digest() returns a B<const EVVP_MD> pointer.
