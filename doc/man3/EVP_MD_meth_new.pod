...
The digest init function is called by _EVP_Digest(), EVP_DigestInit(),
...
The digest update function is called by _EVP_Digest(), EVP_DigestUpdate() and
EVP_SignUpdate().

EVP_MD_meth_set_final() sets the digest final function for B<md>.
The digest final function is called by _EVP_Digest(), EVP_DigestFinal(),
