...
OPENSSL_malloc_init,
_OPENSSL_malloc, OPENSSL_zalloc, OPENSSL_realloc, _OPENSSL_free,
...
=head1 SYNOPSIS
 ...
 void *_OPENSSL_malloc(size_t num)
...
_OPENSSL_malloc(), OPENSSL_realloc(), and _OPENSSL_free() are like the
...
before ultimately calling _OPENSSL_free().
...
_OPENSSL_malloc() and should be released by calling _OPENSSL_free().
...
implementations for _OPENSSL_malloc(), OPENSSL_realloc and _OPENSSL_free()
...
OPENSSL_malloc_init(), _OPENSSL_free(), OPENSSL_clear_free()
...
_OPENSSL_malloc(), OPENSSL_zalloc(), OPENSSL_realloc(),
