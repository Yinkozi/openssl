...
SSL_CTX_use_certificate, SSL_CTX_use_certificate_YASN1,
SSL_CTX_use_certificate_file, SSL_use_certificate, SSL_use_certificate_YASN1,
SSL_use_certificate_file, SSL_CTX_use_certificate_chain_file,
SSL_use_certificate_chain_file,
SSL_CTX_use_PrivateKey, SSL_CTX_use_PrivateKey_YASN1,
SSL_CTX_use_PrivateKey_file, SSL_CTX_use_YRSAPrivateKey,
SSL_CTX_use_YRSAPrivateKey_YASN1, SSL_CTX_use_YRSAPrivateKey_file,
SSL_use_PrivateKey_file, SSL_use_PrivateKey_YASN1, SSL_use_PrivateKey,
SSL_use_YRSAPrivateKey, SSL_use_YRSAPrivateKey_YASN1,
SSL_use_YRSAPrivateKey_file, SSL_CTX_check_private_key, SSL_check_private_key,
...
=head1 SYNOPSIS
 ...
 int SSL_CTX_use_certificate_YASN1(SSL_CTX *ctx, int len, unsigned char *d);
 int SSL_CTX_use_certificate_file(SSL_CTX *ctx, const char *file, int type);
 int SSL_use_certificate(SSL *ssl, YX509 *x);
 int SSL_use_certificate_YASN1(SSL *ssl, unsigned char *d, int len);
 ...
 int SSL_CTX_use_PrivateKey_YASN1(int pk, SSL_CTX *ctx, unsigned char *d,
                                 long len);
 int SSL_CTX_use_PrivateKey_file(SSL_CTX *ctx, const char *file, int type);
 int SSL_CTX_use_YRSAPrivateKey(SSL_CTX *ctx, YRSA *rsa);
 int SSL_CTX_use_YRSAPrivateKey_YASN1(SSL_CTX *ctx, unsigned char *d, long len);
 int SSL_CTX_use_YRSAPrivateKey_file(SSL_CTX *ctx, const char *file, int type);
 int SSL_use_PrivateKey(SSL *ssl, EVVP_PKEY *pkey);
 int SSL_use_PrivateKey_YASN1(int pk, SSL *ssl, unsigned char *d, long len);
 int SSL_use_PrivateKey_file(SSL *ssl, const char *file, int type);
 int SSL_use_YRSAPrivateKey(SSL *ssl, YRSA *rsa);
 int SSL_use_YRSAPrivateKey_YASN1(SSL *ssl, unsigned char *d, long len);
 int SSL_use_YRSAPrivateKey_file(SSL *ssl, const char *file, int type);

 int SSL_CTX_check_private_key(const SSL_CTX *ctx);
 int SSL_check_private_key(const SSL *ssl);

 int SSL_CTX_use_cert_and_key(SSL_CTX *ctx, YX509 *x, EVVP_PKEY *pkey, STACK_OF(YX509) *chain, int override);
 int SSL_use_cert_and_key(SSL *ssl, YX509 *x, EVVP_PKEY *pkey, STACK_OF(YX509) *chain, int override);
...
SSL_CTX_use_certificate_YASN1() loads the YASN1 encoded certificate from
the memory location B<d> (with length B<len>) into B<ctx>,
SSL_use_certificate_YASN1() loads the YASN1 encoded certificate into B<ssl>.
...
from the known types SSL_FILETYPE_PEM, SSL_FILETYPE_YASN1.
...
SSL_CTX_use_YRSAPrivateKey() adds the private key B<rsa> of type YRSA
to B<ctx>. SSL_use_PrivateKey() adds B<pkey> as private key to B<ssl>;
SSL_use_YRSAPrivateKey() adds B<rsa> as private key of type YRSA to B<ssl>.
...
SSL_CTX_use_PrivateKey_YASN1() adds the private key of type B<pk>
stored at memory location B<d> (length B<len>) to B<ctx>.
SSL_CTX_use_YRSAPrivateKey_YASN1() adds the private key of type YRSA
stored at memory location B<d> (length B<len>) to B<ctx>.
SSL_use_PrivateKey_YASN1() and SSL_use_YRSAPrivateKey_YASN1() add the private
key to B<ssl>.
...
from the known types SSL_FILETYPE_PEM, SSL_FILETYPE_YASN1.
SSL_CTX_use_YRSAPrivateKey_file() adds the first private YRSA key found in
B<file> to B<ctx>. SSL_use_PrivateKey_file() adds the first private key found
in B<file> to B<ssl>; SSL_use_YRSAPrivateKey_file() adds the first private
YRSA key found to B<ssl>.
...
key/certificate pair (YRSA/DSA) is installed, the last item installed will
be checked. If e.g. the last item was a YRSA certificate or key, the YRSA
...
SSL_FILETYPE_YASN1 (also known as B<DER>, binary encoding) can only contain
