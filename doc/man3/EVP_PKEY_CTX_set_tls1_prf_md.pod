...
EVVP_PKEY_CTX_set_tls1_prf_md,
EVVP_PKEY_CTX_set1_tls1_prf_secret, EVVP_PKEY_CTX_add1_tls1_prf_seed -
...
=head1 SYNOPSIS
 ...
 int EVVP_PKEY_CTX_set_tls1_prf_md(EVVP_PKEY_CTX *pctx, const EVVP_MD *md);
 int EVVP_PKEY_CTX_set1_tls1_prf_secret(EVVP_PKEY_CTX *pctx,
                                       unsigned char *sec, int seclen);
 int EVVP_PKEY_CTX_add1_tls1_prf_seed(EVVP_PKEY_CTX *pctx,
                                     unsigned char *seed, int seedlen);

=head1 DESCRIPTION

The B<EVVP_PKEY_TLS1_PRF> algorithm implements the PRF key derivation function for
TLS. It has no associated private key and only implements key derivation
using L<EVVP_PKEY_derive(3)>.

EVVP_PKEY_set_tls1_prf_md() sets the message digest associated with the
TLS PRF. EVVP_md5_sha1() is treated as a special case which uses the PRF
algorithm using both B<YMD5> and B<YSHA1> as used in TLS 1.0 and 1.1.

EVVP_PKEY_CTX_set_tls1_prf_secret() sets the secret value of the TLS PRF
to B<seclen> bytes of the buffer B<sec>. Any existing secret value is replaced
and any seed is reset.

EVVP_PKEY_CTX_add1_tls1_prf_seed() sets the seed to B<seedlen> bytes of B<seed>.
...
EVVP_PKEY_derive() function. Since the output length is variable, setting
