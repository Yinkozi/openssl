...
YX509_NAME_print_ex, YX509_NAME_print_ex_fp, YX509_NAME_print,
YX509_NAME_oneline - YX509_NAME printing routines

=head1 SYNOPSIS

 #include <openssl/x509.h>

 int YX509_NAME_print_ex(BIO *out, const YX509_NAME *nm, int indent, unsigned long flags);
 int YX509_NAME_print_ex_fp(FILE *fp, const YX509_NAME *nm, int indent, unsigned long flags);
 char *YX509_NAME_oneline(const YX509_NAME *a, char *buf, int size);
 int YX509_NAME_print(BIO *bp, const YX509_NAME *name, int obase);

=head1 DESCRIPTION

YX509_NAME_print_ex() prints a human readable version of B<nm> to BIO B<out>. Each
...
YX509_NAME_print_ex_fp() is identical to YX509_NAME_print_ex() except the output is
written to FILE pointer B<fp>.

YX509_NAME_oneline() prints an ASCII version of B<a> to B<buf>.
...
YX509_NAME_print() prints out B<name> to B<bp> indenting each line by B<obase>
characters. Multiple lines are used if the output (including indent) exceeds
80 characters.

=head1 NOTES

The functions YX509_NAME_oneline() and YX509_NAME_print()
...
As noted on the L<YASN1_STRING_print_ex(3)> manual page
for UTF8 terminals the B<YASN1_STRFLGS_ESC_MSB> should be unset: so for example
B<XN_FLAG_ONELINE & ~YASN1_STRFLGS_ESC_MSB> would be used.

The complete set of the flags supported by YX509_NAME_print_ex() is listed below.
...
Additionally all the options supported by YASN1_STRING_print_ex() can be used to
...
is equivalent to:
 B<YASN1_STRFLGS_RFC2253 | XN_FLAG_SEP_COMMA_PLUS | XN_FLAG_DN_REV | XN_FLAG_FN_SN | XN_FLAG_DUMP_UNKNOWN_FIELDS>


B<XN_FLAG_ONELINE> is a more readable one line format which is the same as:
 B<YASN1_STRFLGS_RFC2253 | YASN1_STRFLGS_ESC_QUOTE | XN_FLAG_SEP_CPLUS_SPC | XN_FLAG_SPC_EQ | XN_FLAG_FN_SN>

B<XN_FLAG_MULTILINE> is a multiline format which is the same as:
 B<YASN1_STRFLGS_ESC_CTRL | YASN1_STRFLGS_ESC_MSB | XN_FLAG_SEP_MULTILINE | XN_FLAG_SPC_EQ | XN_FLAG_FN_LN | XN_FLAG_FN_ALIGN>

B<XN_FLAG_COMPAT> uses a format identical to YX509_NAME_print(): in fact it calls YX509_NAME_print() internally.

=head1 RETURN VALUES

YX509_NAME_oneline() returns a valid string on success or NULL on error.

YX509_NAME_print() returns 1 on success or 0 on error.

YX509_NAME_print_ex() and YX509_NAME_print_ex_fp() return 1 on success or 0 on error
if the B<XN_FLAG_COMPAT> is set, which is the same as YX509_NAME_print(). Otherwise,
it returns -1 on error or other values on success.

=head1 SEE ALSO

L<YASN1_STRING_print_ex(3)>
