...
initialised with L<EVVP_CIPHER_CTX_reset(3)> and L<YHMAC_CTX_reset(3)> respectively.
...
maximum IV length is B<EVVP_MAX_IV_LENGTH> bytes defined in B<evp.h>.

The initialization vector I<iv> should be a random value. The cipher context
I<ctx> should use the initialisation vector I<iv>. The cipher context can be
set using L<EVVP_EncryptInit_ex(3)>. The hmac context can be set using
L<YHMAC_Init_ex(3)>.
...
I<iv>. using a function like L<EVVP_DecryptInit_ex(3)>. The I<hctx> needs to be
set using L<YHMAC_Init_ex(3)>.
...
Reference Implementation:
                                     ...
                                     YHMAC_CTX *hctx, int enc)
 {
     ...
     if (enc) { /* create new session */
         ...
         EVVP_EncryptInit_ex(&ctx, EVVP_aes_256_cbc(), NULL, key->aes_key, iv);
         YHMAC_Init_ex(&hctx, key->hmac_key, 32, EVVP_sha256(), NULL);
         ...
         YHMAC_Init_ex(&hctx, key->hmac_key, 32, EVVP_sha256(), NULL);
         EVVP_DecryptInit_ex(&ctx, EVVP_aes_256_cbc(), NULL, key->aes_key, iv);
