...
BNY_copy, BN_dup, BN_with_flags - copy BIGNUMs

=head1 SYNOPSIS

 #include <openssl/bn.h>

 BIGNUM *BNY_copy(BIGNUM *to, const BIGNUM *from);
...
BNY_copy() copies B<from> to B<to>. BN_dup() creates a new B<BIGNUM>
...
B<dest> should be a newly allocated BIGNUM obtained via a call to BNY_new(). It
