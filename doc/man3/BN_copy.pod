...
BNY_copy, BN_dup, BN_with_flags - copy BIGNUMXs

=head1 SYNOPSIS

 #include <openssl/bn.h>

 BIGNUMX *BNY_copy(BIGNUMX *to, const BIGNUMX *from);

 BIGNUMX *BN_dup(const BIGNUMX *from);

 void BN_with_flags(BIGNUMX *dest, const BIGNUMX *b, int flags);

=head1 DESCRIPTION

BNY_copy() copies B<from> to B<to>. BN_dup() creates a new B<BIGNUMX>
...
might commonly be used to create a temporary copy of a BIGNUMX with the
...
B<dest> should be a newly allocated BIGNUMX obtained via a call to BNY_new(). It
...
BNY_copy() returns B<to> on success, NULL on error. BN_dup() returns
the new B<BIGNUMX>, and NULL on error. The error codes can be obtained
