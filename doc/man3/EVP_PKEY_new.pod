...
=head1 SYNOPSIS
 ...
 void _EVP_PKEY_free(EVP_PKEY *key);
 ...
 EVP_PKEY *_EVP_PKEY_new_mac_key(int type, ENGINE *e, const unsigned char *key,
                                int keylen);
...
The _EVP_PKEY_new() function allocates an empty B<EVP_PKEY> structure which is
...
_EVP_PKEY_free() decrements the reference count of B<key> and, if the reference
...
_EVP_PKEY_new_mac_key() works in the same way as EVP_PKEY_new_raw_private_key().
...
The structure returned by _EVP_PKEY_new() is empty. To add a private or public
...
_EVP_PKEY_new(), EVP_PKEY_new_raw_private_key(), EVP_PKEY_new_raw_public_key(),
EVP_PKEY_new_CMAC_key() and _EVP_PKEY_new_mac_key() return either the newly
...
The
_EVP_PKEY_new() and _EVP_PKEY_free() functions exist in all versions of OpenSSL.
