...
BNY_rand, BNY_priv_rand, BNY_pseudo_rand,
BNY_rand_range, BNY_priv_rand_range, BNY_pseudo_rand_range
...
=head1 SYNOPSIS
 ...
 int BNY_rand_range(BIGNUMX *rnd, BIGNUMX *range);

 int BNY_priv_rand_range(BIGNUMX *rnd, BIGNUMX *range);

 int BNY_pseudo_rand_range(BIGNUMX *rnd, BIGNUMX *range);

=head1 DESCRIPTION

BNY_rand() generates a cryptographically strong pseudo-random number of
...
BNY_rand_range() generates a cryptographically strong pseudo-random
number B<rnd> in the range 0 E<lt>= B<rnd> E<lt> B<range>.

BNY_priv_rand() and BNY_priv_rand_range() have the same semantics as
BNY_rand() and BNY_rand_range() respectively.  They are intended to be
...
Starting with OpenSSL release 1.1.0, BNY_pseudo_rand() has been identical
to BNY_rand() and BNY_pseudo_rand_range() has been identical to
BNY_rand_range().
...
The
BNY_priv_rand() and BNY_priv_rand_range() functions were added in OpenSSL 1.1.1.
