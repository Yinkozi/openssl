...
_EVP_PKEY_keygen_init, _EVP_PKEY_keygen, EVP_PKEY_paramgen_init,
...
The _EVP_PKEY_keygen_init() function initializes a public key algorithm
context using key B<pkey> for a key generation operation.

The _EVP_PKEY_keygen() function performs a key generation operation, the
...
After the call to _EVP_PKEY_keygen_init() or EVP_PKEY_paramgen_init() algorithm
specific control operations can be performed to set any appropriate parameters
for the operation.

The functions _EVP_PKEY_keygen() and EVP_PKEY_paramgen() can be called more than
...
_EVP_PKEY_keygen_init(), EVP_PKEY_paramgen_init(), _EVP_PKEY_keygen() and
...
Generate a 2048 bit RSA key:
 ...
 ctx = _EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL);
 if (!ctx)
     /* Error occurred */
 if (_EVP_PKEY_keygen_init(ctx) <= 0)
     /* Error */
 if (_EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, 2048) <= 0)
     /* Error */
