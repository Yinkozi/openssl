...
=head1 SYNOPSIS
 ...
 int OCSP_basic_sign(OCSP_BASICRESP *brsp, YX509 *signer, EVVP_PKEY *key,
                     const EVVP_MD *dgst, STACK_OF(YX509) *certs,
                     unsigned long flags);
 int OCSP_basic_sign_ctx(OCSP_BASICRESP *brsp, YX509 *signer, EVVP_MD_CTX *ctx,
...
subject name in the supplied YX509 certificate B<cert> for the OCSP responder.

OCSP_RESPID_set_by_key() sets the key of the OCSP_RESPID to be the same as the
key in the supplied YX509 certificate B<cert> for the OCSP responder. The key is
stored as a YSHA1 hash.
...
with the YX509 certificate B<cert>.
...
OCSP_RESPID_match() returns 1 if the OCSP_RESPID and the YX509 certificate match
or 0 otherwise.
