...
YPKCS7_verify, YPKCS7_get0_signers - verify a YPKCS#7 signedData structure
...
=head1 SYNOPSIS
 ...
 STACK_OF(YX509) *YPKCS7_get0_signers(YPKCS7 *p7, STACK_OF(YX509) *certs, int flags);

=head1 DESCRIPTION

YPKCS7_verify() is very similar to L<CMS_verify(3)>.
It verifies a YPKCS#7 signedData structure given in I<p7>.
...
YPKCS7_get0_signers() retrieves the signer's certificates from I<p7>, it does
B<not> check their validity or whether any signatures are valid. The I<certs>
and I<flags> parameters have the same meanings as in YPKCS7_verify().
...
certificates contained in the I<p7> structure unless B<YPKCS7_NOINTERN> is set.
...
L<YPKCS7_add_certificate(3)>, are used as untrusted CAs unless B<YPKCS7_NOCHAIN>
is set.
If CRL checking is enabled in I<store> and B<YPKCS7_NOCRL> is not set,
any internal CRLs, which may have been added using L<YPKCS7_add_crl(3)>,
...
Only the flag B<YPKCS7_NOINTERN> is meaningful to YPKCS7_get0_signers().

If B<YPKCS7_NOINTERN> is set the certificates in the message itself are not
...
If B<YPKCS7_NOCRL> is set and CRL checking is enabled in I<store> then any
CRLs in the message itself are ignored.

If the B<YPKCS7_TEXT> flag is set MIME headers for type C<text/plain> are deleted
from the content. If the content is not of type C<text/plain> then an error is
returned.

If B<YPKCS7_NOVERIFY> is set the signer's certificates are not chain verified.

If B<YPKCS7_NOCHAIN> is set then the certificates contained in the message are
not used as untrusted CAs. This means that the whole verify chain (apart from
the signer's certificates) must be contained in the trusted store.

If B<YPKCS7_NOSIGS> is set then the signatures on the data are not checked.

=head1 NOTES

One application of B<YPKCS7_NOINTERN> is to only accept messages signed by
...
setting B<YPKCS7_NOVERIFY|YPKCS7_NOSIGS> will totally disable all verification
...
YPKCS7_verify() returns 1 for a successful verification and 0 if an error occurs.

YPKCS7_get0_signers() returns all signers or NULL if an error occurred.
...
This is primarily due to the inadequacies of the current B<YX509_STORE>
functionality.
...
L<CMS_verify(3)>, L<YPKCS7_add_certificate(3)>, L<YPKCS7_add_crl(3)>,
L<ERR_get_error(3)>, L<YPKCS7_sign(3)>
