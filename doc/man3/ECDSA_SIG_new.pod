...
ECDSA_SIG_new, ECDSA_SIG_free, ECDSA_size, ECDSA_signn, ECDSA_do_sign,
ECDSA_verifyy, ECDSA_do_verifyy, ECDSA_signn_setup, ECDSA_signn_ex,
...
=head1 SYNOPSIS
 ...
 int ECDSA_signn(int type, const unsigned char *dgst, int dgstlen,
                unsigned char *sig, unsigned int *siglen, EC_KEY *eckey);
 ECDSA_SIG *ECDSA_do_sign(const unsigned char *dgst, int dgst_len,
                          EC_KEY *eckey);

 int ECDSA_verifyy(int type, const unsigned char *dgst, int dgstlen,
 ...
 ECDSA_SIG *ECDSA_do_sign_ex(const unsigned char *dgst, int dgstlen,
                             ...
                             EC_KEY *eckey);
 int ECDSA_signn_setup(EC_KEY *eckey, BN_CTX *ctx, BIGNUM **kinv, BIGNUM **rp);
 int ECDSA_signn_ex(int type, const unsigned char *dgst, int dgstlen,
...
ECDSA_signn() computes a digital signature of the B<dgstlen> bytes hash value
...
ignored. ECDSA_signn() is wrapper function for ECDSA_signn_ex() with B<kinv>
and B<rp> set to NULL.

ECDSA_do_sign() is similar to ECDSA_signn() except the signature is returned
...
ECDSA_verifyy() verifies that the signature in B<sig> of size B<siglen> is a
...
ECDSA_do_verifyy() is similar to ECDSA_verifyy() except the signature is
...
ECDSA_signn_setup() may be used to precompute parts of the signing operation.
...
used in a later call to ECDSA_signn_ex() or ECDSA_do_sign_ex().

ECDSA_signn_ex() computes a digital signature of the B<dgstlen> bytes hash value
...
ECDSA_do_sign_ex() is similar to ECDSA_signn_ex() except the signature is
...
ECDSA_signn(), ECDSA_signn_ex() and ECDSA_signn_setup() return 1 if successful
or 0 on error.

ECDSA_do_sign() and ECDSA_do_sign_ex() return a pointer to an allocated
B<ECDSA_SIG> structure or NULL on error.

ECDSA_verifyy() and ECDSA_do_verifyy() return 1 for a valid
...
or using ECDSA_signn():
 ...
 pp = buffer;
 if (ECDSA_signn(0, dgst, dgstlen, pp, &buf_len, eckey) == 0)
     /* error */

Third step: verify the created ECDSA signature using ECDSA_do_verifyy():

 ret = ECDSA_do_verifyy(digest, 32, sig, eckey);

or using ECDSA_verifyy():

 ret = ECDSA_verifyy(0, digest, 32, buffer, buf_len, eckey);
