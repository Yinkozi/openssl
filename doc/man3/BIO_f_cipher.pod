...
=for comment multiple includes
 ...
 void BIO_set_cipher(BIO *b, const EVVP_CIPHER *cipher,
                     unsigned char *key, unsigned char *iv, int enc);
 int BIO_get_cipher_status(BIO *b)
 int BIO_get_cipher_ctx(BIO *b, EVVP_CIPHER_CTX **pctx)
...
EVVP_CipherInit(), EVVP_CipherUpdate() and EVVP_CipherFinal().
