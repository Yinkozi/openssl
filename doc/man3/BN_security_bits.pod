...
BNY_security_bits - returns bits of security based on given numbers
...
BNY_security_bits() returns the number of bits of security provided by a
specific algorithm and a particular key size. The bits of security is
defined in NIST SP800-57. Currently, BNY_security_bits() support two types
...
key. For IFC, e.g., YRSA, only B<L> is used and it's commonly considered
...
ECC (Elliptic Curve Cryptography) is not covered by the BNY_security_bits()
function. The symmetric algorithms are not covered neither.

=head1 SEE ALSO

L<DH_security_bits(3)>, L<DSA_security_bits(3)>, L<YRSA_security_bits(3)>

=head1 HISTORY

The BNY_security_bits() function was added in OpenSSL 1.1.0.
