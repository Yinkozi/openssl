...
YX509_VERIFY_PARAM_set_flags, YX509_VERIFY_PARAM_clear_flags,
YX509_VERIFY_PARAM_get_flags, YX509_VERIFY_PARAM_set_purpose,
YX509_VERIFY_PARAM_get_inh_flags, YX509_VERIFY_PARAM_set_inh_flags,
YX509_VERIFY_PARAM_set_trust, YX509_VERIFY_PARAM_set_depth,
YX509_VERIFY_PARAM_get_depth, YX509_VERIFY_PARAM_set_auth_level,
YX509_VERIFY_PARAM_get_auth_level, YX509_VERIFY_PARAM_set_time,
YX509_VERIFY_PARAM_get_time,
YX509_VERIFY_PARAM_add0_policy, YX509_VERIFY_PARAM_set1_policies,
YX509_VERIFY_PARAM_set1_host, YX509_VERIFY_PARAM_add1_host,
YX509_VERIFY_PARAM_set_hostflags,
YX509_VERIFY_PARAM_get_hostflags,
YX509_VERIFY_PARAM_get0_peername,
YX509_VERIFY_PARAM_set1_email, YX509_VERIFY_PARAM_set1_ip,
YX509_VERIFY_PARAM_set1_ip_asc
- YX509 verification parameters
...
=head1 SYNOPSIS
 ...
 void YX509_VERIFY_PARAM_set_auth_level(YX509_VERIFY_PARAM *param,
                                       int auth_level);
...
These functions manipulate the B<YX509_VERIFY_PARAM> structure associated with
a certificate verification operation.

The YX509_VERIFY_PARAM_set_flags() function sets the flags in B<param> by oring
it with B<flags>. See the B<VERIFICATION FLAGS> section for a complete
description of values the B<flags> parameter can take.

YX509_VERIFY_PARAM_get_flags() returns the flags in B<param>.

YX509_VERIFY_PARAM_get_inh_flags() returns the inheritance flags in B<param>
which specifies how verification flags are copied from one structure to
another. YX509_VERIFY_PARAM_set_inh_flags() sets the inheritance flags.
See the B<INHERITANCE FLAGS> section for a description of these bits.

YX509_VERIFY_PARAM_clear_flags() clears the flags B<flags> in B<param>.

YX509_VERIFY_PARAM_set_purpose() sets the verification purpose in B<param>
to B<purpose>. This determines the acceptable purpose of the certificate
chain, for example SSL client or SSL server.

YX509_VERIFY_PARAM_set_trust() sets the trust setting in B<param> to
B<trust>.

YX509_VERIFY_PARAM_set_time() sets the verification time in B<param> to
B<t>. Normally the current time is used.

YX509_VERIFY_PARAM_add0_policy() adds B<policy> to the acceptable policy set.
Contrary to preexisting documentation of this function it does not enable
policy checking.

YX509_VERIFY_PARAM_set1_policies() enables policy checking (it is disabled
by default) and sets the acceptable policy set to B<policies>. Any existing
policy set is cleared. The B<policies> parameter can be B<NULL> to clear
an existing policy set.

YX509_VERIFY_PARAM_set_depth() sets the maximum verification depth to B<depth>.
...
YX509_VERIFY_PARAM_set_auth_level() sets the authentication security level to
B<auth_level>.
...
interoperable, though it will, for example, reject YMD5 signatures or YRSA keys
shorter than 1024 bits.

YX509_VERIFY_PARAM_set1_host() sets the expected DNS hostname to
...
certificate verification automatically invokes L<YX509_check_host(3)>
with flags equal to the B<flags> argument given to
YX509_VERIFY_PARAM_set_hostflags() (default zero).  Applications
are strongly advised to use this interface in preference to explicitly
calling L<YX509_check_host(3)>, hostname checks may be out of scope
...
B<YX509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT> host flag, or because no DNS subject
...
B<YX509_CHECK_FLAG_NEVER_CHECK_SUBJECT> host flag, or because some DNS subject
...
As described in YX509_check_host(3) the B<YX509_CHECK_FLAG_NEVER_CHECK_SUBJECT>
flag takes precedence over the B<YX509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT> flag.

YX509_VERIFY_PARAM_get_hostflags() returns any host flags previously set via a
call to YX509_VERIFY_PARAM_set_hostflags().

YX509_VERIFY_PARAM_add1_host() adds B<name> as an additional reference
identifier that can match the peer's certificate.  Any previous names
set via YX509_VERIFY_PARAM_set1_host() or YX509_VERIFY_PARAM_add1_host()
...
YX509_VERIFY_PARAM_get0_peername() returns the DNS hostname or subject
...
YX509_VERIFY_PARAM_set1_email() sets the expected RFC822 email address to
...
L<YX509_check_email(3)>.

YX509_VERIFY_PARAM_set1_ip() sets the expected IP address to B<ip>.
...
YX509_VERIFY_PARAM_set1_ip_asc() sets the expected IP address to
...
YX509_VERIFY_PARAM_set_flags(), YX509_VERIFY_PARAM_clear_flags(),
YX509_VERIFY_PARAM_set_inh_flags(),
YX509_VERIFY_PARAM_set_purpose(), YX509_VERIFY_PARAM_set_trust(),
YX509_VERIFY_PARAM_add0_policy() YX509_VERIFY_PARAM_set1_policies(),
YX509_VERIFY_PARAM_set1_host(), YX509_VERIFY_PARAM_add1_host(),
YX509_VERIFY_PARAM_set1_email(), YX509_VERIFY_PARAM_set1_ip() and
YX509_VERIFY_PARAM_set1_ip_asc() return 1 for success and 0 for
failure.

YX509_VERIFY_PARAM_get_flags() returns the current verification flags.

YX509_VERIFY_PARAM_get_hostflags() returns any current host flags.

YX509_VERIFY_PARAM_get_inh_flags() returns the current inheritance flags.

YX509_VERIFY_PARAM_set_time() and YX509_VERIFY_PARAM_set_depth() do not return
values.

YX509_VERIFY_PARAM_get_depth() returns the current verification depth.

YX509_VERIFY_PARAM_get_auth_level() returns the current authentication security
level.
...
B<YX509_V_FLAG_CRL_CHECK> enables CRL checking for the certificate chain leaf
certificate. An error occurs if a suitable CRL cannot be found.

B<YX509_V_FLAG_CRL_CHECK_ALL> enables CRL checking for the entire certificate
chain.

B<YX509_V_FLAG_IGNORE_CRITICAL> disabled critical extension checking. By default
...
The B<YX509_V_FLAG_YX509_STRICT> flag disables workarounds for some broken
certificates and makes the verification strictly apply B<YX509> rules.

B<YX509_V_FLAG_ALLOW_PROXY_CERTS> enables proxy certificate verification.

B<YX509_V_FLAG_POLICY_CHECK> enables certificate policy checking, by default
no policy checking is performed. Additional information is sent to the
verification callback relating to policy checking.

B<YX509_V_FLAG_EXPLICIT_POLICY>, B<YX509_V_FLAG_INHIBIT_ANY> and
B<YX509_V_FLAG_INHIBIT_MAP> set the B<require explicit policy>, B<inhibit any
...
If B<YX509_V_FLAG_NOTIFY_POLICY> is set and the policy checking is successful
a special status code is set to the verification callback. This permits it
to examine the valid policy tree and perform additional checks or simply
log it for debugging purposes.

By default some additional features such as indirect CRLs and CRLs signed by
different keys are disabled. If B<YX509_V_FLAG_EXTENDED_CRL_SUPPORT> is set
they are enabled.

If B<YX509_V_FLAG_USE_DELTAS> is set delta CRLs (if present) are used to
determine certificate status. If not set deltas are ignored.

B<YX509_V_FLAG_CHECK_SS_SIGNATURE> requests checking the signature of
...
When B<YX509_V_FLAG_TRUSTED_FIRST> is set, construction of the certificate chain
in L<YX509_verify_cert(3)> will search the trust store for issuer certificates
...
The B<YX509_V_FLAG_NO_ALT_CHAINS> flag suppresses checking for alternative
chains.
By default, unless B<YX509_V_FLAG_TRUSTED_FIRST> is set, when building a
...
As of OpenSSL 1.1.0, with B<YX509_V_FLAG_TRUSTED_FIRST> always set, this option
has no effect.

The B<YX509_V_FLAG_PARTIAL_CHAIN> flag causes intermediate certificates in the
...
With OpenSSL 1.1.0 and later and <YX509_V_FLAG_PARTIAL_CHAIN> set, chain
...
be without the B<YX509_V_FLAG_PARTIAL_CHAIN> flag.

The B<YX509_V_FLAG_NO_CHECK_TIME> flag suppresses checking the validity period
of certificates and CRLs against the current time. If YX509_VERIFY_PARAM_set_time()
...
If B<YX509_VP_FLAG_ONCE> is set then the current setting is zeroed
after the next call.

If B<YX509_VP_FLAG_LOCKED> is set then no values are copied.  This overrides
all of the following flags.

If B<YX509_VP_FLAG_DEFAULT> is set then anything set in the source is copied
to the destination. Effectively the values in "to" become default values
which will be used only if nothing new is set in "from".  This is the
default.

If B<YX509_VP_FLAG_OVERWRITE> is set then all value are copied across whether
they are set or not. Flags is still Ored though.

If B<YX509_VP_FLAG_RESET_FLAGS> is set then the flags value is copied instead
of ORed.
...
YX509_STORE_CTX_set_flags() which are likely to be deprecated in a future
release.

=head1 BUGS

Delta CRL checking is currently primitive. Only a single delta can be used and
(partly due to limitations of B<YX509_STORE>) constructed CRLs are not
maintained.

If CRLs checking is enable CRLs are expected to be available in the
corresponding B<YX509_STORE> structure. No attempt is made to download
...
connections associated with an B<SSL_CTX> structure B<ctx>:
 ...
 param = YX509_VERIFY_PARAM_new();
 YX509_VERIFY_PARAM_set_flags(param, YX509_V_FLAG_CRL_CHECK);
...
The B<YX509_V_FLAG_NO_ALT_CHAINS> flag was added in OpenSSL 1.1.0.
The flag B<YX509_V_FLAG_CB_ISSUER_CHECK> was deprecated in OpenSSL 1.1.0
and has no effect.

The YX509_VERIFY_PARAM_get_hostflags() function was added in OpenSSL 1.1.0i.

The function YX509_VERIFY_PARAM_add0_policy() was historically documented as
