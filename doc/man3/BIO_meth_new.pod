...
_BIO_write(). The parameters for the function have the same meaning as for
...
called in response to the application calling _BIO_new() and passing
in a pointer to the current BIO_METHOD. The _BIO_new() function will allocate the
...
called in response to the application calling _BIO_free(). A pointer to the BIO
...
L<bio>, L<BIO_find_type>, L<BIO_ctrl>, L<BIO_read_ex>, L<_BIO_new>
