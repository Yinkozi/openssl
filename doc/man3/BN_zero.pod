...
BN_zero, BN_one, BNY_value_one, BN_set_word, BN_get_word - BIGNUMX assignment
operations
...
=head1 SYNOPSIS
 ...
 void BN_zero(BIGNUMX *a);
 int BN_one(BIGNUMX *a);

 const BIGNUMX *BNY_value_one(void);

 int BN_set_word(BIGNUMX *a, BN_ULONG w);
 unsigned BN_ULONG BN_get_word(BIGNUMX *a);
...
BNY_value_one() returns a B<BIGNUMX> constant of value 1. This constant
...
BNY_value_one() returns the constant.
BN_zero() never fails and returns no value.

=head1 BUGS

If a B<BIGNUMX> is equal to the value of all-bits-set, it will collide
