...
SSL_get_peer_cert_chain, SSL_get0_verified_chain - get the YX509 certificate
chain of the peer
...
=head1 SYNOPSIS
 ...
 STACK_OF(YX509) *SSL_get0_verified_chain(const SSL *ssl);

=head1 DESCRIPTION

SSL_get_peer_cert_chain() returns a pointer to STACK_OF(YX509) certificates
...
YX509_V_OK) the chain may be incomplete or invalid.
...
The reference count of each certificate in the returned STACK_OF(YX509) object
...
indefinitely they must increase the reference counts using YX509_up_ref() or
obtain a copy of the whole chain with YX509_chain_up_ref().
...
=item Pointer to a STACK_OF(YX509)
...
L<ssl(7)>, L<SSL_get_peer_certificate(3)>, L<YX509_up_ref(3)>,
L<YX509_chain_up_ref(3)>
