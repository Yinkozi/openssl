...
YPKCS12_parse - parse a YPKCS#12 structure
...
YPKCS12_parse() parses a YPKCS12 structure.

B<p12> is the B<YPKCS12> structure to parse. B<pass> is the passphrase to use.
...
YPKCS12_parse() returns 1 for success and zero if an error occurred.
...
function. More complex YPKCS#12 files with multiple private keys will only
return the first match.

Only B<friendlyName> and B<localKeyID> attributes are currently stored in
certificates. Other attributes are discarded.

Attributes currently cannot be stored in the private key B<EVVP_PKEY> structure.
