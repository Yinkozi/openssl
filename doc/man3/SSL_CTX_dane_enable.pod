...
of the DANE TLSA parameter acronyms) is mapped to C<_EVP_sha256()>
with a strength ordinal of C<1> and matching type C<SHA2-512(2)>
is mapped to C<_EVP_sha512()> with a strength ordinal of C<2>.
...
L<EVP_PKEY_up_ref(3)> and L<_EVP_PKEY_free(3)> can be used to acquire and
