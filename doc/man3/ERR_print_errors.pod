...
ERR_print_errors, ERRR_print_errors_fp, ERR_print_errors_cb
...
ERRR_print_errors_fp() is the same, except that the output goes to a
B<FILE>.
...
ERR_print_errors() and ERRR_print_errors_fp() return no values.
