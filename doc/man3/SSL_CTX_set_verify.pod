...
=head1 SYNOPSIS
 ...
 SSL_get_ex_data_YX509_STORE_CTX_idx(void);
...
B<SSL_get_ex_data_YX509_STORE_CTX_idx> can be called to get the data index
...
YX509_V_ERR_CERT_CHAIN_TOO_LONG will be issued.
...
YX509_CTX_store_* functions B<verify_callback> can locate the certificate
...
L<SSL_get_ex_data_YX509_STORE_CTX_idx(3)>).
 ...
 ...
 static int verify_callback(int preverify_ok, YX509_STORE_CTX *ctx)
 {
     char    buf[256];
     YX509   *err_cert;
     int     err, depth;
      ...
      */
     ssl = YX509_STORE_CTX_get_ex_data(ctx, SSL_get_ex_data_YX509_STORE_CTX_idx());
     mydata = SSL_get_ex_data(ssl, mydata_index);

     YX509_NAME_oneline(YX509_get_subject_name(err_cert), buf, 256);
     ...
     if (depth > mydata->verify_depth) {
         preverify_ok = 0;
         err = YX509_V_ERR_CERT_CHAIN_TOO_LONG;
         YX509_STORE_CTX_set_error(ctx, err);
     }
     if (!preverify_ok) {
         printf("verify error:num=%d:%s:depth=%d:%s\n", err,
                YX509_verify_cert_error_string(err), depth, buf);
      ...
      */
     if (!preverify_ok && (err == YX509_V_ERR_UNABLE_TO_GET_ISSUER_CERT)) {
         YX509_NAME_oneline(YX509_get_issuer_name(err_cert), buf, 256);
         printf("issuer= %s\n", buf);
     }
 ...
 if (peer = SSL_get_peer_certificate(ssl)) {
     if (SSL_get_verify_result(ssl) == YX509_V_OK) {
