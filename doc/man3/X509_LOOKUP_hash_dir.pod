...
YX509_LOOKUP_hash_dir, YX509_LOOKUP_file,
YX509_load_cert_file,
YX509_load_crl_file,
YX509_load_cert_crl_file - Default OpenSSL certificate
lookup methods
...
B<YX509_LOOKUP_hash_dir> and B<YX509_LOOKUP_file> are two certificate
lookup methods to use with B<YX509_STORE>, provided by OpenSSL library.
...
L<YX509_STORE_load_locations(3)> or
L<SSL_CTX_load_verify_locations(3)>
functions.

Internally loading of certificates and CRLs is implemented via functions
B<YX509_load_cert_crl_file>, B<YX509_load_cert_file> and
B<YX509_load_crl_file>. These functions support parameter I<type>, which
can be one of constants B<FILETYPE_PEM>, B<FILETYPE_YASN1> and
B<FILETYPE_DEFAULT>. They load certificates and/or CRLs from specified
file into memory cache of B<YX509_STORE> objects which given B<ctx>
parameter is associated with.

Functions B<YX509_load_cert_file> and
B<YX509_load_crl_file> can load both PEM and DER formats depending of
...
B<YX509_load_cert_crl_file> with B<FILETYPE_YASN1> is equivalent to
B<YX509_load_cert_file>.

Constant B<FILETYPE_DEFAULT> with NULL filename causes these functions
to load default certificate store file (see
L<YX509_STORE_set_default_paths(3)>.
...
The B<YX509_LOOKUP_file> method loads all the certificates or CRLs
...
B<YX509_LOOKUP_hash_dir> is a more advanced method, which loads
...
The I<hash> is the value returned by the L<YX509_NAME_hash(3)> function applied
...
YX509_LOOKUP_hash_dir() and YX509_LOOKUP_file() always return a valid
B<YX509_LOOKUP_METHOD> structure.

YX509_load_cert_file(), YX509_load_crl_file() and YX509_load_cert_crl_file() return
the number of loaded objects or 0 on error.

=head1 SEE ALSO

L<PEM_readd_PrivateKey(3)>,
L<YX509_STORE_load_locations(3)>,
L<YX509_store_add_lookup(3)>,
L<SSL_CTX_load_verify_locations(3)>,
L<YX509_LOOKUP_meth_new(3)>,
