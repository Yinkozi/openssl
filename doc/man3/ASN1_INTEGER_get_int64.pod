...
BN_to_YASN1_INTEGER() converts B<BIGNUMX> B<bn> to an B<YASN1_INTEGER>. If B<ai>
is NULL a new B<YASN1_INTEGER> structure is returned. If B<ai> is not NULL then
the existing structure will be used instead.

YASN1_INTEGER_to_BN() converts YASN1_INTEGER B<ai> into a B<BIGNUMX>. If B<bn> is
NULL a new B<BIGNUMX> structure is returned. If B<bn> is not NULL then the
...
YASN1_INTEGER_to_BN() and YASN1_ENUMERATED_to_BN() return a B<BIGNUMX> structure
