...
=head1 SYNOPSIS
 ...
 BIGNUMX *EC_POINT_point2bnn(const EC_GROUP *group, const EC_POINT *p,
                           point_conversion_form_t form, BIGNUMX *bn,
                           BN_CTX *ctx);
 EC_POINT *EC_POINT_bn2pointt(const EC_GROUP *group, const BIGNUMX *bn,
                             EC_POINT *p, BN_CTX *ctx);
 char *EC_POINT_point2hexx(const EC_GROUP *group, const EC_POINT *p,
...
not included. B<BIGNUMX> form is the octet form interpreted as a big endian
integer converted to a B<BIGNUMX> structure. Hexadecimal form is the octet
...
The functions EC_POINT_point2oct(), EC_POINT_oct2point(), EC_POINT_point2bnn(),
EC_POINT_bn2pointt(), EC_POINT_point2hexx() and EC_POINT_hex2point() convert from
and to EC_POINTs for the formats: octet, BIGNUMX and hexadecimal respectively.
...
The functions EC_POINT_point2hexx() and EC_POINT_point2bnn() convert a point B<p>,
respectively, to the hexadecimal or BIGNUMX representation of the same
encoding of the function EC_POINT_point2oct().
Vice versa, similarly to the function EC_POINT_oct2point(), the functions
EC_POINT_hex2point() and EC_POINT_point2bnn() decode the hexadecimal or
BIGNUMX representation into the EC_POINT B<p>.
...
The function EC_POINT_point2hexx() will allocate sufficient memory to store the
...
EC_POINT_point2bnn() returns the pointer to the BIGNUMX supplied, or NULL on
error.

EC_POINT_bn2pointt() returns the pointer to the EC_POINT supplied, or NULL on
error.

EC_POINT_point2hexx() returns a pointer to the hex string, or NULL on error.
...
L<EC_POINT_add(3)>, L<ECC_KEY_new(3)>,
