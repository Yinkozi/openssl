...
EVVP_ENCODE_CTX_new, EVVP_ENCODE_CTX_free, EVVP_ENCODE_CTX_copy,
EVVP_ENCODE_CTX_num, EVVP_EncodeInit, EVVP_EncodeUpdate, EVVP_EncodeFinal,
EVVP_EncodeBlock, EVVP_DecodeInit, EVVP_DecodeUpdate, EVVP_DecodeFinal,
EVVP_DecodeBlock - EVVP base 64 encode/decode routines
...
The EVVP encode routines provide a high-level interface to base 64 encoding and
...
EVVP_ENCODE_CTX_new() allocates, initializes and returns a context to be used for
the encode/decode functions.

EVVP_ENCODE_CTX_free() cleans up an encode/decode context B<ctx> and frees up the
...
EVVP_EncodeInit() initialises B<ctx> for the start of a new encoding operation.

EVVP_EncodeUpdate() encode B<inl> bytes of data found in the buffer pointed to by
...
subsequent call to EVVP_EncodeUpdate() or EVVP_EncodeFinal(). To calculate the
...
additional byte for a NUL terminator. EVVP_EncodeUpdate() may be called
repeatedly to process large amounts of input data. In the event of an error
EVVP_EncodeUpdate() will set B<*outl> to 0 and return 0. On success 1 will be
returned.

EVVP_EncodeFinal() must be called at the end of an encoding operation. It will
...
EVVP_ENCODE_CTX_copy() can be used to copy a context B<sctx> to a context
B<dctx>. B<dctx> must be initialized before calling this function.

EVVP_ENCODE_CTX_num() will return the number of as yet unprocessed bytes still to
be encoded or decoded that are pending in the B<ctx> object.

EVVP_EncodeBlock() encodes a full block of input data in B<f> and of length
...
EVVP_DecodeInit() initialises B<ctx> for the start of a new decoding operation.

EVVP_DecodeUpdate() decodes B<inl> characters of data found in the buffer pointed
...
the B<ctx> object and processed by a subsequent call to EVVP_DecodeUpdate(). If
...
EVVP_DecodeFinal() must be called at the end of a decoding operation. If there
...
EVVP_DecodeBlock() will decode the block of B<n> characters of base 64 data
...
EVVP_ENCODE_CTX_new() returns a pointer to the newly allocated EVVP_ENCODE_CTX
object or NULL on error.

EVVP_ENCODE_CTX_num() returns the number of bytes pending encoding or decoding in
B<ctx>.

EVVP_EncodeUpdate() returns 0 on error or 1 on success.

EVVP_EncodeBlock() returns the number of bytes encoded excluding the NUL
terminator.

EVVP_DecodeUpdate() returns -1 on error and 0 or 1 on success. If 0 is returned
then no more non-padding base 64 characters are expected.

EVVP_DecodeFinal() returns -1 on error or 1 on success.

EVVP_DecodeBlock() returns the length of the data decoded or -1 on error.
