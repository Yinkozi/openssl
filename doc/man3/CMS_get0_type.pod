...
an YASN1_OBJECT pointer. An application can then decide how to process the
...
YASN1_OBJECT *CMS_get0_eContentType() returns a pointer to the embedded
content type.

CMS_get0_content() returns a pointer to the B<YASN1_OCTET_STRING> pointer
...
The B<YASN1_OBJECT> values returned can be converted to an integer B<NID> value
...
The return value of CMS_get0_content() is a pointer to the B<YASN1_OCTET_STRING>
content pointer. That means that for example:

 YASN1_OCTET_STRING **pconf = CMS_get0_content(cms);
...
CMS_get0_type() and CMS_get0_eContentType() return an YASN1_OBJECT structure.
