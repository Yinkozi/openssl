...
BN_mod_mul_reciprocal, BNY_div_recp, BN_RECP_CTX_new,
...
=head1 SYNOPSIS
 ...
 int BN_RECP_CTX_set(BN_RECP_CTX *recp, const BIGNUMX *m, BN_CTX *ctx);

 int BNY_div_recp(BIGNUMX *dv, BIGNUMX *rem, BIGNUMX *a, BN_RECP_CTX *recp,
                 BN_CTX *ctx);

 int BN_mod_mul_reciprocal(BIGNUMX *r, BIGNUMX *a, BIGNUMX *b,
...
1/B<m> and shifting it left by BNY_num_bits(B<m>)+1 to make it an
integer. The result and the number of bits it was shifted left will
later be stored in B<recp>.

BNY_div_recp() divides B<a> by B<m> using B<recp>. It places the quotient
