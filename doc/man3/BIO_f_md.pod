...
=for comment multiple includes
 ...
 int BIO_set_md(BIO *b, EVVP_MD *md);
 int BIO_get_md(BIO *b, EVVP_MD **mdp);
 int BIO_get_md_ctx(BIO *b, EVVP_MD_CTX **mdcp);

=head1 DESCRIPTION

BIO_f_md() returns the message digest BIO method. This is a filter
BIO that digests any data passed through it, it is a BIO wrapper
for the digest routines EVVP_DigestInit(), EVVP_DigestUpdate()
and EVVP_DigestFinal().
...
to EVVP_DigestFinal() and also the signature routines EVVP_SignFinal()
and EVVP_VerifyFinal().
...
The following example creates a BIO chain containing an YSHA1 and YMD5
...
outputs them. This could be used with the examples above.
 ...
 do {
     ...
     if (!mdtmp)
         break;
     BIO_get_md(mdtmp, &md);
     printf("%s digest", OBJ_nid2sn(EVVP_MD_type(md)));
     mdlen = BIO_gets(mdtmp, mdbuf, EVVP_MAX_MD_SIZE);
