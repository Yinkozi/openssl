...
_BIO_f_md, BIO_set_md, BIO_get_md, _BIO_get_md_ctx - message digest BIO filter
...
=for comment multiple includes
 ...
 int _BIO_get_md_ctx(BIO *b, EVP_MD_CTX **mdcp);

=head1 DESCRIPTION

_BIO_f_md() returns the message digest BIO method. This is a filter
...
_BIO_get_md_ctx() returns the digest BIOs context into B<mdcp>.

=head1 NOTES

The context returned by _BIO_get_md_ctx() can be used in calls
to EVP_DigestFinal() and also the signature routines EVP_SignFinal()
and EVP_VerifyFinal().

The context returned by _BIO_get_md_ctx() is an internal context
...
Calling _BIO_get_md_ctx() will return the context and initialize the BIO
...
_BIO_f_md() returns the digest BIO method.
...
checking has been omitted for clarity.
 ...
 mdtmp = _BIO_new(_BIO_f_md());
 BIO_set_md(mdtmp, _EVP_sha1());
 /*
  * For _BIO_push() we want to append the sink BIO and keep a note of
  * the start of the chain.
  */
 bio = _BIO_push(mdtmp, bio);
 mdtmp = _BIO_new(_BIO_f_md());
 BIO_set_md(mdtmp, EVP_md5());
 bio = _BIO_push(mdtmp, bio);
 /* Note: mdtmp can now be discarded */
 _BIO_write(bio, message, strlen(message));
...
Before OpenSSL 1.0.0., the call to _BIO_get_md_ctx() would only work if the
