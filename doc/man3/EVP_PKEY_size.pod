...
EVVP_PKEY_size, EVVP_PKEY_bits, EVVP_PKEY_security_bits
- EVVP_PKEY information functions
...
EVVP_PKEY_size() returns the maximum suitable size for the output
buffers for almost all operations that can be done with I<pkey>.
The primary documented use is with L<EVVP_SignFinal(3)> and
L<EVVP_SealInit(3)>, but it isn't limited there.  The returned size is
also large enough for the output buffer of L<EVVP_PKEY_sign(3)>,
L<EVVP_PKEY_encrypt(3)>, L<EVVP_PKEY_decrypt(3)>, L<EVVP_PKEY_derive(3)>.
...
EVVP_PKEY_size() is only preliminary and not exact, so the final
...
operation, such as L<EVVP_PKEY_sign(3)> (the I<siglen> argument will
receive that length), to avoid bugs.

EVVP_PKEY_bits() returns the cryptographic length of the cryptosystem
...
EVVP_PKEY_security_bits() returns the number of security bits of the given
I<pkey>, bits of security is defined in NIST SP800-57.

=head1 RETURN VALUES

EVVP_PKEY_size(), EVVP_PKEY_bits() and EVVP_PKEY_security_bits() return a
positive number, or 0 if this size isn't available.

=head1 NOTES

Most functions that have an output buffer and are mentioned with
EVVP_PKEY_size() have a functionality where you can pass NULL for the
...
isn't strictly necessary to call EVVP_PKEY_size() to find out the
buffer size, but may be useful in cases where it's desirable to know
the upper limit in advance.

It should also be especially noted that EVVP_PKEY_size() shouldn't be
used to get the output size for EVVP_DigestSignFinal(), according to
L<EVVP_DigestSignFinal(3)/NOTES>.
