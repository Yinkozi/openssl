...
ECC_KEY_get_enc_flags, ECC_KEY_set_enc_flags
...
=head1 SYNOPSIS
 ...
 void ECC_KEY_set_enc_flags(EC_KEY *eckey, unsigned int flags);
...
EC_PKEY_NO_PUBKEY has been used - see below), then d2i_ECCPrivateKey() generates
...
The functions ECC_KEY_get_enc_flags() and ECC_KEY_set_enc_flags() get and set the
...
define the behaviour of how the  B<key> is converted into YASN1 in a call to
...
ECC_KEY_get_enc_flags() returns the value of the current encoding flags for the
EC_KEY.
