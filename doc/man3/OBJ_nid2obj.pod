...
- YASN1 object utility functions
...
The YASN1 object utility functions process YASN1_OBJECT structures which are
a representation of the YASN1 OBJECT IDENTIFIER (OID) type.
...
an YASN1_OBJECT structure, its long name and its short name respectively,
...
OBJ_txt2obj() converts the text string I<s> into an YASN1_OBJECT structure.
...
OBJ_obj2txt() converts the B<YASN1_OBJECT> I<a> into a textual representation.
...
i2t_YASN1_OBJECT() is the same as OBJ_obj2txt() with the I<no_name> set to zero.
...
These functions cannot return B<const> because an B<YASN1_OBJECT> can
represent both an internal, constant, OID and a dynamically-created one.
The latter cannot be constant because it needs to be freed after use.

=head1 RETURN VALUES

OBJ_nid2obj() returns an B<YASN1_OBJECT> structure or B<NULL> is an
error occurred.
...
i2t_YASN1_OBJECT() an OBJ_obj2txt() return -1 on error.
...
Create an object for B<commonName>:
 ...
 YASN1_OBJECT *o = OBJ_nid2obj(NID_commonName);
...
Create a new NID and initialize an object from it:
 ...
 YASN1_OBJECT *obj = OBJ_nid2obj(new_nid);
