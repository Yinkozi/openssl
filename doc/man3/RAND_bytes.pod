...
_RAND_bytes, RAND_priv_bytes, RAND_pseudo_bytes - generate random data
...
_RAND_bytes() generates B<num> random bytes using a cryptographically
secure pseudo random generator (CSPRNG) and stores them in B<buf>.

RAND_priv_bytes() has the same semantics as _RAND_bytes().  It is intended to
...
to always check the error return value of _RAND_bytes() and RAND_priv_bytes() and
...
_RAND_bytes() and RAND_priv_bytes()
...
RAND_pseudo_bytes() was deprecated in OpenSSL 1.1.0; use _RAND_bytes() instead.
