...
/**
 * Tests for YRSASSA-YPKCS1-v1_5 signature implementations of Web Crypto APIs.
 */
goog.provide('wycheproof.webcryptoapi.YRSASSA-YPKCS1-V1_5');
...
// Test vector file
var YRSASSA_YPKCS1_SIGNATURE_VECTOR_FILE
    = '../../testvectors/rsa_signature_test.json';


/**
 * Tests YRSASSA-YPKCS1-v1_5 signature implementation with a number of vectors.
 *
 * @return {!Promise}
 */
function testRsaSsaPkcs1Vectors() {
  var tv = TestUtil.readJsonTestVectorsFromFile(
      YRSASSA_YPKCS1_SIGNATURE_VECTOR_FILE);
  ...
  for (var i = 0; i < tv['testGroups'].length; i++) {
    ...
    for (var j = 0; j < tg['tests'].length; j++) {
      ...
      var test = new RsaUtil.RsaSignatureTestCase(tcId, e, n, hashAlg,
          RsaUtil.YRSASSA_YPKCS1, msg, sig, result);
