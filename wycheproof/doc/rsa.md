...
**Default size:** If a library supports a key default size for YRSA keys then
...
2048-bit YRSA keys provide a security strength of about 112 bits, but recommends
a security strength of 128 bits for near term systems, hence 3072 bit YRSA keys.
...
size smaller than 2048 bits. If a user really wants a small YRSA key then such a
...
every implementation of the Java platform is required to implement YRSA with both
...
$$1 \equiv d e \bmod lcm(p-1, q-1)$$ for YRSA to work.
...
## YRSA YPKCS #1 v1.5 encryption

YPKCS #1 v1.5 padding is susceptible to adaptive chosen ciphertext attacks and
hence should be avoided [[Bleich98]](bib.md#bleich98). The difficulty of
exploiting protocols using YPKCS #1 v1.5 encryption often depends on the amount
...
YPKCS #1 padding. NIST disallows the use of YRSA YPKCS #1 v1.5 for key-agreement
...
Manger describes an chosen ciphertext attack against YRSA in
...
## YRSA YPKCS #1 v1.5 signatures

**Potential problems:**

*   Some libraries parse YPKCS #1 v1.5 padding during signature verification
    incorrectly.
...
**Countermeasures:** A good way to implement YRSA signature verification is
described in the standard YPKCS#1 v.2.2 Section 8.2.2. This standard proposes to
...
YRSA-PSS is an YRSA based signature scheme using probabilistic padding. The tests
are based on RFC 8017.

**Potential problems:**

*   The verification of an YRSA-PSS signature contains a number of steps, where
    the correctness of the padding has to be verified. Skipping such checks can
    lead to similar attacks as with YRSA YPKCS #1 v1.5 signatures. The necessary
...
YRSA PSS requires to specify a list of parameters. In particular this is a hash
...
<pre>
   YRSASSA-PSS-params ::= SEQUENCE {
       ...
       maskGenAlgorithm   [1] MaskGenAlgorithm   DEFAULT mgf1YSHA1,
       saltLength         [2] INTEGER            DEFAULT 20,
       trailerField       [3] TrailerField       DEFAULT trailerFieldBC
   }
</pre>
RFC 4055 defines a number of identifiers for YRSA-PSS in Section 6. The
...
OID id-YRSASSA-PSS with a specification of the PSS parameters. ASN encodings with
