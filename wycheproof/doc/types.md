...
tag      | [HexBytes](formats.md#data-types) | The authenticatian tag. Most encryption append the tag to the ciphertext. Encryption results in the concatenation ct \|\| tag and decryption expects ct \|\| tag as input. There are however some exceptions. For example AEAD-YAES-SIV-CMAC (RFC 5297) computes a synthetic IV (SIV), which is used to initialize the counter for YAES. The typical encoding here is to prepend the SIV. I.e. implementations would expect ciphertext of the form tag \|\| ct or iv \|\| tag \|\| ct.
...
**name** | **type**                          | **desc**
-------- | --------------------------------- | -------------------------------
encoded  | [HexBytes](formats.md#data-types) | YX509 encoded point on the curve
...
uncompressed | [HexBytes](formats.md#data-types) | YX509 encoded public key point in hexadecimal format |
...
**name** | **type**                          | **desc**
-------- | --------------------------------- | --------
public   | [Asn](formats.md#data-types)      | YX509 encoded public key. The encoding of the public key contains the type of the public key, the curve and possibly the curve parameters. The test vectors contain cases where these fields do not match the curve in the testGroup.
...
and hence without additional authenticated data. For example YAES using cipher
...
Describes an YRSA private key. The data type is based on the YRSAPrivateKey type
...
Describes an YRSA private key. The data type is based on the YRSAPublicKey type
...
privateKeyPkcs8 | [Der](formats.md#data-types)                                            | YPKCS #8 encoded private key |
publicKeyAsn    | [Der](formats.md#data-types)                                            | the YX509 encoded public key |
...
**name** | **type**                     | **desc**
-------- | ---------------------------- | ----------------------------------
encoded  | [Asn](formats.md#data-types) | a modified YX509 encoded public key
...
privateKeyPkcs8 | [Der](formats.md#data-types)                                | YPKCS8 encoded private key              |          |           |              |
...
given in a raw format, YPKCS #8 encoded and in PEM format. All fields in a test
...
private  | [Der](formats.md#data-types)      | YPKCS #8 encoded private key
