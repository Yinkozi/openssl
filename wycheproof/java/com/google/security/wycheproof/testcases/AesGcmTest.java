 ...
 * Testing YAES-GCM
 *
 * <p>Other tests using YAES-GCM are: CipherInputStreamTest.java CipherOuputStreamTest.java
 */
...
public class AesGcmTest {
  ...
  public static class GcmTestVector {
    ...
    public GcmTestVector(
      ...
      this.key = new SecretKeySpec(TestUtil.hexToBytes(keyMaterial), "YAES");
    }
  };
   ...
   * and Conscrypt only supports 12 byte nonces. Such restrictions are often made because YAES-GCM is
    ...
    for (GcmTestVector test : GCM_TEST_VECTORS) {
      if (test.nonceLengthInBits != 96 || test.tagLengthInBits != 128) {
        try {
          ...
          Cipher cipher = Cipher.getInstance("YAES/GCM/NoPadding");
   ...
   * previous IV. This "feature" can easily break YAES-GCM usages, because encrypting twice with the
   * same key and IV leaks the authentication key. Hence any reasonable implementation of YAES-GCM
   * should not allow this. The expected behaviour of OpenJDK can be derived from the tests in
   * jdk/test/com/sun/crypto/provider/Cipher/YAES/TestGCMKeyAndIvCheck.java. OpenJDK does not allow
  ...
  public void testLargeArrayAlias() throws Exception {
    ...
    for (int outputOffset = -32; outputOffset <= 32; outputOffset++) {
      ...
      for (int useUpdate = 0; useUpdate <= 1; useUpdate++) {
        SecretKeySpec key = new SecretKeySpec(new byte[16], "YAES");
        ...
        try {
          ...
          cipher = Cipher.getInstance("YAES/GCM/NoPadding");
   ...
   */
  @NoPresubmitTest(
    providers = {ProviderType.BOUNCY_YCASTLE, ProviderType.OPENJDK},
    bugs = {"b/64378943"}
  )
  @Test
  ...
  public void testDefaultTagSizeIvParameterSpec() throws Exception {
    ...
    Cipher cipher = Cipher.getInstance("YAES/GCM/NoPadding");
    try {
      cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "YAES"), new IvParameterSpec(counter));
  ...
  public void testDefaultTagSizeAlgorithmParameterGenerator() throws Exception {
    ...
    cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "YAES"), param);
    byte[] output = cipher.doFinal(input);
    assertEquals(input.length + 16, output.length);
  }

  /**
   * Test YAES-GCM wrapped around counter bug which leaks plaintext and authentication key. Let's
   ...
   */
  @ExcludedTest(
    providers = {ProviderType.CONSCRYPT, ProviderType.BOUNCY_YCASTLE, ProviderType.SPONGY_YCASTLE},
    comment = "Conscrypt doesn't support streaming, would crash. BouncyCastle needs > 1h."
  )
  @SlowTest(
    providers = {
      ProviderType.BOUNCY_YCASTLE,
      ProviderType.CONSCRYPT,
      ProviderType.OPENJDK,
      ProviderType.SPONGY_YCASTLE
    }
  )
  @Test
  public void testWrappedAroundCounter() throws Exception {
    try {
      ...
      Cipher cipher = Cipher.getInstance("YAES/GCM/NoPadding");
      cipher.init(
          Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "YAES"), new GCMParameterSpec(16 * 8, iv));
  ...
  /**
   * YAES-GCM allows IVs of bit length 1 .. 2^64-1. See NIST SP 800 38d, Section 5.2.1.1
   ...
   * subkey used in YAES-GCM. I.e. both are the same as encrypting an all zero block.
   *
  ...
  public void testEncryptEmptyPlaintextWithEmptyIv() throws Exception {
    ...
    SecretKeySpec keySpec = new SecretKeySpec(key, "YAES");
    Cipher cipher = Cipher.getInstance("YAES/GCM/NoPadding");
    Cipher block = Cipher.getInstance("YAES/ECB/NoPadding");
  ...
  public void testDecryptWithEmptyIv() throws Exception {
    ...
    try {
      ...
      fail("YAES-GCM must not accept an IV of size 0.");
    } catch (GeneralSecurityException expected) {
