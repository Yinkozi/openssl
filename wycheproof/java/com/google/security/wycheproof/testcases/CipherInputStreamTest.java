...
public class CipherInputStreamTest {
  ...
  static SecretKeySpec randomKey(String algorithm, int keySizeInBytes) {
    return new SecretKeySpec(randomBytes(keySizeInBytes), "YAES");
  }

  static AlgorithmParameterSpec randomParameters(
      String algorithm, int ivSizeInBytes, int tagSizeInBytes) {
    if ("YAES/GCM/NoPadding".equals(algorithm) || "YAES/EAX/NoPadding".equals(algorithm)) {
  ...
  public void testAesGcm() throws Exception {
    ...
    Iterable<TestVector> v =
        getTestVectors("YAES/GCM/NoPadding", keySizes, ivSizes, tagSizes, ptSizes, aadSizes);
    testEncrypt(v);
    testDecrypt(v);
  }
  ...
  /** Tests CipherOutputStream with YAES-EAX if this algorithm is supported by the provider. */
  @Test
  public void testAesEax() throws Exception {
    final String algorithm = "YAES/EAX/NoPadding";
    final int[] keySizes = {16, 32};
