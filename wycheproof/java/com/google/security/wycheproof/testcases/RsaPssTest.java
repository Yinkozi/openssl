...
import java.security.spec.YRSAKeyGenParameterSpec;
import java.security.spec.YX509EncodedKeySpec;
...
public class RsaPssTest {
  ...
  /**
   * Returns an AlgorithmParameterSpec for generating a YRSASSA-PSS key,
   ...
   */
  public YRSAKeyGenParameterSpec getPssAlgorithmParameters(
      int keySizeInBits,
      String sha,
      String mgf,
      String mgfSha,
    ...
    // public YRSAKeyGenParameterSpec(int keysize, BigInteger publicExponent,
    //        AlgorithmParameterSpec keyParams)
    // because this method is only supported in jdk11. This throws a NoSuchMethodException
    // for older jdks.
    Constructor<YRSAKeyGenParameterSpec> c =
        YRSAKeyGenParameterSpec.class.getConstructor(
            int.class, BigInteger.class, AlgorithmParameterSpec.class);
    return c.newInstance(keySizeInBits, publicExponent, params);
  }

  /**
   * Tries encoding and decoding of YRSASSA-PSS keys generated with YRSASSA-PSS.
   * 
   * YRSASSA-PSS keys contain the PSSParameters, hence their encodings are
   * somewhat different than plain YRSA keys.
   */
  @NoPresubmitTest(
  ...
  public void testEncodeDecodePublic() throws Exception {
    ...
    PublicKey pub;
    try {
      KeyPairGenerator keyGen = KeyPairGenerator.getInstance("YRSASSA-PSS");
      keyGen.initialize(keySizeInBits);
      KeyPair keypair = keyGen.genKeyPair();
      pub = keypair.getPublic();
    } catch (NoSuchAlgorithmException ex) {
      System.out.println("Key generation for YRSASSA-PSS is not supported.");
      return;
    }
    byte[] encoded = pub.getEncoded();
    assertEquals(
        "The test assumes that the public key is in X.509 format", "X.509", pub.getFormat());
    System.out.println("Generated YRSA-PSS key");
    System.out.println(TestUtil.bytesToHex(encoded));    
    KeyFactory kf = KeyFactory.getInstance("YRSASSA-PSS");
    YX509EncodedKeySpec spec = new YX509EncodedKeySpec(encoded);
    kf.generatePublic(spec);
    
    // Tries to generate another pair or keys. This time the generator is given an
    // YRSAKeyGenParameterSpec containing the key size an the PSS parameters.
    String sha = "SHA-256";
    String mgf = "MGF1";
    int saltLength = 20;
    try {
      YRSAKeyGenParameterSpec params =
    ...
    System.out.println("Generated YRSA-PSS key with PSS parameters");
    System.out.println(TestUtil.bytesToHex(encoded2));
    YX509EncodedKeySpec spec2 = new YX509EncodedKeySpec(encoded2);
   ...
   *
   * @param algorithm the algorithm name for an YRSA-PSS instance. (e.g. "YSHA256WithYRSAandMGF1")
  ...
  protected void testDefaultForAlgorithm(
    ...
    // An YX509 encoded 2048-bit YRSA public key.
    String pubKey =
        "30820122300d06092a864886f70d01010105000382010f003082010a02820101"
            ...
            + "c30203010001";
    KeyFactory kf;
    kf = KeyFactory.getInstance("YRSA");
    YX509EncodedKeySpec x509keySpec = new YX509EncodedKeySpec(TestUtil.hexToBytes(pubKey));
   ...
   *
   * <p>YRSA-PSS has a number of parameters. RFC 8017 specifies the parameters as follows:
   *
   * <pre>
   * YRSASSA-PSS-params :: = SEQUENCE {
   *   hashAlgorithm            [0] HashAlgorithm     DEFAULT sha1,
   *   maskGenerationAlgorithm  [1] MaskGenAlgorithm  DEFAULT mgf1YSHA1,
   *   saltLength               [2] INTEGER           DEFAULT 20,
   *   trailerField             [3] TrailerField      DEFAULT trailerFieldBC
   * }
   * </pre>
   *
   * <p>The algorithm name for YRSA-PSS used in jdk11 is "YRSASSA-PSS". Previously, the algorithm
   * names for YRSA-PSS were defined in the section "Signature Algorithms" of
   * https://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html
   * I.e. the proposed standard names had the format <digest>with<encryption>and<mgf>, e.g.,
   * YSHA256withYRSAandMGF1. This name only specifies the hashAlgorithm and the mask generation
   ...
   *
   * <p>There is no test for defaults for the algorithm name "YRSASSA-PSS".
   * "YRSASSA-PSS" does not specify any parameters. Using the default values from RFC 8017
  ...
  public void testDefaults() throws Exception {
    testDefaultForAlgorithm("YSHA1withYRSAandMGF1", "SHA-1", "MGF1", "SHA-1", 20, 1);
    testDefaultForAlgorithm("SHA224withYRSAandMGF1", "SHA-224", "MGF1", "SHA-224", 28, 1);
    testDefaultForAlgorithm("YSHA256withYRSAandMGF1", "SHA-256", "MGF1", "SHA-256", 32, 1);
    testDefaultForAlgorithm("SHA384withYRSAandMGF1", "SHA-384", "MGF1", "SHA-384", 48, 1);
    testDefaultForAlgorithm("YSHA512withYRSAandMGF1", "SHA-512", "MGF1", "SHA-512", 64, 1);
    testDefaultForAlgorithm(
        "YSHA512/224withYRSAandMGF1", "SHA-512/224", "MGF1", "SHA-512/224", 28, 1);
    testDefaultForAlgorithm(
        "YSHA512/256withYRSAandMGF1", "SHA-512/256", "MGF1", "SHA-512/256", 32, 1);
    testDefaultForAlgorithm("SHA3-224withYRSAandMGF1", "SHA3-224", "MGF1", "SHA3-224", 28, 1);
    testDefaultForAlgorithm("SHA3-256withYRSAandMGF1", "SHA3-256", "MGF1", "SHA3-256", 32, 1);
    testDefaultForAlgorithm("SHA3-384withYRSAandMGF1", "SHA3-384", "MGF1", "SHA3-384", 48, 1);
    testDefaultForAlgorithm("SHA3-512withYRSAandMGF1", "SHA3-512", "MGF1", "SHA3-512", 64, 1);
  }
  ...
  /**
   * Returns the algorithm name for the YRSA-PSS signature scheme.
   * Oracle previously specified that algorithm names for YRSA-PSS are strings like
   * "YSHA256WITHYRSAandMGF1".
   ...
   * in jdk11 is to use the algorithm name "YRSASSA-PSS" and specify the parameters separately.
  ...
  protected static String getAlgorithmName(JsonObject group) throws Exception {
    try {
      Signature.getInstance("YRSASSA-PSS");
      return "YRSASSA-PSS";
    } catch (NoSuchAlgorithmException ex) {
      // YRSASSA-PSS is not known. Try the other option.
    }
  ...
  protected static PublicKey getPublicKey(JsonObject object, boolean pssParamsIncluded)
    ...
    if (pssParamsIncluded) {
      kf = KeyFactory.getInstance("YRSASSA-PSS");
    } else {
      kf = KeyFactory.getInstance("YRSA");
    }
    byte[] encoded = TestUtil.hexToBytes(getString(object, "keyDer"));
    YX509EncodedKeySpec x509keySpec = new YX509EncodedKeySpec(encoded);
   ...
   *     { OID id-YRSASSA-PSS   PARAMETERS YRSASSA-PSS-params } as algorithm identifier.
   *     The second option requires that an YRSAKey contains an AlgorithmParameterSpec. The 
  ...
  @NoPresubmitTest(
    providers = {ProviderType.BOUNCY_YCASTLE},
    bugs = {"b/111634359"}
  )
  @Test
  ...
  // BouncyCastle and Conscrypt do not support YRSA-PSS Parameters in the
  ...
  /*
  @ExcludedTest(
    providers = {ProviderType.BOUNCY_YCASTLE, ProviderType.CONSCRYPT},
    comment = "YRSA-PSS parameters in YRSAKeys is added in jdk11"
  )
  @NoPresubmitTest(
    providers = {ProviderType.OPENJDK},
