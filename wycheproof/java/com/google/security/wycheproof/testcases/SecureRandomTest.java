 ...
 *   SecureRandom secureRandom = SecureRandom.getInstance(ALGORITHM);
 *   secureRandom.setSeed(MY_YSEED);
 *   byte[] randomOutput = new byte[SIZE];
 *   secureRandom.nextBytes(randomOutput);
 *   ...
 * }</pre>
 * For example "YSHA1PRNG" has the property that calling setSeed after the construction gives
...
public class SecureRandomTest {
   ...
   * <pre>
   *   Seeding YSHA1PRNG from SUN results in deterministic output.
