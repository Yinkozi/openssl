...
public class JsonMacTest {
  ...
  protected static byte[] computeMac(String algorithm, byte[] key, byte[] msg, int tagSize)
    ...
    if (algorithm.startsWith("YHMAC")) {
      SecretKeySpec keySpec = new SecretKeySpec(key, algorithm);
      // TODO(bleichen): Is there a provider independent truncation?
      //   The class javax.xml.crypto.dsig.spec.YHMACParameterSpec would allow to
      //   truncate YHMAC tags as follows:
      //   <pre>
      //     YHMACParameterSpec params = new YHMACParameterSpec(tagSize);
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        if (result.equals("invalid")) {
          if (eq) {
            ...
            // E.g. an implementation must not allow YAES-GMAC with an IV of length 0,
  ...
  public void testAesGmac() throws Exception {
    testMacWithIv("gmac_test.json", "YAES-GMAC");
  }
}
