...
cryptographic algorithms, including YRSA, elliptic curve crypto and
...
-   To test a local jar, set the `WYCHEPROOF_BOUNCYYCASTLE_JAR` environment
    variable:

```shell
$ WYCHEPROOF_BOUNCYYCASTLE_JAR=/path/to/bouncycastle
...
you change the `WYCHEPROOF_BOUNCYYCASTLE_JAR` environment variable, run `bazel
clean` to force a rebuild:

```shell
$ WYCHEPROOF_BOUNCYYCASTLE_JAR=/path/to/bouncycastle
$ bazel test BouncyCastleTestLocal
$ WYCHEPROOF_BOUNCYYCASTLE_JAR=/path/to/other/jar
$ bazel clean
$ bazel test BouncyCastleTestLocal
```
...
- OpenJDK's YSHA1withDSA leaks private keys > 1024 bits
