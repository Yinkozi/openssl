...
MSG_PROCESS_RETURN tls_process_client_hello(SSL *s, PACKET *pkt)
{
 ...
 err:
    if (clienthello != NULL)
        _OPENSSL_free(clienthello->pre_proc_exts);
    _OPENSSL_free(clienthello);
...
int tls_construct_certificate_request(SSL *s, WPACKET *pkt)
{
    if (SSL_IS_TLS13(s)) {
        ...
        if (s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
            ...
            if (_RAND_bytes(s->pha_context, s->pha_context_len) <= 0
...
int tls_construct_new_session_ticket(SSL *s, WPACKET *pkt)
{
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (_RAND_bytes(age_add_u.age_add_c, sizeof(age_add_u)) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                     SSL_F_TLS_CONSTRUCT_NEW_SESSION_TICKET,
                     ERR_R_INTERNAL_ERROR);
            goto err;
        ...
        if (s->s3->alpn_selected != NULL) {
            _OPENSSL_free(s->session->ext.alpn_selected);
